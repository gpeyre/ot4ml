...
Formulation~\eqref{eq-w1-cont} is a special case of a dual norm. A dual norm is a convenient way to design ``weak'' discrepancies by testing signed differences of measures against a controlled class of functions. For a symmetric convex set $B$ of measurable functions, one defines
...
The total variation norm (Example~\ref{exmp-tv}) is the dual norm associated with the unit ball of continuous functions
\eq{
	B = \enscond{f \in \Cc(\X)}{\norm{f}_\infty \leq 1}.
}
...
\end{tabular}
\caption{Dual witnesses for integral probability metrics. The red and blue curves are two one-dimensional probability densities and the violet curve is a representative normalized maximizer in the corresponding test class. $\Wass_1$ restricts the slope, MMD restricts the RKHS norm and produces a smooth witness, while total variation can saturate pointwise and therefore reacts sharply to signed density differences.}
...
\setlength{\tabcolsep}{2pt}
\begin{tabular}{@{}cc@{}}
\small kernel mean distance & \small signed kernel witness \\[-.15em]
\includegraphics[width=.40\linewidth]{figures/dualnorms-mmd-kernel-mean-embedding/feature-mean-distance.pdf} &
\includegraphics[width=.54\linewidth]{figures/dualnorms-mmd-kernel-mean-embedding/signed-kernel-sum.pdf}
\end{tabular}
\caption{Kernel mean embedding interpretation of MMD. The left panel represents each distribution by its RKHS mean element $m_\al$ or $m_\be$, and the MMD is their Hilbert distance. The right panel shows the same difference in the input space: each Dirac contributes a kernel bump with its signed weight, and the optimal RKHS witness is proportional to $m_\al-m_\be$.}
\label{fig:dualnorms-mmd-kernel-mean-embedding}
\end{figure}
...
\begin{thm}[Pinsker inequality]\label{thm-pinsker}
	Let $\al,\be\in\Mm_+^1(\X)$ be probability measures on a measurable space. With the full-variation convention~\eqref{eq-defn-tv},
	\[
		\TV(\al|\be)^2=\norm{\al-\be}_{\TV}^2\leq2\KL(\al|\be).
	\]
	In particular, on a finite space, for $\a,\b\in\simplex_n$,
	\[
		\norm{\a-\b}_{\ell^1}^2\leq2\KLD(\a|\b).
	\]
\end{thm}

\begin{proof}
	The claim is immediate if $\KL(\al|\be)=+\infty$, so assume $\al\ll\be$. Let $A$ be a positive Hahn set for $\al-\be$, and set $a=\al(A)$ and $b=\be(A)$. Since $(\al-\be)(\X)=0$, one has $a-b=\frac12\norm{\al-\be}_{\TV}$. Coarse-graining onto $(A,A^c)$ gives
	\[
		\KL(\al|\be)\geq a\log\frac ab+(1-a)\log\frac{1-a}{1-b}.
	\]
	For fixed $b\in(0,1)$, subtracting $2(a-b)^2$ from the binary relative entropy gives a function whose value and first derivative vanish at $a=b$, while its second derivative is $1/a+1/(1-a)-4\geq0$. Hence $\KL(\al|\be)\geq2(a-b)^2=\frac12\norm{\al-\be}_{\TV}^2$. Boundary cases follow by lower semicontinuity, and the finite-space statement uses $\norm{\al-\be}_{\TV}=\norm{\a-\b}_{\ell^1}$.
\end{proof}
