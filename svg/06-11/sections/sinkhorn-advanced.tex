...
which is a smooth approximation of the minimum operator. The two soft $c$-transforms are
...
\begin{tabular}{@{}cccc@{}}
\small $\epsilon=0$ & \small $\epsilon=.55$ & \small $\epsilon=.14$ & \small $\epsilon=.035$ \\[-.15em]
...
Gaussian log-densities and soft transforms are quadratic, so every dual Sinkhorn iterate is described by finitely many coefficients.
...
Closure of the iterates suggests, but does not by itself prove, Gaussian optimality. Entropy maximization at fixed covariance supplies the variational argument.
...
\begin{prop}[Balanced entropic OT between Gaussians]\label{prop-gaussian-sinkhorn-closed-form}
	...
	\[
		...
		U\diag(\sigma_i)V^\top
	\]
	be a singular-value decomposition. For the balanced entropic problem~\eqref{eq-entropic-generic} with $c(x,y)=\norm{x-y}^2$,
...
\eq{
	\norm{f_\ell-f^\star}_V = O(\la^\ell)
	\qandq
		\norm{g_\ell-g^\star}_V = O(\la^\ell)
}
\eq{
	\norm{ \log \frac{\d\pi_\ell}{\d\pi^\star} }_\infty
	=
	\norm{ (f_\ell-f^\star) \oplus (g_\ell-g^\star) }_\infty
	\leq
...
\eq{
	\norm{f_\ell-f^\star}_V \leq \frac{ \norm{\log\frac{\d \pi_{\ell,1}}{\d\al}}_\infty }{1-\la}
}
...
\begin{proof}
	...
	\[
		...
		-\epsilon\log\int e^{(g'-c)/\epsilon}\d\beta.
	\]
	The composition of two order-reversing transforms is therefore order preserving. The transform also commutes with additive constants in the projective sense, which is why one fixes a normalization. Starting from a subsolution gives $f_0\leq f_1$, and order preservation gives $f_\ell\leq f_{\ell+1}$ for all $\ell$. Soft-transform oscillation bounds, controlled by $\sup c-\inf c$, prevent escape to infinity after normalization. Monotone pointwise convergence, compactness of equicontinuous soft transforms, and continuity of $\mathcal A$ then give a fixed point. Uniqueness of entropic potentials up to constants, Proposition~\ref{prop-entropic-unique} and the dual uniqueness statement above identify this fixed point with the Sinkhorn solution.
\end{proof}
...
\begin{prop}[A compact $O(1/\ell)$ dual rate]\label{prop-sinkhorn-dual-rate}
	...
	\[
		\Delta_\ell \leq \frac{C R^2}{\epsilon(\ell+1)}.
	\]
\end{prop}
...
\begin{proof}
	...
	\[
		\mathcal D_\epsilon(f_{\ell+1},g_{\ell+1})-
		\mathcal D_\epsilon(f_\ell,g_\ell)
		=
		\epsilon\big[\KL(\pi^\star|\pi_\ell)-\KL(\pi^\star|\pi_{\ell+1})\big],
	\]
	where $\pi_\ell=e^{(f_\ell\oplus g_\ell-c)/\epsilon}\alpha\otimes\beta$ and $\pi^\star$ is the optimal entropic coupling. The KL drop controls the marginal residuals through Pinsker's inequality, Theorem~\ref{thm-pinsker}. Third, convexity of the exponential dual objective gives a one-step estimate of the form
	\[
		\Delta_\ell^2
		\leq
		\frac{C R^2}{\epsilon}
		\big(\mathcal D_\epsilon(f_{\ell+1},g_{\ell+1})-
		\mathcal D_\epsilon(f_\ell,g_\ell)\big).
	\]
	...
	\[
		\frac{1}{\Delta_{\ell+1}}-\frac{1}{\Delta_\ell}
		\geq \frac{\epsilon}{C R^2},
	\]
	and therefore $\Delta_\ell\leq C R^2/(\epsilon(\ell+1))$.
\end{proof}

\begin{cor}[Approximating unregularized OT by regularized dual costs]\label{cor-sinkhorn-dual-complexity}
	Consider discrete histograms $\a\in\simplex_n$, $\b\in\simplex_m$ and a finite cost matrix $\C$. Let $\mathcal D_{\epsilon,\ell}$ be the KL-normalized entropic dual value after $\ell$ Sinkhorn cycles, and let $\Delta_\ell$ be its dual gap. Define the entropy-corrected lower bound
	\[
		L_{\epsilon,\ell}
		\eqdef
	...
	\[
		...
		\leq
		\epsilon\log(nm)+\Delta_\ell .
	\]
	Consequently, choosing $\epsilon\leq\delta/(2\log(nm))$ and running Sinkhorn until $\Delta_\ell\leq\delta/2$ gives a $\delta$-accurate lower bound on the unregularized OT value. Under Proposition~\ref{prop-sinkhorn-dual-rate}, the intermediate condition is $\ell+1 \geq 2 C R^2/(\epsilon\delta)$. With the above choice of $\epsilon$, it is sufficient to take
	\[
		\ell+1 \geq \frac{4 C R^2\log(nm)}{\delta^2},
	\]
	hence $\ell=O(R^2\log(nm)/\delta^2)$, up to constants and logarithmic stabilization factors.
\end{cor}
\begin{proof}
	...
	\[
		...
		\MKD_\C(\a,\b).
	\]
	The corrected iterate satisfies $L_{\epsilon,\ell}=E_\epsilon-\Delta_\ell$, which gives the displayed value bound. The final iteration estimate follows by combining $\Delta_\ell\leq C R^2/(\epsilon(\ell+1))$ with the target $\Delta_\ell\leq\delta/2$.
\end{proof}
