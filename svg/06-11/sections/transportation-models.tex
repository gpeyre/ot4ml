...
%
Optimal transport is one approach to achieving this, but it is computationally expensive and raises questions about how to estimate it from samples. A recent idea, first introduced in diffusion models~\cite{Ho2020DDPM,Song2021ScoreSDE} and later systematically developed in flow matching by Yaron Lipman and collaborators~\cite{Lipman2022FlowMatching}, is to obtain $T$ by integrating a time-dependent vector field $v_t$.
%
...
\end{equation}
A common case is $d'=2d$. We write $(x,y) \in \RR^{d'} = \RR^d \times \RR^d$ and assume $P_0(x,y)=x$ and $P_1(x,y)=y$, so that $\pi$ is a probabilistic coupling between $\alpha_0$ and $\alpha_1$, i.e. $\pi$ has marginals $(\alpha_0, \alpha_1)$. For instance, one can use $\pi = \alpha_0 \otimes \alpha_1$, the trivial coupling. An even more special case is the linear interpolation $P_t(x,y)=(1-t)x+ty$. More complex constructions are also possible, provided that sampling from $\pi$ remains simple; stochastic interpolants provide a general framework unifying deterministic flows and diffusions~\cite{Albergo2025StochasticInterpolants}.
...
The same probability-flow intuition is visible in two dimensions. When the data law is a Gaussian mixture, the noising density remains a Gaussian mixture and its score can be evaluated explicitly. This makes it possible to draw backward trajectories without training a neural network.

\begin{figure}[ht]
\centering
\begin{tabular}{@{}cc@{}}
\small forward density evolution &
\small backward probability flow \\[-.15em]
\includegraphics[width=.58\linewidth]{figures/generative-diffusion-2d-forward-backward/forward-density.pdf} &
\includegraphics[width=.30\linewidth]{figures/generative-diffusion-2d-forward-backward/backward-trajectories.pdf}
\end{tabular}
\caption{Two-dimensional diffusion bridge from a three-component Gaussian mixture to an isotropic Gaussian. The left panel shows the forward noising densities of $Z_t=(1-t)X+tY$. The right panel integrates the probability-flow ODE backward from near the Gaussian endpoint; trajectories split toward the three red data modes while keeping the smooth, non-straight geometry characteristic of diffusion-type samplers.}
\label{fig:generative-diffusion-2d-forward-backward}
\end{figure}

\paragraph{When is the induced map optimal?}

Integrating the learned velocity gives a deterministic map from $\alpha_0$ to $\alpha_1$, but this map is not automatically the Brenier optimal map. It is optimal only in special cases where the accumulated flow remains the gradient of a convex potential. For Gaussian marginals, the flow is affine. If the covariance matrices co-diagonalize, the affine map is symmetric positive definite in the common eigenbasis, hence it coincides with the Gaussian Brenier map. More generally, non-commuting covariance dynamics or mixture-like paths can create rotational or shearing components, and the final map need not be optimal even if every instantaneous velocity looks natural. This distinction is closely related to counterexamples showing that flow maps associated with Fokker--Planck or diffusion-type evolutions do not in general provide optimal transport maps~\cite{LavenantSantambrogio2022FlowMap}. In particular, starting from an isotropic Gaussian does not by itself guarantee optimality once the target distribution is non-Gaussian; additional structural assumptions on the path or on the coupling are needed.
...
\end{tabular}
\caption{Diffusion-style sampling trajectories compared with OT rays in a small two-dimensional example. The initial particles are sampled near the center and each trajectory is colored, including its initial disk, by the final target cluster. Curved multi-step trajectories can approach modes through non-straight paths, while the OT map uses straight displacement rays selected by the quadratic matching.}
...
Drifting methods need not start from an exact Wasserstein gradient. They often prescribe a normalized attraction-minus-repulsion field and then regress this field in $L^2(\al_t)$. A simple continuous version uses a positive kernel $K_\epsilon(x,y)$ and defines, for any measure $\nu$,
...
\centering
\begin{tabular}{@{}ccc@{}}
\small unnormalized drift & \small normalized drift & \small normalized field \\[-.15em]
\includegraphics[width=.30\linewidth]{figures/generative-drifting-model-trajectories/unnormalized-trajectories.pdf} &
\includegraphics[width=.30\linewidth]{figures/generative-drifting-model-trajectories/normalized-trajectories.pdf} &
\includegraphics[width=.30\linewidth]{figures/generative-drifting-model-trajectories/normalized-field.pdf}
\end{tabular}
\caption{Drifting fields for a small particle generator. The unnormalized Laplacian-kernel drift has weak long-range attraction and can leave particles away from the data modes. The normalized drift uses the local average $B_\epsilon[\beta]-B_\epsilon[\al_t]$, so particles reach the blue modes while the self-normalized correction prevents collapse. The right panel displays the normalized vector field around an intermediate violet model distribution.}
...
\centering
\begin{tabular}{@{}cc@{}}
\small attention-induced velocity & \small token flow through depth \\[-.15em]
\includegraphics[width=.34\linewidth]{figures/transformer-token-measure-flow/attention-edges.pdf} &
\includegraphics[width=.34\linewidth]{figures/transformer-token-measure-flow/depth-flow.pdf}
\end{tabular}
\caption{Attention as a transportation dynamics on token measures. The schematic uses the attractive setup $Q=K=I$ and velocity $W(X)X-X$, where $W(X)$ is the softmax attention matrix. The left panel shows one layer: faint violet edges encode attention weights and gray arrows show the residual velocity applied to red tokens. The right panel follows the same tokens through residual depth, colored from red to blue, and shows local clustering induced by attention.}
\label{fig:transformer-token-measure-flow}
\end{figure}
...
%
This formulation is related to continuous-depth neural networks~\cite{Chen2018NeuralODE} and was introduced for attention dynamics in the Sinkformers paper~\cite{Sander2022Sinkformers}, building on the transformer architecture of~\cite{Vaswani2017Attention}. It models deep transformers as PDEs on token measures. The key challenge is to understand training, which corresponds to optimizing the time-dependent parameters $(\theta_t)_t$.
