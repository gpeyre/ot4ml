...
Denote by $\Ee_0(f,g)$ the full dual objective, extended by $-\infty$ outside the feasible set. Optimizing out the source potential gives
\eql{\label{eq-semi-dual}
	\MK_\c(\al,\be)=\usup{g\in\Cc(\Yy)}\Ee(g),
	\qquad
	\Ee(g)\eqdef\Ee_0(g^{\bar c},g)
	=\int_\Xx g^{\bar c}\d\al+\int_\Yy g\d\be.
}
Partial maximization of a concave problem preserves concavity, so $\Ee$ is concave. The major advantage of the semi-dual is that it removes the explicit inequality constraint, which allows the use of simpler optimization algorithms.
