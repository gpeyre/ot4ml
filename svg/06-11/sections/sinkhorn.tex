...
\end{tabular}
\caption{Coupling matrices along Sinkhorn iterations for the same one-dimensional setting as Figure~\ref{fig:sinkhorn-dual-potentials-epsilon}, with fixed $\epsilon=0.045$. The panels show the matrix after $\ell$ complete row-column scaling cycles and use a common grayscale normalization. The initial Gibbs kernel is a diagonal-like affinity matrix; scaling bends and reweights it until its row and column sums match the prescribed marginals.}
