...
\index{flow!matching}% strictindex
\textbf{Input:} Interpolant $P_t(u)$, training coupling/sample source $u\sim\pi$, parametrized field $v_\theta(t,z)$.
...
\index{probability-flow ODE}% strictindex
\index{Gaussian mixture}% strictindex
\textbf{Input:} Gaussian-mixture data law, schedule $(a_t,b_t)$, noise level $\sigma$.
...
\]
\textbf{For} each terminal particle \textbf{do}:
\begin{algblock}

\textbf{Draw} $Z_1$ from the Gaussian endpoint.

\textbf{Integrate} $\dot Z_t=v_t(Z_t)$ backward in time.
...
\index{one-step!generative model}% strictindex
\textbf{Input:} Generator $G_{\theta_k}$, latent law $\zeta$, data law $\beta$, step size $\tau$.

\textbf{Output:} Updated generator $G_{\theta_{k+1}}$.

\textbf{Sample} $z\sim\zeta$.

\textbf{Set} $x=G_{\theta_k}(z)$.

\textbf{Estimate} the Wasserstein descent field:
\[
	w_k(x)=-\nabla\delta_\alpha\mathcal E_\beta(\alpha_{\theta_k})(x).
\]
\textbf{Fit} a residual field $U_{\eta_k}$:
\[
	\EE_{z\sim\zeta}\,
...
\index{drifting!model}% strictindex
\textbf{Input:} Particles $x_i^k$ for $\mu_k$, data samples from $\beta$, kernel scale $\epsilon$, step $h$.
...
\]
using~\eqref{eq-normalized-kernel-drift}.
