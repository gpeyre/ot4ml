...
The solution of \eqref{eq:flow-matching} is the conditional expectation of the velocities $\partial_t P_t$. Intuitively, $v_t(z)$ is the average velocity of all trajectories passing through $z$.
%
...
The same probability-flow intuition is visible in two dimensions. For a discrete data law, or more generally for a Gaussian mixture, the noising density is a Gaussian mixture whose score can be evaluated explicitly. This makes it possible to draw backward trajectories without training a neural network. In the plots below, the Gaussian endpoint has covariance $\sigma^2\Id$ to keep the geometry visible at the scale of the three atoms; then $p_t(z)=\sum_j w_j\Gaussian((1-t)c_j,(t\sigma)^2\Id)$, with $s_t=\nabla\log p_t$, and the scaled version of Proposition~\ref{prop:flow} gives $v_t(z)=-(z+t\sigma^2s_t(z))/(1-t)$.

\begin{figure}[ht]
\centering
\includegraphics[width=.72\linewidth]{figures/generative-diffusion-2d-forward-backward/forward-density.pdf}
\caption{Two-dimensional noising path from three Dirac masses to a single Gaussian. The interpolation $Z_t=(1-t)X+tY$ starts from three red atoms and progressively turns them into Gaussian components whose centers move toward the origin and whose covariance grows like $(t\sigma)^2\Id$. At $t=1$ the three components coincide, giving the blue centered Gaussian endpoint.}
...
\begin{proof}
	...
	\]
	which proves~\eqref{eq-gaussian-product-fm-velocity}. To solve the characteristic equation, whiten the source by setting $C=\Sigma_0^{-1/2}\Sigma_1\Sigma_0^{-1/2}$ and $\widetilde Z_t=\Sigma_0^{-1/2}Z_t$. In these coordinates the source covariance is $\Id$ and
	\[
		\widetilde\Sigma_t=(1-t)^2\Id+t^2 C.
	\]
	...
	Both $T_1^{\rm FM}$ and $T^{\rm OT}$ push $\Gaussian(0,\Sigma_0)$ to $\Gaussian(0,\Sigma_1)$. The Brenier map between nondegenerate Gaussians is the unique symmetric positive definite linear map with this property. Hence $T_1^{\rm FM}=T^{\rm OT}$ if and only if $T_1^{\rm FM}$ is symmetric positive definite. Since $C^{1/2}$ is symmetric positive definite,
	\[
		(T_1^{\rm FM})^\top
		=
...
Transformers were introduced as sequence-to-sequence architectures driven by self-attention~\cite{Vaswani2017Attention}, and rapidly became the dominant architecture for language models and representation learning, from bidirectional encoders~\cite{Devlin2019BERT} to large autoregressive models~\cite{Brown2020LanguageModels} and vision transformers~\cite{Dosovitskiy2021Image}. Their distinctive feature is that each token is updated by a data-dependent average of all other tokens. This makes the layer permutation-equivariant before positional encoding, context dependent after conditioning on the input sequence, and naturally compatible with a measure viewpoint in which a prompt is regarded as an empirical distribution of tokens. The same viewpoint is increasingly relevant in the long-context regime, where sparse, block, or low-rank approximations of attention were introduced to reduce the quadratic token cost~\cite{Beltagy2020Longformer,Zaheer2020BigBird,Wang2020Linformer}, while recent token-sample complexity results quantify how an empirical attention map converges to its infinite-token operator~\cite{Bohbot2025TokenSampleComplexity}. This separates uniform convergence of the attention map itself from convergence of moments of the transformed token law, which is often the quantity that propagates through depth.

The mathematical analysis of depth takes a complementary limit: instead of increasing the number of tokens, one lets the number of residual attention layers grow. Continuous-depth neural networks already motivate replacing residual blocks by ODEs~\cite{Chen2018NeuralODE}. For attention, the resulting system is nonlinear in the empirical measure because the velocity of each token is normalized by the whole context. This was first made explicit for doubly stochastic attention in Sinkformers~\cite{Sander2022Sinkformers} and is part of a broader measure-theoretic view of attention as a Lipschitz or interacting-particle operator~\cite{Vuckovic2020MathematicalAttention,Geshkovski2023MathematicalPerspective}. In this language, the infinite-depth limit is a transport or Vlasov-type PDE on token measures. Recent work gives a unified well-posedness and mean-field-limit theory for several attention mechanisms, including softmax, $L^2$, Sinkhorn, sigmoid, multi-head and masked attention~\cite{Castin2025DynamicsTransformers}.

This dynamical viewpoint also explains geometric phenomena observed across depth. With fixed weights, self-attention behaves as a nonlinear interacting particle system whose trajectories can concentrate into clusters or leaders determined by the input context~\cite{Geshkovski2023EmergenceClusters,Geshkovski2023MathematicalPerspective}. Causal masks change this picture by destroying some gradient-flow structures, but clustering can still be proved in masked dynamics~\cite{Karagodin2024CausalMasking}. Related mean-field models on the sphere exhibit metastable clustering phases~\cite{Bruno2024MetastableClustering}. Gaussian closures provide another tractable window: for several attention variants, Gaussian token laws remain Gaussian, so the PDE reduces to ODEs for means and covariances and reveals anisotropy amplification and rank-deficiency mechanisms related to clustering~\cite{Castin2025DynamicsTransformers}. The same measure formalism separates two approximation questions: the depth limit, which is the focus here, and the token limit, where one controls the deviation between a finite context and the limiting attention operator~\cite{Bohbot2025TokenSampleComplexity}.

We now consider very deep transformers, focusing on a single-head attention mechanism for simplicity while ignoring MLP layers, layer normalization, causality, and masking. This stripped-down framework is best suited to modeling encoders and vision transformers, but the preceding literature explains how it can be extended to richer attention mechanisms.
