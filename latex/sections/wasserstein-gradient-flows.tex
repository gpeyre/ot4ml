...
\index{gravity interaction}% strictindex
	Consider the interaction energy
	\begin{equation}\label{eq-quadratic-interaction-momentum}
		f(\mu)
		=
		\frac12\iint k(x,y)\,\d\mu(x)\d\mu(y)
		+
		\int V(x)\,\d\mu(x),
	\end{equation}
	...
	\]
	and therefore
	\begin{equation}\label{eq-quadratic-momentum-force}
		v_\mu(x)
		=
		-\int \nabla_x k(x,y)\,\d\mu(y)-\nabla V(x).
	\end{equation}
...
\]
implemented by chunked PyTorch tensor operations, rather than by differentiating a density on a grid. To keep the entropy-driven expansion in a fixed display window, the simulation adds the quadratic confinement $V_\kappa(x)=\kappa\norm{x}^2/2$, whose force is $-\kappa x$. The displayed dynamics use a small mobility factor $\tau$ only to choose a readable time scale: either the first-order closure $\dot x_i=-\tau\nabla\log\widehat\rho_h(x_i)-\kappa x_i$, or the Newton closure $\dot x_i=s_i$ and $\dot s_i=-\tau\nabla\log\widehat\rho_h(x_i)-\kappa x_i$.
This should be read as a regularized particle discretization of the continuum equation, not as the exact heavy-ball flow of the singular empirical entropy. Figure~\ref{fig:gradflow-second-order-momentum-entropy} shows this KDE-score closure for a longer simulation initialized from a Gaussian mixture with non-identical component variances. Each row displays a trajectory panel on the left and density snapshots at increasing times on the right, all computed from the same full particle cloud.

\begin{figure}[H]
\centering
\newcommand{\entropyclosurepanelheight}{.112\linewidth}
\begin{tabular}{@{}c@{\hspace{.006\linewidth}}c@{\hspace{.006\linewidth}}c@{\hspace{.006\linewidth}}c@{\hspace{.006\linewidth}}c@{\hspace{.006\linewidth}}c@{}}
\small GF & \small $t=0$ & \small $t=1.05$ & \small $t=2.1$ & \small $t=3.15$ & \small $t=4.2$\\[-.25em]
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/no-momentum-trajectories.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/no-momentum-density-t000.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/no-momentum-density-t025.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/no-momentum-density-t050.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/no-momentum-density-t075.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/no-momentum-density-t100.pdf}\\[.25em]
\small Newton & \small $t=0$ & \small $t=1.05$ & \small $t=2.1$ & \small $t=3.15$ & \small $t=4.2$\\[-.25em]
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/newton-trajectories.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/newton-density-t000.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/newton-density-t025.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/newton-density-t050.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/newton-density-t075.pdf}
&
\includegraphics[height=\entropyclosurepanelheight]{figures/gradflow-second-order-momentum-entropy/newton-density-t100.pdf}
\end{tabular}
\caption{\emph{KDE-score particle closure for an entropy-driven inertial flow.} Shannon entropy is not finite on empirical Dirac measures, so the particle dynamics estimate the density by a Gaussian KDE and use its direct particle score $\nabla\log\widehat\rho_h$, here with an added quadratic confining force $-\kappa x$. The left panel of each row shows a farthest-point subset of particle trajectories, while the five panels on the right show density snapshots computed from all particles over the longer time interval $[0,4.2]$. The first row is the overdamped Wasserstein gradient flow; the second row is the undamped Newton law $\ddot x=-\tau\nabla\log\widehat\rho_h(x)-\kappa x$.}
\label{fig:gradflow-second-order-momentum-entropy}
\end{figure}

\section{Normalized Spectral Wasserstein Dynamics}
...
\end{equation}
This is the infinitesimal analogue of the displacement covariance used in the spectral distance $\Wass_\gamma$ in Definition~\ref{def-spectral-wasserstein}. It measures the covariance of the velocity field before applying the gauge. Thus the geometry is global in direction: two velocities with the same pointwise $L^2$ norm may have different costs if their velocity covariance is distributed differently across directions.
...
\index{JKO scheme}% strictindex
\index{minimizing movement}% strictindex
The normalized dynamics can be obtained formally from the JKO scheme associated with the spectral distance:
