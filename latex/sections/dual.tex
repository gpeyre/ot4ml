...
\small shifted target &
\small unequal widths \\[-.15em]
...
\includegraphics[width=.30\linewidth]{figures/dual-kantorovich-discrete-potentials/target-unequal.pdf}
\end{tabular}
\caption{Discrete Kantorovich dual potentials for the quadratic cost $C_{i,j}=|x_i-y_j|^2$.  In each panel the upper strip shows the fixed source histogram in red and the target histogram in blue.  The lower strip shows an optimal pair of dual vectors $(\fD,\gD)$, with gauge chosen so that $\dotp{\fD}{\a}=0$.  Complementary slackness states that mass can be transported only through entries where $\fD_i+\gD_j=C_{i,j}$.}
...
\]
The target $j_i$ is then assigned to $i$, and its previous owner, if any, becomes unassigned. The iteration stops when all sources are assigned.
...
Keeping a dual potential $\f$ fixed, one can maximize in closed form over the second potential in the dual problem~\eqref{eq-dual-generic}, which leads to consider
\eq{
...
\centering
\begin{tabular}{@{}cc@{}}
\small source-side closure & \small target-side closure \\[-.15em]
\includegraphics[width=.46\linewidth]{figures/dual-alternating-c-transform-failure/source-concave-closure.pdf} &
\includegraphics[width=.46\linewidth]{figures/dual-alternating-c-transform-failure/target-concave-closure.pdf}
\end{tabular}
\caption{Hard $c$-transforms for the bilinear cost $c(x,y)=-xy$. Red curves are deliberately non-concave initial potentials $f$ and $g$. Violet curves are the double-transform closures $f^{c\bar c}$ and $g^{\bar c c}$, which are concave majorants. Dashed blue curves are the one-sided best responses $g^{\bar c}$ and $f^c$, displayed after a harmless vertical gauge shift. The figure illustrates why exact alternating best responses are useful for dual certificates but do not give the smooth iterative dynamics later provided by entropic regularization.}
