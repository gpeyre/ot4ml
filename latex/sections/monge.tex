...
\end{tabular}
\caption{Jacobian determinant in the density push-forward formula. A smooth diffeomorphism compresses the middle of a colored square grid and then applies a slight rotation. The source density is uniform, but the target density, rendered as a blue image with contours of $\rho_\be$, forms a central bump where the deformed grid cells have smaller area. This illustrates the factor $|\det \T'(x)|^{-1}$ in~\eqref{eq-pfwd-density}.}
