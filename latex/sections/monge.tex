...
This can be applied in particular to the subspace of continuous functions that are measurable.
%
...
%
On compact spaces, the converse is true, namely that any continuous linear form $\ell : f \mapsto \ell(f)$ on $(\Cc(\Xx),\norm{\cdot}_\infty)$ is represented as an integral against a measure $\ell(f)=\int f \d \al$. This is the  Riesz-Markov-Kakutani representation theorem, which is often stated that Borel measures can be identified with Radon measures.
%
...
%
This duality pairing $\dotp{f}{\al}$ between continuous functions and measures will be crucial to developing duality theory for the convex optimization problem we will consider later. 
...
In general, an optimal map $T$ solving~\eqref{eq-monge-continuous} might fail to exist. The constraint set $\{ T : \T_\sharp \al = \be \}$ might be empty, which is the case for instance if $\al=\de_x$ and $\be$ is not a single Dirac. Intuitively, the support of $\alpha$ should be ``larger'' than the one of $\be$ for this constraint to be non-empty. One can show that if $\alpha$ is non atomic, i.e. $\alpha(\{x\})=0$ for all $x$, then $\{ T : \T_\sharp \al = \be \} \neq \emptyset$ for all $\beta$. This is the case in $\Xx=\RR^d$ if $\alpha$ has a density with respect to Lebesgue measure, which is the key hypothesis of Brenier's theorem~\ref{thm-brenier}.
