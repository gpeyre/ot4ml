...
Figure~\ref{fig:monge-jacobian-pushforward-density} makes the determinant factor visible. Even when the source density is uniform, a nonlinear diffeomorphism can compress area in one region and expand it elsewhere; the target density is therefore not obtained by the naive composition $\rho_\al\circ\T^{-1}$ alone. The high-density band appears exactly where the deformed grid cells have small area.
...
\end{tabular}
\caption{Jacobian determinant in the density push-forward formula. A smooth diffeomorphism bends a colored square grid and compresses a narrow vertical region. The source density is uniform, but the target density, rendered as a blue image with contours of $\rho_\be$, becomes larger where the deformed grid cells have smaller area. This illustrates the factor $|\det \T'(x)|^{-1}$ in~\eqref{eq-pfwd-density}.}
...
\begin{rem}[Non-quadratic convex costs]
