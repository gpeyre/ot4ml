...
\textbf{Initialize:} Choose $\P^{(0)}\in\CouplingsD(\a,\b)$.
...
\textbf{Initialize:} Hermitian potentials $F^{(0)}$ and $G^{(0)}$.
...
\textbf{Initialize:} Set $U,V\succ0$.
...
\textbf{Until} both operator-marginal residuals are at most $\mathrm{tol}$.
