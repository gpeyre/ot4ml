...
\textbf{Choose} cyclic orderings $(x_{(k)})_{k=1}^n$ and $(y_{(k)})_{k=1}^n$.
...
\]
\end{algblock}
\textbf{Set} $s^\star\in\argmin_sE_s$.

\textbf{Open} circle at a gap compatible with $s^\star$.

\textbf{Lift} all points to the unfolded interval.
...
\textbf{Initialize:} Choose dual feasible labels $(u,v)$.
...
\]
\textbf{Choose} unmatched source.

\textbf{Grow} an alternating tree in $E(u,v)$.

\textbf{If} the tree reaches an unmatched target \textbf{then}:
\begin{algblock}

\textbf{Augment} $M$ along the alternating path.
...
\textbf{Set} $S=$ reached sources and $T=$ reached targets.
