...
Figure~\ref{fig:kantorovich-dro-ambiguity} gives the corresponding geometric picture for binary logistic classification. A Wasserstein ball is not just a scalar uncertainty level: it permits an adversary to move data points in feature space, so increasing the radius forces the classifier to account for a thicker neighborhood around its decision boundary.
...
{\small large $\rho$}
\end{tabular}
\caption{Wasserstein distributional robustness as a geometric ambiguity set for logistic classification. Red and blue dots are the two classes, the black line is the learned linear decision boundary and the faint background shows the logistic score. From left to right, the classifier is trained with no robustness, a medium $\Wass_2$ radius and a larger $\Wass_2$ radius. The violet strip is the Euclidean $\rho$-neighborhood of the boundary, used as a local proxy for feature perturbations; the actual Wasserstein constraint is a global average transport budget.}
