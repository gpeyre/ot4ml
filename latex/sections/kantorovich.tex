...
\]
\textbf{Run} Algorithm~\ref{alg:north-west-corner} on sorted weights $(\a_i)_i$ and $(\b_j)_j$.
...
\textbf{Find} a permutation $\sigma$ supported by positive entries:
\[
	R_{i,\sigma(i)}>0
	\qquad\text{for all }i.
\]
...
\textbf{Solve} quadratic Kantorovich problem.

\textbf{Choose} $\pi^\star$ among the minimizers.
