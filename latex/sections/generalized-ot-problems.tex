...
\\[-.1em]
\small horizontal rank one, $A=-e_1e_1^\top$ &
\small vertical rank one, $A=-e_2e_2^\top$
\\[.35em]
...
\end{tabular}
\caption{Forward solutions of the bilinear cost $c_A(x,y)=\dotp{Ax}{y}$ on the OT4ML logo point clouds. Each panel solves the equal-weight assignment problem with a different matrix $A$; the source atoms are red, the target atoms are blue, and the gray segments give one deterministic optimal bijection.}
...
\centering
\begin{tabular}{@{}ccc@{}}
\includegraphics[width=.35\linewidth]{figures/inverse-ot-gap-loss/observed-map-n20.pdf} &
\includegraphics[width=.29\linewidth]{figures/inverse-ot-gap-loss/gap-loss-n20.pdf} &
\includegraphics[width=.29\linewidth]{figures/inverse-ot-gap-loss/gap-loss-n100.pdf}
\\[-.1em]
\small observed map, $n=20$ &
\small loss curve, $n=20$ &
\small loss curve, $n=100$
\end{tabular}
\caption{Inverse-OT gap loss for a bilinear cost. Panel (a): two empirical mixtures of two Gaussians are matched with the cost $c_{A_\star}(x,y)=\dotp{A_\star x}{y}$ for $A_\star=-I$, which gives the same optimizer as the quadratic $\Wass_2$ cost; the faint ellipses indicate the sampling components. Panels (b,c): the unregularized Fenchel--Young Kantorovich gap $\mathcal L_{\mathrm{iOT}}(C(A_t);\widehat P)$ along $A_t=-\diag(1+t,1-t)$ for $n=20$ and $n=100$, using the same vertical scale. The red dot marks the generating parameter $t=0$; the curves are convex and piecewise affine.}
