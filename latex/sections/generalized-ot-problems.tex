...
\]
\textbf{Initialize:} Choose $\cov^{(0)}\succ0$.
...
\textbf{Initialize:} Set $\K_s=e^{-\C_s/\epsilon}$ and choose $\uD_s^{(0)}\in\RR_{+,*}^n$.
...
\textbf{Input:} Marginals $\a_s\in\simplex_{n_s}$, tensor cost $C$, regularization $\epsilon>0$.
...
\]
\textbf{Initialize:} Set $u_s\in\RR_{+,*}^{n_s}$ for all $s$.
...
\textbf{Until} all marginal residuals are below $\mathrm{tol}$.
...
\textbf{Output:} Fitted cost $C_{\theta^\star}$ and potentials $(f^\star,g^\star)$.
