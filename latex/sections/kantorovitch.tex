...
Kantorovitch, aiming for the planification of the economy, made a strong simplifying assumption: the cost of transportation should be linear in the amount of transport mass. 
%
...
%
In this case, one can solve Monge's optimal matching problem~\eqref{eq-optimal-assignment}, and it is convenient to rewrite it using permutation matrices. 
%
...
\begin{proof}
	...
	%
	\removed{As shown in figure~\ref{fig-bipartite}, }
	...
	so that $0 < \epsilon < 1$. \removed{As shown in Figure~\ref{fig-extremal}, right}
...
Optimal transport plans behave well when one looks at any finite
sub-collection of points in their support: the restriction is still an
optimal matching for those points alone. This simple remark, which is obvious for finitely supported marginals, leads to the notion of $c$-cyclical Monotonicity.

\paragraph{Support and $c$-cyclical monotonicity.}

To formalise this, one needs a precise notion of \emph{support}, i.e.\ the set that carries the mass of the coupling.  
Because we work with Radon measures, the support is automatically closed.

\begin{defn}[Support]\label{def:support}
For a Radon measure $\pi$ on $\mathcal{X}\times \mathcal{Y}$
\[
  \operatorname{supp}\pi
  :=\bigl\{(x,y)\;:\;
  \pi(U\times V)>0 \text{ for every open }U\ni x,\;V\ni y\bigr\}.
\]
\end{defn}

\begin{defn}[$c$-cyclical monotonicity]\label{def:ccm}
A set $\Gamma\subset \mathcal{X}\times \mathcal{Y}$ is \emph{$c$-cyclically monotone} if,  
for every $k\ge2$, every finite family
$\{(x_i,y_i)\}_{i=1}^k\subset\Gamma$, and every permutation
$\sigma\in\mathfrak{S}_k$,
\[
  \sum_{i=1}^k c(x_i,y_i)\;\le\;
  \sum_{i=1}^k c\bigl(x_i,y_{\sigma(i)}\bigr).
\]
Any permutation is a product of cycles, so it suffices to verify the
inequality for cycles, 
\[
  \sum_{i=1}^k c(x_i,y_i)\;\le\;
  \sum_{i=1}^k c\bigl(x_i,y_{i+1}\bigr).
\]
\end{defn}

\paragraph{Optimal marching to optimal matching.}

Let the marginals be uniform on $n$ points: $\alpha=\tfrac1n\sum_{i=1}^n\delta_{x_i}$, $\beta =\tfrac1n\sum_{i=1}^n\delta_{y_i}$.
In this case, there exist solutions to the Kantorovitch problem that are permutations. 
An such permutation defines, by definition of optimality, a coupling with support $\Gamma := (x_i,y_{\sigma(i)})_i$ that is cyclical $c$-cyclical monotone. 
...
$\operatorname{supp}\pi$ is $c$-cyclically monotone.
\end{thm}
...
$T:\mathcal{X}\to \mathcal{Y}$, i.e.\ $\pi=(\mathrm{id},T)_\#\alpha$.  
