...
\end{tabular}
\caption{Principal components in linear OT coordinates for MNIST digit-zero histograms. The left panel shows the Sinkhorn barycenter reference \(\rho\). Each mode panel is a triptych obtained by pushing \(\rho\) through \(x\mapsto x+\bar z(x)+a e_k(x)\) with a negative, zero, and positive coefficient \(a\). The displayed densities use black for zero mass and white for high mass. The modes capture rotations, aspect-ratio changes, and stroke-thickness deformations in the tangent chart around the barycenter.}
\index{linear!OT}% strictindex
\index{principal component analysis}% strictindex
