...
\textbf{Output:} Lifted feasible coupling $\pi_{\theta^\star}$.

\textbf{For} each $\theta\in\Theta$ \textbf{do}:
\begin{algblock}

\textbf{Sort} projected values $\dotp{\theta}{x_i}$ and $\dotp{\theta}{y_j}$.

\textbf{Match} equal ranks.

\textbf{Store} permutation $\sigma_\theta$.
...
\]
\end{algblock}
\textbf{Set} $\theta^\star\in\argmin_{\theta\in\Theta}E(\theta)$.
