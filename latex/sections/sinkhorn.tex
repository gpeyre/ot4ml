...
\textbf{Input:} Weights $\a,\b$, cost matrix $\C$, regularization $\epsilon>0$.

\textbf{Output:} Entropic coupling $\P$.

\textbf{Initialize:} Set $\K_{ij}=e^{-\C_{ij}/\epsilon}$ and choose $\vD^{(0)}\in\RR_{+,*}^m$.
...
\textbf{Solve} static entropic endpoint problem:
\[
	\pi_\epsilon^\star
	\in
...
\textbf{Initialize:} Choose $\gD^{(0)}\in\RR^m$.
...
\]
\textbf{If} potential changes are below $\mathrm{tol}$ \textbf{then}:
\begin{algblock}
