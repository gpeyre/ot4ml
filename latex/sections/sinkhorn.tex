...
\index{KL!proximal operator}% strictindex
\index{total variation!marginal penalty}% strictindex
...
\small larger $\tau$
\end{tabular}
\caption{Geodesics-in-heat approximation of the distance to three red source points on a two-dimensional grid. The first panel shows the exact Euclidean distance. The other panels diffuse each source by one backward-Euler resolvent step with Neumann boundary conditions, recover a distance by the normalized-gradient Poisson solve, and take the lower envelope over the three sources. Each panel is robustly normalized to emphasize the geometry of its level sets. Increasing the resolvent scale $\tau$ stabilizes the sparse linear solves but rounds and merges small-scale distance features.}
\index{geodesics in heat}% strictindex
...
The same heat kernel also explains why log-sum-exp is the entropic version of a hard transform. For $t>0$, the quadratic Hopf--Lax transform is
\[
	Q_t f(x)=\inf_y \left\{ f(y)+\frac{\norm{x-y}^2}{2t}\right\}.
\]
After completing the square, this is equivalent to the Legendre--Fenchel transform
\[
	Q_t f(x)=\frac{\norm{x}^2}{2t}-
	\left(f+\frac{\norm{\cdot}^2}{2t}\right)^*\!\left(\frac{x}{t}\right).
\]
Its entropic version replaces the supremum in this transform by a log-sum-exp, or equivalently by a Gaussian convolution at heat time $\epsilon t/2$:
\[
	Q_t^\epsilon f(x)
	=
	-\epsilon\log
	\int
	\exp\!\left(
	-\frac{f(y)+\norm{x-y}^2/(2t)}{\epsilon}
	\right)\d y .
\]
Laplace's principle gives $Q_t^\epsilon f\to Q_t f$ as $\epsilon\to0$, up to an additive constant. Equivalently,
\[
	Q_t^\epsilon f(x)
	=
	-\epsilon\log\Big(G_{\epsilon t/2}\ast e^{-f/\epsilon}\Big)(x)
	+\text{\upshape cst},
\]
where $G_s$ denotes the normalized heat kernel at time $s$; the additive constant absorbs its normalization and depends only on $t$, $\epsilon$ and the dimension. This formula is the continuous analogue of the soft $c$-transform used later in the Sinkhorn dual.
\index{heat!kernel}% strictindex
\index{Legendre-Fenchel transform}% strictindex
\index{soft!c-transform}% strictindex

The Hopf--Cole transform gives the corresponding PDE linearization. If $\nu>0$ and
\index{Hopf-Cole transform}% strictindex
\[
	\partial_t\phi+\frac12\norm{\nabla\phi}^2=\nu\Delta\phi,
\]
then $u=e^{-\phi/(2\nu)}$ solves the linear heat equation $\partial_tu=\nu\Delta u$. Hence $v=\nabla\phi=-2\nu\nabla\log u$ solves the viscous Burgers equation
\[
	\partial_t v+(v\cdot\nabla)v=\nu\Delta v .
\]
Thus nonlinear Hamilton--Jacobi/Burgers dynamics can be computed through a Laplacian-generated heat flow followed by a logarithmic change of variables. This is also the conceptual bridge between soft-minimum formulas, Gaussian convolution and entropic transport potentials.
...
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}ccc@{}}
\includegraphics[width=.30\linewidth]{figures/sinkhorn-hopf-cole-transform/soft-hopf-lax.pdf} &
\includegraphics[width=.30\linewidth]{figures/sinkhorn-hopf-cole-transform/heat-variable.pdf} &
\includegraphics[width=.30\linewidth]{figures/sinkhorn-hopf-cole-transform/burgers-velocity.pdf} \\[-.1em]
\small soft Hopf--Lax &
\index{Hopf-Lax formula}% strictindex
\small heat variable $u_t$ &
\small Burgers velocity $v_t$
\end{tabular}
\caption{Hopf--Cole and heat-kernel numerics in one dimension. Left: the hard Hopf--Lax/Legendre transform is approximated by Gaussian log-sum-exp transforms at two temperatures; the pale curve is the input potential shifted downward for reference. Middle: the Hopf--Cole variable $u=e^{-\phi/(2\nu)}$ evolves linearly by the heat equation. Right: the Burgers velocity $v=-2\nu\,\partial_x\log u$ recovered from the same heat evolution smooths shocks while preserving the red-to-blue time ordering.}
\index{Hopf-Cole transform}% strictindex
\index{Burgers equation}% strictindex
