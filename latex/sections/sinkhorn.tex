...
\begin{rem}[Mutual-information view]\label{rem-schrodinger-mutual-information}
	If $(X,Y) \sim \pi$ have marginals $X \sim \al$ and $Y \sim \be$, then $\KL(\pi|\al \otimes \be) = \Ii(X,Y)$ is the mutual information of the endpoint pair, which is $0$ if and only if $X$ and $Y$ are independent. The entropic problem~\eqref{eq-entropic-generic} is thus equivalent to
	\[
...
The previous construction regularizes OT by a density-ratio divergence. This differs from using a Bregman divergence generated by a convex functional on the space of measures. If $\Phi$ is a differentiable convex functional and $\xi$ is a reference measure in its domain, its measure Bregman divergence is
...
\end{equation}
where $\delta\Phi(\xi)$ is the first variation, in the sense of Chapter~\ref{sec-wasserstein-gradient-flows}. In finite dimension this reduces to Definition~\ref{def-bregman-divergence}. The KL divergence is special because it is simultaneously a density-ratio divergence and a Bregman divergence.
...
The raw entropic OT value has a large-temperature attraction toward the product coupling. The standard debiasing subtracts the two self-interaction energies, in the same spirit as passing from a positive kernel value to the associated squared distance. This gives the Sinkhorn divergence
