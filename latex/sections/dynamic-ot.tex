...
\begin{equation}\label{eq:least-square-field}
	...
	\partial_t\alpha_t+\diverg(\alpha_t v_t)=0.
\end{equation}
Introducing a Lagrange multiplier $\psi_t$ for the continuity equation gives the saddle problem
\[
	\umin{v}\umax{\psi}
	\int_0^1
	\left[
		\frac12\int_{\RR^d}\norm{v_t(x)}^2\,\d\alpha_t(x)
		+\int_{\RR^d}\psi_t(x)\big(\diverg(\alpha_t v_t)(x)+\partial_t\alpha_t(x)\big)\d x
	\right]\d t.
\]
After integrating by parts in the divergence term, the minimization in $v_t$ gives $v_t=\nabla\psi_t$. Equivalently, when $\alpha_t=\rho_t\,\d x$ is smooth and positive, the minimizing velocity is the gradient field $v_t=\nabla\psi_t$ where
\begin{equation}\label{eq:least-square-field-explicit}
	-\diverg(\rho_t\nabla\psi_t)=\partial_t\rho_t,
	\qquad
	v_t=-\nabla\Delta_{\alpha_t}^{-1}(\partial_t\alpha_t),
	\quad
	\Delta_{\alpha_t}\psi=\diverg(\alpha_t\nabla\psi).
\end{equation}
