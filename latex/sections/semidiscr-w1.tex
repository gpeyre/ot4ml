...
\begin{figure}[H]
\centering
\includegraphics[width=.70\linewidth]{figures/w1-graph-transport-flow/flow.pdf}
\index{graph!transport}% strictindex
\caption{Graph Beckmann formulation of $\Wass_1$ on a nonuniform Delaunay graph $G=(V,E)$ whose vertices are sampled from a two-component Gaussian mixture.  Red and blue disks encode the positive and negative parts of $r=\alpha-\beta$, localized on six vertices each.  The darker graph edges show the triangulation, while the violet arrows display the optimal signed edge flow $m$: orientation gives the sign, width is proportional to $\sqrt{|m_e|}$, and the flow satisfies $\operatorname{div}_G m=r$.}
