...
\textbf{Find} the active Laguerre cell:
\[
	j_\ell\in\argmin_j\bigl(c(x_\ell,y_j)-\gD_j^{(\ell)}\bigr).
\]
...
\textbf{Input:} Source measure $\alpha$, number of codepoints $m$, squared Euclidean cost.

\textbf{Output:} Codepoints $Y=(y_j)_{j=1}^m$.

\textbf{Initialize:} Choose $Y^{(0)}=(y_j^{(0)})_{j=1}^m$.
...
\textbf{Keep or reseed} $y_j^{(k+1)}$.

\end{algblock}
\textbf{If} codepoint displacement is below $\mathrm{tol}$ \textbf{then}:
\begin{algblock}
...
\]
\textbf{Initialize:} Choose a feasible spanning-tree basis, tree flow, and node potentials.

\textbf{While} some non-tree arc has negative reduced cost \textbf{do}:
\begin{algblock}

\textbf{Choose} entering arc with negative reduced cost.

\textbf{Add} it to the tree.

\textbf{Set} $\mathcal C=$ unique induced cycle.

\textbf{Send} the largest admissible flow around $\mathcal C$.

\textbf{Remove} the leaving arc that becomes tight.
