...
\textbf{Input:} Source measure $\alpha$, initial codepoints $Y^{(0)}=(y_j^{(0)})_{j=1}^m$, squared Euclidean cost.

\textbf{Output:} Codepoints $Y=(y_j)_{j=1}^m$.

\textbf{Initialize:} Set $k=0$.
...
\textbf{Remove} the first zero-flow arc met on the directed cycle after augmentation.
