...
\(\dotp{f_\epsilon}{G_a}\), whose variance is the displayed covariance formula. To justify the bias expansion, fix a relative-interior neighborhood \(U\) of \(a\) on which the Hessian is bounded. A multinomial concentration bound gives \(\PP(\hat a_N\notin U)\leq C e^{-cN}\). On \(U\), Taylor's formula has a centered linear term and a quadratic remainder bounded by \(C\norm{\hat a_N-a}^2\), whose expectation is \(O(N^{-1})\). On the exponentially unlikely complement, the entropic value remains bounded by a constant depending only on \(\C\) and \(\epsilon\); the natural definition on a boundary face simply discards zero-weight atoms. This proves the \(O(N^{-1})\) bias. The same expansion, together with bounded moments of the multinomial fluctuation, gives convergence of the rescaled variance. The two-sample formula follows by applying the argument independently to the two marginals.
\end{proof}
