...
KL and total variation are two very different $\phi$-divergences, yet the former controls the square of the latter.
\begin{thm}[Pinsker inequality]\label{thm-pinsker}
Let $\al,\be\in\Mm_+^1(\Xx)$. With the full-variation convention,
\[
\TV(\al|\be)^2=\norm{\al-\be}_{\TV}^2\leq2\KL(\al|\be).
\]
On a finite space, this reads $\norm{\a-\b}_{\ell^1}^2\leq2\KLD(\a|\b)$ for $\a,\b\in\simplex_n$.
\end{thm}
\begin{proof}
If the KL divergence is finite, let $A$ be a positive Hahn set for $\al-\be$ and set $a=\al(A)$, $b=\be(A)$, so that $a-b=\frac12\norm{\al-\be}_{\TV}$. Coarse-graining onto $(A,A^c)$ gives
\[
\KL(\al|\be)\geq a\log\frac ab+(1-a)\log\frac{1-a}{1-b}\geq2(a-b)^2,
\]
where the last inequality follows because subtracting $2(a-b)^2$ gives a function of $a$ that vanishes to second order at $a=b$ and has second derivative $1/a+1/(1-a)-4\geq0$. Boundary cases follow by lower semicontinuity. The finite-space formula uses $\norm{\al-\be}_{\TV}=\norm{\a-\b}_{\ell^1}$.
\end{proof}
