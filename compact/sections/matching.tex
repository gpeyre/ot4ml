...
We now present a first algorithm for solving the optimal matching problem. The Hungarian method gives a self-contained preview of duality by computing an assignment together with an optimality certificate. The auction algorithm is postponed to Section~\ref{sec-auction-dual-ascent}, after the discrete semi-dual has been introduced.

\paragraph{Hungarian primal-dual method.}
The Hungarian method constructs a partial matching and a dual certificate together. The certificate can be isolated as follows.
\begin{prop}[Dual certificate for an assignment]\label{prop-assignment-dual-certificate}
If $\fD_i+\gD_j\leq\C_{i,j}$ for all $i,j$, then every $\tau\in\Perm(n)$ satisfies
\(\sum_i\C_{i,\tau(i)}\geq\sum_i\fD_i+\sum_j\gD_j.\)
If equality holds on every edge $(i,\sigma(i))$ of a permutation $\sigma$, then $\sigma$ is optimal and $(\fD,\gD)$ is dual optimal.
\end{prop}
\begin{proof}
Sum $\C_{i,\tau(i)}\geq\fD_i+\gD_{\tau(i)}$ over $i$ and use that $\tau$ permutes the target indices. Tightness along $\sigma$ makes this lower bound equal to its assignment cost.
\end{proof}
This is the uniform-mass instance of the discrete dual in Section~\ref{sec-discrete-dual}. The equality graph contains the zero-slack edges $(i,j)$ satisfying $\fD_i+\gD_j=\C_{i,j}$. By Proposition~\ref{prop-assignment-dual-certificate}, a perfect matching in this graph is optimal. Starting at an unmatched source, an alternating tree follows equality edges to targets and matched edges back to sources. Reaching an unmatched target gives an augmenting path. If no equality edge leaves the reached source set $S$ toward $T^c$, use
\[
...
\]
which preserves feasibility and creates a new equality edge. Since $|S|=|T|+1$, this shift increases the dual objective by $\delta$. Section~\ref{sec-auction-dual-ascent} contrasts this exact augmenting-tree mechanism with the $\epsilon$-relaxed auction ascent.
\begin{prop}[Correctness and complexity of the Hungarian primal-dual method]\label{prop-hungarian-correct}
For every $\C\in\RR^{n\times n}$, the Hungarian method terminates after $n$ augmentation phases and returns an optimal permutation $\sigma$. With maintained slacks, it uses $O(n^3)$ arithmetic and comparison operations and $O(n^2)$ storage.
\end{prop}
\begin{proof}
Initialize $\fD_i=\min_j\C_{i,j}$ and $\gD_j=0$. Feasibility holds and the empty matching uses only equality edges. Before reaching a free target, the tree has $|S|=|T|+1$, so $T$ omits a target. A dual shift leaves slacks unchanged on $S\times T$, decreases them by $\delta$ on $S\times T^c$, and increases them on $S^c\times T$. Hence it preserves feasibility and tight matched edges while creating a new equality edge. The tree therefore reaches a free target in at most $n$ expansions; flipping the resulting alternating path adds one matched pair. Exactly $n$ phases produce a perfect matching $\sigma$. Its edges are tight, so Proposition~\ref{prop-assignment-dual-certificate} proves optimality. Maintaining $\ell_j=\min_{i\in S}(\C_{i,j}-\fD_i-\gD_j)$ and a minimizing parent costs $O(n)$ per tree expansion: when a source enters $S$, one compares its row of slacks with the current $\ell_j$. Hence one phase costs $O(n^2)$ and all $n$ phases cost $O(n^3)$.
\end{proof}
