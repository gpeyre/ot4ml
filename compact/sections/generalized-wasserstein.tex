...
\end{prop}
\begin{proof}
Write $\alpha=r_0\bar\alpha$, $\beta=r_1\bar\beta$ with normalized Gaussian
$\bar\alpha,\bar\beta$. If a plan of mass $M>0$ is written as $M\bar\pi$
with normalized marginals $\tilde\alpha,\tilde\beta$, then
\[
\KL(M\tilde\alpha\mid r_0\bar\alpha)
=M\KL(\tilde\alpha\mid\bar\alpha)+M\log(M/r_0)-M+r_0.
\]
Hence, with
$\mathcal A=\Wass_2^2(\tilde\alpha,\tilde\beta)
+\tau\KL(\tilde\alpha\mid\bar\alpha)
+\tau\KL(\tilde\beta\mid\bar\beta)$, minimizing in $M$ gives
$M=\sqrt{r_0r_1}e^{-\mathcal A/(2\tau)}$ and value
$\tau(r_0+r_1)-2\tau M$.

Gelbrich's inequality and moment matching show that replacing each normalized
marginal by the Gaussian with the same mean and covariance cannot increase
$\mathcal A$. The mean stationarity equations give a minimum
$\frac{\tau}{2}\norm{m_0-m_1}_{X_\tau^{-1}}^2$. For the covariances, set
$\eta=2/\tau$, $A_i=\Sigma_i^{-1}$, $C_i=A_i+\eta\Id$, and let $L$ be the
positive solution of $LC_1L=C_0$. With
$P=[A_0+\eta(\Id-L)]^{-1}$ and $Q=LPL$, the covariance stationarity equations
hold. Their traces cancel the Bures term against the trace terms in the two
Gaussian KL divergences, leaving
\[
\mathcal A_{\rm cov}
=\frac{\tau}{2}\log\frac{\det\Sigma_0\det\Sigma_1}{\det P\det Q}
=\tau\log\det J_\tau.
\]
Thus $M=\sqrt{r_0r_1}\mathfrak a_\tau$, which gives
\eqref{eq-gaussian-ghk-closed-form}. Quadratic KL-dual potentials obtained from
the density ratios of these adjusted Gaussian marginals satisfy
$\varphi(x)+\psi(y)\leq\norm{x-y}^2$, with equality on the Gaussian optimal
transport graph, and certify global optimality.
\end{proof}
...
For $\alpha=\Gaussian(m_\alpha,\Sigma_\alpha)$ and
...
+\int_{\Sphere^{d-1}}
\pa{\sqrt{\transp{\theta}\Sigma_\alpha\theta}
-\sqrt{\transp{\theta}\Sigma_\beta\theta}}^2\d\sigma(\theta)
...
\end{align}
One always has $\SW_2^2\leq\Wass_2^2/d$. For positive-definite covariances,
equality holds exactly when they are proportional. More generally, if
$\Sigma_\alpha=s^2\Sigma$ and $\Sigma_\beta=t^2\Sigma$, then
$\SW_2^2=\Wass_2^2/d=
d^{-1}[\norm{m_\alpha-m_\beta}^2+(s-t)^2\tr(\Sigma)]$.
Thus two isotropic covariances saturate the bound, but one isotropic covariance
does so only when the other is also isotropic. For
$\Sigma_\alpha=a^2u\transp{u}$, $\Sigma_\beta=b^2v\transp{v}$, and
$\chi=\abs{\dotp{u}{v}}$,
\[
\SW_2^2=\frac1d\left[\norm{m_\alpha-m_\beta}^2+a^2+b^2
-\frac{4ab}{\pi}\pa{\sqrt{1-\chi^2}+\chi\arcsin\chi}\right],
\quad
