...
\end{equation}
The sign follows from the one-dimensional identity: the first variation of $\frac12\Wass_2^2(\rho,\nu)$ has spatial derivative $s-T(s)$, so the descent velocity is $T(s)-s$, and composing with $P_\theta$ lifts it in the direction $\theta$. Thus empirical implementations only require one-dimensional sorting along sampled directions, followed by averaging the lifted projected displacements.
...
\(\frac{\d}{\d t}\mathcal C_\al(\rho_t)\Big|_{t=0^+} \geq \int \dotp{T(x)-x}{\nabla u(x)}\,\d\rho(x),\)
...
The following centered catalogue tracks representative cases where the Wasserstein gradient is linear on Gaussian inputs. Thus Gaussianity is a conclusion of the flow, not an ansatz imposed in advance.
\begin{prop}[Centered Gaussian closure catalogue]\label{prop-centered-gaussian-covariance-catalogue}
Let $\gamma=\Gaussian(0,\Id)$ and let $\al_0=\Gaussian(0,C_0)$ with $C_0\succ0$. For each row below, let $\al_t$ be the corresponding Wasserstein gradient flow initialized at $\al_0$. Then the centered Gaussian family is invariant: as long as $C_t\succ0$,
\[
\al_t=\Gaussian(0,C_t),
\qquad
\dot C_t=H(C_t),
...
\mathcal I(\al|\gamma) &:& H(C)=4(C^{-1}-C), \\[.35em]
\Wass_2^2(\al,\gamma) &:& H(C)=4(C^{1/2}-C), \\[.35em]
\MMD_k^2(\al,\gamma),\quad k(x,y)=\dotp{x}{y}^2
&:& H(C)=8(C-C^2), \\[.35em]
\bar\MK_{\norm{\cdot-\cdot}^2}^{\epsilon}(\al,\gamma)
&:& H(C)=
4\left(C+\frac{\epsilon^2}{16}\Id\right)^{1/2}
-4\left(C^2+\frac{\epsilon^2}{16}\Id\right)^{1/2}, \\[.75em]
\SW_2^2(\al,\gamma)
&:& H(C)=V(C)C+CV(C),
\end{array}
\]
where $\bar\MK_{\norm{\cdot-\cdot}^2}^{\epsilon}$ is the debiased Sinkhorn divergence for the quadratic cost $\norm{x-y}^2$ and KL regularization strength $\epsilon$, and
\[
V(C)=
2\int_{\Sphere^{d-1}}
...
\end{prop}
\begin{proof}
Each row is obtained by computing the ambient Wasserstein first variation on centered Gaussian inputs and identifying the affine descent velocity $v(x)=M_Cx$. Proposition~\ref{prop-gaussian-affine-closure} then gives $\dot C=M_CC+CM_C^\top$. For $\KL(\cdot|\gamma)$, the Fokker--Planck velocity is $(C^{-1}-\Id)x$, hence $\dot C=2(\Id-C)$. For the Fisher row, the ambient first variation is quadratic on centered Gaussian inputs; equivalently, the restriction of $\frac12\mathcal I$ to centered Gaussians is
\[
\frac12\left(\tr(C)+\tr(C^{-1})-2d\right).
\]
Multiplying by two for $\mathcal I$ gives the descent velocity $2(C^{-2}-\Id)x$, hence $\dot C=4(C^{-1}-C)$. Although the ambient Fisher flow is a fourth-order PDE for a generic density, its velocity is linear on centered Gaussian inputs, so the centered Gaussian family is closed.
...
\end{proof}
As a simple exact family, if a functional depends on a measure only through its first two moments,
\(f(\alpha)=g(\mean_\alpha,\cov_\alpha)\), then
\[
\delta f(\alpha)(x)
=
\dotp{\nabla_\mean g}{x}
+\dotp{\nabla_\cov g}{(x-\mean_\alpha)(x-\mean_\alpha)^\top},
\qquad
\nabla_x\delta f(\alpha)(x)=\nabla_\mean g+2\nabla_\cov g(x-\mean_\alpha),
\]
where \(\nabla_\cov g\) is the symmetric Frobenius gradient. Thus Gaussian laws are invariant under the full Wasserstein gradient flow and
\[
\dot\mean_t=-\nabla_\mean g,
\qquad
\dot\cov_t=-2(\cov_t\nabla_\cov g+\nabla_\cov g\,\cov_t).
...
The affine-gradient cases above are exceptional: a generic Wasserstein gradient flow does not remain Gaussian. The constrained substitute projects the evolution onto
\(\mathcal G=\{\Gaussian(\mean,\cov):\mean\in\RR^d,\ \cov\succ0\}\),
the Gaussian submanifold of $\Pp_2(\RR^d)$. The Wasserstein gradient of a functional constrained to a smooth submanifold $\mathcal M\subset\Pp_2$ is defined as the Riesz representative of the differential restricted to tangent velocities of $\mathcal M$.
