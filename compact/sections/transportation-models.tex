...
so the descent velocity is \(v(x)=-A\delta_m+(\Sigma^{-1}-A)(x-m).\)
...
For \(\Wass_2^2(\alpha,\gamma)\), the Brenier map from \(\Gaussian(m,\Sigma)\) to \(\gamma\) is \(T(x)=\bar m+M_{\Sigma,\bar\Sigma}(x-m).\)
...
Gaussian preservation concerns the full law, whereas moment closure concerns only selected statistics. If \(f(\alpha)=g(m_\alpha,\Sigma_\alpha)\), its affine descent velocity gives, for every sufficiently regular flow starting from an arbitrary \(\alpha_0\in\Pp_2(\RR^d)\),
...
\end{equation}
Thus mean--covariance closure is distribution-free even though Gaussian preservation is not.
...
For \(f_g(\alpha)=g(m_\varphi(\alpha))\), every sufficiently regular Wasserstein gradient flow satisfies
\[
\dot m_t=-g'(m_t)\int_\Omega\norm{\nabla\varphi(x)}^2\d\alpha_t(x).
\]
This equation is autonomous for every smooth \(g\) and every admissible law if and only if there are $a,b\in\RR$ such that
\begin{equation}\label{eq-scalar-moment-affine-carre}
\norm{\nabla\varphi(x)}^2=a+b\varphi(x).
\end{equation}
In this case \(\dot m_t=-(a+b m_t)g'(m_t)\). Moreover, on every connected open $U\subset\{x:\nabla\varphi(x)\neq0\}$, the restriction of this condition to \(U\) is equivalent to the existence of $r\in C^2(U)$ and $c,\kappa,\lambda\in\RR$ such that
\begin{equation}\label{eq-scalar-moment-eikonal-form}
\norm{\nabla r}=1,
\qquad
\varphi=c+\kappa r+\frac{\lambda}{2}r^2.
\end{equation}
One may then take \(\lambda=b/2\).
\end{prop}
\begin{proof}
The first-variation formula gives the displayed moment equation and proves sufficiency. For necessity, use \(g(s)=s\). Dirac laws show that \(\norm{\nabla\varphi}^2=q\circ\varphi\); comparing a Dirac law with a two-point mixture of the same \(\varphi\)-moment shows that \(q\) preserves all convex combinations. Since \(\varphi(\Omega)\) is an interval, \(q\) is affine. On a regular set, define
\[
r(x)=\int_{s_0}^{\varphi(x)}\frac{\d s}{\sqrt{a+bs}}.
\]
Then $\norm{\nabla r}=1$, and inversion gives the asserted normal form with $\lambda=b/2$. The converse follows from
