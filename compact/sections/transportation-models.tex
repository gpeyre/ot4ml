...
The first construction below is deterministic once the latent variable has been sampled: the randomness is in $U\sim\pi$, while $t\mapsto P_t(U)$ is differentiable. Thus $\alpha_t$ is a push-forward curve and satisfies a continuity equation. Genuine temporal noise is a different extension; it leads to a Fokker--Planck equation, as recalled after Proposition~\ref{prop-flow-matching-vector-field}.

We assume first that $\alpha_t$ is obtained by pushing a latent distribution $\pi \in \Pp(\RR^{d'})$ through a time-dependent map $P_t: \RR^{d'} \to \RR^d$, i.e.
...
In the terminology of Albergo--Boffi--Vanden-Eijnden, a stochastic interpolant is not first an SDE. It is an explicit random bridge
\[
X_t=I_t(X_0,X_1,Z),\qquad X_0\sim\alpha_0,\quad X_1\sim\alpha_1,
\]
with $Z$ an auxiliary, often Gaussian, random variable and $I_0(X_0,X_1,Z)=X_0$, $I_1(X_0,X_1,Z)=X_1$. For instance \(X_t=a(t)X_0+b(t)X_1+\gamma(t)Z\), with $\gamma(0)=\gamma(1)=0$. The noise is static: conditionally on $(X_0,X_1,Z)$, the path is differentiable. This is the previous push-forward setting with $u=(X_0,X_1,Z)$ and $P_t=I_t$, hence
\[
v_t(x)=\EE[\partial_t I_t(X_0,X_1,Z)\mid X_t=x]
\]
and $\partial_t\alpha_t+\diverg(\alpha_t v_t)=0$. SDEs are alternative dynamics realizing the same marginals, not the definition of the interpolant.
\end{rem}

\begin{rem}[Brownian realizations of interpolant marginals]\label{rem-noisy-stochastic-interpolants}
One can also realize an interpolating marginal curve by Brownian-in-time dynamics. This should not be confused with the static-noise bridge of Remark~\ref{rem-static-noise-stochastic-interpolants}. If
\(\d Z_t=r_t(U,Z_t)\d t+\Sigma_t(U,Z_t)\d B_t\), $\alpha_t=\operatorname{Law}(Z_t)$, then the Eulerian drift and diffusion matrix defined by
\[
 v_t(z)=\EE[r_t(U,Z_t)|Z_t=z], \qquad
 D_t(z)=\EE[\Sigma_t(U,Z_t)\Sigma_t(U,Z_t)^\top|Z_t=z]
\]
satisfy
\[
...
\]
Thus the noisy regression estimates the drift in a Fokker--Planck equation, not a pure continuity equation. If $\alpha_t=\rho_t\d x$, an equivalent probability-flow ODE uses
\(\bar v_t=v_t-(2\rho_t)^{-1}\diverg(\rho_tD_t)\), hence
\(\bar v_t=v_t-\frac{\sigma_t^2}{2}\nabla\log\rho_t\) when $D_t=\sigma_t^2\Id$.
\end{rem}
...
\end{proof}
Proposition~\ref{prop-gaussian-flow-matching-optimality} explains why the statement ``flow matching gives an optimal map'' is fragile. The same terminal map~\eqref{eq-gaussian-product-fm-terminal-map} is obtained for any scalar schedule $Z_t=a_tX_0+b_tX_1$ with the same endpoints, because after whitening the covariance path remains $a_t^2\Id+b_t^2C$.
...
Moment measures encode a target law by a single convex potential: the same $u$ defines the log-concave source and the Brenier map. The useful point is hidden convexity: after rewriting the problem with OT, a nonlinear equation on convex potentials becomes a convex minimization over densities. If $u:\RR^d\to\RR\cup\{+\infty\}$ is proper convex and \(Z_u=\int e^{-u(x)}\d x<+\infty,\) set
\[
\rho_u=Z_u^{-1}e^{-u(x)}\d x,
\qquad
\mathfrak M(u)=(\nabla u)_\sharp\rho_u .
\]
The normalization removes additive constants, and translations of the argument do not change $\mathfrak M(u)$. Integration by parts gives \(\int y\,\d\mathfrak M(u)(y)=0\), so moment measures are centered. The Cordero--Erausquin--Klartag theorem states that a probability $\mu\in\Pp_1(\RR^d)$ is a moment measure iff it is centered and its support is not contained in a hyperplane; the convex potential is unique up to translations and additive constants. For example, if \(u(x)=x^\top A x/2+c\) with $A\succ0$, then \(\rho_u=\Nn(0,A^{-1})\) and \(\mathfrak M(u)=\Nn(0,A)\).

Santambrogio's OT formulation fixes centered $\mu$ and minimizes
\[
\min_{\rho\in\Pp_1(\RR^d)} \mathcal H(\rho)+\mathcal C_\mu(\rho),
\qquad
\mathcal C_\mu(\rho)=\sup_{\pi\in\Couplings(\rho,\mu)}
\int x\cdot y\,\d\pi(x,y),
\]
where \(\mathcal H(r\d x)=\int r\log r\). Duality gives
\[
\mathcal C_\mu(\rho)=
\inf_{v\ {\rm convex}}
\left\{\int v\,\d\rho+\int v^*\,\d\mu\right\},
\]
and, when second moments are finite,
\[
\mathcal C_\mu(\rho)
=
\frac12\int\norm{x}^2\d\rho
...
\]
The centering of $\mu$ makes the objective invariant under translations of $\rho$. The hidden convexity is that, on \(\Pp_2\), both \(\mathcal H\) and \(\mathcal C_\mu\) are displacement convex; for \(\mathcal C_\mu\), this follows from the identity above and the convexity of the quadratic moment combined with the semiconvexity of \(-\Wass_2^2(\cdot,\mu)/2\). The minimizer has the form \(\rho=Z_u^{-1}e^{-u}\d x\) and satisfies \(\mu=(\nabla u)_\sharp\rho\). Conversely, if this equation holds, then along a geodesic from $\rho$ to a competitor the derivative of entropy is \(-\int\dotp{T-\Id}{\nabla u}\d\rho\), while the derivative of \(\mathcal C_\mu\) is bounded below by the opposite quantity; displacement convexity gives global minimality. Eliminating $\rho$ instead gives \(u\mapsto \int u^*\d\mu-\log\int e^{-u}\d x\), whose convexity is hidden until one passes to the conjugate potential. For generative modeling, the conjugate variant uses
\[
\beta=(\nabla w^*)_\sharp\left(Z_w^{-1}e^{-w(z)}\d z\right),
\]
so sampling and mapping are still controlled by a single convex potential, but the map is placed on the Brenier-conjugate side.
