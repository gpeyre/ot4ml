...
This keeps the preceding discrepancy-minimization viewpoint but replaces Euclidean parameter descent by the Wasserstein gradient flow of the generated law, a distribution-space natural-gradient-type dynamics less tied to parametrization and potentially less prone to parameter-space local minima. Let $\zeta$ be a simple latent distribution and let $\alpha_\theta=(G_\theta)_\sharp\zeta$ be the model distribution. Assume that the target data distribution is $\beta$. The Wasserstein-flow construction chooses a discrepancy
...
For \(\mathcal E_\beta(\alpha)=\SW_2^2(\alpha,\beta)/2\), the \(\Wass_2\)-gradient flow has velocity
\[
w_{\SW}[\alpha,\beta](x)=\int_{\Sphere^{d-1}}\big(T_\theta(P_\theta x)-P_\theta x\big)\theta\,\d\sigma(\theta),
\]
where \(T_\theta\) is the one-dimensional monotone map between projected measures. This only needs sorting along sampled directions. For Gaussian targets the flow converges under suitable hypotheses; when both laws are Gaussian it closes on means/covariances, see the Gaussian closure subsection.

\paragraph{Stein variational gradient descent.}\label{sec-svgd-generative-flow}
For a target probability $\beta=\rho_\beta\,\d x$ with smooth positive density, SVGD replaces the $\Wass_2$ gradient flow of $\KL(\alpha|\beta)$ by the kernelized geometry of Section~\ref{sec-kernelized-bb-distance}. If $k$ has vector RKHS $\RKHS_k^d$, then
\[
\left.\frac{\d}{\d\epsilon}\KL((\Id+\epsilon v)_\sharp\alpha|\beta)\right|_{0}
=-\int(\dotp{\nabla\log\rho_\beta}{v}+\diverg v)\d\alpha,
\]
so the RKHS steepest-descent direction is
\[
v_{\alpha}^{\rm SVGD}(x)=\int\big(k(y,x)\nabla\log\rho_\beta(y)+\nabla_y k(y,x)\big)\d\alpha(y).
\]
Thus $\partial_t\alpha_t+\diverg(\alpha_t v_{\alpha_t}^{\rm SVGD})=0$, and particles follow
\[
X_i^{\ell+1}=X_i^\ell+\tau n^{-1}\sum_j\big(k(X_j^\ell,X_i^\ell)\nabla\log\rho_\beta(X_j^\ell)+\nabla_{X_j}k(X_j^\ell,X_i^\ell)\big).
\]
The target-score term attracts particles and the kernel-gradient term repels them.
