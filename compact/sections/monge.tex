...
Equivalently, for $y=\T(x)$, \(\density{\be}(y)=\density{\al}(\T^{-1}y)\, \frac{1}{|\det(\T'(\T^{-1}y))|}.\)
\end{prop}
\begin{proof}
...
\(\sum_{x,y}d_{\mathsf{T}}(x,y)\pi_{xy} = \sum_{e\in E}\ell_e \sum_{x,y:\ e\in[x,y]}\pi_{xy} \geq \sum_{e\in E}\ell_e|\sigma(V_e)|,\)
...
For $\Sigma,\Lambda\in\mathbb S_+^d$, \(\min_{\Phi_2(\al)=\Sigma,\ \Phi_2(\nu)=\Lambda} \Wass_2(\al,\nu)^2 = \Bb(\Sigma,\Lambda)^2.\)
