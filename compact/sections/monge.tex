...
\end{proof}
\paragraph{1D Monge solutions.}
The quantile construction gives a Monge map only when $\al$ has no atoms, because then the cumulative distribution function can be used as a change of variable. If $\al$ has atoms, quantile matching may require splitting mass; the relaxed statement is treated in Section~\ref{sec-kantorovich-continuous}, paragraph \hyperref[sec-1d-kantorovich-solution]{Kantorovich solution in 1D}.
\begin{thm}[One-dimensional Monge solution]\label{prop-1d-quantile-map}
Let $\al,\be\in\Mm_+^1(\RR)$, assume that $\al$ has no atoms, and let $h:\RR\to\RR$ be convex. For $c(x,y)=h(x-y)$, set $q_\al=\cumul{\al}^{-1}$ and $q_\be=\cumul{\be}^{-1}$, and assume $h(q_\al-q_\be)\in L^1(0,1)$.
\eql{\label{eq-1d-monge-map}
\T=q_\be\circ\cumul{\al}=\cumul{\be}^{-1}\circ\cumul{\al}
}
satisfies $\T_\sharp\al=\be$ and minimizes $\int h(x-S(x))\d\al(x)$ over maps $S_\sharp\al=\be$.
\end{thm}
\begin{proof}
The push-forward follows from Proposition~\ref{prop-quantile-pushforward}. For optimality, use Theorem~\ref{prop-1d-kantorovich-quantile-coupling}: the quantile coupling $\pi^\star=(q_\al,q_\be)_\sharp\mathrm{Leb}_{[0,1]}$ is optimal among all couplings, and atomlessness gives $(\Id,\T)_\sharp\al=\pi^\star$.
\end{proof}
...
\end{proof}
For $p=2$, Bobkov--Ledoux also give a CDF-only formula: if $\al,\be\in\Mm_+^1([a,b])$ and $s_+=\max(s,0)$, then
\[
\Wass_2^2(\al,\be)
=2\int_a^b\int_x^b
\Big[
(\cumul{\al}(x)-\cumul{\be}(y))_+
+(\cumul{\be}(x)-\cumul{\al}(y))_+
\Big]\d y\,\d x .
\]
It follows from writing $|q_\al(r)-q_\be(r)|^2$ as twice the triangular area between the two quantile levels and then integrating first in $r$. This representation is useful when CDFs are easier to aggregate than quantiles.
\begin{rem}[One-dimensional Wasserstein spaces are isometric to Banach spaces]
Formula~\eqref{eq-wass-cumul} means that $\al\mapsto\cumul{\al}^{-1}$ embeds one-dimensional Wasserstein geometry isometrically into $L^p(0,1)$. For $p=2$, it is therefore Hilbertian, unlike higher-dimensional Wasserstein geometry.
\end{rem}

\(\cumul{\al_t}^{-1}(r) = (1-t)\cumul{\al}^{-1}(r)+t\cumul{\be}^{-1}(r), \qquad r\in(0,1).\)
\begin{rem}[$\Wass_1$ is a norm]
For $p=1$, $\Wass_1(\al,\be)=\norm{\cumul{\al}-\cumul{\be}}_{L^1}$ on the line, so $\Wass_1$ is a norm on zero-mass signed measures through cumulative primitives. This is the one-dimensional form of the general Kantorovich--Rubinstein norm on zero-mass signed measures with finite norm; see Section~\ref{sec-W1}.
\end{rem}
