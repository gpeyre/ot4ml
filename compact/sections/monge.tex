...
\end{defn}
Beyond Euclidean spaces, a key infinite-dimensional example is the path space $\Ss=C([0,1];\RR^d)$ with the uniform metric. It is complete by stability of continuity under uniform limits, separable by rational piecewise-linear approximation, and its continuous endpoint maps make it the natural state space for the dynamical and Schr\"odinger path formulations in Section~\ref{sec-path-space-schrodinger}.
...
For discrete measures, first recast both measures as weight vectors on a common finite support. If $\al=\sum_{i=1}^n\tilde a_i\delta_{x_i}$ and $\be=\sum_{j=1}^m\tilde b_j\delta_{y_j}$, let $z_1,\ldots,z_r$ be the distinct points in $\{x_i\}_i\cup\{y_j\}_j$ and set
\(a_k=\sum_{i:x_i=z_k}\tilde a_i\), \(b_k=\sum_{j:y_j=z_k}\tilde b_j\). Then $\al=\sum_{k=1}^r a_k\delta_{z_k}$ and $\be=\sum_{k=1}^r b_k\delta_{z_k}$. If the input supports are disjoint and have no repetitions, this is just $(z_k)_k=(x_1,\ldots,x_n,y_1,\ldots,y_m)$ with $a=(\tilde a,0_m)$ and $b=(0_n,\tilde b)$. Thus

\(\norm{\al-\be}_{\TV} = \sum_{k=1}^r |a_k-b_k|.\)
\paragraph{Probabilistic interpretation.}
Radon probability measures represent the laws of random variables. Let $(\Omega,\Ff,\PP)$ be an abstract probability space. A random variable with values in $\X$ is a measurable map $X:(\Omega,\Ff)\to(\X,\Bb(\X))$, or simply $X:\Omega\to\X$ once the measurable structures are understood.
...
A smooth cost $c\in C^1(X\times Y)$ satisfies the twist condition if, for every $x$, the map \(y\mapsto\nabla_x c(x,y)\) is injective. If \((f,g)\) are optimal dual potentials and \((x,y)\in\supp(\pi)\), then \(z\mapsto c(z,y)-g(y)\) touches \(f\) from above at \(x\); hence, at differentiability points of \(f\),
\(\nabla f(x)=\nabla_xc(x,y).\)
Twist makes this equation single-valued in \(y\), so an optimal plan cannot split mass from a typical source point. The quadratic cost, the bilinear cost \(-\dotp{x}{y}\), and \(c(x,y)=h(x-y)\) with \(h\) smooth strictly convex are twisted; separated costs \(a(x)+b(y)\) are not.
For the quadratic cost, the resulting Brenier map is a convex gradient and is therefore monotone:
\(\dotp{\nabla\phi(x)-\nabla\phi(x')}{x-x'}\geq0.\)
...
Twist gives a map, but not necessarily a smooth one. For a general \(C^4\) twisted cost with invertible mixed Hessian, the Ma--Trudinger--Wang condition controls regularity. On the image of the change of variables \((x,y)\mapsto(x,\nabla_xc(x,y))\), define \(Y(x,p)\) by \(\nabla_xc(x,Y(x,p))=p\), and set \(A_{ij}(x,p)=\partial^2_{x_ix_j}c(x,Y(x,p))\). The weak MTW condition asks
\[
\sum_{i,j,k,l}\partial^2_{p_kp_l}A_{ij}(x,p)\xi_i\xi_j\eta_k\eta_l\geq0
\qquad\text{for }\dotp{\xi}{\eta}=0.
\]
It is a curvature condition on the cost: combined with \(c\)-convexity of the domains and positive smooth densities, it yields continuity and higher regularity of optimal maps. Quadratic and bilinear costs have zero MTW curvature. For the squared Riemannian distance, MTW is a refined cost-curvature condition: near the diagonal it recovers sectional curvature, but global regularity also depends on cut-locus and domain-convexity issues.
...
The Monge--Amp\`ere operator is a fully nonlinear degenerate elliptic analogue of the Laplacian: its linearization is the weighted Poisson equation below, but the determinant is elliptic only on the convex branch. Numerically, the hard part is not merely solving a nonlinear system; it is building a scheme that selects the convex solution and discretizes both the operator and the second boundary condition consistently. Newton, monotone wide-stencil finite-difference, variational and power-diagram solvers are different answers to this discretization problem.
...
\begin{rem}[Fisher--Rao metric with mean variation]\label{rem-fisher-rao-gaussian-mean}
For one-dimensional Gaussians,
\[
\KL(\Gaussian(m,\sigma^2)\mid\Gaussian(m',(\sigma')^2))
=\log\frac{\sigma'}{\sigma}+\frac{\sigma^2+(m-m')^2}{2(\sigma')^2}-\frac12.
\]
The local expansion gives
\(
\KL(\Gaussian(m+\epsilon h,(\sigma+\epsilon s)^2)\mid\Gaussian(m,\sigma^2))
=\frac{\epsilon^2}{2}(h^2/\sigma^2+2s^2/\sigma^2)+o(\epsilon^2),
\)
so
\(g_{(m,\sigma)}^{\mathrm{FR}}((h,s),(h',s'))=(hh'+2ss')/\sigma^2.\)
With $z=m/\sqrt2+i\sigma$, this is twice the hyperbolic metric; its geodesics are vertical lines or semicircles orthogonal to $\sigma=0$. Hence
\[
d_{\mathrm{FR}}=\sqrt2\,\operatorname{arcosh}\left(1+\frac{(m-m')^2+2(\sigma-\sigma')^2}{4\sigma\sigma'}\right).
\]
Thus $\Wass_2$ is Euclidean in $(m,\sigma)$, while Fisher--Rao bends along hyperbolic geodesics and puts the boundary $\sigma=0$ at infinite distance.
\end{rem}
...
\end{proof}
\begin{prop}[Bures as a second-moment quotient]\label{prop-bures-second-moment-lift}
For $\mu\in\Pp_2(\RR^d)$ set the raw second moment
\(\Phi_2(\mu)=\int uu^\top\d\mu(u)\in\mathbb S_+^d\).  For $\Sigma,\Lambda\in\mathbb S_+^d$, define
\[
...
\Wass_2(\mu,\nu)^2 .
\]
Then \(D_{\Phi_2}(\Sigma,\Lambda)=\Bb(\Sigma,\Lambda)\).
\end{prop}
\begin{proof}
Assume first that \(\Sigma,\Lambda\) are positive definite. For any coupling of admissible laws, set \(K=\int uv^\top\d\pi(u,v)\). Its cost is
\(\tr\Sigma+\tr\Lambda-2\tr K\), while the block matrix
\(\left(\begin{smallmatrix}\Sigma&K\\K^\top&\Lambda\end{smallmatrix}\right)\)
is positive semidefinite. Thus the Schur complement gives
\(K=\Sigma^{1/2}R\Lambda^{1/2}\) with \(\|R\|_{\mathrm{op}}\leq1\). Hence, by nuclear/operator norm duality,
\[
\tr K\leq \sup_{\|R\|_{\mathrm{op}}\leq1}\tr(\Sigma^{1/2}R\Lambda^{1/2})
=\| \Lambda^{1/2}\Sigma^{1/2}\|_*
=\tr((\Sigma^{1/2}\Lambda\Sigma^{1/2})^{1/2}).
\]
This gives the lower bound \(\Bb^2\). Singular matrices follow by adding \(\eta I\) to the diagonal blocks only in the Schur-complement estimate, then letting \(\eta\downarrow0\). Equality is attained by the centered Gaussian pair \(\Gaussian(0,\Sigma),\Gaussian(0,\Lambda)\), using Proposition~\ref{prop-gaussian-w2-bures}. Thus the Bures metric is the quotient of \(\Wass_2\) by the raw second-moment map; on centered laws, this is the covariance map.
\end{proof}
\begin{rem}[Common-generator elliptical laws]\label{rem-elliptical-bures}
The Gaussian formula extends beyond Gaussian tails, but only along common-generator elliptical families. Let $\rho$ be absolutely continuous, centered and radial, with covariance $\Id$. If
\(\al=(z\mapsto \mean_\al+\cov_\al^{1/2}z)_\sharp\rho\)
and
\(\be=(z\mapsto \mean_\be+\cov_\be^{1/2}z)_\sharp\rho,\)
then the same Brenier matrix~\eqref{eq-bures-map} transports $\al$ to $\be$ optimally and
\(\Wass_2^2(\al,\be)=\|\mean_\al-\mean_\be\|^2+\Bb^2(\cov_\al,\cov_\be).\)
The common generator is essential: with different radial profiles, one must also transport the radial variable, and covariance data alone no longer determines the transport cost.
\end{rem}
For $2\times2$ covariance matrices
\(\Sigma=\left(\begin{smallmatrix}a&c\\c&b\end{smallmatrix}\right)\), the coordinates
\(t=(a+b)/\sqrt2\), \(u=(a-b)/\sqrt2\), \(v=\sqrt2c\) identify symmetric matrices with $\RR^3$, with \(t^2-u^2-v^2=2\det\Sigma\). Hence $\Sigma\succeq0$ is exactly the Lorentz cone \(t\geq\sqrt{u^2+v^2}\). On its positive definite interior, the Bures metric is the geodesic distance of the nonlinear covariance geometry induced by Gaussian OT, not the ambient Euclidean metric.
\begin{rem}[Fisher--Rao versus Bures for centered Gaussians]\label{rem-fisher-rao-bures-gaussian}
For $\Sigma,\Sigma'\in\mathbb S_{++}^d$,
\[
\KL(\Sigma\mid\Sigma')\eqdef\KL(\Gaussian(0,\Sigma)\mid\Gaussian(0,\Sigma'))=\frac12\left(\tr((\Sigma')^{-1}\Sigma)-d-\log\det((\Sigma')^{-1}\Sigma)\right).
\]
For symmetric $D$,
\[
\KL(\Sigma+\epsilon D\mid\Sigma)=\frac{\epsilon^2}{2}\langle Q_\Sigma(D),D\rangle_F+o(\epsilon^2),
\qquad
Q_\Sigma(D)=\frac12\Sigma^{-1}D\Sigma^{-1}.
\]
Thus \(g_\Sigma(D,E)=\frac12\tr(\Sigma^{-1}D\Sigma^{-1}E)\) is the Fisher--Rao, or affine-invariant, metric, with
\[
d_{\mathrm{FR}}(\Sigma,\Sigma')^2=\frac12\norm{\log(\Sigma^{-1/2}\Sigma'\Sigma^{-1/2})}_F^2,
\]
where $\log$ is the principal matrix logarithm. Bures extends continuously to $\mathbb S_+^d$; for instance $\Bb^2(\Sigma,0)=\tr(\Sigma)$, so singular covariances are at finite distance. Fisher--Rao has the logarithmic distance above, so every rank-deficient covariance is at infinite distance from the positive definite cone.
\end{rem}
