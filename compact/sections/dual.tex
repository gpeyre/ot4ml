...
For $\al=\sum_i a_i\de_{x_i}$ and $\be=\sum_j b_j\de_{y_j}$, apply the definition to $\X=\{x_i\}_i$ and $\Y=\{y_j\}_j$. With $\C_{i,j}=c(x_i,y_j)$ and dual vectors $(\fD,\gD)\in\RR^n\times\RR^m$, the sign convention $f\oplus g\leq c$ gives
\[
(\fD^\C)_j=\min_i \C_{i,j}-\fD_i,\qquad
\gD^{\bar\C}\eqdef\gD^{\C^\top},\qquad
(\gD^{\bar\C})_i=\min_j \C_{i,j}-\gD_j.
\]
The minimum is forced by $\gD_j\leq\C_{i,j}-\fD_i$ for every $i$; a maximum would correspond to another sign convention. Thus $c$-transforms become column minima for $\fD^\C$, or row minima for $\gD^{\bar\C}$, in shifted cost matrices. Section~\ref{sec-semidiscr-w1} studies the hybrid case where $\al$ has a density and $\be$ is discrete, where $g^{\bar c}(x)=\min_j c(x,y_j)-g_j$ generates Laguerre cells for the quadratic cost.
\end{rem}
