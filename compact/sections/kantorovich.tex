...
For $n=m=1$, $(2,1)$ and $(1,2)$, the coupling set is a singleton. The first nontrivial case is $(2,2)$. For $\a=(p,1-p)$ and $\b=(q,1-q)$, choosing $s\eqdef \P_{1,1}$ forces
\[
\P(s)=
\begin{pmatrix}
s & p-s\\
...
\end{pmatrix},
\qquad
s\in[\max(0,p+q-1),\min(p,q)].
\]
Thus $\CouplingsD(\a,\b)$ is a segment, possibly collapsed to a point. With positive marginals, the general affine dimension is $(n-1)(m-1)$ before imposing nonnegativity.
\end{rem}
...
\int f\d\al,
\]
Echoing Proposition~\ref{prop-discrete-product-coupling-degenerate}, optimality of $\al\otimes\be$ means flatness over all admissible couplings; for continuous costs, this is the additive form $c=u+v$ on the product support.
...
\end{prop}
\begin{proof}
The constraint set is non-empty because it contains $\al\otimes\be$ and is closed for weak convergence. With $\Omega=\Xx\times\Yy$ compact, Proposition~\ref{prop-wasserstein-space-polish} gives compactness of $\Pp(\Omega)$ in Wasserstein topology, hence in the weak topology by Proposition~\ref{prop-wass-metrizes-weak-compact}. Thus $\Couplings(\al,\be)$ is compact, and weak continuity of $\pi\mapsto\int c\d\pi$ gives a minimizer.
\end{proof}
...
\end{proof}
If $\al$ does not have a density, then $\phi$ may be non-smooth on a set charged by $\al$, and non-smooth points can lead to mass splitting. This is the continuous counterpart of the gap between the uniform matching case of Corollary~\ref{cor-kantorovich-matching} and the general splitting case.
...
The Kantorovich problem is a relaxation of Monge in the lifted plan variable.
\begin{prop}[Plan-space relaxation of Monge]\label{prop-kantorovich-relaxation-monge}
Let $(\Xx,\dist)$ be compact, $p\geq1$, and $\al,\be\in\Pp(\Xx)$ with $\al$ atomless. For every $\pi\in\Couplings(\al,\be)$ there are measurable maps $T_k$ with $(T_k)_\sharp\al=\be$ such that $(\Id,T_k)_\sharp\al\rightharpoonup\pi$ and
\[
\int_\Xx \dist(x,T_k(x))^p\d\al(x)\to \int_{\Xx\times\Xx}\dist(x,y)^p\d\pi(x,y).
\]
Thus $\pi\mapsto\int_{\Xx\times\Xx}\dist(x,y)^p\d\pi(x,y)$ is the weak lower-semicontinuous envelope of the same cost restricted to graph couplings, and
\[
\Wass_p(\al,\be)^p=\inf_{T_\sharp\al=\be}\int\dist(x,T(x))^p\d\al(x)=\tilde\Wass_p(\al,\be)^p.
\]
\end{prop}
\begin{proof}
For a coupling $\pi$, choose finite partitions $(A_i)_i,(B_j)_j$ with small diameters and set $m_{ij}=\pi(A_i\times B_j)$. Since $\al$ is atomless, split $A_i$ into $A_{ij}$ with $\al(A_{ij})=m_{ij}$. On each positive-mass $A_{ij}$, Proposition~\ref{prop-existence-transport-map-atomless} gives a map sending $\al|_{A_{ij}}/m_{ij}$ to $\be|_{B_j}/\be(B_j)$. Pasting these maps gives $T_\sharp\al=\be$ and a graph coupling with the same masses as $\pi$ on all rectangles $A_i\times B_j$. Uniform continuity of test functions on the compact product gives weak convergence as the mesh goes to zero, and applying this to $\dist^p$ gives convergence of costs. The lower-semicontinuous-envelope statement follows because any lsc minorant of the graph functional is bounded above by its values along these approximating graph couplings.
\end{proof}
Since the cost is affine in $\pi$ and the coupling set is convex, this is the closed convex relaxation in plan space.
\begin{cor}[Envelope of the Monge \(p\)-cost]\label{cor-wasserstein-lsc-envelope-monge-distance}
Assume that atomless probabilities are dense in $\Pp(\Xx)$ for $\Wass_p$. Let $D_M(\al,\be)\eqdef\tilde\Wass_p(\al,\be)^p$, with value $+\infty$ if no map exists. Then $\Wass_p(\al,\be)^p$ is the lower-semicontinuous envelope of $D_M$ on $\Pp(\Xx)^2$.
\end{cor}
\begin{proof}
One has $\Wass_p^p\leq D_M$, and $\Wass_p^p$ is continuous. If $H\leq D_M$ is lsc and $\al_k\to\al$ are atomless, Proposition~\ref{prop-kantorovich-relaxation-monge} gives $D_M(\al_k,\be)=\Wass_p(\al_k,\be)^p$, hence $H(\al,\be)\leq\liminf_k H(\al_k,\be)\leq\lim_k\Wass_p(\al_k,\be)^p=\Wass_p(\al,\be)^p$.
\end{proof}
The density assumption is essential: on finite spaces, no closure repairs the impossibility of splitting a source atom. Then Kantorovich genuinely adds mass splitting.
...
If $\mu$ is the common law of i.i.d. centered unit-variance variables $X_i$, the law of $n^{-1/2}\sum_i X_i$ is $\alpha_n=(D_{1/\sqrt n})_\sharp\mu^{*n}$, where $D_a(x)=a x$ and $\mu^{*n}$ is the $n$-fold convolution. Here $\mu*\nu=\operatorname{add}_\sharp(\mu\otimes\nu)$, $\operatorname{add}(x,y)=x+y$, is the law of the sum of two independent variables with laws $\mu,\nu$; with densities, $(f*g)(z)=\int f(x)g(z-x)\d x$. The qualitative CLT says that these rescaled convolutions converge weak$^*$ to the standard Gaussian; the next proposition quantifies this convergence in $\Wass_1$.
