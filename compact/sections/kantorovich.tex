...
\end{proof}
\(\mathcal P_p(\Xx) \eqdef \enscond{\al\in\Mm_+^1(\Xx)}{\int d(x,x_0)^p\d\al(x)<+\infty},\)
...
The atomless assumption in the Monge formula is a limitation of maps, not of one-dimensional optimality. The relaxed quantile coupling works for arbitrary measures because atoms may be split over quantile subintervals.
\begin{thm}[One-dimensional Kantorovich solution]\label{prop-1d-kantorovich-quantile-coupling}
Let $\al,\be\in\Mm_+^1(\RR)$, let $h:\RR\to\RR$ be convex, and set $c(x,y)=h(x-y)$. With $q_\al=\cumul{\al}^{-1}$ and $q_\be=\cumul{\be}^{-1}$, assume $h(q_\al-q_\be)\in L^1(0,1)$. Then
\[
\pi^\star=(q_\al,q_\be)_\sharp\mathrm{Leb}_{[0,1]}
\]
minimizes $\int h(x-y)\d\pi(x,y)$ over $\pi\in\Couplings(\al,\be)$.
\end{thm}
\begin{proof}
The coupling property follows from Proposition~\ref{prop-quantile-pushforward}. Optimality follows from the one-dimensional uncrossing inequality: for $x<x'$ and $y>y'$, convexity gives \(h(x-y)+h(x'-y')\geq h(x-y')+h(x'-y)\). Repeated uncrossing on ordered quantile grids gives the diagonal quantile plan, and approximation by uniform partitions gives the general case.
\end{proof}
