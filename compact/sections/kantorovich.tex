...
Let $(\Xx,d)$ be Polish, $1\leq p<+\infty$, and $\al_k,\al\in\Pp_p(\Xx)$. Then $\Wass_p(\al_k,\al)\to0$ if and only if $\al_k\rightharpoonup\al$ and, for one (equivalently every) $x_0\in\Xx$,
\(\int d(x,x_0)^p\d\al_k(x)\to\int d(x,x_0)^p\d\al(x).\)
Equivalently, weak convergence holds and
\(\lim_{R\to+\infty}\sup_k\int_{\{d(x,x_0)>R\}}d(x,x_0)^p\d\al_k(x)=0.\)
\end{prop}
\begin{proof}
Wasserstein convergence implies weak convergence and moment convergence by the reverse triangle inequality in $L^p$ under a coupling. Conversely, represent weak convergence by random variables $X_k\to X$ almost surely. Moment convergence makes $d(X_k,x_0)^p$ uniformly integrable, hence $\EE d(X_k,X)^p\to0$, which bounds $\Wass_p^p$. Truncating $d(\cdot,x_0)^p$ proves the equivalent tail criterion.
\end{proof}
On compact spaces the moment condition is automatic, so $\Wass_p$ metrizes weak convergence.
