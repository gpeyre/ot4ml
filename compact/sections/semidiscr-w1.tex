\section{\texorpdfstring{Semi-discrete and $\Wass_1$}{Semi-discrete and W1}}
\label{sec-semidiscr-w1}
\subsection{Semi-dual}
\(\usup{f,g \in \Cc(\Xx) \times \Cc(\Yy)} \Ee(f,g)\)
where $\Ee(f,g)$ is the dual objective, with value $-\infty$ when the feasibility constraint fails. One can optimize out $g$ exactly and obtain the following semi-dual problem

\eql{\label{eq-semi-dual}
\usup{f \in \Cc(\Xx)} \tilde\Ee(f) \eqdef \Ee(f,f^c) = \usup{g} \Ee(f,g) =
\int_\Xx f \d \al + \int_\Yy f^c \d \be.
}
\subsection{Semi-discrete}
\paragraph{Discrete target and Laguerre cells.}
For $\be=\sum_j\b_j\de_{y_j}$, restrict Definition~\ref{def-c-transform} to the finite support of $\be$, equivalently apply it with the discrete target space $\Y=\{y_j\}_j$ and identify $\gD$ with $g:\Y\to\RR$, $g(y_j)=\gD_j$:
\eql{\label{eq-disc-c-transfo}
\foralls \gD \in \RR^m, \;
\foralls x \in \Xx, \quad
\gD^{\bar \c}(x) \eqdef \umin{j \in \range{m}} \c(x,y_j) - \gD_j.
}
\eql{\label{eq-semi-dual-discr}
\MK_\c(\al,\be) =
\umax{\gD \in \RR^m}
\Ee(\gD) \eqdef
\int_\X \gD^{\bar \c}(x) \d\al(x) + \sum_j \gD_j \b_j.
}
\begin{defn}[Laguerre cells and power diagrams]\label{def-laguerre-power-cells}
For sites $(y_j)_{j=1}^m$ and weights $\gD\in\RR^m$, the Laguerre cell associated with $y_j$ is
\eql{\label{eq-laguerre-cells}
\Laguerre_{j}(\gD) \eqdef \enscond{x \in \Xx}{ \foralls j' \neq j, \c(x,y_j) - \gD_j \leq \c(x,y_{j'}) - \gD_{j'} }.
}
The cells cover $\Xx$; after arbitrary tie-breaking on common boundaries, they induce a disjoint partition. When $c(x,y)=\norm{x-y}^2$, this Laguerre decomposition is also called a power diagram. If $\gD$ is constant, it reduces to the ordinary Voronoi diagram.
\end{defn}
\paragraph{Mass balance.}
\eql{\label{eq-semi-disc-energy}
\Ee(\gD) = \sum_{j=1}^m \int_{\Laguerre_{j}(\gD)} \pa{ c(x,y_j) - \gD_j } \d\al(x) + \dotp{\gD}{\b}.
}
\begin{prop}[Gradient of the semi-discrete dual]
If $\al$ gives zero mass to the Laguerre cell boundaries, then $\Ee$ is differentiable at $\gD$ and
\(\foralls j \in \range{m}, \quad \nabla\Ee(\gD)_j = \b_j - \int_{\Laguerre_{j}(\gD)} \d\al.\)
\end{prop}
\begin{proof}
For $\al$-almost every $x$, the minimizing index in $\min_j c(x,y_j)-\gD_j$ is unique. If this index is $j(x)$, then the directional derivative in a direction $h\in\RR^m$ is
\[
\left.\frac{\d}{\d t}\right|_{t=0}
\min_j \bigl(c(x,y_j)-(\gD_j+t h_j)\bigr)
=
-h_{j(x)}.
\]
Dominated convergence gives \(\d\Ee(\gD)[h] = -\sum_j h_j\int_{\Laguerre_j(\gD)}\d\al +\sum_j h_j\b_j,\)
which is the announced gradient formula.
\end{proof}
\paragraph{Stochastic optimization.}
\eql{\label{eq-semi-disc-energy-entropy}
\Ee(\gD)
=
\int_\X E(\gD,x)\d\al(x)
=
\EE_X(E(\gD,X)),
\qquad
E(\gD,x)\eqdef \gD^{\bar c}(x)+\dotp{\gD}{\b}.
}
\[
\nabla_{\gD}E(\gD,x)
=
\left(\b_j-\ones_{\Laguerre_j(\gD)}(x)\right)_{j=1}^m,
\]
\eql{\label{eq-sgd}
\gD^{(\ell+1)}
\eqdef
\gD^{(\ell)}+\tau_\ell\nabla_{\gD}E(\gD^{(\ell)},x_\ell).
}
\eql{\label{eq-step-size-sgd}
\tau_\ell\eqdef \frac{\tau_0}{1+\ell/\ell_0},
}
where $\ell_0$ is a warmup scale. Under standard stochastic-approximation assumptions, one obtains the usual sublinear rate

\(\Ee(\gD^\star)-\EE\big(\Ee(\gD^{(\ell)})\big) = O(\ell^{-1/2}),\)
where $\gD^\star$ is a maximizer and the expectation is over the i.i.d. samples.

\subsection{Optimal Quantization}
\label{sec-optimal-quantization}
\eql{\label{eq-optimal-quantization}
\Qq_m(\al)
\eqdef
\umin{Y=(y_j)_{j=1}^m,\ \b\in\simplex_m}
\Wass_p\left(\al,\sum_{j=1}^m \b_j\de_{y_j}\right).
}
\paragraph{Free masses and prescribed weights.}
This free-mass problem optimizes both codepoint positions $Y$ and weights $\b$; prescribing the weights gives a different problem. The equal-weight case $\b_j=1/m$ is used below.
\begin{prop}[Quantization rate and curse of dimensionality]\label{prop-quantization-rate}
Let $\Omega\subset\RR^d$ be a bounded Lipschitz domain and assume $\al=\rho\,\d x$ on $\Omega$, with $0<\rho_-\leq\rho\leq\rho_+<+\infty$. Then, for fixed $p\geq1$, there exist constants $0<c\leq C<+\infty$ such that
\(c\,m^{-1/d} \leq \Qq_m(\al) \leq C\,m^{-1/d}.\)
\end{prop}
\begin{proof}
For the upper bound, partition $\Omega$ into $m$ cells of diameter at most $Cm^{-1/d}$, up to boundary effects, and put one codepoint in each nonempty cell. Sending each point to the codepoint in its cell gives a transport distance bounded by $Cm^{-1/d}$.

For the lower bound, fix any set $Y$ of $m$ codepoints and write $d_Y(x)=\min_j\norm{x-y_j}$. Since the density is bounded above, the mass of the $t$-neighborhood of $Y$ is at most $Cmt^d$. Choosing $t_0\simeq m^{-1/d}$ small enough gives $\al(\{d_Y>t\})\geq c$ for $0<t<t_0$. Hence
\(\int d_Y(x)^p\d\al(x) = \int_0^{+\infty} p t^{p-1}\al(\{d_Y>t\})\d t \geq c t_0^p \simeq c m^{-p/d}.\)
Taking the $p$-th root and minimizing over $Y$ proves the lower bound.
\end{proof}
\paragraph{Lloyd algorithm.}
Quantization becomes computationally useful because fixed sites make the assignment step local: the optimal cells are nearest-site Voronoi cells, and the second step recenters the sites.
\begin{prop}[Free masses give Voronoi cells]\label{prop-free-masses-voronoi}
For the cost $c(x,y)=d(x,y)^p$, fix distinct codepoints $Y=(y_j)_{j=1}^m$. Duplicate codepoints can be merged beforehand. Minimizing over the weights $\b\in\simplex_m$ gives
\[
\min_{\b\in\simplex_m}
\Wass_p^p\left(\al,\sum_j \b_j\de_{y_j}\right)
=
\int_\X \min_{1\leq j\leq m} c(x,y_j)\d\al(x).
\]
An optimal coupling is induced by sending each $x$ to a nearest codepoint. The corresponding cells are the Voronoi cells
\(\VV_j(Y) \eqdef \enscond{x}{\foralls j',\ c(x,y_j)\leq c(x,y_{j'})},\)
up to arbitrary tie-breaking on common boundaries.
\end{prop}
\begin{proof}
For any coupling between $\al$ and a measure supported on $Y$, the conditional destination of a point $x$ belongs to $Y$, hence its conditional cost is at least $\min_j c(x,y_j)$. Integrating gives the lower bound. Conversely, choose a measurable nearest-codepoint map $T_Y(x)\in\argmin_j c(x,y_j)$, breaking ties measurably, and set $\b_j=\al(T_Y^{-1}(y_j))$. Then $(T_Y)_\sharp\al=\sum_j \b_j\de_{y_j}$ and the induced transport reaches the displayed lower bound.
\end{proof}
\(\Qq_m(\al)^p = \min_Y \Ff(Y), \qquad \Ff(Y) \eqdef \int_\X \min_{1\leq j\leq m} c(x,y_j)\d\al(x).\)
\(y_j \in \uargmin{y} \int_{\VV_j(Y)} c(x,y)\d\al(x).\)
\(y_j = \frac{\int_{\VV_j(Y)} x\d\al(x)} {\int_{\VV_j(Y)} \d\al}.\)
Lloyd's algorithm alternates nearest-cell assignment and centroid update; it decreases the squared Euclidean objective but generally reaches only a local minimum.
\paragraph{Continuous Lloyd flow.}
For \(c(x,y)=\norm{x-y}^2\), assume \(\al\) does not charge Voronoi boundaries and set
\(a_j(Y)=\al(\VV_j(Y))\), \(b_j(Y)=a_j(Y)^{-1}\int_{\VV_j(Y)}x\d\al(x)\) on nonempty cells.
The relaxed Lloyd update
\(y_j^{(k+1)}=y_j^{(k)}+\tau(b_j(Y^{(k)})-y_j^{(k)})\)
formally gives, as \(\tau\to0\), the cell-mass preconditioned gradient flow of \(\Ff\),
\[
\dot y_j(t)=b_j(Y_t)-y_j(t).
\]
Indeed \(\nabla_{y_j}\Ff(Y)=2a_j(Y)(y_j-b_j(Y))\), hence
\(\dot y_j=-(2a_j(Y_t))^{-1}\nabla_{y_j}\Ff(Y_t)\). This is the gradient flow for the site metric \(g_Y(U,V)=2\sum_j a_j(Y)\dotp{u_j}{v_j}\), not the unweighted Euclidean metric, and \(\frac{\d}{\d t}\Ff(Y_t)=-2\sum_j a_j(Y_t)\norm{b_j(Y_t)-y_j(t)}^2\leq0\).
For fixed particle weights \(\mu_t=\sum_jw_j\de_{y_j(t)}\), this is the weak continuity equation
\(\partial_t\mu_t+\diverg(v_{\mu_t}\mu_t)=0\) with \(v_{\mu_t}(y_j)=b_j(Y)-y_j\).
These \(w_j\) are auxiliary weights. The free-weight projection \(\nu_Y=\sum_j a_j(Y)\de_{y_j}\) instead satisfies formally
\[
\partial_t\nu_{Y_t}+\diverg(v_t\nu_{Y_t})=\sum_j\dot a_j(Y_t)\de_{y_j(t)},
\qquad v_t(y_j(t))=\dot y_j(t),
\]
so varying cell masses create a balance term.

\paragraph{Mean-field limit and ultrafast diffusion.}
When the number \(M\) of codepoints tends to infinity, the continuum variable is the density \(f\) of sites: about \(M f(x)\d x\) codepoints lie in \(\d x\). For \(p\)-quantization in dimension \(d\), set \(r=p/d\), hence \(r=2/d\) for the quadratic cost. The rescaled high-resolution functional is, up to a cell-shape constant,
\[
\Gg_\rho(f)=\int_\Omega \rho(x)f(x)^{-r}\d x,\qquad \int_\Omega f\d x=1.
\]
Its formal \(\Wass_2\)-gradient flow is the weighted ultrafast diffusion
\[
\partial_t f=-r\,\diverg\!\left(f\nabla\!\left(\frac{\rho}{f^{r+1}}\right)\right),
\]
with periodic or no-flux boundary conditions. The one-dimensional particle-to-continuum limit is due to Caglioti, Golse and Iacobelli, the one-dimensional very-fast-diffusion analysis to Iacobelli, and the weighted Wasserstein/JKO theory to Iacobelli, Patacchini and Santambrogio. With \(\omega=\rho^{1/(r+1)}\) and \(u=f/\omega\), the equation reads
\[
\partial_t u=-\frac{r+1}{\omega}\diverg\!\left(\omega\nabla(u^{-r})\right).
\]

\paragraph{Quantization with fixed equal weights.}
For fixed equal weights, set \(\nu_Y=m^{-1}\sum_j\de_{y_j}\) and
\(\Ff_{\rm eq}(Y)=\frac12\Wass_2^2(\al,\nu_Y)\). If \(\al\) has a density and \(C_j(Y)\) are the optimal equal-mass Laguerre cells, define \(\bar x_j(Y)=m\int_{C_j(Y)}x\d\al(x)\). Then
\(\nabla_{y_j}\Ff_{\rm eq}(Y)=m^{-1}(y_j-\bar x_j(Y))\). For the empirical \(\Wass_2\) metric \(g_Y(U,V)=m^{-1}\sum_j\dotp{u_j}{v_j}\), the gradient flow is
\[
\dot y_j(t)=\bar x_j(Y_t)-y_j(t),\qquad j=1,\ldots,m,
\]
the so-called finite-particle \(\Wass_2\) gradient flow studied in Section~\ref{sec-wasserstein-gradient-flows}. These fixed-weight cells are Laguerre cells, not the free-mass Voronoi cells of Lloyd's method.

\paragraph{Equal-weight quantization on the line.}
\begin{prop}[Equal-weight quantization on the line]\label{prop-1d-equal-weight-quantization}
Let $\al\in\Mm_+^1(\RR)$ have quantile $Q=\cumul{\al}^{-1}$ and define
\[
\Qq_m^{\mathrm{eq}}(\al)^2
\eqdef
\min_{y_1\leq\cdots\leq y_m}\Wass_2^2\left(\al,\frac1m\sum_{i=1}^m\de_{y_i}\right).
\]
For $I_i=((i-1)/m,i/m]$, the sorted minimizer is \(y_i^\star=m\int_{I_i}Q(u)\d u\); for the uniform law this is \(y_i^\star=(i-1/2)/m\). Moreover,
\[
\Qq_m^{\mathrm{eq}}(\al)^2
=
\sum_{i=1}^m\int_{I_i}\abs{Q(u)-y_i^\star}^2\d u.
\]
If \(Q\in C^1([0,1])\), then
\[
m^2\Qq_m^{\mathrm{eq}}(\al)^2\to \frac1{12}\int_0^1|Q'(u)|^2\d u.
\]
\end{prop}
\begin{proof}
By the one-dimensional quantile formula, the objective is \(\sum_i\int_{I_i}|Q(u)-y_i|^2\d u\). The minimizer on each bin is the \(L^2\) projection onto constants, hence the bin average \(\bar Q_{I_i}\). If \(h=1/m\), uniform Taylor expansion gives \(Q(a+hv)=Q(a)+hvQ'(a)+o(h)\) on all bins, so the bin variance is \(h^3|Q'(a)|^2/12+o(h^3)\); summing gives the Riemann limit.
\end{proof}
\begin{prop}[Random placement on the line]\label{prop-1d-random-quantile-process}
Let \(Q\in C^1([0,1])\) and \(\widehat\al_m=m^{-1}\sum_i\de_{X_i}\), \(X_i\sim\al\) iid. If \(B\) is a standard Brownian bridge, then
\[
m\,\Wass_2^2(\al,\widehat\al_m)\overset{\mathrm{law}}{\longrightarrow}
\int_0^1 B(u)^2|Q'(u)|^2\d u,
\qquad
m\,\EE[\Wass_2^2(\al,\widehat\al_m)]
\to
\int_0^1u(1-u)|Q'(u)|^2\d u.
\]
Thus deterministic inverse-CDF particles have \(O(m^{-2})\) squared error, while iid particles have \(O(m^{-1})\) average squared error.
\end{prop}
\begin{proof}
Write \(X_i=Q(U_i)\) with \(U_i\) uniform. The uniform quantile-process theorem gives \(\sqrt m(\widehat U_m^{-1}-\Id)\Rightarrow -B\) in \(L^2\), and the \(C^1\) delta method gives \(\sqrt m(\widehat Q_m-Q)\Rightarrow -BQ'\). Since \(\Wass_2^2(\al,\widehat\al_m)=\|\widehat Q_m-Q\|_{L^2}^2\) and \(B\) is symmetric, the continuous mapping theorem gives the law limit; uniform integrability and \(\EE B(u)^2=u(1-u)\) give the expectation limit.
\end{proof}


\subsection{Wasserstein-1 norm}
\label{sec-W1}
\paragraph{\texorpdfstring{$c$-transform for $\Wass_1$.}{c-transform for W1.}}
Assume that $d$ is a distance on $\X=\Y$ and take the ground cost $c(x,y)=d(x,y)$. We denote the Lipschitz constant of $f\in\Cc(\X)$ by

\begin{defn}[Lipschitz constant]\label{def-lipschitz-constant}
For a function $f:\X\to\RR$ on a metric space $(\X,d)$, its Lipschitz constant is
\eql{\label{eq-lip-constant}
\Lip(f)
\eqdef
\sup\enscond{\frac{|f(x)-f(y)|}{d(x,y)}}{x\neq y}.
}
The function is $1$-Lipschitz when $\Lip(f)\leq1$.
\end{defn}
\begin{prop}[$c$-transforms and $1$-Lipschitz functions]\label{prop-w1-c-transform-lipschitz}
Suppose $\X=\Y$ and $c(x,y)=d(x,y)$. Then there exists $g$ such that $f=g^c$ if and only if $\Lip(f)\leq1$. Furthermore, if $\Lip(f)\leq1$, then $f^c=-f$.
\end{prop}
\begin{proof}
First suppose $f=g^c$ for some $g$. For $x,y\in\X$,
\[
|f(x)-f(y)|
=
\left|\inf_z[d(x,z)-g(z)]-\inf_z[d(y,z)-g(z)]\right|
\leq
\sup_z |d(x,z)-d(y,z)|
\leq d(x,y),
\]
where the last inequality is the reverse triangle inequality. Thus $\Lip(f)\leq1$.

If $\Lip(f)\leq1$, then $f(x)\leq f(y)+d(x,y)$, so $d(x,y)-f(x)\geq -f(y)$ for all $x$ and hence $f^c(y)\geq -f(y)$. Taking $x=y$ gives $f^c(y)\leq -f(y)$. Therefore $f^c=-f$. Applying the same property to $-f$ gives $(-f)^c=f$, so every $1$-Lipschitz function is $c$-concave.
\end{proof}
\eql{\label{eq-w1-metric}
\Wass_1(\al,\be)
=
\umax{f}
\enscond{\int_\X f\d(\al-\be)}{\Lip(f)\leq1}
=:\norm{\al-\be}_{W_1}.
}
This defines the Kantorovich--Rubinstein norm $\norm{\cdot}_{W_1}$ on signed measures of total mass zero.
For a discrete signed measure $\al-\be=\sum_k r_k\de_{z_k}$ with $\sum_k r_k=0$,~\eqref{eq-w1-metric} becomes the finite-dimensional linear program

\eql{\label{eq-w1-discr}
\Wass_1(\al,\be)
=
\umax{(f_k)_k}
\enscond{\sum_k f_k r_k}{\foralls k,\ell,\ |f_k-f_\ell|\leq d(z_k,z_\ell)}.
}
This LP has $O(N^2)$ Lipschitz constraints, so the dual form alone does not remove the dense all-pairs structure. For metrics generated by local moves, local inequalities suffice: in $1$-D, after sorting $z_1\leq z_2\leq\cdots$,
\(\Wass_1(\al,\be) = \umax{(f_k)_k} \enscond{\sum_k f_k r_k}{\foralls k,\ |f_{k+1}-f_k|\leq z_{k+1}-z_k}.\)
\paragraph{\texorpdfstring{$\Wass_1$ on Euclidean spaces.}{W1 on Euclidean spaces.}}
\eql{\label{eq-w1-cont}
\Wass_1(\al,\be) =
\usup{f} \enscond{ \int_{\RR^\dim} \f (\d\al-\d\be) }{ \norm{\nabla \f}_\infty \leq 1 }.
}
\(-\iota_{\norm{\cdot}_{\RR^d} \leq 1}(u) = \uinf{v} \dotp{u}{v} + \norm{v}_{\RR^d},\)
\begin{align*}
\Wass_1(\al,\be) &=
\usup{f} \uinf{\flow(x) \in \RR^d} \int_{\RR^\dim} \f \d\xi + \int \dotp{\nabla f(x)}{\flow(x)} \d x + \int \norm{\flow(x)}_{\RR^d} \d x \\
&=
\uinf{\flow(x) \in \RR^d} \int \norm{\flow(x)} \d x
+ \usup{f} \int f(x) ( \d \xi - \diverg(\flow) \d x)
\end{align*}
\eql{\label{eq-w1-cont-div}
\Wass_1(\al,\be) =
\uinf{\flow} \enscond{ \int_{\RR^\dim} \norm{\flow(x)}_{\RR^d} \d x }{ \diverg(\flow)=\al-\be },
}
\paragraph{Graph distances.}
\begin{defn}[Graph geodesic distance]\label{def-graph-geodesic-distance}
Let $G=(V,E)$ be a connected finite graph with positive edge lengths $(\ell_e)_{e\in E}$. The graph geodesic distance between two vertices is \(d_G(i,j)=\min_{\gamma:i\leadsto j}\sum_{e\in\gamma}\ell_e.\)
The minimum is over all paths $\gamma$ joining $i$ to $j$.
\end{defn}
\begin{prop}[$\Wass_1$ and Beckmann flow on a graph]\label{prop-graph-w1-beckmann}
Let $G=(V,E)$ be a connected finite graph with positive edge lengths $(\ell_e)_{e\in E}$ and graph geodesic distance $d_G$.
For two probability vectors $\a,\b$ on $V$, set $r=\a-\b$ and orient each edge $e=(i,j)$. If
\((\nabla_G f)_e=f_j-f_i,\qquad \operatorname{div}_G=-\nabla_G^*\)
are the finite-difference gradient and its negative adjoint, then a positive flow on the oriented edge $i\to j$ has positive divergence at $i$ and negative divergence at $j$. With this convention,
\[
\Wass_{1,G}(\a,\b)
=
\max_f \enscond{\sum_{i\in V} f_i r_i}{|f_i-f_j|\leq \ell_e\quad\forall e=(i,j)}
=
\min_m \enscond{\sum_{e\in E}\ell_e |m_e|}{\operatorname{div}_G m=r}.
\]
The vector $m_e$ is an oriented edge flow, and the constraint $\operatorname{div}_G m=r$ is conservation of mass at each vertex.
\end{prop}
\begin{proof}
The edge constraint $|f_i-f_j|\leq\ell_e$ implies, by summing along paths, that $|f_i-f_j|\leq d_G(i,j)$ for all vertices. Conversely, any $1$-Lipschitz function for $d_G$ satisfies the edge constraints because each edge is a path of length $\ell_e$. The first equality is therefore the Kantorovich--Rubinstein formula on the metric space $(V,d_G)$.

For the second equality, write the graph Beckmann problem and dualize its equality constraint with a potential $f$: \(\inf_m \sum_e\ell_e|m_e| +\sup_f \sum_i f_i(r_i-(\operatorname{div}_G m)_i).\)
Using $\operatorname{div}_G=-\nabla_G^*$, the coupling term is $\sum_e m_e(\nabla_G f)_e$. The minimization over each scalar flow $m_e$ is finite exactly when $|(\nabla_G f)_e|\leq\ell_e$, and is then equal to zero. The dual problem is therefore precisely the graph Lipschitz dual above. Strong duality holds because this is a finite-dimensional linear program with a nonempty feasible set: connectedness and $\sum_i r_i=0$ allow the signed surplus to be routed along paths. This proves the graph Beckmann formula.
\end{proof}
