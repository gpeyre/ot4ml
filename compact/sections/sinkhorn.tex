...
\]
For $c(x,y)=\norm{x-y}^2$, the Sinkhorn kernel satisfies
\[
\K_\epsilon(x,y)=e^{-\norm{x-y}^2/\epsilon}=(\pi\epsilon)^{d/2}h_{\epsilon/4}(x,y).
\]
On a manifold, one therefore replaces dense multiplication by $\K_\epsilon$ with the heat operator $\mathsf H_\epsilon=e^{-(\epsilon/4)L}$; Varadhan's formula gives

\(-\epsilon\log h_{\epsilon/4}(x,y)\longrightarrow d_M(x,y)^2\) as $\epsilon\downarrow0$.
\[
\mathsf H_\epsilon
=
\lim_{q\to\infty}\left(\Id+\frac{\epsilon}{4q}L\right)^{-q}.
\]
For a sparse $L_h$, factor $\Id+\epsilon L_h/(4q)$ once by sparse Cholesky and reuse the factor in every Sinkhorn kernel application. This avoids both the dense kernel and all-pairs geodesic distances.
