...
Hence $\KL(\pi_\epsilon|\al\otimes\be)\leq \epsilon^{-1}\int c\,\d\al\d\be$. Theorem~\ref{thm-pinsker} gives total-variation convergence to $\al\otimes\be$, and the convergence of the values follows by continuity of $c$.
\end{proof}
...
For the $\phi$-divergence dual~\eqref{eq-phi-regularized-ot-dual}, the exact block maximizers are the pointwise transforms
\eql{\label{eq-phi-soft-c-transform}
\begin{aligned}
g^{\bar c,\epsilon,\phi}(x)
&\in\uargmin{u\in\RR}
\left\{\epsilon\int_{\Yy}\phi^*\!\left(\frac{u+g(y)-c(x,y)}{\epsilon}\right)\d\be(y)-u\right\},\\
f^{c,\epsilon,\phi}(y)
&\in\uargmin{v\in\RR}
\left\{\epsilon\int_{\Xx}\phi^*\!\left(\frac{f(x)+v-c(x,y)}{\epsilon}\right)\d\al(x)-v\right\}.
\end{aligned}
}
When $\phi^*$ is differentiable, the first minimizer satisfies
\(\int_{\Yy}(\phi^*)'((u+g(y)-c(x,y))/\epsilon)\d\be(y)=1,\)
and the second satisfies the symmetric equation. These scalar updates are monotone, although outside KL they are generally not log-sum-exp transforms.

The Bregman dual~\eqref{eq-bregman-regularized-ot-dual} instead defines the function-space transforms
\eql{\label{eq-bregman-soft-c-transform}
\begin{aligned}
g^{\bar c,\epsilon,\Phi}
&\in\uargmin{u\in\Cc(\Xx)}
\left\{\epsilon\Phi^*\!\left(\delta\Phi(\xi)+\frac{u\oplus g-c}{\epsilon}\right)-\int u\d\al\right\},\\
f^{c,\epsilon,\Phi}
&\in\uargmin{v\in\Cc(\Yy)}
\left\{\epsilon\Phi^*\!\left(\delta\Phi(\xi)+\frac{f\oplus v-c}{\epsilon}\right)-\int v\d\be\right\}.
\end{aligned}
}
They become pointwise when $\Phi$ is separable. Alternate dual maximization is
...
For KL the two lines coincide and recover Sinkhorn. Quadratic $\phi$ and Bregman penalties instead give the positive-part laws
\eql{\label{eq-quadratic-regularized-density-laws}
\P^\star_{i,j}=a_i b_j\left(1+\frac{f_i^\star+g_j^\star-\C_{i,j}}{\epsilon}\right)_+,
\qquad
\P^\star_{i,j}=\left(a_i b_j+\frac{f_i^\star+g_j^\star-\C_{i,j}}{\epsilon}\right)_+,
}
respectively, and can therefore yield sparse plans, as advocated by Blondel, Seguy, and Rolet. Each scalar transform is a piecewise-affine threshold problem; sorting its breakpoints, equivalently projecting onto a weighted simplex, costs $O(m\log m)$ for a row of length $m$.
...
\textbf{Case $\epsilon \rightarrow 0$.}
The first limit follows from $\Gamma$-convergence of entropic OT to the Kantorovich problem. Non-negativity of relative entropy and continuity of $c$ give the liminf inequality; approximating an optimal coupling by finite-entropy couplings, at a scale for which $\epsilon$ times the entropy vanishes, gives the limsup inequality. Since $c(x,x)=0$, the two self-costs converge to zero and the cross term converges to $\MK_\c(\al,\be)$.
...
Assume that the entropic dual admits optimal potentials and that the symmetric kernel $k_\epsilon(x,y)=e^{-c(x,y)/\epsilon}$ is positive semidefinite in the sense of Definition~\ref{def-positive-kernels}. Then $\bar\MK_\c^\epsilon(\al,\be)\geq0$. If, in addition, the common state space is compact, $c$ is continuous, and $k_\epsilon$ is universal, meaning that its kernel sections span a uniformly dense subset of $\Cc(\Xx)$, then $\bar\MK_\c^\epsilon(\al,\be)=0$ if and only if $\al=\be$, and convergence of the divergence to zero is equivalent to weak convergence.
\end{prop}
\begin{proof}
...
If the kernel is universal and the divergence vanishes, equality in these bounds gives identical kernel mean embeddings for $\tilde\al$ and $\tilde\be$, hence $\tilde\al=\tilde\be\eqdef\tilde\mu$. The self-consistency equations then imply $\d\al=(k_\epsilon\tilde\mu)\d\tilde\mu=\d\be$, so $\al=\be$. Continuity and compactness give the weak-convergence statement. Positive semidefiniteness without universality guarantees only non-negativity.
\end{proof}
\subsection{Complex \texorpdfstring{$\epsilon$}{epsilon}}
\label{sec-complex-epsilon}

The real Sinkhorn temperature admits a local complex continuation, but only at the level of the scaling equations: positivity and global convergence no longer apply.

\paragraph{Measure fixed point.}
For $\epsilon\in\CC\setminus\{0\}$, the continuous soft-transform equations~\eqref{eq-soft-c-cont-f}--\eqref{eq-soft-c-cont-g}, equivalently their multiplicative scaling form, remain valid verbatim for complex-valued scalings whenever the defining integrals exist and the divisions are well defined.

\paragraph{Discrete histograms.}
For discrete histograms, the factorization~\eqref{eq-scaling-form} and updates~\eqref{eq-sinkhorn} are used verbatim over $\CC$ whenever $\epsilon\neq0$ and all divisions are defined. Carlier, Pegon and Tamanini proved analytic dependence of the normalized Schr\"odinger potentials and entropic cost on every real $\epsilon>0$, and explicitly observed the resulting local continuation to complex temperatures. The following scaling-variable version makes this continuation precise in finite dimension.

\begin{thm}[Local holomorphic continuation of Sinkhorn scalings]\label{thm-carlier-complex-sinkhorn}
Fix positive histograms $\a^0\in\simplex_n$, $\b^0\in\simplex_m$, a finite real cost matrix $\C^0$, and $\epsilon_0>0$. Choose positive scalings $(\uD^0,\vD^0)$ of the corresponding Sinkhorn coupling. Locally around $(\epsilon_0,\a^0,\b^0,\C^0)$, within $\sum_i\a_i=\sum_j\b_j$, there is a unique holomorphic map $(\epsilon,\a,\b,\C)\mapsto(\uD_\epsilon,\vD_\epsilon)$ satisfying
\[
\sum_i\a_i^0u_{\epsilon,i}=\sum_i\a_i^0u_i^0,
\qquad
\P_\epsilon=\diag(\uD_\epsilon)\K_\epsilon(\C)\diag(\vD_\epsilon),
\qquad
\P_\epsilon\ones_m=\a,
\qquad
...
\begin{proof}
Use the $n$ row residuals, the first $m-1$ column residuals, and the displayed gauge in the holomorphic implicit-function theorem. If $(\delta\uD,\delta\vD)$ lies in the kernel of the derivative, set $r_i=\delta u_i/u_i^0$ and $s_j=\delta v_j/v_j^0$. Then $\delta\P_{i,j}=\P^0_{i,j}(r_i+s_j)$ has zero row and column sums, and
\[
0=\sum_{i,j}\P^0_{i,j}|r_i+s_j|^2.
\]
Positivity gives $r_i=\theta$ and $s_j=-\theta$. The linearized gauge gives $\theta\sum_i\a_i^0u_i^0=0$, hence $\theta=0$. The derivative is invertible, so the implicit-function theorem applies.
\end{proof}
