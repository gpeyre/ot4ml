...
\]
Thus a single inverse shifted Laplacian $(\Id+\tau L)^{-1}$ gives a Laplace/Yukawa smoothing, while repeated resolvent steps approximate the Gaussian heat kernel. On a triangulated surface or image grid, $L$ is replaced by a discrete Laplacian $L_h$, and one applies sparse linear solves rather than dense Gaussian matrices.
...
\right\}.
\]
Define the Bregman-regularized value, using the same product reference as the density-ratio penalty, \(\MK_{\c,\Phi}^{\epsilon}(\al,\be) \eqdef \inf_{\pi\in\Couplings(\al,\be)} \int c\,\d\pi+\epsilon B_\Phi(\pi|\xi).\)
...
\(e^{f_{\al,\al}(x)/\epsilon}\int k_\epsilon(x,y)\d\tilde\al(y)=1 \qquad \text{for }\al\text{-a.e. }x,\)
and hence
\eq{
...
\(\fD=\widehat\fD+\epsilon\log\a, \qquad \gD=\widehat\gD+\epsilon\log\b,\)
