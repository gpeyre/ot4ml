...
\begin{rem}[Entropy barriers versus generic LP barriers]
The entropy used in Sinkhorn is not the generic logarithmic barrier of interior-point linear programming. Its special feature is separability over the entries of $\P$, which turns Bregman projections onto marginal constraints into diagonal matrix scalings.
\end{rem}
...
The KL term measures the dependence created by the coupling.
...
\(f_\epsilon+\frac{d\epsilon}{2}\log(\pi\epsilon)\longrightarrow f_0, \qquad g_\epsilon\longrightarrow g_0,\)
...
\(T\mathscr C_T(\al,\be) = \frac12\Wass_2^2(\al,\be) +\frac{T}{2}\pa{\mathrm H(\al)+\mathrm H(\be)} +\frac{T^2}{8}\Ii_0(\al,\be)+o(T^2),\)
under the stated regularity assumptions. Since
\(\mathscr C_T(\al,\be) = \inf_{\pi\in\Couplings(\al,\be)} \KL(\pi|p_T(x,y)\,\d x\d y), \qquad p_T(x,y)=(2\pi T)^{-d/2}e^{-\norm{x-y}^2/(2T)},\)
multiplying by $2T$ and setting $\epsilon=2T$ gives \(\MK_{\norm{\cdot}^2}^{\epsilon}(\al,\be) = 2T\mathscr C_T(\al,\be) -2T\pa{\mathrm H(\al)+\mathrm H(\be)} -dT\log(2\pi T),\)
...
This compact subsection connects entropic OT with path-space Schr\"odinger bridges.
