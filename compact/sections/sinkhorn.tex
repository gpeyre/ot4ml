...
If $(X,Y)\sim\pi$ have marginals $X\sim\al$ and $Y\sim\be$, the mutual information of the pair is \(\Ii(X,Y)\eqdef\KL(\pi|\al\otimes\be)\). It is nonnegative and vanishes if and only if $X$ and $Y$ are independent.
\end{defn}
With this terminology,~\eqref{eq-entropic-generic} is \(\inf_{X\sim\al,\;Y\sim\be}\EE(c(X,Y))+\epsilon\Ii(X,Y)\). Large $\epsilon$ favors nearly independent endpoints, while small $\epsilon$ recovers an optimal Monge--Kantorovich coupling; for quadratic transport with a Brenier map, this limiting coupling is deterministic.
