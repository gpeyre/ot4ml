...
The same heat-kernel calculus gives the soft Hopf--Lax transform
\[
Q_t^\epsilon f(x)=-\epsilon\log\int
\exp\!\left(-\frac{f(y)+\norm{x-y}^2/(2t)}{\epsilon}\right)\d y
=-\epsilon\log\big(G_{\epsilon t/2}\ast e^{-f/\epsilon}\big)(x)+\mathrm{cst},
\]
where \(G_s\) is the normalized heat kernel and the constant absorbs its normalization. It converges to \(Q_t f(x)=\inf_y\{f(y)+\norm{x-y}^2/(2t)\}=\norm{x}^2/(2t)-(f+\norm{\cdot}^2/(2t))^*(x/t)\). The Hopf--Cole change \(u=e^{-\phi/(2\nu)}\) maps
\(\partial_t\phi+\frac12\norm{\nabla\phi}^2=\nu\Delta\phi\)
to the heat equation \(\partial_tu=\nu\Delta u\), and \(v=\nabla\phi=-2\nu\nabla\log u\) solves viscous Burgers.
