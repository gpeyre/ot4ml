...
Thus~\eqref{eq-regularized-discr-rescaled} has exactly the same minimizer as the original entropic problem~\eqref{eq-regularized-discr}, while its optimal value is \( \MKD_\C^\epsilon(\a,\b)+\epsilon\bigl(\HD(\a)+\HD(\b)\bigr) \). This normalization will matter again for unbalanced OT, where changing the reference measure is no longer merely a harmless notational choice.
...
Thus the coupling density reconstructed from the current potentials is \( u^{(\ell)}(x)k_\epsilon(x,y)v^{(\ell)}(y) \) with respect to $\al\otimes\be$, exactly as in the scaling law~\eqref{eq-continuous-sinkhorn-scaling}. At a fixed point, its two marginals are $\al$ and $\be$; equivalently, the potentials jointly maximize the continuous dual.
...
\(\epsilon\KL(\pi|\Rr^\epsilon_{0,1}) = \int c(x,y)\,\d\pi(x,y) + \epsilon\KL(\pi|\al\otimes\be) + \mathrm{constant},\)
...
where $\mathsf H_\epsilon$ denotes the discretized integral operator, including quadrature weights, and $\mathsf H_\epsilon^\top$ its discrete adjoint. In a symmetric mass-normalized discretization, the two operators coincide.
