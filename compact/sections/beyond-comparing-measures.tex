...
In Euclidean applications, the intrinsic profile lower bound can be paired with an extrinsic upper certificate obtained by rigid registration.
\begin{prop}[Wasserstein--Procrustes upper certificate]\label{prop-gw-procrustes-upper-certificate}
Let $\alpha,\beta$ be probability measures on $\RR^d$, equipped with the Euclidean distance, and take $\De(u,v)=|u-v|$. If $\Wass_{p,\mathrm E(d)}$ is the quotient Wasserstein distance for the Euclidean group, then
\[
\GW((\RR^d,\norm{\cdot},\alpha),(\RR^d,\norm{\cdot},\beta))
\leq
...
\end{prop}
\begin{proof}
Rigid motions preserve pairwise distances and hence do not change the metric-measure isometry class. For any rigid motions $g,h$, Proposition~\ref{prop-gw-controlled-by-wasserstein} applied to $g_\sharp\alpha,h_\sharp\beta$ gives
\(\GW((\RR^d,\norm{\cdot},\alpha),(\RR^d,\norm{\cdot},\beta))\leq2\Wass_p(g_\sharp\alpha,h_\sharp\beta).\)
Taking the infimum over $g,h\in\mathrm E(d)$ proves the claim.
\end{proof}
Together with the M\'emoli profile lower bound, this gives the Euclidean sandwich
\[
...
\YY=(\RR^d,\norm{\cdot},\beta).
\]
The left term is intrinsic and cheap to compute; the right term is an ambient rigid-registration certificate.
...
The finite-dimensional case is the quantum analogue of Kantorovich couplings: probabilities become positive semidefinite trace-one matrices, and a coupling is a positive semidefinite operator on a tensor product with prescribed partial traces.
