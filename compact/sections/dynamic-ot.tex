...
\end{cases}
\]
For a curve \(\partial_t\alpha_t+\diverg\mu_t=0\), with \(\alpha_t=\rho_t\d x\) and \(\mu_t=m_t\d x\), write \(\mathbb J_A(\alpha_t,\mu_t)=\int J_A(\rho_t,m_t)\d x\). If \(J_A\) is lower semicontinuous, convex, even in \(m\), \(r\)-homogeneous in \(m\), and vanishes only for \(m=0\), define
\[
\mathsf D_A(\alpha_0,\alpha_1)
\eqdef
\inf_{\substack{\partial_t\alpha_t+\diverg\mu_t=0\\
\alpha_{t=0}=\alpha_0,\ \alpha_{t=1}=\alpha_1}}
\left(\int_0^1 \mathbb J_A(\alpha_t,\mu_t)\d t\right)^{1/r}.
\]
...
\end{prop}
\begin{proof}
The constant curve gives zero self-distance. Conversely, zero action implies \(\mu_t=0\) a.e.; the continuity equation gives \(\partial_t\alpha_t=0\), hence equal endpoints.
