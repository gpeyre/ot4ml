...
Here the action is quadratic and represented by an operator \(Q_\alpha\) on \(L^2(\alpha)\), or on a closed subspace in constrained applications.
\[
\mathbb A(\alpha;w,z)
=
...
The local action formalism keeps the continuity equation but replaces the quadratic cost \(|m|^2/\rho\) by a convex momentum perspective. Starting from a velocity action \(A(a,w)\), set
\[
J_A(a,m)=
\begin{cases}
A(a,m/a), & a>0,\\
...
\end{cases}
\]
For a curve \(\partial_t\alpha_t+\diverg\omega_t=0\), with \(\alpha_t=\rho_t\d x\) and \(\omega_t=m_t\d x\), write \(\mathbb J_A(\alpha_t,\omega_t)=\int J_A(\rho_t,m_t)\d x\). If \(J_A\) is lower semicontinuous, convex, even in \(m\), \(r\)-homogeneous in \(m\), and vanishes only for \(m=0\), define
\[
\mathsf D_A(\alpha_0,\alpha_1)
\eqdef
\inf_{\substack{\partial_t\alpha_t+\diverg\omega_t=0\\
\alpha_{t=0}=\alpha_0,\ \alpha_{t=1}=\alpha_1}}
\left(\int_0^1 \mathbb J_A(\alpha_t,\omega_t)\d t\right)^{1/r}.
\]
Equivalently, away from zero density, \(A(a,-w)=A(a,w)\), \(A(a,\xi w)=|\xi|^rA(a,w)\), and \(A(a,w)=0\) iff \(w=0\). For a smooth state \(\alpha=\rho\d x\) and velocity \(v\), the associated local squared action is

\begin{equation}\label{eq-local-action-density-induced}
q_{A,\alpha}(v)
\eqdef
\left(
\int J_A(\rho(x),\rho(x)v(x))\,\d x
\right)^{2/r}.
\end{equation}
\begin{prop}[Homogeneous dynamic actions define distances]\label{prop-homogeneous-dynamic-action-distance}
Under the hypotheses above and after the usual lower-semicontinuous relaxation assuming that zero-action values are attained, \(\mathsf D_A\) is an extended distance on each finite-action component: it is symmetric, satisfies the triangle inequality, and \(\mathsf D_A(\alpha_0,\alpha_1)=0\) only when \(\alpha_0=\alpha_1\).
\end{prop}
\begin{proof}
The constant curve gives zero self-distance. Conversely, zero action implies \(\omega_t=0\) a.e.; the continuity equation gives \(\partial_t\alpha_t=0\), hence equal endpoints.

Symmetry follows by time reversal and evenness in \(m\). For the triangle inequality, concatenate two admissible curves with actions \(a\) and \(b\) using time fractions \(\tau\) and \(1-\tau\). Homogeneity gives the action \(\tau^{1-r}a+(1-\tau)^{1-r}b\). Optimizing in \(\tau\) yields \((a^{1/r}+b^{1/r})^r\), hence the triangle inequality after taking infima.
\end{proof}
...
\end{cases}
\]
The factors \(\zeta^{-1}\) and \((1-\zeta)^{-1}\) are exactly those needed for the weak continuity equation after changing time. Since \(\mathcal A_K\) is quadratic in the velocity,
\(\int_0^1\mathcal A_K(\rho_t,v_t)\,\d t = \frac{A_0}{\zeta}+\frac{A_1}{1-\zeta}.\)
...
\]
For \(a\in\Sigma_n\), write the relative density \(\rho_i(a)=a_i/\pi_i\).
...
\end{equation}
\((\mathcal L_a\psi)_i\eqdef\pi_i(\mathcal K_{\rho(a)}\psi)_i.\)
\begin{equation}
\label{eq-discrete-markov-action}
\mathcal A(a,\psi)
\eqdef
\frac12\sum_{i,j}\pi_iK_{ij}\theta(\rho_i(a),\rho_j(a))(\psi_i-\psi_j)^2.
\end{equation}
\begin{equation}
...
\eqdef
\inf_{a_t,\psi_t}
\int_0^1\mathcal A(a_t,\psi_t)\,\d t,
\qquad
