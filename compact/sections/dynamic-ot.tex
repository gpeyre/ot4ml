...
\end{equation}
whenever this limit exists. The notation \(q_\alpha\) is intentionally broader than a quadratic form: it is the squared local norm, or more generally the squared local action, selected by \(d\).

\paragraph{Geodesic distances from quadratic tangent norms.}
Here \(q_\alpha\) is quadratic and represented by an operator \(Q_\alpha\).
\[
q_\alpha(w,z)
=
...
\(A(\rho,\lambda m)=|\lambda|^rA(\rho,m) \qquad \text{for all }\lambda\in\RR,\)
and that \(A(\rho,m)=0\) if and only if \(m=0\). Set
\(\mathsf D_A(\rho_0,\rho_1) \eqdef \mathcal A_A(\rho_0,\rho_1)^{1/r}.\)
...
\end{cases}
\]
The rescaled curve is admissible from \(\rho_0\) to \(\rho_2\). By \(r\)-homogeneity in \(m\), its action is \(\tau^{1-r}a+(1-\tau)^{1-r}b.\)
