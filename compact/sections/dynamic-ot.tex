...
A second class of variants changes the mobility of the medium: the quadratic action $|m|^2/\rho$ is replaced by $|m|^2/\theta(\rho)$ for a suitable concave mobility $\theta$. The finite-state version, where the mobility is dictated by a reversible Markov chain, is developed in Section~\ref{sec-discrete-wasserstein-markov}.
