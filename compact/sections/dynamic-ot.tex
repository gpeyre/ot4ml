...
\end{equation}
For the classical linear density path, let $\alpha_i=\rho_i\d x$ be smooth positive densities of equal mass and set $\alpha_t=(1-t)\alpha_0+t\alpha_1=\rho_t\d x$, where $\rho_t=(1-t)\rho_0+t\rho_1$. If \(w\) solves \(\diverg w=\rho_0-\rho_1\) with \(w\cdot n=0\) on the boundary, for instance \(w=-\nabla\phi\), \(\Delta\phi=\rho_1-\rho_0\), then \(v_t=w/\rho_t\) satisfies \(\partial_t\rho_t+\diverg(\rho_t v_t)=0\). Its flow transports \(\rho_0\d x\) to \(\rho_t\d x\), hence \(T_1\) solves \(\rho_1(T_1(x))\det\nabla T_1(x)=\rho_0(x)\).
...
Let $\gamma$ be a monotone spectral gauge as in Definition~\ref{def-spectral-wasserstein}. Define
\[
\mathcal N_{\gamma,\alpha}(v)^2\eqdef\gamma\!\left(\int vv^\top\d\alpha\right),
\qquad
...
\]
\begin{prop}[Static/dynamic spectral equality]\label{prop-static-dynamic-spectral-wasserstein}
For $\alpha_0,\alpha_1\in\Pp_2(\RR^d)$,
\(\mathsf W_{\gamma,\mathrm{dyn}}(\alpha_0,\alpha_1)=\Wass_\gamma(\alpha_0,\alpha_1)\).
\end{prop}
\begin{proof}
For a static coupling $\pi$, the interpolation $Z_t=(1-t)X+tY$ with Eulerian velocity \(v_t(z)=\EE[Y-X\mid Z_t=z]\) gives \(C_t=\int v_tv_t^\top\d\alpha_t\preceq M_\pi\) by conditional Jensen, hence dynamic action at most $\gamma(M_\pi)$ by monotonicity of $\gamma$. Conversely, finite spectral action implies finite kinetic energy because $\gamma$ is equivalent to the trace on $\mathbb S_+^d$. The superposition principle therefore represents any dynamic competitor by paths \(\omega_t\) with $\dot\omega_t=v_t(\omega_t)$. For the endpoint coupling, Jensen gives \(M_\pi\preceq\int_0^1C_t\d t\). Monotonicity and convexity give \(\gamma(M_\pi)\leq\int_0^1\gamma(C_t)\d t\). Infimizing on both sides proves equality. The monotonicity of $\gamma$ is essential: the proof only produces Loewner-order covariance inequalities.
\end{proof}
...
\]
Its square root is the associated kernelized transport length and gives the dynamic geometry behind SVGD: the $L^2(\alpha)$ norm of velocities is replaced by the vector-valued RKHS norm on smooth vector fields. This length is generally extended: for smooth kernels, finite-action curves are regular flows, so atoms cannot split and a Dirac mass is at infinite distance from an absolutely continuous law.
