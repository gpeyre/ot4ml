...
Assume that every \(\cov_s\succ0\). The forward KL barycenter
\(\al_{\mathrm{KL}}=\argmin_{\al}\sum_s\la_s\KL(\al\mid\be_s)\) is
\(\Gaussian(\mean_{\mathrm{KL}},\cov_{\mathrm{KL}})\), where
\[
\cov_{\mathrm{KL}}=\left(\sum_s\la_s\cov_s^{-1}\right)^{-1},
\qquad
\mean_{\mathrm{KL}}=\cov_{\mathrm{KL}}\sum_s\la_s\cov_s^{-1}\mean_s.
\]
Thus its covariance is the weighted harmonic mean and, unlike for the Wasserstein barycenter, its mean depends on the covariances. This follows by normalizing the geometric mean of the Gaussian densities. The KL direction matters: reverse-KL averaging gives the mixture \(\sum_s\la_s\be_s\), generally not a Gaussian.
\end{rem}
...
and therefore $\sum_{\ell\geq0}\mathcal R(X_\ell)<+\infty$. Equivalently, \(\Bb\bigl(X_\ell,X_{\ell+1}\bigr)^2 = \eta^2\mathcal R(X_\ell).\)
\end{prop}
\begin{proof}
...
Let $d\geq2$, let $\sigma$ be the uniform probability measure on $\Sphere^{d-1}$, and let $R\rho(\theta,t)=\int_{\{x:\dotp{\theta}{x}=t\}}\rho(x)\,\d\mathcal H^{d-1}(x)$ be the density Radon transform. Write
