...
\subsection{Capacity-constrained OT}
\label{sec-capacity-constrained-ot}
Capacity constraints bound the local density of a coupling, preventing a cheap pair from carrying arbitrary mass. For a measurable cap $\kappa\geq0$,
\[
\MK_c^\kappa(\alpha,\beta)
=
\inf_{\pi\in\Couplings(\alpha,\beta)}
\left\{\int c\,\d\pi:\;
\pi\ll\alpha\otimes\beta,\;
\frac{\d\pi}{\d(\alpha\otimes\beta)}\leq\kappa
\right\}.
\]
The product plan is feasible when $\kappa\geq1$; large $\kappa$ recovers ordinary OT, while small $\kappa$ forces a diffuse plan. Discretely, one prescribes an upper matrix \(U\) and solves
\[
\min_{P\in\CouplingsD(\a,\b)}\langle C,P\rangle
\quad\text{s.t.}\quad 0\leq P_{ij}\leq U_{ij}.
\]
For the density-ratio discretization, \(U_{ij}=\kappa_{ij}a_i b_j\). The entropic version
\[
\min_{P\in\CouplingsD(\a,\b),\,P\leq U}
\langle C,P\rangle+\epsilon\KLD(P|\a\otimes\b)
\]
is the KL projection of \(K_{ij}=a_i b_j e^{-C_{ij}/\epsilon}\) onto row constraints, column constraints and the box \(P\leq U\). It can therefore be solved by KL-Dykstra projections: alternately rescale rows, rescale columns and clip to the cap, while updating the usual Dykstra correction factors.
