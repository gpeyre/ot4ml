\documentclass[10pt,a4paper]{article}
\pdfoutput=1
\usepackage[latin1]{inputenc}
\usepackage[
bookmarks,
bookmarksdepth=2,
colorlinks=true,
linkcolor=blue,
urlcolor=blue,
pdftitle={Optimal Transport for Machine Learners: Compact Teaching Notes},
pdfauthor={Gabriel Peyre},
pdfsubject={Compact teaching notes on optimal transport for machine learning},
pdfkeywords={optimal transport, machine learning, Wasserstein distance, Sinkhorn algorithm, teaching notes}
]{hyperref}
\usepackage[a4paper,top=6mm,bottom=6mm,left=6mm,right=6mm,includefoot,footskip=4mm]{geometry}
\usepackage[compact]{titlesec}
\usepackage{enumitem}
\usepackage{longtable}
\usepackage{mystyle}
\definecolor{otcite}{HTML}{7A3B2E}
\definecolor{otparagraph}{HTML}{315F83}
\hypersetup{citecolor=otcite}
\usepackage{wrapfig}
\usepackage{notations_ot}
\usepackage{tikz}

\setlength{\parindent}{0pt}
\setlength{\parskip}{0.2pt}
\linespread{0.90}
\setlength{\abovedisplayskip}{2pt plus .5pt minus .5pt}
\setlength{\belowdisplayskip}{2pt plus .5pt minus .5pt}
\setlength{\abovedisplayshortskip}{1pt plus .5pt minus .5pt}
\setlength{\belowdisplayshortskip}{1pt plus .5pt minus .5pt}
\setlength{\jot}{1pt}
\setlength{\topsep}{0pt}
\setlength{\partopsep}{0pt}
\setlength{\parsep}{0pt}
\setlength{\itemsep}{0pt}
\setlist{nosep,leftmargin=*}
\titlespacing*{\section}{0pt}{.55ex plus .15ex}{.15ex}
\titlespacing*{\subsection}{0pt}{.45ex plus .15ex}{.1ex}
\titlespacing*{\subsubsection}{0pt}{.35ex plus .1ex}{.05ex}
\titlespacing*{\paragraph}{0pt}{.45ex plus .15ex}{.45em}
\titleformat{\section}{\large\bfseries}{\thesection}{.45em}{}
\titleformat{\subsection}{\normalsize\bfseries}{\thesubsection}{.4em}{}
\titleformat{\subsubsection}{\normalsize\itshape}{\thesubsubsection}{.35em}{}
\titleformat{\paragraph}[runin]{\bfseries\color{otparagraph}}{}{0pt}{}
\allowdisplaybreaks[2]
\emergencystretch=2em
\sloppy

\newcommand{\todo}[1]{}
\newcommand{\blue}[1]{#1}
\newcommand{\removed}[1]{}
\newcommand{\dims}{d}
\providecommand{\mathscr}{\mathcal}

\begin{document}
\begin{center}
{\Large\bfseries Optimal Transport for Machine Learners}\\[-.15em]
{\large\itshape Compact teaching notes}\\[.35em]
Gabriel Peyr{\'e}\\[-.1em]
{\small\today}
\end{center}
\vspace{-.6em}

\input{sections/matching}
\input{sections/monge}
\input{sections/kantorovich}
\input{sections/dual}
\input{sections/semidiscr-w1}
\input{sections/dual-norms}
\input{sections/sinkhorn}
\input{sections/sinkhorn-advanced}
\input{sections/statistical-ot}
\input{sections/generalized-wasserstein}
\input{sections/generalized-ot-problems}
\input{sections/beyond-comparing-measures}
\input{sections/dynamic-ot}
\input{sections/wasserstein-gradient-flows}
\input{sections/transportation-models}

\end{document}
