...
\index{Kurdyka-Lojasiewicz inequality}% strictindex
\index{Wasserstein-Kurdyka-Lojasiewicz inequality}% strictindex
...
\index{metric slope}% strictindex
\index{gradient flow!Wasserstein}% strictindex
\index{Kurdyka-Lojasiewicz inequality}% strictindex
...
\begin{proof}
	Let \(\alpha=\rho\gamma_d\), and let \(T=\nabla\phi\) be the Brenier map with \(T_\sharp\alpha=\gamma_d\). Write \(T(x)=x+\theta(x)\). The Monge--Amp\`ere equation reads
	\[
		\rho(x)e^{-\norm{x}^2/2}
		=
...
\begin{example}[Linearized forms for standard discrepancies]\label{ex-linearized-discrepancy-forms}
\index{phi-divergence}% strictindex
\index{maximum mean discrepancy}% strictindex
\index{negative Sobolev norm}% strictindex
	The two forms above can be evaluated explicitly under the indicated smoothness and integrability assumptions.
	\begin{enumerate}
		\item Suppose that \(\beta\) has a smooth positive density and that \(\phi\) is twice continuously differentiable near \(1\), with \(\phi(1)=0\) and \(\phi''(1)>0\). For the Csisz\'ar divergence \(f(\alpha)=\Divergm_\phi(\alpha|\beta)\), one has
		\[
			\mathsf H^f_\beta(\xi)
			=
		...
		\item For \(f(\alpha)=\MMD_k^2(\alpha,\beta)\), using the convention
		\(\MMD_k^2(\alpha,\beta)=\iint k\,\d(\alpha-\beta)\d(\alpha-\beta)\), assume that \(k\) is symmetric, differentiable in its first variable, and positive semidefinite on signed measures of zero total mass, and that the displayed integrals are finite. Then
		\[
			\mathsf H^f_\beta(\xi)
			=
		...
		\item For \(f(\alpha)=\Wass_2^2(\alpha,\beta)\), the calculation is naturally written in the tangent space of \(\Wass_2\). If \(\beta=\rho_\beta\d x\) is smooth and positive and the weighted Poisson problem
		\[
...
\begin{proof}
	...
	\[
		\int\norm{\theta}^2\,\d\gamma_d=\Wass_2^2(\gamma_d,\alpha),
	\]
	which proves the claim. Approximation gives the general case. Equality holds for translations \(\alpha=\mathcal N(a,\Id)\), so the constant is sharp.
\end{proof}
