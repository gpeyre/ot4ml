...
\]
The Wasserstein analogue is the following elementary estimate.
	\begin{prop}[Finite metric length from dissipation]\label{prop-gradient-flow-finite-length}
		Let \((\alpha_r)_{r\in[0,T]}\) be an absolutely continuous curve in \((\Pp_2(\RR^d),\Wass_2)\). If
		\[
			f(\alpha_s)-f(\alpha_t)
			=
			\int_s^t |\dot\alpha_r|_{\Wass_2}^2\d r
		\]
		for all \(0\leq s<t\leq T\), then
		\[
		...
		\]
		For a smooth Wasserstein gradient flow with velocity \(v_r=-\Wgrad f(\alpha_r)\), the dissipation identity is equivalently
		\[
			f(\alpha_s)-f(\alpha_t)
			=
			\int_s^t |\dot\alpha_r|_{\Wass_2}^2\d r
		...
		\]
		With only the weaker maximal-slope energy-dissipation inequality, the same estimate holds with the right-hand side multiplied by \(\sqrt2\).
	\end{prop}
	\begin{proof}
		...
		\[
			\int_s^t |\dot\alpha_r|_{\Wass_2}\d r
			\leq
			\sqrt{t-s}\left(\int_s^t|\dot\alpha_r|_{\Wass_2}^2\d r\right)^{1/2}.
		\]
		The exact dissipation identity gives the stated bound. The maximal-slope energy-dissipation inequality gives \(\frac12\int_s^t|\dot\alpha_r|_{\Wass_2}^2\d r\leq f(\alpha_s)-f(\alpha_t)\), hence the factor \(\sqrt2\).
	\end{proof}
In the abstract metric theory, this is formulated through the energy-dissipation identity for curves of maximal slope~\cite{ambrosio2006gradient,SantambrogioBook}. Thus an energy-dissipating trajectory has finite metric length on every finite interval as soon as the energy is bounded below there. More quantitatively, if \(m_T\leq f(\alpha_r)\) for \(0\leq r\leq T\), then
\[
...
\]
up to the factor \(\sqrt2\) for the metric energy-dissipation inequality. Hence the curve is \(1/2\)-H\"older continuous in Wasserstein space on finite-energy intervals. Non-coercivity means that sublevel sets need not be compact, but it does not by itself allow finite-time escape to infinity in the metric. Convergence rates require additional structure, such as geodesic convexity or a Polyak--{\L}ojasiewicz slope inequality. The latter originates in finite-dimensional optimization~\cite{Polyak1963GradientMethods,KarimiNutiniSchmidt2016PL}; in Wasserstein space it is obtained by replacing the Euclidean gradient norm by the metric slope. The tail-length refinement giving an explicit \(\Wass_2\)-distance to the minimizer set was communicated to us by Rapha{\"e}l Barboni.
...
\]
or, equivalently, a probability measure on $[0,1]\times\Theta$ whose first marginal is Lebesgue measure. In this ResNet setting, the condition is continuous depth, $s\in[0,1]$, the condition law is $\lambda(\d s)=\d s$, and the fiber space is the neuron parameter space $\Theta$. For two conditional neuron laws $\alpha(\d s,\d\theta)=\alpha_s(\d\theta)\d s$ and $\beta(\d s,\d\theta)=\beta_s(\d\theta)\d s$, the depthwise quadratic Wasserstein metric is
...
\end{equation}
For a small displacement $\alpha=(\Id+\tau v)_\sharp\alpha^k$, the dynamic interpretation of $\Wass_\gamma$ developed in~\cite{peyre2026muon} gives the local-action expansion
\[
	\Wass_\gamma(\alpha^k,(\Id+\tau v)_\sharp\alpha^k)^2
	=
	\tau^2q_{\gamma,\alpha^k}(v)+o(\tau^2)
	=
