...
Before asking for rates, energy dissipation already controls the length travelled by the flow. For a smooth Euclidean flow $\dot x_t=-\nabla h(x_t)$,
\[
	\frac{\d}{\d t}h(x_t)=-\norm{\nabla h(x_t)}^2=-\norm{\dot x_t}^2,
\]
and hence, for $0\leq s<t$,
\[
	\int_s^t\norm{\dot x_r}\d r
	\leq
	\sqrt{t-s}\,\bigl(h(x_s)-h(x_t)\bigr)^{1/2}.
\]
For a smooth Wasserstein gradient flow with metric derivative $|\dot\alpha_t|_{\Wass_2}$ and velocity $v_t=-\Wgrad f(\alpha_t)$,
\[
	f(\alpha_s)-f(\alpha_t)
	=
...
\]
In the abstract metric theory, the same conclusion is formulated through the energy-dissipation identity for curves of maximal slope~\cite{ambrosio2006gradient,SantambrogioBook}; with the weaker inequality convention only the numerical constant changes. Thus an energy-dissipating trajectory has finite metric length on every finite interval as soon as the energy is bounded below there. Non-coercivity means that sublevel sets need not be compact, but it does not by itself allow finite-time escape to infinity in the metric. Convergence rates require additional structure, such as geodesic convexity or a Polyak--{\L}ojasiewicz slope inequality. The latter originates in finite-dimensional optimization~\cite{Polyak1963GradientMethods,KarimiNutiniSchmidt2016PL}; in Wasserstein space it is obtained by replacing the Euclidean gradient norm by the metric slope. The tail-length refinement giving an explicit \(\Wass_2\)-distance to the minimizer set was communicated to us by Rapha{\"e}l Barboni.
...
\index{linearized Wasserstein metric}% strictindex
	...
	\[
		...
		\int h\norm{\nabla h}^2\d\beta .
	\]
		The exact PL interpretation is obtained by freezing the mobility at equilibrium. This gives the \(H^{-1}(\beta)\) metric, which is precisely the linearization of \(\Wass_2\) at \(\beta\).
