...
\end{tabular}
\caption{Jacobian determinant in the density push-forward formula. A smooth nonlinear diffeomorphism radially compresses the middle of a cropped colored grid and adds a radius-dependent twist. The source density is locally uniform, but the target density forms an almost isotropic central bump where deformed grid cells have smaller area. The target panel is a crop of a larger warped grid, so no artificial square boundary is visible. This is the geometric content of the factor $|\det\nabla T|^{-1}$ in~\eqref{eq-pfwd-density}, and later becomes the divergence structure of continuity equations.}
