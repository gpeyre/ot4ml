...
\paragraph{Weak convergence and moment classes.}
The PDE constructions below use both weak convergence of laws and the stronger topology induced by transport. A sequence $(\alpha_n)_n\subset\Pp(\X)$ converges weakly to $\alpha$, written $\alpha_n\rightharpoonup\alpha$, when
\[
	\int g\,\d\alpha_n\longrightarrow\int g\,\d\alpha
\]
for every bounded continuous function $g$. After fixing $x_0\in\X$, define
\begin{equation}\label{eq-primer-p-moment-space}
	\Pp_p(\X)
	\eqdef
	\left\{\alpha\in\Pp(\X):\int d(x,x_0)^p\,\d\alpha(x)<+\infty\right\}.
\end{equation}
This definition does not depend on the choice of $x_0$. The finite $p$-moment assumption is exactly what makes the $p$-Wasserstein distance finite. On a Polish space, $\Pp_p(\X)$ is itself Polish for $\Wass_p$, and
\begin{equation}\label{eq-primer-wasserstein-convergence}
	...
	\qquad\text{and}\qquad
	\int d(x,x_0)^p\,\d\alpha_n(x)
	\longrightarrow
	\int d(x,x_0)^p\,\d\alpha(x).
\end{equation}
These standard facts explain why Wasserstein compactness arguments combine tightness with moment bounds~\cite{Villani09,ambrosio2006gradient}.

\paragraph{Conditional laws and disintegration.}
Flow matching repeatedly conditions a latent trajectory on its current position, so the underlying measure-theoretic operation should be explicit. If $\pi\in\Pp(\X\times\Y)$ has first marginal $\alpha$, the disintegration theorem on Polish spaces provides a measurable family $(\pi_x)_{x\in\X}\subset\Pp(\Y)$ such that
\begin{equation}\label{eq-primer-disintegration}
	\int_{\X\times\Y} h(x,y)\,\d\pi(x,y)
	=
	\int_\X\left(\int_\Y h(x,y)\,\d\pi_x(y)\right)\d\alpha(x)
\end{equation}
for every bounded measurable $h$. The family is unique for $\alpha$-almost every $x$ and is the conditional law of $Y$ given $X=x$. In particular, for an integrable vector-valued function $q(x,y)$,
\[
	\EE[q(X,Y)\mid X=x]=\int q(x,y)\,\d\pi_x(y).
\]
...
Figure~\ref{fig:monge-jacobian-pushforward-density} makes the determinant factor visible. This is the first geometric warning behind transport PDEs: moving particles changes both their positions and the local volume element, and several local compressions create several density bumps.
...
\end{tabular}
\caption{\emph{One-dimensional optimal matching by quantile sorting.} The red and blue curves are smooth laws used to generate equal-weight empirical measures; the dots are inverse-CDF samples at common quantile levels. For convex costs on the line, the monotone assignment connects equal ranks.}
...
\small $\norm{x-y}^6$
\end{tabular}
\caption{\emph{Optimal assignments between the same two planar point clouds for four powers of the Euclidean distance.} The feasible permutations are unchanged, but the selected matching changes with the cost geometry: small powers tolerate long exchanges, whereas larger powers suppress the longest edges.}
...
The dual formulation exposes the scalar potentials that later reappear in Hamilton--Jacobi equations, entropic transport and adversarial objectives. Under the preceding compactness assumptions,
\begin{equation}\label{eq-primer-kantorovich-duality}
	...
	\right\},
\end{equation}
where the supremum may be taken over continuous potentials. Weak duality follows by integrating the pointwise constraint against any coupling. Strong duality is the infinite-dimensional linear-programming duality theorem for transport~\cite{Kantorovich42,Villani09}. At optimality, complementary slackness gives $f(x)+g(y)=c(x,y)$ on the support of an optimal plan.
...
The discrete coupling formulation makes precise why assignments are restricted to equal cardinalities and equal weights. Figure~\ref{fig:matching-resolution-and-weights} contrasts the uniform assignment case with plans that split or merge mass when the supports or weights differ.
...
\end{tabular}
\caption{\emph{One-dimensional Wasserstein geodesic by quantiles.} The optimal coupling matches equal quantile levels, so the interpolation is linear in inverse-CDF coordinates: $Q_t=(1-t)Q_0+tQ_1$. The right panel shows the corresponding density path.}
...
\small $t=1$
\end{tabular}
\caption{\emph{Interpolation induced by a Kantorovich plan.} Each nonzero entry of an optimal discrete coupling transports mass along the segment $(1-t)x_i+ty_j$. This construction is the static plan-level ancestor of the dynamic Benamou--Brenier viewpoint developed in Section~\ref{sec-dynamic-optimal-transport}.}
...
\subsection{Functionals and Discrepancies on Measures}
...
\paragraph{First variations and scores.}
Let $f:\Pp(\RR^d)\to\RR\cup\{+\infty\}$. A first variation $\delta f(\alpha)$ is a function, defined up to an additive constant, such that for signed perturbations $\eta$ of zero total mass,
\begin{equation}\label{eq-primer-first-variation}
	f(\alpha+\varepsilon\eta)
	=
	f(\alpha)+\varepsilon\int\delta f(\alpha)(x)\,\d\eta(x)+o(\varepsilon).
\end{equation}
When $\alpha=\rho\,\d x$ has a positive differentiable density, its score is $\nabla\log\rho$. The spatial gradient $\nabla\delta f(\alpha)$, rather than the additive normalization of $\delta f(\alpha)$, determines the Wasserstein velocity in Section~\ref{sec-wasserstein-gradient-flows}.

\paragraph{Relative entropy and kernel discrepancies.}
If $\alpha\ll\beta$, the relative entropy is
\begin{equation}\label{eq-primer-relative-entropy}
	\KL(\alpha|\beta)
	=
	\int\log\left(\frac{\d\alpha}{\d\beta}\right)\d\alpha,
\end{equation}
and it is $+\infty$ otherwise. For smooth positive densities, its first variation is $\log(\rho_\alpha/\rho_\beta)$ up to a constant. If $k$ is a positive-definite kernel with reproducing-kernel Hilbert space $\RKHS_k$, the maximum mean discrepancy is
\begin{equation}\label{eq-primer-mmd}
	\MMD_k^2(\alpha,\beta)
	=
	\left\|\int k(x,\cdot)\,\d(\alpha-\beta)(x)\right\|_{\RKHS_k}^2
	=
	\iint k(x,y)\,\d(\alpha-\beta)(x)\d(\alpha-\beta)(y).
\end{equation}
Its first variation with respect to $\alpha$ is $2\int k(x,y)\,\d(\alpha-\beta)(y)$. Entropy produces local score-driven diffusion, whereas MMD produces a nonlocal interaction field; this contrast is central in the particle examples below.

\paragraph{Sliced Wasserstein discrepancy.}
For $\theta\in\Sphere^{d-1}$, set $P_\theta(x)=\dotp{\theta}{x}$ and let $\sigma$ be the uniform probability measure on the sphere. The sliced quadratic Wasserstein distance is
\begin{equation}\label{eq-primer-sliced-wasserstein}
	\SW_2^2(\alpha,\beta)
	=
	\int_{\Sphere^{d-1}}
	\Wass_2^2\bigl((P_\theta)_\sharp\alpha,(P_\theta)_\sharp\beta\bigr)
	\d\sigma(\theta).
\end{equation}
Each integrand is computed by the one-dimensional quantile formula~\eqref{eq-primer-quantile-wasserstein}. This makes $\SW_2$ computationally attractive, while its Wasserstein gradient still defines a genuinely multidimensional flow through the average of projected one-dimensional velocities.
