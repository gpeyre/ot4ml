...
\index{maximum mean discrepancy}% strictindex
	...
	\]
	Here \(\tau_\epsilon\), defined below, is applied to positive matrices by spectral calculus; equivalently
	\(G_\epsilon=\nabla_\Sigma \Bb_\epsilon(\Sigma,\bar\Sigma)^2\).
	The quantity \(\Bb_\epsilon\) is the debiased smoothed Bures term: if \(\sigma_i(\Sigma,\Lambda)\) are the singular values of \(\Sigma^{1/2}\Lambda^{1/2}\) and \(\lambda_i(\Sigma)\) are the eigenvalues of \(\Sigma\), then
	\[
