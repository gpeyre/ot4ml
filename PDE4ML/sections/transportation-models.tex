...
The conditional expectation in~\eqref{eq:flow-match-conditional} has a simple measure-theoretic meaning. Let $\alpha_t=(P_t)_\sharp\pi$ and define the vector-valued flux measure $\omega_t$ on $\RR^d$ by
\[
...
\]
For $\tau=1$ this is discrete blurring mean shift; for small $\tau$ it approximates a transport PDE that moves each token uphill along the log of the smoothed token density. This distinction between the frozen and self-consistent processes, and between the raw barycentric output $m_\epsilon$ and the velocity $M_\epsilon=m_\epsilon-\Id$, is important: adding $m_\epsilon$ directly as a residual would produce a different drift. The mean-shift form isolates a purely metric attention mechanism from the learned bilinear geometry of $\dotp{Qx}{Ky}$.
\index{mean shift!iteration}% strictindex
\index{barycentric attention}% strictindex

Figure~\ref{fig:generative-mean-shift-pde} visualizes this positive-feedback mechanism before complete modal collapse: particles climb the current smoothed density while the density itself sharpens.
...
\paragraph{Consensus contraction.}
When the averaging law evolves with the particles, blurring mean shift is also a consensus model, closely related to Hegselmann--Krause and Motsch--Tadmor opinion dynamics~\cite{HegselmannKrause2002OpinionDynamics,BlondelHendrickxTsitsiklis2009Krause,MotschTadmor2014Heterophilious}. This viewpoint gives a direct convergence mechanism. For a positive kernel $K$ and an empirical law $\alpha=\sum_i a_i\delta_{x_i}$, define
\begin{equation}\label{eq-pde4ml-mean-shift-markov}
	(\mathsf P_X)_{ij}
	=
	\frac{a_jK(x_i,x_j)}{\sum_r a_rK(x_i,x_r)}.
\end{equation}
Then $\mathsf P_X$ is row-stochastic and the damped update is $X^+=((1-\tau)I+\tau\mathsf P_X)X$. Its contraction is measured by the Dobrushin coefficient
\begin{equation}\label{eq-pde4ml-dobrushin}
	\delta(\mathsf P)
	=
...
\end{equation}
This is the variation-seminorm analogue of projective contraction for positive operators: it measures how much two normalized averaging rows can still disagree~\cite{GaubertQu2015Dobrushin}.

\begin{thm}[Dobrushin consensus for positive blurring mean shift]\label{thm-pde4ml-mean-shift-consensus}
	Let $\alpha_0=\sum_i a_i\delta_{x_i^0}$ have positive weights, and let $C_0$ be the convex hull of its support. Assume that $K:C_0\times C_0\to(0,+\infty)$ is continuous. Set
	\[
		\bar\delta=\sup_{X\in C_0^n}\delta(\mathsf P_X)<1,
		\qquad
		q_\tau=1-\tau(1-\bar\delta),
		\qquad 0<\tau\leq1.
	\]
	For the iteration $X^{k+1}=((1-\tau)I+\tau\mathsf P_{X^k})X^k$,
	\begin{equation}\label{eq-pde4ml-mean-shift-diameter}
		\max_{i,j}\|x_i^k-x_j^k\|
		\leq
		q_\tau^k\max_{i,j}\|x_i^0-x_j^0\|.
	\end{equation}
...
\begin{proof}
	Every updated point is a convex combination of the current points, hence the convex hulls are nested. For a scalar vector $z\in\RR^n$, the common-mass decomposition of two rows of a stochastic matrix gives
	\[
		\max_i(\mathsf Pz)_i-\min_i(\mathsf Pz)_i
		\leq
		\delta(\mathsf P)(\max_i z_i-\min_i z_i).
	\]
	Apply this inequality to every projection $z_i=\dotp{e}{x_i^k}$, $\|e\|=1$, and to $(1-\tau)I+\tau\mathsf P_{X^k}$, whose Dobrushin coefficient is at most $1-\tau(1-\delta(\mathsf P_{X^k}))\leq q_\tau$. Taking the supremum over $e$ yields~\eqref{eq-pde4ml-mean-shift-diameter}. The increments are bounded by $\tau$ times the current diameter, so the geometric bound makes every particle trajectory Cauchy; the vanishing diameter forces a common limit. Finally, continuity and strict positivity of $K$ on the compact set $C_0\times C_0$ imply $\bar\delta<1$.
\end{proof}
...
The one-dimensional Gaussian family makes the preceding catalogue especially transparent. Fix a target $\gamma=\Gaussian(\bar m,\bar\sigma^2)$, $\bar\sigma>0$, and write $\alpha=\Gaussian(m,\sigma^2)$, $\sigma>0$. By Proposition~\ref{prop-gaussian-w2-bures}, $(m,\sigma)\mapsto\Gaussian(m,\sigma^2)$ identifies this family isometrically with the Euclidean upper half-plane. For a functional $f$, set $g_f(m,\sigma)=f(\Gaussian(m,\sigma^2))$ and $\delta=m-\bar m$. For the Sinkhorn row, define
\[
	\psi_\epsilon(r)
	=
	-\frac{\sqrt{\epsilon^2+16r^2}-\epsilon}{2}
	-\frac{\epsilon}{2}
	\log\!\left(\frac{2\epsilon}{\sqrt{\epsilon^2+16r^2}+\epsilon}\right),
	\qquad r>0.
\]
...
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{.34\linewidth}>{\raggedright\arraybackslash}X}
	...
	\hline
	$\KL(\alpha|\gamma)$
	& $\displaystyle \log\frac{\bar\sigma}{\sigma}+\frac{\sigma^2+\delta^2}{2\bar\sigma^2}-\frac12$ \\
	$\Wass_2^2(\alpha,\gamma)$
	& $\displaystyle \delta^2+(\sigma-\bar\sigma)^2$ \\
	$\displaystyle \bar\MK_{\norm{\cdot-\cdot}^2}^{\epsilon}(\alpha,\gamma)$
	& $\displaystyle \delta^2+\psi_\epsilon(\sigma\bar\sigma)-\frac12\psi_\epsilon(\sigma^2)-\frac12\psi_\epsilon(\bar\sigma^2)$ \\
	$\mathcal I(\alpha|\gamma)$
	& $\displaystyle \frac{\delta^2}{\bar\sigma^4}+\frac{(\sigma^2-\bar\sigma^2)^2}{\sigma^2\bar\sigma^4}$ \\
	\hline
\end{tabularx}
\endgroup
\end{center}
Here $\bar\MK^\epsilon$ denotes the debiased entropic transport cost used in the catalogue, and $\mathcal I$ is relative Fisher information. Since $\psi_\epsilon(r)\to-2r$ as $\epsilon\downarrow0$, the Sinkhorn landscape converges to the squared-Wasserstein landscape. Figure~\ref{fig:gradflow-gaussian-1d-energy-landscapes} compares the four exact ambient Wasserstein flows; none is a projection onto the Gaussian family.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/gradflow-gaussian-1d-energy-landscapes/flows.pdf}
\caption{\emph{One-dimensional Gaussian Wasserstein gradient flows in the parameter half-plane $(m,\sigma)$.} From left to right: relative entropy, squared Wasserstein distance, debiased Sinkhorn divergence, and relative Fisher information. The trajectories start from the same well-spread initial parameters and converge to the common target $(\bar m,\bar\sigma)$.}
