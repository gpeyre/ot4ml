...
Modern generative models often use transport language beyond literal Wasserstein gradient flows. This section is synchronized with the current OT4ML chapter and separates deterministic flow matching, diffusion-style interpolants, one-step discrepancy flows, SVGD, transformer depth dynamics, moment measures and Gaussian closures.\index{flow matching}% strictindex
\index{diffusion model}% strictindex
\index{transportation dynamics}% strictindex
...
\index{one-step generative model}% strictindex
\index{parameter-domain flow}% strictindex
...
\]
for instance a smoothed $\KL(\alpha|\beta)$, an MMD/IPM loss, or the debiased Sinkhorn divergence $\bar\MK_\c^\epsilon(\alpha,\beta)$. The associated formal descent is
...
\]
where $\SW_2$ denotes the sliced Wasserstein distance, obtained by averaging one-dimensional quadratic transport costs over projection directions. With the continuity-equation convention $\partial_t\alpha_t+\diverg(\alpha_t w_t)=0$, the descent velocity points from the current projected law toward the target projected law. More precisely, when the projected transports are well defined, if $T_\theta$ denotes the one-dimensional monotone transport from $(P_\theta)_\sharp\alpha$ to $(P_\theta)_\sharp\beta$, then the formal $\Wass_2$-gradient-flow velocity is
...
\index{kernelized gradient flow}% strictindex
...
\end{figure}
\index{Stein variational gradient descent!particle flow}% strictindex
\index{Fokker--Planck particle closure}% strictindex

\paragraph{Self-corrected drifting fields.}
\index{drifting!field}% strictindex

Drifting methods need not start from an exact Wasserstein gradient. They often prescribe an attraction-minus-repulsion field and then regress this field in $L^2(\al_t)$. A simple continuous version uses a positive kernel $K_\epsilon(x,y)$ and defines, for any measure $\nu$,
\index{Wasserstein gradient}% strictindex
...
\index{continuity equation}% strictindex
	...
	\]
	is the formal Wasserstein gradient descent of the time-dependent functional $\alpha\mapsto\mathcal R_t(\alpha|\al_t)$.
\index{Wasserstein gradient}% strictindex
\end{prop}
...
\centering
\begin{tabular}{@{}ccc@{}}
\includegraphics[width=.30\linewidth]{figures/moment-measure-forward-map/quadratic-source.pdf} &
\includegraphics[width=.30\linewidth]{figures/moment-measure-forward-map/quadratic-map.pdf} &
\includegraphics[width=.30\linewidth]{figures/moment-measure-forward-map/quadratic-moment.pdf} \\[-.2em]
{\small source $\rho_u$} &
{\small map $u'$} &
{\small moment measure $\mathfrak M(u)$} \\[.35em]
\includegraphics[width=.30\linewidth]{figures/moment-measure-forward-map/asymmetric-source.pdf} &
\includegraphics[width=.30\linewidth]{figures/moment-measure-forward-map/asymmetric-map.pdf} &
\includegraphics[width=.30\linewidth]{figures/moment-measure-forward-map/asymmetric-moment.pdf} \\[-.2em]
{\small tilted log-concave source} &
{\small nonlinear gradient map} &
{\small centered push-forward}
\end{tabular}
...
Santambrogio~\cite{Santambrogio2015MomentMeasures} reformulates the moment-measure problem as a minimization over probability densities. For a centered $\al\in\Pp_1(\RR^d)$, define the maximal-correlation transport functional, with values in $\RR\cup\{+\infty\}$,
...
\index{displacement convexity}% strictindex
	Let $\al\in\Pp_1(\RR^d)$ be centered and not supported on a hyperplane. Then the minimization problem~\eqref{eq-moment-measure-variational} admits a solution, unique up to translations. Every minimizer is a log-concave density of the form
	\[
		\rho=Z_u^{-1}e^{-u}\,\d x,
	\]
...
\index{Prekopa inequality}% strictindex
	...
	\]
	This is not visibly convex as a function of $u$: the first term is convex, while the logarithmic partition term is concave in this parametrization. Cordero--Erausquin and Klartag reveal convexity after passing to the dual potential $u^*$ and using a Pr\'ekopa-type argument. Santambrogio's formulation reveals the same mechanism in transport language: the difficult convexity becomes the displacement convexity of an entropy-plus-maximal-correlation functional in the density variable $\rho$.
\end{rem}
...
\end{equation}
and, when $\alpha$ is empirical, $\rho_\epsilon[\alpha]$ is a Gaussian kernel density estimate up to normalization. Thus \(M_\epsilon[\alpha]\) is the classical Gaussian mean-shift vector~\cite{FukunagaHostetler1975,Cheng1995MeanShift,ComaniciuMeer2002MeanShift}. Consequently, the barycentric residual update
\index{kernel density estimate}% strictindex
...
\end{tabular}
\caption{\emph{Continuous-time mean shift for a densely sampled three-Gaussian mixture.} Left: initial density level sets, in red, and representative particle paths of $\dot x=M_\epsilon[\alpha_t](x)$, colored from red to blue. Right: four later kernel-density renderings of $\alpha_t$ at increasing times, with the same red-to-blue time palette; the initial density is omitted because it is shown on the left. The snapshots are chosen before complete mode collapse, so that the flow visibly advects mass uphill along $\log\rho_\epsilon[\alpha_t]$ and sharpens the overlapping modes.}
...
\index{Hermite polynomial}% strictindex
\index{Ornstein-Uhlenbeck operator}% strictindex
...
\index{maximum mean discrepancy}% strictindex
	...
	\]
	The debiased Sinkhorn row uses the notation displayed here: for Gaussian inputs,
	\[
		\bar\MK_{\norm{\cdot-\cdot}^2}^{\epsilon}(\alpha,\gamma)
		=
	...
	\]
	Here \(\Bb_\epsilon\) is the debiased smoothed Bures term: if \(\sigma_i(\Sigma,\Lambda)\) are the singular values of \(\Sigma^{1/2}\Lambda^{1/2}\) and \(\lambda_i(\Sigma)\) are the eigenvalues of \(\Sigma\), then
	\[
...
\index{first variation}% strictindex
	...
	For the KL row, the ambient first variation is \(\log(\rho/\gamma)\) up to an additive constant. If \(\alpha=\Gaussian(m,\Sigma)\), then
	\[
	...
	\]
	Proposition~\ref{prop-gaussian-affine-closure} gives \(h(m,\Sigma)=-A\delta_m\) and \(H(m,\Sigma)=2\Id-\Sigma A-A\Sigma\). For the Fisher row, writing \(u=\log(\rho/\gamma)\), the ambient first variation of \(\frac12\mathcal I\) is, up to constants,
	\[
		-\Delta u-\frac12|\nabla u|^2-\dotp{\nabla u}{\nabla\log\gamma}.
	\]
	For Gaussian \(\rho\) and Gaussian \(\gamma\), this is quadratic in \(x\). Multiplying by two for \(\mathcal I\) gives the descent velocity
	\[
...
\index{second moment}% strictindex
	...
	For the Sinkhorn row, the closed Gaussian formula is a squared mean displacement plus the smoothed Bures term \(\Bb_\epsilon^2\). This is not only a formula on the Gaussian submanifold. The first variation of the entropic value with respect to the first marginal is a Sinkhorn dual potential, and for Gaussian marginals these cross and self dual potentials are quadratic. Their debiased combination is therefore quadratic, so its spatial gradient is affine. Since the restriction of the functional to Gaussians is
	\[
		(m,\Sigma)\mapsto \norm{m-\bar m}^2+\Bb_\epsilon(\Sigma,\bar\Sigma)^2,
	\]
	the quadratic coefficient is \(G_\epsilon=\nabla_\Sigma\Bb_\epsilon(\Sigma,\bar\Sigma)^2\), and the moment-functional computation gives the displayed \(h\) and \(H\). When \(\bar\Sigma=\Id\), this covariance contribution is a spectral function of \(\Sigma\). Since \(\psi_\epsilon'(r)=-2\tau_\epsilon(r)\), if \(\lambda\) is an eigenvalue of \(\Sigma\), the corresponding scalar velocity is
	\[
		4\sqrt{\lambda+\epsilon^2/16}-4\sqrt{\lambda^2+\epsilon^2/16},
	\]
	which gives the displayed matrix formula by functional calculus.
...
\index{two-layer neural network}% strictindex
	...
	\]
	Thus the square-loss energy of~\eqref{eq-mlp-square-loss-quadratic-linear} depends on the neuron law only through the raw cross moment \(Q_\alpha=\int v u^\top\d\alpha(u,v)\). If the neuron law has mean and covariance
	\[
		m=
...
\index{neuron law}% strictindex
	...
	\[
		...
		+\frac12\int\norm{y}^2\d\rho(z,y).
	\]
	This puts the model exactly in the moment-functional row of Proposition~\ref{prop-centered-gaussian-covariance-catalogue}. To see that it recovers the usual particle equation, write the derivative of the outer square loss with respect to the matrix \(Q\) as
	\[
		E_\alpha\eqdef Q_\alpha S-R.
	\]
...
\index{first variation}% strictindex
	...
	\[
		...
		\end{pmatrix}.
	\]
	The factor \(1/2\) in the covariance gradient comes from the symmetry of \(\Sigma\): the upper-right block is the transpose of the lower-left block. Substituting these gradients in Proposition~\ref{prop-centered-gaussian-covariance-catalogue} gives the closed ODE for \((m_t,\Sigma_t)\). In particular, a Gaussian law of neurons remains Gaussian. When the neuron law is centered, \(Q_\alpha=\Sigma_{vu}\), so the energy is a function of the cross-covariance block alone. This exact closure is special to the linear activation; for nonlinear activations, Gaussian closures are usually projections rather than invariant families.
...
\index{Gelbrich theorem}% strictindex
	...
	\[
		\mathcal R\al\eqdef \Gaussian(\mean_\al,\cov_\al)
	\]
...
\index{covariance!matrix}% strictindex
	\[
		\tr C\leq \tr\left((\cov_\al^{1/2}\cov_\nu\cov_\al^{1/2})^{1/2}\right).
	\]
...
\index{Gaussian measure}% strictindex
	...
	\]
	then $\mathcal R\be_s=\be_s$, and Theorem~\ref{thm-gelbrich-projection} gives $F(\mathcal R\al)\leq F(\al)$. Thus the moment-matched Gaussian projection of any barycenter is again a barycenter; when the barycenter is unique, it must itself be Gaussian.
\index{moment matching}% strictindex
\index{Gaussian!projection}% strictindex
\end{rem}
