...
\end{equation}
This viewpoint includes Wasserstein GANs, MMD-GANs and Sinkhorn generative models~\cite{WassersteinGAN,MMD-GAN,2017-Genevay-AutoDiff}. When $\mathcal D$ is written through a dual potential or discriminator, this is the adversarial training viewpoint. The resulting training dynamics is the ordinary Euclidean gradient flow in parameter space,
...
\]
for instance a smoothed $\KL(\alpha|\beta)$, an MMD/IPM loss, or a debiased Sinkhorn divergence $\bar\MK_\c^\epsilon(\alpha,\beta)$. The associated formal descent is
...
\index{Fokker--Planck equation!particle closure}% strictindex
