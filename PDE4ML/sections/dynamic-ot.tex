...
The same variational grammar extends beyond the quadratic Wasserstein distance. One changes either the kinetic exponent, the mobility or the balance equation, while keeping a continuity-type constraint and a convex action density.
\index{balance equation}% strictindex
\index{Wasserstein!distance}% strictindex
\index{perspective function}% strictindex

\paragraph{General convex action densities.}\label{rem-generalized-bb}
\index{Benamou-Brenier}% strictindex
...
\index{unbalanced OT}% strictindex
	Fix the action above and let $\CW_\kappa$ be the static cone value of unbalanced optimal transport, with the cone metric normalized to the same growth scale $\kappa$. For nonnegative finite measures $\alpha_0,\alpha_1$ on $\RR^d$, the dynamic value
...
Let $k$ be a positive definite kernel with scalar RKHS $\RKHS_k$ and vector RKHS $\RKHS_k^d$, $\norm{v}_{\RKHS_k^d}^2=\sum_{\ell=1}^d\norm{v_\ell}_{\RKHS_k}^2$. The kernelized dynamic action is
\[
\mathcal W_k^2(\alpha_0,\alpha_1)
=\inf_{\partial_t\alpha_t+\diverg(\alpha_t v_t)=0}
\int_0^1\norm{v_t}_{\RKHS_k^d}^2\d t.
\]
Its square root is the associated kernelized transport length and gives the dynamic geometry behind SVGD~\cite{LiuWang2016SVGD,Liu2017SVGDGradientFlow,DuncanNueskenSzpruch2019SVGDGeometry}: the $L^2(\alpha)$ norm of velocities is replaced by the vector-valued RKHS norm on smooth vector fields. This length is generally extended: for smooth kernels, finite-action curves are regular flows, so atoms cannot split and a Dirac mass is at infinite distance from an absolutely continuous law.
...
Although the minimizing-movement construction is introduced only in Section~\ref{sec-wasserstein-gradient-flows}, it is useful to keep its intuition in mind before the next statement. A metric gradient flow can be read as the continuous-time limit of the implicit Euler, or JKO, step
\begin{equation}
