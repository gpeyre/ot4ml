...
Proposition 2.26: Caffarelli regularity -> [to review] added and polished `fig:monge-caffarelli-nonconvex-map`, generated by `monge-caffarelli-nonconvex-map.ipynb`. The figure solves a 1524-point equal-weight assignment from a nearly area-uniform concentric-ring sample of the disk to a low-discrepancy sample of a smooth non-convex bean domain. The right panel connects the images of each source circle in angular order, making the bending and bunching near the inward dent visible without presenting it as a literal counterexample to the regularity theorem.
...
2.7 Gaussian Measures and the Bures Metric -> [to review] added `fig:monge-gaussian-fr-vs-bures-cone`, generated by `monge-gaussian-fr-vs-bures-cone.ipynb`. The two side-by-side cone panels compare a Bures--Wasserstein covariance geodesic reaching a rank-one covariance with a Fisher--Rao affine-invariant geodesic toward a small full-rank regularization of the same rank-one limit, emphasizing that Bures extends to the closed PSD cone while Fisher--Rao sends the rank-deficient boundary to infinite distance.
