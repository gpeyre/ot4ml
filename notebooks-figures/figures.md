...
Proposition 2.26: Caffarelli regularity -> [to review] refined `fig:monge-caffarelli-nonconvex-map`, generated by `monge-caffarelli-nonconvex-map.ipynb`. The figure now solves a 2400-point equal-weight empirical OT problem from a farthest-point sample of a non-convex banana-shaped source to a farthest-point sample of the disk. The target dual potential is used through its quadratic c-transform to extrapolate the empirical map to level curves inside the banana; the notebook checks that the assigned target belongs to the c-transform argmin set on the source samples up to numerical precision.
...
Heat kernels and Hopf--Cole transforms -> [to review] added `fig:sinkhorn-geodesics-in-heat`, generated by `sinkhorn-geodesics-in-heat.ipynb`, and `fig:sinkhorn-hopf-cole-transform`, generated by `sinkhorn-hopf-cole-transform.ipynb`. The first notebook compares exact two-dimensional grid distances to geodesics-in-heat approximations at three resolvent scales using sparse shifted-Laplacian solves with Neumann boundary conditions. The second notebook illustrates Gaussian log-sum-exp Hopf--Lax/Legendre smoothing with heat time $\epsilon t/2$ and the Hopf--Cole linearization of viscous Burgers through a heat equation.
...
Figure: Three-marginal Coulomb Sinkhorn -> [to review] added `fig:multimarginal-coulomb-sinkhorn`, generated by `multimarginal-coulomb-sinkhorn.ipynb`. The notebook solves a lightweight one-dimensional equal-marginal three-body problem with softened pairwise Coulomb repulsion using tensor Sinkhorn scaling. The three panels display the pairwise $(X_1,X_2)$ marginal for $\epsilon=0.06,0.18,0.70$, showing how small regularization pushes mass away from the collision diagonal while larger regularization blurs the repulsive arrangement.
