...
Complex Sinkhorn continuation -> [checked] `fig:sinkhorn-complex-epsilon-continuation`, generated by `sinkhorn-complex-epsilon-continuation.ipynb`. The notebook fixes two one-dimensional Gaussian-mixture histograms and $\epsilon_0=0.55$, then follows the gauge-fixed complex scaling equations by Newton continuation. Four square panels display $|P_{\epsilon_0+i\eta}|$ for $\eta=0,0.20,0.40,0.60$ on a common intensity scale. The attached red and blue profiles are the prescribed complex marginals, while violet profiles show the row and column sums after taking the entrywise modulus. The maximum numerical marginal residual is below $5\times10^{-14}$.
