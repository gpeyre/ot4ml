...
Push-forward formula for densities -> [to review] polished `fig:monge-jacobian-pushforward-density`, generated by `monge-jacobian-pushforward-density.ipynb`. The figure uses a smooth diffeomorphism of the unit square with an analytic positive Jacobian determinant. A uniform source density is shown with a colored square grid, while the target panel overlays the deformed grid with the pushed-forward density image and contours of $\rho_\beta$. The narrow blue high-density strip appears exactly where the grid cells are compressed, making the determinant factor in the density push-forward formula visually explicit.
...
Wasserstein DRO ambiguity set -> [to review] polished `fig:kantorovich-dro-ambiguity`, generated by `kantorovich-dro-ambiguity.ipynb`. The figure displays a readable two-dimensional empirical cloud next to a linear classifier, smaller local perturbation disks suggesting the Wasserstein ambiguity ball, and a simple budgeted adversary that moves atoms closest to the decision boundary along a fixed loss-gradient direction. The notebook intentionally avoids solving a full DRO problem and focuses on the spatial meaning of the inner adversarial perturbation.
...
Complementary slackness contacts -> [to review] added `fig:dual-complementary-slackness-contacts`, generated by `dual-complementary-slackness-contacts.ipynb`. The figure uses one-dimensional Gaussian-mixture densities and the exact quantile map for the quadratic cost. It displays the nonnegative slack $c(x,y)-f(x)-g(y)$ as a continuous heatmap, with the source and target marginal densities as red/blue side strips and the transported zero-contact curve highlighted in violet.

Birkhoff--von Neumann proof cycle -> [to review] added and polished `fig:birkhoff-von-neumann-cycle`, generated by `birkhoff-von-neumann-cycle.ipynb`. The notebook builds a $7\times7$ bistochastic non-permutation matrix with a single $2\times2$ fractional block and isolated unit entries, then exports a matrix panel and a red/blue bipartite support graph where thin purple edges represent $0<P_{ij}<1$, bold black edges represent $P_{ij}=1$, and an orange halo marks the minimal fractional cycle used in the alternating perturbation proof. The graph panel orders the column nodes so the isolated unit edges are visually separated from the fractional cycle.
...
Figure: Partial OT active regions between shapes -> [to review] added `fig:partial-ot-shape-active-mass`, generated by `partial-ot-shape-active-mass.ipynb`. The notebook samples the cat and annulus indicator supports by dense farthest-point sampling, solves exact fixed-mass partial OT with POT for transported masses $m=0.82,0.58,0.36,0.18$, and displays the active source/target marginals as saturated red/blue points while unmatched available mass is kept at opacity `0.1`.

Figure: Capacity-constrained OT in one dimension -> [to review] added `fig:capacity-constrained-ot-1d`, generated by `capacity-constrained-ot-1d.ipynb`. The notebook solves the entropic capped problem by KL-Dykstra projections and displays three coupling matrices with attached source/target marginals for density-ratio caps $\kappa=80,10,2.6$, making the transition from a nearly Monge-like plan to a saturated diffuse plan visible.
