...
Figure: One-dimensional linear OT PCA -> [to review] added `fig:linear-ot-1d-pca`, generated by `linear-ot-1d-pca.ipynb`. The notebook builds a synthetic dataset of two-Gaussian mixtures with varying component locations and scales, computes exact one-dimensional linear OT coordinates by sampling quantile functions on a fine grid, performs PCA in $L^2(0,1)$, and displays the dataset plus the first three quantile-PCA directions as red-to-blue density evolutions.
