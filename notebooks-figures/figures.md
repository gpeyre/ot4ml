...
Figure: Hungarian algorithm progression -> [to review] regenerated `fig:matching-hungarian-progression`, generated by `matching-hungarian-progression.ipynb`. The five geometric panels use the canonical disk-to-annulus clouds of Figure 1.5 and record actual Hungarian states with $|M|=0,9,18,27,36$. Thick violet edges show the current matching, while thin translucent edges show the $2n$ lowest-slack unmatched pairs; exact zero-slack candidates receive slightly stronger emphasis. The notebook checks dual feasibility and matching-edge tightness at every snapshot, and verifies final primal-dual equality.
...
Push-forward formula for densities -> [to review] updated `fig:monge-jacobian-pushforward-density`, generated by `monge-jacobian-pushforward-density.ipynb`. The figure is a one-line triptych of stronger smooth local-compression diffeomorphisms creating one, three, and five density bumps. Each panel uses an analytic positive Jacobian determinant, a tighter crop around the bumps, a fine red deformed grid, and black level sets of the pushed density $|\det \T'|^{-1}$.
...
Tree geodesic interpolation -> [to review] polished `fig:monge-tree-mccann-interpolation`, generated by `monge-tree-mccann-interpolation.ipynb`. The notebook solves a discrete quadratic OT problem between two non-uniform histograms on a finite weighted tree and displays the associated McCann interpolation: transported packets move at constant speed along unique tree paths, circle areas encode mass, colors interpolate from red to blue, and faint colored corridors show which branches carry the optimal plan.

Example 2.16: Semi-discrete Monge maps -> [to review] added and polished `fig:monge-semidiscrete-maps`, generated by `monge-semidiscrete-maps.ipynb`. The two panels show a continuous Gaussian transported to a 15-atom regularized empirical Gaussian, and a two-mode continuous source transported to a 20-atom central empirical Gaussian. Target atoms and full-window Laguerre cells share a mildly randomized red-violet-blue palette driven by the target horizontal coordinate, with faint cell-barycenter transport segments.
...
Wasserstein DRO ambiguity set -> [updated] rebuilt `fig:kantorovich-dro-ambiguity`, generated by `kantorovich-dro-ambiguity.ipynb`, using two noisy interlocking moons with localized overlap near their opposing tips. The robust panels show hollow adversarial samples, violet transport segments and the reorganization of the winding Laplacian-kernel decision boundary under increasing global quadratic budgets.
...
Figure: Geometric auction progression -> [reviewed] regenerated `fig:dual-auction-progression`, generated by `dual-auction-progression.ipynb`. The notebook runs the unit-mass transportation auction on the canonical 36-point planar disk-to-annulus assignment, validates ownership and $\varepsilon$-complementary slackness after every bid, and independently checks the final objective with an exact assignment solver. Five panels show the first states with matching sizes $0,9,18,27,36$ after $0,10,40,98,505$ bids. Thick violet edges are current ownerships; faint edges expose the smallest auction reduced costs.
...
Complex Sinkhorn continuation -> [checked] `fig:sinkhorn-complex-epsilon-continuation`, generated by `sinkhorn-complex-epsilon-continuation.ipynb`. The notebook fixes two one-dimensional Gaussian-mixture histograms and follows the constant-modulus arc $\epsilon_\theta=0.12e^{i\theta}$ by gauge-fixed Newton continuation. Four square panels display $|P_{\epsilon_\theta}|$ for $\theta=0,0.40,0.80,1.20$ on a common intensity scale. The attached red and blue profiles are the prescribed complex marginals, while violet profiles show the row and column sums after taking the entrywise modulus. Every angular continuation step is certified, and the maximum numerical marginal residual is below $10^{-11}$.

Positive-feature Sinkhorn sketches -> [to review] updated `fig:sinkhorn-positive-feature-sketching`, generated by `sinkhorn-positive-feature-sketching.ipynb`. The notebook now compares the dense one-dimensional Sinkhorn plan with positive Gaussian-feature sketches of ranks \(40\), \(10\), and \(3\), and adds a matched second row displaying the effective costs \( -\epsilon\log \widetilde k_\epsilon(x,y) \) with level sets. This makes visible that coarse sketches preserve marginals after scaling but distort the logarithmic kernel geometry.

Doubly-positive kernel counterexample -> [to review] added `fig:sinkhorn-doubly-positive-counterexample`, generated by `sinkhorn-doubly-positive-counterexample.ipynb`. The notebook visualizes the periodic kernel \(k_\lambda(t)=\lambda+\cos^2(t/2)\) for \(\lambda=1/20\), its five-point Gram matrix, and the Horn copositive certificate showing that the matrix is PSD and entrywise positive but not completely positive. This supports the new discussion of why positive-feature Sinkhorn requires total positivity rather than mere pointwise-positive PSD kernels.
...
Figure: Quotient Wasserstein and Wasserstein--Procrustes rigid alignment -> [to review] added `fig:wasserstein-procrustes-rigid-motion`, generated by `wasserstein-procrustes-rigid-motion.ipynb`. The figure uses farthest-point samples of the bunny silhouette and displays pose-damped iterates \(1,2,3,5,10\) of an alternating OT/Procrustes continuation under a strong translation and moderate rotation. Rotation is interpolated on \(\mathrm{SO}(2)\), translation is interpolated linearly, and every displayed source remains a rigid transform; black is the fixed target, the moving source is colored from red to blue, and selected current correspondences are drawn faintly.
...
Figure 41: Trace and spectral gauges for displacement second moments. -> [to review] regenerated from one common equal-weight support pair. The trace and spectral plans are computed on that pair, validated against their marginals, and displayed directly. Every interpolation panel is the exact weighted pushforward of the corresponding displayed plan, followed only by KDE smoothing on a common spatial frame and density scale.
...
Figure: Sliced/Radon barycentric interpolation -> [to review] polished `fig:sliced-radon-barycenter`, generated by `sliced-radon-barycenter.ipynb`. The figure keeps the Radon-domain quantile interpolation between cat and heart densities, but now uses a regularized nonnegative filtered back-projection and suppresses low-amplitude inversion ghosts in the displayed density row. The caption now explicitly distinguishes this pseudo-inverse reconstruction from the exact constrained sliced barycenter problem.
...
Figure: Variational mean field game with hard congestion -> [to review] added `fig:dynamic-variational-mfg-hard-congestion`, generated by `dynamic-variational-mfg-hard-congestion.ipynb`. The notebook solves the convex density--momentum problem by Douglas--Rachford splitting on a $40\times24$ two-room grid with a narrow doorway and 16 time slabs, followed by a Dykstra projection onto affine conservation and the global density box. It compares no cap with the tight feasible capacity $\kappa=\|\rho_0\|_\infty$ for a crowd initially uniform on three large disks and attracted toward a mirrored three-disk terminal profile. The executable assertions certify fixed-point, cap, positivity, exterior-leakage, continuity, and mass residuals.
...
Figure: Gradient flows of discrepancies toward a Gaussian-mixture target -> [to review] added `fig:gradflow-discrepancy-density-comparison`, generated by `gradflow-discrepancy-density-comparison.ipynb`. The figure compares Wasserstein gradient flows of the energy distance, the $\mathcal W_2$ distance and the relative entropy toward the same one-dimensional Gaussian-mixture target. The ED and $\mathcal W_2$ panels use many quantile particles and render the evolving law by KDE density curves; the KL panel solves the reversible Fokker--Planck equation by an implicit finite-volume scheme whose discrete stationary density is the target. Each panel shows red-to-blue density snapshots on the same axes with the target density as a dashed black curve.
...
Figure: Wasserstein versus sliced-Wasserstein shape flows -> [to review] added `fig:generative-w2-vs-sliced-flow-shapes`, generated by `generative-w2-vs-sliced-flow-shapes.ipynb`. The source particles are sampled from the cat silhouette and the target particles from the heart silhouette. The top row follows the exact empirical gradient flow of \(\frac12 W_2^2(\cdot,\beta)\) using one optimal assignment, while the bottom row integrates the \(\frac12\mathrm{SW}_2^2(\cdot,\beta)\) flow by repeatedly sorting projected particles along 128 directions. Each row shows representative red-to-blue particle trajectories on the left and six KDE density snapshots on the right.

Figure: Wasserstein relative-entropy flow versus SVGD -> [to review] added `fig:generative-w2-vs-svgd-entropy-flow`, generated by `generative-w2-vs-svgd-entropy-flow.ipynb`. The notebook compares two deterministic particle descents of \(\mathrm{KL}(\alpha|\beta)\) toward the same three-Gaussian target: a sharp KDE particle closure of the \(W_2\) Fokker--Planck flow and the RBF-kernel SVGD update. Both panels use the same initial particles, gray target contours, and red-to-blue representative trajectories.

Figure: Forward moment-measure construction -> [to review] updated `fig:moment-measure-forward-map`, generated by `moment-measure-forward-map.ipynb`. The figure gives a one-dimensional exact push-forward calculation for \(u\mapsto\mathfrak M(u)\) using three convex potentials. The top row overlays \(u\) with the log-concave source \(Z_u^{-1}e^{-u}\), and the bottom row shows moment measures shaped as a skewed unimodal density, a two-bump mixture, and a three-bump mixture with varying widths and heights.
