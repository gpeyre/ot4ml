...
\]
Dual feasibility is the statement $s\geq0$, while complementary slackness says that an optimal coupling can only live where this slack vanishes. Figure~\ref{fig:dual-complementary-slackness-contacts} shows this zero set for a simple Gaussian example and for a two-component Gaussian-mixture example: the transported graph is not guessed from the potentials separately, but appears as the contact valley of the dual inequality.
\index{complementary slackness}% strictindex
\index{dual!slack}% strictindex

\begin{figure}[H]
\centering
\includegraphics[width=.74\linewidth]{figures/dual-complementary-slackness-contacts--contacts.pdf}
\caption{Complementary slackness contacts for one-dimensional quadratic OT.  Left: transport between two Gaussian densities.  Right: transport between two-component Gaussian-mixture densities.  Each central heatmap displays the nonnegative dual slack $s(x,y)=|x-y|^2-f(x)-g(y)$: pale colors mark the contact valley and darker blue means larger strict slack.  The red top strip and blue side strip show the source and target densities.  The violet curve is the quantile transport contact graph $(x,T(x))$, where $s(x,T(x))=0$ and the optimal Monge plan is supported.}
\index{complementary slackness}% strictindex
\index{dual!slack}% strictindex
