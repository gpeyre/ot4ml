...
\]
\index{semi-dual}% strictindex
Partial maximization of a jointly concave objective preserves concavity, so $\Ee_0$ is concave. The semi-dual is invariant under $g\mapsto g+s$: indeed, $(g+s)^{\bar c}=g^{\bar c}-s$ and both measures have unit mass. Potentials are therefore determined only up to an additive constant, exactly as in the full dual. The main computational advantage is that the semi-dual is unconstrained.
...
\]
The function $\Ee_0$ is concave and piecewise affine. It is invariant under $\gD\mapsto\gD+s\ones_m$, because the two marginals have equal mass. If a tie-breaking rule selects
\[
	\sigma_\gD(i)\in\argmin_j(\C_{i,j}-\gD_j),
\]
the induced target masses determine a supergradient explicitly.
...
In one dimension, the Laguerre geometry of Definition~\ref{def-laguerre-power-cells} becomes explicit. Consider the quadratic cost $c(x,y)=\abs{x-y}^2$, assume that $\al$ has a density and that $y_1<\cdots<y_m$, and set $B_0=0$ and $B_j=\sum_{k=1}^j\b_k$. For $j<k$, the difference between the weighted costs $c(x,y_j)-\gD_j$ and $c(x,y_k)-\gD_k$ is affine and increasing in $x$. The active site index is therefore nondecreasing, so the nonempty Laguerre cells are intervals ordered like the sites. If $\gD^\star$ maximizes the semi-dual problem~\eqref{eq-semi-dual-discr}, its induced map pushes $\al$ to $\be$, and hence its $j$-th cell has $\al$-mass $\b_j$. Writing $q_\al=\cumul{\al}^{-1}$ therefore gives
...
with the harmless endpoint conventions $q_\al(0)=-\infty$ and $q_\al(1)=+\infty$. Thus the optimal semi-discrete map is $T^\star(x)=y_j$ on $I_j$; this is the Laguerre-cell realization of the monotone rearrangement in Theorem~\ref{prop-1d-quantile-map}. For equal target weights, $B_j=j/m$, and the same interval partition yields a closed-form quadratic cost through the integrated quantile.
...
\textbf{Compute cells:}
\(\Laguerre_j(\gD^{(\ell)}) = \enscond{x}{c(x,y_j)-\gD^{(\ell)}_j\leq c(x,y_r)-\gD^{(\ell)}_r\quad\forall r}\).

\textbf{Compute masses:}
\(m_j^{(\ell)}=\int_{\Laguerre_j(\gD^{(\ell)})}\d\al \).

\textbf{If} $\max_j\abs{m_j^{(\ell)}-\b_j}\leq\mathrm{tol}$ \textbf{then}:
\begin{algblock}
\textbf{Return} $\gD^{(\ell)}$ and $\bigl(\Laguerre_j(\gD^{(\ell)})\bigr)_{j=1}^m$.
\end{algblock}
...
\end{algblock}
\algreturnskip
\textbf{Compute} $\bigl(\Laguerre_j(\gD^{(L)})\bigr)_{j=1}^m$.

\textbf{Return} $\gD^{(L)}$ and these cells.
\end{alg}
...
\)
can be described by a site density \(\sigma\). Thus \(m\sigma(x)\d x\) represents the number of codepoints near \(x\): regions where \(\sigma\) is large are quantized more finely. If \(\al=\rho\d x\) and \(r=p/d\), high-resolution quantization leads, up to a constant depending only on \(p\) and \(d\), to the effective energy~\cite{graf2000foundationsquantization}
\[
	\Gg_\rho(\sigma)
	\eqdef
