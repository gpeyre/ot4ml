...
Under the compactness and continuity assumptions of Chapter~\ref{sec-dual}, denote the extended full-dual objective by
\index{dual!problem}% strictindex
...
\begin{alg}[Semi-discrete Laguerre ascent]\label{alg:semidiscrete-laguerre-ascent}\label{alg:semidiscrete-laguerre-descent}
\textbf{Input:} Source measure $\al$, target atoms $(y_j,\b_j)$, cost $c$, steps $(\tau_k)_{k=0}^{K-1}$, tolerance $\mathrm{tol}$, maximum iteration count $K$.
...
\textbf{Input:} Source measure $\al$, initial codepoints $Y^{(0)}=(y_j^{(0)})_{j=1}^m$, squared Euclidean cost, tolerance $\mathrm{tol}$, maximum iteration count $K$.
