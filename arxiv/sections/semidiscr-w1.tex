...
In one dimension, the Laguerre geometry of Definition~\ref{def-laguerre-power-cells} becomes explicit. Consider the quadratic cost $c(x,y)=\abs{x-y}^2$, assume that $\al$ has a density and that $y_1<\cdots<y_m$, and set $B_0=0$ and $B_j=\sum_{k=1}^j\b_k$. For $j<k$, the difference between the weighted costs $c(x,y_j)-\gD_j$ and $c(x,y_k)-\gD_k$ is affine and increasing in $x$. The active site index is therefore nondecreasing, so the nonempty Laguerre cells are intervals ordered like the sites. If $\gD^\star$ maximizes the semi-dual problem~\eqref{eq-semi-dual-discr}, its induced map pushes $\al$ to $\be$. Consequently, after modifying the cells on $\al$-null sets if necessary, the optimal cells are
...
Here the harmless endpoint conventions are $\cumul{\al}^{-1}(0)=-\infty$ and $\cumul{\al}^{-1}(1)=+\infty$. Thus the optimal semi-discrete map is $T^\star(x)=y_j$ for $x\in\Laguerre_j(\gD^\star)$; this is the Laguerre-cell realization of the monotone rearrangement in Theorem~\ref{prop-1d-quantile-map}. For equal target weights, $B_j=j/m$, and the same interval partition yields a closed-form quadratic cost through the integrated quantile.
