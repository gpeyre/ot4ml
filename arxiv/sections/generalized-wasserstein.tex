...
\begin{proof}
	Quadratic dual potentials are closed under the KL optimality equations. Solving the resulting matrix fixed point and integrating the associated scaled Gaussian plan gives~\eqref{eq-gaussian-ghk-closed-form}; equivalently, it is the zero-coupling-entropy limit of the formula below~\cite{janati2020gaussian}. The scalar $\mathfrak a_\tau\in(0,1]$ is the resulting Gaussian affinity. In particular, as $\tau\to+\infty$,
	\[
		\mathfrak a_\tau
		=
		1-\frac{1}{2\tau}
		\Wass_2^2\bigl(\Gaussian(m_0,\Sigma_0),\Gaussian(m_1,\Sigma_1)\bigr)
		+o(\tau^{-1}),
	\]
