...
Figure~\ref{fig:monge-jacobian-pushforward-density} makes the determinant factor visible. Even when the source density is uniform, a nonlinear diffeomorphism can compress area near the center and expand it elsewhere; the target density is therefore not obtained by the naive composition $\rho_\al\circ\T^{-1}$ alone. The high-density bump appears exactly where the deformed grid cells have small area.
...
\begin{tabular}{@{}cc@{}}
\includegraphics[width=.39\linewidth]{figures/monge-jacobian-pushforward-density--source-grid.pdf} &
\includegraphics[width=.39\linewidth]{figures/monge-jacobian-pushforward-density--target-density-grid.pdf} \\[-.1em]
\small uniform source grid &
\small pushed density and deformed grid
\end{tabular}
\caption{Jacobian determinant in the density push-forward formula. A smooth nonlinear diffeomorphism radially compresses the middle of a cropped colored grid and adds a radius-dependent twist. The source density is locally uniform, but the target density, rendered as a blue image with contours of $\rho_\be$, forms an almost isotropic central bump where the deformed grid cells have smaller area. The target panel is a crop of a larger warped grid, so no artificial square boundary is visible. This illustrates the factor $|\det \T'(x)|^{-1}$ in~\eqref{eq-pfwd-density}.}
...
\index{Wasserstein distance!Wp@$\Wass_p$}% strictindex
...
\begin{rem}[One-dimensional Wasserstein spaces are isometric to Banach spaces]\label{rem-1d-wasserstein-banach-isometry}
...
\index{earth mover distance!tree metric}% strictindex
