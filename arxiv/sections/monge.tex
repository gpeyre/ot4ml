...
Figure~\ref{fig:monge-caffarelli-nonconvex-map} illustrates the geometric role of the convexity assumption. It should be read as a finite-sample visualization rather than as a counterexample: a quadratic assignment sends a nearly area-uniform concentric-ring sample of the disk to a non-convex bean-shaped target. The images of initially smooth circles remain visible, but they bend and bunch near the inward dent of the target.
\index{regularity theory}% strictindex
\index{non-convex!target}% strictindex

\begin{figure}[ht]
\centering
\begin{tabular}{@{}cc@{}}
\includegraphics[width=.33\linewidth]{figures/monge-caffarelli-nonconvex-map--source-circles.pdf} &
\includegraphics[width=.43\linewidth]{figures/monge-caffarelli-nonconvex-map--bean-image-circles.pdf} \\[-.1em]
\small source concentric circles &
\small images in a non-convex target
\end{tabular}
\caption{Empirical quadratic OT map from a disk to a non-convex bean-shaped domain. The left panel shows the source through nearly area-uniform concentric circles, without displaying the individual samples. The right panel shows the assigned images of these circles in the bean-shaped target; the target samples used by the assignment are not displayed. Colors record the source radius, and thin curves connect the matched images of each source circle in angular order. The bunching along the inward dent visualizes the target-geometry obstruction that Caffarelli's convexity assumptions rule out.}
...
\index{Monge-Ampere equation!numerical solver}% strictindex
...
\index{mass!splitting}% strictindex
	Assume that $c\in C^1(X\times Y)$ satisfies the twist condition from $X$ to $Y$. Suppose that the optimality conditions for the cost $c$ are represented by a $c$-convex potential $f$ which is differentiable on a full $\al$-measure subset of $X$. Then, for $\al$-almost every source point $x$, there is at most one target point $y$ that can be paired optimally with $x$. Consequently, whenever an optimal relaxed transport exists under these hypotheses, it is induced by a Monge map.
\end{prop}
\begin{proof}
