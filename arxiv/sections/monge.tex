...
Radial symmetry gives a useful higher-dimensional case where the Brenier map reduces to a one-dimensional monotone rearrangement. A measure $\mu$ on $\RR^d$ is radial if $Q_\sharp\mu=\mu$ for every orthogonal map $Q$. Such a measure is determined by the law of the radius $\norm{x}$, and the optimal map transports this radius while keeping the angular direction fixed.
...
Let $\Phi_2$ keep only raw second moments. Taking the infimum of $\Wass_2$ over
...
important: for a non-centered law, $\Phi_2(\mu)$ is not its covariance matrix.
