...
\begin{rem}[$\Wass_1$ is a norm]\label{rem-w1-norm-1d}
	For $p=1$, formula~\eqref{eq-w1-1d} shows that $\Wass_1$ is a norm on zero-mass signed measures identified with their cumulative primitives. More generally, Kantorovich--Rubinstein duality defines such a norm on every metric space whenever its supremum is finite; see Section~\ref{sec-W1}. What is special to the line is the explicit primitive representation $\norm{\cumul{\al}-\cumul{\be}}_{L^1(\RR)}$.
\end{rem}
