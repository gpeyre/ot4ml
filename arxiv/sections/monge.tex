...
%
We denote by $\Mm_+(\X)$ the cone of positive finite measures on $\X$, characterized by $\al(A)\geq0$ for every Borel set $A$. The probability measures form the subset $\Mm_+^1(\X)=\enscond{\al\in\Mm_+(\X)}{\al(\X)=1}$.
\index{probability measure}% strictindex
...
A positive Borel measure is Radon if it is inner regular, meaning that the mass of every Borel set is the supremum of the masses of its compact subsets. Every finite Borel measure on a Polish space is Radon. Using Lebesgue integration, a Borel measure can be used to integrate measurable functions, and we denote this pairing by
\index{Borel set}% strictindex
...
\index{density!relative}% strictindex
	...
	\]
	Equivalently, for every nonnegative measurable function $h$, and hence for every integrable $h$,
	\[
...
Probability measures represent the laws of random variables. Let $(\Omega,\Ff,\PP)$ be an abstract probability space and let $\Xx$ be Polish. A random variable with values in $\Xx$ is a measurable map $X:(\Omega,\Ff)\to(\Xx,\Bb(\Xx))$, or simply $X:\Omega\to\Xx$ once the measurable structures are understood. Its law is the Radon probability measure $\al=X_\sharp\PP$ defined by
...
\end{align*}
	Since the identity holds for every such $h$, uniqueness of Lebesgue densities gives
\eq{
	\density{\be}(y) = \density{\al}(\T^{-1}y) \frac{1}{|\det(\T'(\T^{-1} y))|}.
}
	The equivalent source-variable formula follows by setting $y=\T(x)$:
\eq{
		\density{\al}(x) = |\det(\T'(x))| \density{\be}(\T(x))
}
where $\T'(x) \in \RR^{\dims \times \dims}$ is the Jacobian matrix of $\T$.
\end{proof}
%%%%%%%
...
The constraint $\T_\sharp\al=\be$ means that $\T$ pushes the mass of $\al$ onto $\be$ in the sense of Definition~\ref{defn-pushfwd}; by convention, the infimum is $+\infty$ when no such map exists.
...
\index{probability measure}% strictindex
\index{Polish space}% strictindex
\index{atomless measure}% strictindex
\end{prop}
\begin{proof}
...
\end{thm}
\begin{proof}
	The proof uses the Kantorovich relaxation and duality developed later in Chapter~\ref{sec-dual}. Choose optimal $c$-concave potentials $(f,g)$ for the cost $\norm{x-y}^2$ and define
	\[
		\phi(x)\eqdef\frac12\norm{x}^2-\frac12 f(x),
		\qquad
		\psi(y)\eqdef\frac12\norm{y}^2-\frac12 g(y).
...
Figure~\ref{fig:monge-polar-factorization} separates these two factors on a colored grid. The first deformation is only a relabeling: it swirls the grid inside the square while preserving area. The second is a symmetric positive definite affine map, hence the gradient of a convex quadratic potential.
\index{measure-preserving!map}% strictindex
\index{convex!potential}% strictindex
...
\small original labels & \small area-preserving relabeling & \small Brenier stretch
\end{tabular}
\caption{\emph{Polar factorization as a relabeling followed by a Brenier map.} The middle panel applies the area-preserving flow of a divergence-free Hamiltonian vector field; it changes the parametrization inside the square but keeps the square boundary fixed. The right panel then applies a symmetric positive definite affine stretch, the gradient of a convex quadratic potential. Faint arrows indicate the successive maps.}
\index{polar factorization}% strictindex
\index{Brenier!map}% strictindex
...
\index{Caffarelli regularity}% strictindex
	Let $\Omega,\Lambda\subset\RR^d$ be bounded uniformly convex domains with $C^2$ boundaries. Let $\al=\rho(x)\d x$ and $\be=\eta(y)\d y$ be supported on $\Omega$ and $\Lambda$, respectively, with
	\[
		0<m\leq\rho,\eta\leq M<+\infty.
	\]
	If $\rho\in C^\alpha(\overline\Omega)$ and $\eta\in C^\alpha(\overline\Lambda)$ for some $\alpha\in(0,1)$, then the Brenier potential $\phi$ is strictly convex in $\Omega$ and belongs to $C^{2,\alpha}_{\mathrm{loc}}(\Omega)$. Consequently, $\nabla\phi\in C^{1,\alpha}_{\mathrm{loc}}(\Omega)$. Under the standard compatibility and higher boundary-regularity assumptions, the same regularity extends to the boundary.
\end{prop}
\begin{proof}
...
\index{Ma-Trudinger-Wang condition}% strictindex
...
\index{Wasserstein!p-distance}% strictindex
...
On a Riemannian manifold, the natural analogue of the quadratic Euclidean cost is the squared geodesic distance \(c(x,y)=d_M(x,y)^2/2\). The optimal map is no longer written with a vector-space subtraction, but with the exponential map:
	\[
		\T(x)=\exp_x(-\nabla\phi(x)),
	\]
	where \(\phi\) is \(c\)-convex. This is the intrinsic version of the formula \(T=x-\nabla\phi\) in normal coordinates. The main additional issues are the cut locus, possible non-uniqueness of minimizing geodesics, and the regularity of the exponential map, which is why the Euclidean statement is usually presented first. The Riemannian polar-factorization theorem of McCann~\cite{mccann2001polar} gives the corresponding optimal-map framework, and these ideas feed into displacement convexity and the general manifold theory of optimal transport~\cite{mccann1997convexity,Villani09}.
\index{displacement convexity}% strictindex
...
\index{Ma-Trudinger-Wang condition}% strictindex
\index{MTW condition}% strictindex
...
\index{Ma-Trudinger-Wang condition}% strictindex
	\[
		\sum_{i,j,k,l}\partial^2_{p_k p_l} A_{ij}(x,p)\,\xi_i\xi_j\eta_k\eta_l\geq0
...
\index{strong MTW condition}% strictindex
\end{defn}
...
\index{quantile!map}% strictindex
\index{one-dimensional!transport}% strictindex
\textbf{Input:} One-dimensional probability measures $\al,\be$; time $t\in[0,1]$.

\textbf{Output:} Quantile coupling, Monge map when defined, and geodesic point $\al_t$.
...
\textbf{If} $\al$ has no atoms \textbf{then}:
\begin{algblock}
...
\textbf{Set} \(\al_t=(Q_t)_\sharp\mathrm{Leb}_{[0,1]}\).

\textbf{Return} $(X,Y)$, $T$ if defined, and $\al_t$.
\end{alg}
...
\begin{rem}[$\Wass_1$ is a norm]\label{rem-w1-norm-1d}
	For $p=1$, formula~\eqref{eq-w1-1d} shows that $\Wass_1$ is a norm on signed measures with zero total mass once they are identified with their cumulative primitives. This norm property is not specific to the line: in any dimension, and more generally on metric spaces after restricting to zero-mass signed measures for which the Kantorovich--Rubinstein supremum is finite, duality defines the norm studied in Section~\ref{sec-W1}. The one-dimensional formula is special only because the norm has the explicit primitive representation $\norm{\cumul{\al}-\cumul{\be}}_{L^1(\RR)}$.
\end{rem}

Other classical one-dimensional distances are obtained by replacing the $L^1$ norm of cumulative functions with $L^2$ or $L^\infty$ norms; under suitable tightness assumptions, such norms also metrize convergence in law and lead for instance to Cram\'er--von Mises and Kolmogorov--Smirnov-type distances.
...
The same tree can nevertheless be used as a genuine geodesic space. If $\pi$ is an optimal coupling for the squared tree distance $d_{\mathsf{T}}^2$, the associated displacement interpolation moves each packet of mass $\pi_{ij}$ at constant speed along the unique path from $x_i$ to $x_j$. Figure~\ref{fig:monge-tree-mccann-interpolation} shows this tree analogue of McCann interpolation. The intermediate measures are not necessarily supported on the original vertices: mass may sit inside edges while it travels through the branching structure.
\index{McCann interpolation}% strictindex
...
\index{Knothe-Rosenblatt rearrangement@Knothe--Rosenblatt rearrangement}% strictindex
\textbf{Input:} Probability measures $\al,\be$ on $\RR^d$ with conditional laws.
...
\begin{proof}
	We summarize the scale-separated argument of~\cite{carlier2010knothe}. Let $\pi_\epsilon=(\Id,T_\epsilon)_\sharp\al$ and set
	\[
		I_k(\gamma)\eqdef\int\abs{x_k-y_k}^2\d\gamma(x,y),
		\qquad
...
\end{tabular}
\caption{\emph{One-dimensional Gaussian $\Wass_2$ geodesics.} In the coordinates $(m,\sigma)$, the geodesics are Euclidean segments in the upper half-plane. The two density panels show the corresponding Gaussian densities along the two segments, with colors interpolating from the red endpoint to the blue endpoint.}
...
\index{Fisher-Rao metric}% strictindex
\index{Gaussian!measure}% strictindex
...
\end{tabular}
\caption{\emph{Wasserstein and Fisher--Rao geodesics in the one-dimensional Gaussian family.} The left panel shows the straight Wasserstein chord with neutral gray samples and the Fisher--Rao geodesic as a colored hyperbolic arc in the rescaled upper-half-plane geometry. The two density panels use the same endpoint Gaussians and the same time samples; the Fisher--Rao path expands the standard deviation along the hyperbolic arc before returning to the target scale.}
...
The same formula can be read without first restricting to Gaussian measures.
Let $\Phi_2$ keep only raw second moments. Taking the minimum of $\Wass_2$ over
the two fibers of $\Phi_2$ defines a quotient distance on $\mathbb S_+^d$.
Since centered Gaussian laws realize every positive semidefinite raw second
moment, this quotient distance is exactly Bures. The adjective ``raw'' is
important: for a non-centered law, $\Phi_2(\al)$ is not its covariance matrix.
...
\begin{proof}
	...
	\[
		\tr(K)
		\leq
		\tr\pa{((\Sigma+\eta I)^{1/2}(\Lambda+\eta I)(\Sigma+\eta I)^{1/2})^{1/2}}.
	\]
...
\index{positive!semidefinite matrix}% strictindex
\index{covariance!term}% strictindex
For $2\times2$ covariance matrices, this geometry can be seen inside a familiar cone. The useful coordinates separate trace, anisotropy and correlation. Writing
\[
...
\end{tabular}
\caption{\emph{Two-dimensional Gaussian $\Wass_2$ geodesics.} Means move linearly, while covariance ellipses follow the Bures--Wasserstein interpolation. The left panel contracts an anisotropic Gaussian toward an isotropic one; the middle panel interpolates between two strongly oriented anisotropic covariances. The right panel displays the same two covariance paths in the $2\times2$ positive-semidefinite cone, with $u$ and $v$ horizontal and $t$ vertical; the solid and dashed colored curves correspond to the left and middle ellipse panels, while faint gray chords show the ambient Euclidean segments for comparison.}
\label{fig:monge-gaussian-w2-geodesic-2d}
\end{figure}

\begin{rem}[Comparison with the Fisher--Rao metric for zero-mean Gaussians]\label{rem-fisher-rao-bures-gaussian}
...
\begin{proof}
	...
	\]
	Indeed, expanding the square gives $\tr\Sigma+\tr\Lambda-2\max_{Q^\top Q=I}\tr(\Sigma^{1/2}Q^\top\Lambda^{1/2})$, and the orthogonal Procrustes formula identifies the maximum with $\tr((\Sigma^{1/2}\Lambda\Sigma^{1/2})^{1/2})$. Symmetry, positivity and separation follow immediately from this representation. For the triangle inequality, choose almost optimal orthogonal matrices $Q_1,Q_2$ for $(\Sigma,\Lambda)$ and $(\Lambda,\Gamma)$, respectively. Since $Q_2Q_1$ is admissible for $(\Sigma,\Gamma)$,
\index{triangle inequality}% strictindex
	\[
		\Bb(\Sigma,\Gamma)
		\leq
