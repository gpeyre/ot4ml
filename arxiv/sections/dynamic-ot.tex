...
Although the minimizing-movement construction is introduced only in Chapter~\ref{sec-wasserstein-gradient-flows}, it is useful to keep its intuition in mind here. If one formally performs one JKO step for the metric \(\mathcal W_K\),
\begin{equation}
...
=K\rho^k+O(\tau),
\]
where \((K\rho)_i=\sum_jK_{ij}(\rho_j-\rho_i)\). Thus the discrete Wasserstein geometry is engineered so that the implicit Euler step for entropy is, to first order, the Markov semigroup. This is precisely the finite-state analogue of the JKO interpretation of the heat equation.
