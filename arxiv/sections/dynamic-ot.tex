...
For a fixed jump kernel, this geometry is genuinely nonlocal and does not coincide with ordinary $\Wass_2$. The local metric is nevertheless recovered as a small-jump limit: for symmetric kernels concentrated at scale $\varepsilon$, with the second moment normalized appropriately, the nonlocal actions and distances converge as $\varepsilon\to0$ to the classical Benamou--Brenier action and $\Wass_2$~\cite{SlepcevWarren2022NonlocalWasserstein}. Thus the continuum and graph constructions below are nonlocal analogues of Wasserstein geometry, while sharply concentrated isotropic jumps return the local theory.
