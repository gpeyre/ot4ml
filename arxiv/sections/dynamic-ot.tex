...
The static spectral distances of Section~\ref{sec-spectral-subspace-wasserstein} penalize a coupling through the covariance of its displacement. A dynamic version keeps the continuity equation but replaces the pointwise kinetic energy by a gauge of the whole velocity covariance. The resulting action is nonlocal in space: velocity directions are charged globally through their covariance. Let $\gamma$ be a monotone spectral gauge on $\mathbb S_+^d$. For a probability measure $\alpha$ and a velocity field $v\in L^2(\alpha;\RR^d)$, set
...
\begin{equation}\label{eq-dynamic-spectral-wasserstein}
	...
	\alpha_{t=0}=\alpha_0,\ \alpha_{t=1}=\alpha_1}}
	\int_0^1\mathcal N_{\gamma,\alpha_t}(v_t)^2\,\d t .
\end{equation}
The trace gauge recovers the usual Wasserstein tangent norm, while $\gamma(M)=\lambda_{\max}(M)$ charges only the largest directional velocity variance.
...
\begin{proof}
For a static coupling $\pi$, let $(X,Y)\sim\pi$, set $Z_t=(1-t)X+tY$ and $\alpha_t=(Z_t)_\sharp\pi$, and define \(v_t(z)=\EE[Y-X\mid Z_t=z]\). Then $(\alpha_t,v_t)$ solves the continuity equation. If \(M_\pi=\int(x-y)(x-y)^\top\d\pi(x,y)\) and \(C_t=\int v_tv_t^\top\d\alpha_t\), conditional Jensen gives \(C_t\preceq M_\pi\). Since $\gamma$ is monotone, \(\int_0^1\gamma(C_t)\d t\leq\gamma(M_\pi)\). Infimizing over $\pi$ gives \(\mathsf W_{\gamma,\mathrm{dyn}}^2\leq\Wass_\gamma^2\).

Conversely, let $(\alpha_t,v_t)$ be a finite-action competitor. Since $\gamma$ is equivalent to the trace on the finite-dimensional cone $\mathbb S_+^d$, finite spectral action gives finite kinetic energy. By the superposition principle, the competitor is represented by a probability law $\eta$ on absolutely continuous paths satisfying $\dot\omega_t=v_t(\omega_t)$. The endpoint law $\pi=(e_0,e_1)_\sharp\eta$ is a coupling of $\alpha_0$ and $\alpha_1$. Jensen along each path yields
\[
	(\omega_1-\omega_0)(\omega_1-\omega_0)^\top
	\preceq
	\int_0^1\dot\omega_t\dot\omega_t^\top\d t,
\]
hence \(M_\pi\preceq\int_0^1 C_t\d t\). Monotonicity and convexity of $\gamma$ give
\[
	\gamma(M_\pi)
	\leq
...
\]
Thus the static value is no larger than any dynamic action. The crucial hypothesis is the Loewner monotonicity of $\gamma$: without it, the covariance comparisons above would not imply action inequalities. This is the static/dynamic spectral identity used in~\cite{peyre2026muon}.
\end{proof}

Consequently, the dynamic formula is the path-length metric generated by the spectral tangent norm. This is the geometry behind the operator-gauge connection with Muon-type normalization developed in~\cite{peyre2026muon}.
