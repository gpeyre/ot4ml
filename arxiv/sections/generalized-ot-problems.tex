...
Once the candidate support is fixed, problem~\eqref{eq-wass-discr} is convex in $\a$. Instead of fixing that support in advance, one may optimize simultaneously over its weights and locations. For quadratic costs on $\RR^d$, this gives
\[
...
\begin{proof}
	...
	\]
	Consequently,
	\eql{\label{eq-gaussian-barycenter-first-variation}
		D\mathcal E(X)[Z]
		=
...
\]
and gives
\eql{\label{eq-gaussian-barycenter-damped-update}
	\mathcal K_\eta(X)
	\eqdef
	B_\eta(X)X B_\eta(X).
}
...
\begin{prop}[Convergence of the Gaussian transport fixed-point iteration]\label{prop-gaussian-barycenter-fixed-point-convergence}
	Assume that $\lambda_s>0$, $\cov_s\succeq0$, and that at least one $\cov_s$ is positive definite. Fix $\eta\in(0,1]$ and $X_0\succ0$, and iterate
	\[
		X_{\ell+1}=\mathcal K_\eta(X_\ell).
	\]
	Then every $X_\ell$ is positive definite and $X_\ell\to\cov$, where $\cov$ is the covariance of the Gaussian barycenter in Proposition~\ref{prop-gaussian-barycenter}. More precisely, with
	\[
		\mathcal R(X)
		\eqdef
	...
	\]
	one has
	\eql{\label{eq-gaussian-barycenter-descent}
		\mathcal E(X_{\ell+1})
		\leq
		\mathcal E(X_\ell)
		-
		\eta(2-\eta)\mathcal R(X_\ell),
	}
	and therefore $\sum_{\ell\geq0}\mathcal R(X_\ell)<+\infty$. Equivalently,
	\[
		\Bb\bigl(X_\ell,X_{\ell+1}\bigr)^2
		=
		\eta^2\mathcal R(X_\ell).
	\]
\end{prop}
...
\begin{proof}
	...
	$\delta(X_\ell)\geq\min\{\delta(X_0),c_0\}>0$ for every $\ell$.
...
\index{discrete!barycenter}% strictindex
	Let $\be_s=\sum_{i_s=1}^{n_s}b_{s,i_s}\de_{x_{s,i_s}}$ be discrete input measures. There exists a quadratic Wasserstein barycenter $\al^\star$ supported on weighted averages $\sum_s\la_s x_{s,i_s}$ of one support point from each input and satisfying
	\[
...
\begin{proof}
	Let $\mathcal A_{\mathrm{marg}}$ map the tensor $\P=(\P_{i_1,\ldots,i_S})$ to its $S$ marginals. Its annihilator consists of vectors $u_s\in\RR^{n_s}$ satisfying $\sum_su_{s,i_s}=0$ for every multi-index. Varying one index at a time shows that $u_s=c_s\ones_{n_s}$, and the remaining condition is $\sum_sc_s=0$. Thus
	\[
		\operatorname{rank}(\mathcal A_{\mathrm{marg}})
		=
		\sum_{s=1}^S n_s-S+1.
	\]
	A linear objective over the multi-marginal polytope has an optimal basic feasible tensor with at most this many positive entries. Proposition~\ref{prop-multimarginal-barycenter} pushes such a tensor forward by $B$; each positive entry produces at most one barycenter atom.
\index{multi-marginal!OT}% strictindex
\index{basic feasible solution}% strictindex
\end{proof}

This linear support bound is the standard sparsity estimate for discrete Wasserstein barycenters~\cite{anderes2016discrete}.
...
The last condition is the \emph{running-intersection property}. A tree decomposition equipped with factors assigned to its bags is also called a \emph{junction tree}. Equivalently, eliminate the vertices in some order, connecting the remaining neighbors of a vertex just before eliminating it. These \emph{fill-in edges} create an induced width equal to the largest number of remaining neighbors encountered; minimizing that width over elimination orders gives $\operatorname{tw}(G)$. This viewpoint explains why treewidth controls exact summation. Higher-order factors are handled by connecting every pair of variables appearing in a common factor and taking the treewidth of that primal interaction graph.
...
\index{complete graph}% strictindex
	A tree, including a chain or a star, has treewidth one. The edge bags $\{q,q+1\}$ decompose a chain. A cycle $C_S$, $S\geq3$, has treewidth two: the bags $\{1,q,q+1\}$, $q=2,\ldots,S-1$, give a width-two decomposition. The complete graph $K_S$ has treewidth $S-1$, so it offers no asymptotic reduction over enumerating all indices. The star obtained by retaining a barycenter variable as a hub also has width one; in that graphical formulation, leaf marginals are prescribed while the hub marginal is induced by the optimizer~\cite{FanHaaslerKarlssonChen2021GraphCostMOT}.
\end{example}
...
The treewidth reduction replaces each full tensor contraction in Sinkhorn by exact sum-product messages. Define
\[
	K^{r,s}_{i_r,i_s}
	=
...
\end{equation}
When $G$ is a tree, directed sum-product messages satisfy
\begin{equation}\label{eq-multimarginal-tree-message}
	...
	m_{q\to r}(i_r),
\end{equation}
where $N_G(r)$ denotes the neighbors of $r$. Cutting $(r,s)$ shows that $m_{r\to s}(i_s)$ is the contribution of the component containing $r$, conditional on $i_s$. A leaf-to-root and root-to-leaf pass gives every current marginal,
...
\end{equation}
after which the block $s$ selected by cyclic scaling is updated by $u_s\leftarrow u_s\odot\a_s/\widehat{\a}_s$. This is an exact coordinate update; updating all blocks at once would instead define a Jacobi scheme.

For a general tree decomposition, choose a host bag for each unary factor, assign each edge factor to a bag containing both endpoints, and denote their product in $B_q$ by $\psi_q(i_{B_q})$. For adjacent bags $q,q'$, with separator $\mathcal S_{q,q'}=B_q\cap B_{q'}$, the junction-tree message is
...
\begin{equation}\label{eq-multimarginal-junction-belief}
	...
	=
	\psi_q(i_{B_q})
	\prod_{\ell\in N_{\mathcal T}(q)}M_{\ell\to q}(i_{\mathcal S_{\ell,q}}).
\end{equation}
Summing this belief over $B_q\setminus\{s\}$ gives $\widehat{\a}_s$ for any $s\in B_q$; the running-intersection property makes the result independent of the chosen bag. After contracting redundant adjacent bags, a width-$w$ decomposition has bags of size at most $w+1$ and separators of size at most $w$. Thus, for $n_s\leq n$, one collect-distribute inference pass at fixed scalings costs $O(|Q|n^{w+1})$ time and $O(|Q|n^w)$ message memory. More precisely, the bounds are
\[
	O\!\left(\sum_{q\in Q}(1+\deg_{\mathcal T}(q))\prod_{s\in B_q}n_s\right),
	\qquad
	O\!\left(\sum_{(q,q')\in F}\prod_{s\in\mathcal S_{q,q'}}n_s\right).
\]
The latter excludes the original factors and scaling vectors and assumes streamed bag contractions; storing all dense beliefs costs $O(|Q|n^{w+1})$ working memory.

These are inference bounds, not the cost of an entire Sinkhorn solve. Assign each constrained marginal $s$ to a host bag $q(s)$. After updating $u_s$, only the messages along the unique path to the host of the next marginal must be refreshed. A path of length $\ell$ costs $O(\ell n^{w+1})$, hence at most $O(\operatorname{diam}(\mathcal T)n^{w+1})$ per block update~\cite{HaaslerSinghZhangKarlssonChen2020PGMMOT,FanHaaslerKarlssonChen2021GraphCostMOT}. A tree costs $O(\sum_{(r,s)\in E}n_rn_s)$ for a full pass, or $O(Sn^2)$ with equal support sizes; a cycle costs $O(Sn^3)$; and a complete graph still costs $O(n^S)$.

The implementation stores only the original local factors and separator messages, evaluates bag products on the fly, uses~\eqref{eq-multimarginal-tree-marginal} or~\eqref{eq-multimarginal-junction-message} for the generalized Sinkhorn contractions, and returns the coupling implicitly. Explicitly materializing all entries of $\P$ would itself cost $\prod_s n_s$, so the reduction applies when only marginals, costs, samples or low-order statistics are required. For small $\epsilon$, the recursions are evaluated in the log domain. This viewpoint connects entropic multi-marginal OT with exact inference in graphical models and Schr\"odinger bridge computations~\cite{HaaslerRinghChenKarlsson2021TreeMOT,HaaslerSinghZhangKarlssonChen2020PGMMOT,FanHaaslerKarlssonChen2021GraphCostMOT,AltschulerBoixAdsera2022StructuredMOT}.

A representative fluid-mechanics example is the time discretization of Brenier's generalized incompressible Euler problem: kinetic action couples neighboring time slices and periodic incompressibility closes the chain into a cycle, hence a graph of treewidth two. Entropic Bregman/Sinkhorn schemes exploit this circular structure through low-order contractions~\cite{2015-benamou-cisc,BenamouCarlierNenna2018GeneralizedIncompressible}.
\index{Sinkhorn!iteration}% strictindex
...
The cut condition can be stated exactly. For index sets $I\subset\{1,\ldots,n\}$ and $J\subset\{1,\ldots,m\}$, write $a(I)=\sum_{i\in I}a_i$, $b(J)=\sum_{j\in J}b_j$, and $U(I,J)=\sum_{i\in I,j\in J}U_{ij}$.
...
\textbf{Input:} Positive marginals $\a\in\simplex_n$, $\b\in\simplex_m$, finite cost $\C$, feasible capacity $U$, regularization $\epsilon>0$, tolerance $\mathrm{tol}$.

\textbf{Output:} Entropic capped coupling $\P$.

\textbf{Restrict} all arrays to the active edge set $E=\{(i,j):U_{ij}>0\}$; entries outside $E$ remain zero.
...
\\[-.1em]
\small unconstrained, $\kappa=+\infty$ &
\small medium capacity, $\kappa=10$ &
\small small capacity, $\kappa=2.6$
\end{tabular}
\caption{Capacity-constrained entropic OT between two one-dimensional Gaussian-mixture histograms. Each matrix solves~\eqref{eq-entropic-capacity-constrained-ot} with the same marginals and the same squared-distance cost. The left panel is the unconstrained entropic OT solution, corresponding to the density-ratio cap $U_{ij}=+\infty$. The other panels impose $U_{ij}=\kappa a_i b_j$. The red side strip is the source marginal and the blue top strip is the target marginal. As the capacity decreases, more entries saturate and the coupling is forced to spread.}
\index{Gaussian!mixture}% strictindex
\index{capacity-constrained OT}% strictindex
...
Let $\Xx$ and $\Yy$ be Polish spaces, let $\alpha\in\Pp(\Xx)$ and $\beta\in\Pp(\Yy)$, and let $C:\Xx\times\Pp(\Yy)\to\RR\cup\{+\infty\}$ be measurable. For each $\pi\in\Couplings(\alpha,\beta)$, let $\pi(\d x,\d y)=\alpha(\d x)\pi_x(\d y)$ be a disintegration. Then
...
\end{tabular}
\caption{Classical and weak quadratic transport between a red disk and a blue annulus. Left: the quadratic OT plan $\pi_{\mathrm{OT}}$, with segment thickness proportional to transported mass. Center: its barycentric projection $\bar T_{\pi_{\mathrm{OT}}}$, shown by violet displacements. Right: the projection $\bar T_{\pi_{\mathrm{weak}}}$ of a minimizer of~\eqref{eq-barycentric-weak-primal}; it remains much closer to the source points because the weak objective ignores the conditional spread needed to reproduce the blue marginal.}
\index{weak!cost}% strictindex
