...
\index{Stein method}% strictindex
	...
	\]
		The solution satisfies uniform derivative bounds depending only on $\Lip(h)$~\cite[Chapter~2]{chen2011normal}. Hence
	\[
...
Figure~\ref{fig:statistical-berry-esseen-w1} confronts the estimate with exact one-dimensional computations. For a centered, unit-variance input law \(\al\), define \(\al_n=(D_{1/\sqrt n})_\sharp\al^{*n}\) for \(n\geq1\), so that \(\al_1=\al\). The two examples are the symmetric Bernoulli law and the continuous uniform law on \([-\sqrt3,\sqrt3]\). Atom masses in the Bernoulli panels are divided by their lattice spacing to obtain a local-density scale.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=.98\linewidth]{figures/statistical-berry-esseen-w1--comparison.pdf}
		\caption{\emph{Sharp lattice and density central-limit asymptotics in \(\Wass_1\).} The two left panels show \(\al_1\), \(\al_2\), and \(\al_6\) for symmetric Bernoulli and continuous-uniform inputs; the gray curve is the standard Gaussian density. The right panel compares the exact numerical distances (solid) with the sharp equivalents \(1/(2\sqrt n)\) for Bernoulli and \((1+4e^{-3/2})/(10\sqrt{2\pi}\,n)\) for the continuous-uniform law (dashed).}
