...
\index{plan!interpolation}% strictindex
\index{McCann interpolation}% strictindex
\textbf{Input:} Measures $\al,\be$ on $\RR^d$, time $t\in[0,1]$.

\textbf{Output:} Displacement interpolant $\al_t$.
...
\end{tabular}
\caption{\emph{Central-limit theorem for normalized Bernoulli sums.} Starting from $\al_0=\frac12(\delta_{-1}+\delta_1)$, the law of $Z_n=n^{-1/2}\sum_{i=1}^nX_i$ remains discrete, but its rescaled atom heights approach the standard Gaussian density shown in gray. The Wasserstein Berry--Esseen bound below quantifies this weak convergence in $\Wass_1$.}
\index{central limit theorem}% strictindex
\index{weak!convergence}% strictindex
