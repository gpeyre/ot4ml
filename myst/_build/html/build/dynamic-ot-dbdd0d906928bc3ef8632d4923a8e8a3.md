...
The most geometric way to generalize the Benamou--Brenier formula is to keep
the continuity equation and change the infinitesimal norm on velocities.
Informally, instead of measuring a tangent vector $v$ at $\alpha$ by

```{math}
\int |v(x)|^2\,\d\alpha(x),
```

one can prescribe a positive self-adjoint linear operator
$Q_\alpha:L^2(\alpha;\RR^d)\to L^2(\alpha;\RR^d)$, or more precisely on the
admissible tangent subspace, and define the quadratic action
...
The usual $\Wass_2$ geometry corresponds to $Q_\alpha=\Id$. This is the
Riemannian-type viewpoint used later to interpret gradient flows: changing
$Q_\alpha$ changes the meaning of steepest descent. When $\alpha=\rho\d x$,
$m=\rho v$, and $Q_\alpha$ is pointwise multiplication by a symmetric positive
matrix $B_\rho(x)$, the integrand becomes
...
```{math}
A(\rho,m,x)=\frac{m^\top B_\rho(x)m}{\rho}.
```

For $B_\rho=\Id$, this reduces to the Benamou--Brenier density
$A(\rho,m)=|m|^2/\rho$. The next paragraph is more general: it is written in
the $(\rho,m)$ variables, suppressing possible $x$-dependence to keep notation
light. This makes convexity and lower-semicontinuous relaxation transparent,
and it also includes nonquadratic, hence non-Riemannian, Finsler-type actions.

(rem-generalized-bb)=
### General Mobilities and Convex Action Densities

A common abstraction is to keep the continuity equation but replace the
quadratic cost $|m|^2/\rho$ by a convex local action. This contains the
quadratic $Q_\alpha$-construction above whenever $Q_\alpha$ is a local
multiplication operator, but it also covers actions whose infinitesimal length
is not induced by a Hilbert norm. Let $I\subset[0,+\infty)$ be a convex
...
In density--momentum variables, this time-dependent action reads
...
Thus it is convex in $(\rho,m)$ by the matrix perspective and the monotonicity
and convexity of $\gamma$, but it is not generally a pointwise action density
of the form $\int A(\rho(x),m(x))\,\d x$. It becomes local only for linear
spectral gauges. For instance, if $\gamma(M)=\operatorname{tr}(GM)$ with
$G\succeq0$, then the spectral action corresponds to the quadratic density
...
This construction is therefore outside the pointwise local action-density
...
It is best viewed as a convex action on the pair space $X\times X$, rather
than as an integral of independent costs attached to single base points $x$.
