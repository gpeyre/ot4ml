...
Figure {numref}`fig:gradflow-fractional-laplacian-diffusion` illustrates the
qualitative effect of lowering $s$. Classical heat diffusion, $s=2$, smooths
local discontinuities by nearby averaging. Fractional diffusion keeps a sharper
memory of localized peaks but immediately creates algebraic long-range tails,
because mass can jump over macroscopic distances.
...
peaks remain more localized at short scales, while heavier tails appear across
