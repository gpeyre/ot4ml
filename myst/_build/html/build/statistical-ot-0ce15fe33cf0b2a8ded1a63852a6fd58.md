...
which is the barycentric projection of the entropic plan. For $\epsilon=0$,
the soft transform becomes the hard $c$-transform and the same formula uses an
optimal unregularized empirical plan. The consistency statement below is
written for uniform empirical weights, which is the standard statistical
setting; deterministic nonuniform weights are handled by the same compactness
argument with the corresponding grid-entropy bound.
...
assume that $\alpha$ is absolutely continuous. Let $T$ be the unique Brenier
map from $\alpha$ to $\beta$, represented by a Borel subgradient selection
$T(x)\in\partial\phi(x)$ of a Brenier potential $\phi$. If uniform empirical measures
$\hat\alpha_n,\hat\beta_m$ converge to $\alpha,\beta$ in $\Wass_2$, if
$\epsilon_{n,m}\to0$ with $\epsilon_{n,m}\log(nm)\to0$, and if
...
On a uniform $n\times m$ grid, the relative entropy with respect to
$\hat\alpha_n\otimes\hat\beta_m$ is at most $\log(nm)$, so the entropic
coupling has asymptotically optimal quadratic cost when
$\epsilon\log(nm)\to0$. Any weak limit is therefore an optimal population
...
Sinkhorn. The drawback of this sketching approach is that the additional
approximation error induced by the kernel sketch is dimension-dependent, so the
number of features needed for a fixed accuracy typically grows with the ambient
dimension.
...
where $\rho$ is a latent distribution. Drawing $z_1,\ldots,z_r\sim\rho$ gives
...
where divisions are componentwise. If $K$ is replaced by a rank-$r$
factorization

```{math}
K\simeq \widetilde K=\Phi_X\Phi_Y^\top,
\qquad
\Phi_X\in\RR^{n\times r},
\quad
\Phi_Y\in\RR^{m\times r},
```

then each scaling sweep costs $O((n+m)r)$ operations. The difficulty is that
Sinkhorn is not a generic kernel method: it divides by $\widetilde K v$ and, in
dual variables, applies logarithms. Thus one needs $\widetilde K_{ij}>0$,
preferably with a relative error on the entries, because
$-\epsilon\log K_{ij}$ is the effective cost. This is a more stringent
requirement than the usual spectral or Frobenius approximation of a PSD Gram
matrix.
...
:class: important
Let $K_{ij}=\mathbb E h_{ij}(Z)>0$ for $1\leq i\leq n$, $1\leq j\leq m$,
where $|h_{ij}(Z)|\leq M$ almost surely. Let
...
mode: negative entries in $\widetilde K$.

The dimension dependence is unavoidable in the worst case. For the quadratic
...
nonnegative factorization $K=BB^\top$. Thus this complete-positive notion of
total positivity is the kernel-level condition behind positive random
features. For rectangular source--target matrices, one applies the same
statement to the Gram matrix on the union of the sampled source and target
points and then extracts its $n\times m$ cross block.
...
positive. In particular, positive autocorrelations
...
The tensor feature remains entrywise nonnegative. The autocorrelation formula
is already a positive feature representation with latent variable $u$ and
scalar feature $g(u-x)$.
:::

(prop-gaussian-positive-features)=
:::{admonition} Proposition: Gaussian Positive Features
:class: important
Let

```{math}
k_\epsilon(x,y)=\exp\left(-\frac{\norm{x-y}^2}{2\epsilon}\right),
\qquad x,y\in\RR^d,\quad \epsilon>0 .
```

Fix a centering point $x_0\in\RR^d$, set
$q_x=(x-x_0)/\sqrt{\epsilon}$, and let $z\sim\mathcal N(0,\Id_d)$. The
positive feature

```{math}
\phi(x,z)=\exp\left(\dotp{z}{q_x}-\norm{q_x}^2\right)
```
...
k_\epsilon(x,y)
=
\mathbb E_z\left(\phi(x,z)\phi(y,z)\right).
```

Consequently, $k_\epsilon$ is totally positive in the complete-positive sense.
:::

:::{dropdown} Proof
The feature $\phi(x,z)$ is positive. Using the moment-generating function of a
standard Gaussian,
...
=
\exp\left(-\norm{q_x}^2-\norm{q_y}^2\right)
\mathbb E_z\left(\exp\dotp{z}{q_x+q_y}\right)
```
...
The arbitrary center $x_0$ cancels in the expectation. The representation is a
positive-feature representation, so every finite Gram matrix of $k_\epsilon$
is completely positive by the preceding discussion.
:::

This positive random feature identity is the one used by Scetbon and Cuturi
{cite:p}`ScetbonCuturi2020PositiveFeatures`; choosing $x_0$ near the data can
reduce the dynamic range of the features.
...
For the quadratic cost $c(x,y)=\norm{x-y}^2/2$, the Sinkhorn Gibbs kernel is
...
$z_1,\ldots,z_r\sim\mathcal N(0,\Id_d)$, define the normalized features

```{math}
\varphi_\ell(x)=r^{-1/2}\phi(x,z_\ell),
...
```{math}
\widetilde k_\epsilon(x,y)
=
\sum_{\ell=1}^r\varphi_\ell(x)\varphi_\ell(y)
\geq0,
\qquad
\mathbb E[\widetilde k_\epsilon(x,y)]=k_\epsilon(x,y).
```
...
$\widetilde c_\epsilon=-\epsilon\log\widetilde k_\epsilon$, and it
approximates the original problem only insofar as this logarithmic kernel
approximation is accurate. This route is complementary to worst-case
near-linear Sinkhorn analyses {cite:p}`altschuler2017near`, low-rank
...
changed by this approximation. The dense Gaussian Gibbs kernel gives the usual
entropic plan between two one-dimensional mixtures. A sufficiently rich
positive-feature sketch produces a visually indistinguishable sketched plan,
whereas an extremely coarse rank-one sketch still has the correct marginals
after Sinkhorn scaling but loses the geometric concentration of the true
coupling.

### Connection with Linear Attention

The same algebra appears in transformer attention. A softmax attention matrix
has entries proportional to $\exp(\dotp{q_i}{k_j})$; linear-attention methods
