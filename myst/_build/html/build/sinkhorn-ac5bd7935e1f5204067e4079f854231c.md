...
normalized-gradient Poisson solve. Larger heat times stabilize the linear solve
...
Completing the square gives $Q_t f(x)=\norm{x}^2/(2t)-(f+\norm{\cdot}^2/(2t))^*(x/t)$. The entropic version replaces this Legendre transform by the Gaussian log-sum-exp formula

```{math}
Q_t^\epsilon f(x)
= -\epsilon\log\int
\exp\!\left(-\frac{f(y)+\norm{x-y}^2/(2t)}{\epsilon}\right)\,dy
= -\epsilon\log\big(G_{\epsilon t}\ast e^{-f/\epsilon}\big)(x)+\text{cst}.
```
...
*Hopf--Cole numerics in one dimension: Gaussian soft Hopf--Lax, heat evolution
of the transformed variable, and the recovered viscous Burgers velocity.*
:::
