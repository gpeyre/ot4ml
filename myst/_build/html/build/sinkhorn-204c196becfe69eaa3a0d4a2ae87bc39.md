...
The local continuation can be visualized on a finite histogram problem. Since
the potentials enter only through exponentials, the complex logarithm has to be
followed on a continuous local branch; this is a display convention, while the
coupling itself is defined by the exponential formula.
...
Gaussian-mixture histograms are fixed, $\epsilon_0=.55$, and
$\epsilon=\epsilon_0+i\eta$ is varied for $\eta$ near zero. The bold black
curve is the real-temperature potential $\eta=0$; colored curves follow the
local branch for negative $\eta$ in blue, positive $\eta$ in red, and
intermediate values in violet. The faint red and blue silhouettes show the
source and target histograms.*
:::
