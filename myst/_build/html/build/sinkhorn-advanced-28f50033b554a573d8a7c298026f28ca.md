...
Choose an interior starting point $P_0$. For $k=0,1,\ldots$, alternate the two Bregman projections
...
Stop when the two constraint defects are below tolerance. In the entropic OT case,

```{math}
\Cc_1=\Cc^1_\a,\qquad \Cc_2=\Cc^2_\b,\qquad B_\Phi=\KL,
```

and the two half-steps are precisely row and column KL projections.
:::
...
Order-preserving additively homogeneous maps are called topical maps in nonlinear Perron--Frobenius theory {cite:p}`lemmens2012nonlinear`. Proposition labeled `prop-topical-variation-nonexpansive` is the basic nonexpansiveness mechanism behind Fortet's monotone argument. The Hilbert-metric analysis in Section labeled `sec-sinkhorn-hilbert` is stronger: under strict positivity assumptions on the kernel it upgrades nonexpansiveness to a genuine projective contraction.
:::
...
Given a target accuracy $\delta>0$ for a finite $n\times m$ cost matrix, choose for instance

```{math}
\epsilon=\frac{\delta}{2\log(nm)}.
```

Run stabilized Sinkhorn iterations and monitor an upper bound $\widehat\Delta_k$ on the entropic dual gap, for example through the marginal residual or the KL drop between half-steps. Stop when

```{math}
\widehat\Delta_k\leq\delta/2.
```

Return the entropy-corrected lower bound
...
Corollary labeled `cor-sinkhorn-dual-complexity` then gives
...
For $\al=\Gaussian(\mean_\al,\cov_\al)$ and $\be=\Gaussian(\mean_\be,\cov_\be)$, compute a singular-value decomposition
...
For each $\sigma_i>0$, set

```{math}
s_i=\frac{\sqrt{\epsilon^2+16\sigma_i^2}-\epsilon}{4\sigma_i}.
```

The optimal entropic coupling is the Gaussian law on $(X,Y)$ with means $(\mean_\al,\mean_\be)$, marginal covariances $(\cov_\al,\cov_\be)$, and cross-covariance
...
The entropic value is then obtained by inserting the same singular values into Proposition labeled `prop-gaussian-sinkhorn-closed-form`.
:::

## Sample Complexity

This section separates two statistical regimes. Exact OT resolves geometry at
all spatial scales and pays dimension-dependent empirical rates. Fixed
temperature Sinkhorn divergences smooth the dual potentials and recover
...
dimensionality. Entropic regularization changes the picture: for a fixed
$\epsilon>0$, Sinkhorn divergences have parametric $n^{-1/2}$ statistical
rates, although the constant deteriorates when $\epsilon\to0$
{cite:p}`genevay2018sample,bigot2017central`. Related two-sample-testing
viewpoints are developed in {cite:p}`ramdas2017wasserstein`, and the
...
exact OT deteriorates with the ambient dimension.
:::
...
$2^{Jd}\simeq n$, hence $n^{-1/d}$. Matching lower bounds follow from packing
...
The parametric rate in Proposition labeled `prop-sinkhorn-sample-rate` holds for fixed $\epsilon$. If the goal is to approximate the unregularized OT value, one must also account for the regularization bias. In a typical bounded-cost finite-dimensional regime,
