...
(ex-book-shifting-w1)=
:::{admonition} Example: Book-shifting and non-uniqueness for $\Wass_1$
:class: ot4ml-example
...
The monotone translation $T_{\rm tr}(x)=x+1$ pushes $\al$ to $\be$. For the
cost $|x-y|$, it is not the only optimal map. The book-shifting map
...
also satisfies $(T_{\rm book})_\sharp\al=\be$: it keeps the overlapping interval
$[1,2]$ fixed and sends the left interval $[0,1]$ to the uncovered interval
$[2,3]$. For any coupling $\pi\in\Couplings(\al,\be)$,
...
Thus $\Wass_1(\al,\be)\geq1$. Equality holds for every coupling supported on
the half-plane $\{(x,y):y\geq x\}$, where the inequality $|y-x|\geq y-x$ is
saturated. Both $T_{\rm tr}$ and $T_{\rm book}$ have this property and have
cost $1$, hence both are $\Wass_1$-optimal. In fact, one obtains infinitely many
optimal maps by replacing $x\mapsto x+2$ on $[0,1]$ by any measure-preserving
map from $[0,1]$ to $[2,3]$, while keeping the identity on $[1,2]$.

This degeneracy disappears for $\Wass_p$ with $p>1$. The unique
one-dimensional optimal coupling is the quantile coupling, induced by the
monotone rearrangement $T_p(x)=x+1$. Its $p$-cost is

```{math}
\int |T_p(x)-x|^p\d\al(x)=1,
```

whereas

```{math}
\int |T_{\rm book}(x)-x|^p\d\al(x)
=
\frac12\int_0^1 2^p\d x
=
2^{p-1}>1.
```

Thus the overlap can remain fixed for $\Wass_1$, but the strictly convex cost
$|x-y|^p$, $p>1$, penalizes this concentration of displacement and selects the
