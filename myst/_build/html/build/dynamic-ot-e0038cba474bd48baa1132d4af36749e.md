...
(rem-generalized-bb)=
### Wasserstein-p Benamou--Brenier Distances

The first extension changes only the kinetic exponent. For measures with finite $p$-th moments and $1<p<+\infty$, the Benamou--Brenier principle gives the dynamic formula
...
under the linear constraint $\partial_t\rho_t+\nabla\!\cdot m_t=0$. The $p$-th root of the value is exactly the Wasserstein distance $\Wass_p$; in particular this construction defines a distance because it is another representation of the static Kantorovich distance. The case $p=2$ is the quadratic action used above, while the limiting case $p=1$ loses strict convexity and is closer to the Beckmann formulation of $\Wass_1$.

### Concave-Mobility Benamou--Brenier Distances

A second extension keeps the quadratic power but replaces the linear mobility $\rho$ by a density-dependent mobility $\theta(\rho)$. On a reference domain, for densities taking values in an interval $I\subset[0,+\infty)$ and a concave function $\theta:I\to[0,+\infty)$, define formally
...
\int_0^1\!\int A_\theta(\rho_t(x),m_t(x))\,\d x\,\d t,
\qquad
A_\theta(\rho,m)=\frac{|m|^2}{\theta(\rho)}.
```

The same perspective convention is used: $A_\theta(\rho,0)=0$ when $\theta(\rho)=0$, and $A_\theta(\rho,m)=+\infty$ when $\theta(\rho)=0$ and $m\neq0$. The concavity of $\theta$ is the structural assumption which makes $A_\theta$ convex in $(\rho,m)$; it is therefore the analogue of the perspective convexity of the usual Benamou--Brenier action. The choice $\theta(\rho)=\rho$ recovers $\Wass_2$. Other choices encode different physics: $\theta(\rho)=\rho^\gamma$ with $0<\gamma\leq1$ changes the cost of moving dilute mass, while $\theta(\rho)=\rho(1-\rho/M)$ on $[0,M]$ models a volume-filling or exclusion effect, since mobility vanishes both at vacuum and at the maximal density. In gradient-flow form, these metrics lead formally to
