...
Total variation gives the sharpest mass-selection limit of this family and
recovers the classical partial-transport problem. For a price $\lambda>0$ for
...
Assume $c\geq0$. Any feasible plan can be replaced, without increasing the
objective, by one whose transported marginals do not exceed the available mass:
removing excess transported mass decreases the transport cost and cannot
increase the sum of the two total-variation penalties, because the decrease on
the excess marginal offsets the possible increase on the other marginal. Hence
an optimal plan may be chosen so that $\pi_1\leq\alpha$ and
$\pi_2\leq\beta$. Writing $m=\pi(\X\times\Y)$, the penalty then reduces to

```{math}
\lambda\big(\alpha(\X)-m\big)+\lambda\big(\beta(\Y)-m\big),
```

so the TV-penalized problem is the Lagrangian form of the mass-constrained
partial OT problem
...
Conversely, a prescribed transported mass $m$ is recovered by choosing a
supporting slope of the convex value function
$m\mapsto\operatorname{POT}_m(\alpha,\beta)$; on flat pieces, one value of
$\lambda$ can select an interval of masses. This is the usual constrained
formulation of partial OT studied by Caffarelli--McCann and
Figalli {cite}`CaffarelliMcCannPartial,FigalliPartial`. Modern computational
and learning applications include partial Wasserstein and partial
