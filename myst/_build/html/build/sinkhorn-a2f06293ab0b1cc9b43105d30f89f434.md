...
implementations, and it connects soft minima with Hopf--Cole transforms.
...
construction uses the shifted-Laplacian resolvent. On the line, the kernel of
...
A single inverse shifted Laplacian gives a Laplace/Yukawa smoothing, while
repeated resolvent steps approximate the Gaussian heat kernel. On a mesh or
grid, the limit is delicate: the heat scale $\sqrt t$ or the one-step
resolvent scale $\sqrt\tau$ must be resolved by the mesh. If this length goes
to zero faster than the grid spacing, metrication artifacts and inconsistent
schemes can dominate the geodesic limit.
