...
up to universal constants and harmless logarithms. Thus the feature rank is
near-linear in $n$ but polynomial in the inverse kernel bandwidth, with
exponent $d$. When the same machinery is used to approximate unregularized
...
For example, the Gaussian kernel is totally positive. Indeed, for

```{math}
g_\epsilon(u)=\left(\frac{2}{\pi\epsilon}\right)^{d/4}
\exp\left(-\frac{\norm{u}^2}{\epsilon}\right),
```

one has

```{math}
\int_{\RR^d}g_\epsilon(u-x)g_\epsilon(u-y)\,du
=
...
The positive random features of Scetbon and Cuturi
{cite:p}`ScetbonCuturi2020PositiveFeatures` use an equivalent exponential
identity. Fix any centering point $x_0$, write
$q_x=(x-x_0)/\sqrt{\epsilon}$, draw $\omega\sim\mathcal N(0,\Id_d)$, and set

```{math}
\psi_\omega(x)
\eqdef
\exp\left(\dotp{\omega}{q_x}-\norm{q_x}^2\right).
```

Then

```{math}
\mathbb E_\omega[\psi_\omega(x)\psi_\omega(y)]
=
\exp\left(-\frac{\norm{x-y}^2}{2\epsilon}\right).
```

The arbitrary center cancels in the expectation, but it can reduce the dynamic
range of the features.

With $r$ draws, the normalized features
$\varphi_\ell(x)=r^{-1/2}\psi_{\omega_\ell}(x)$ satisfy
