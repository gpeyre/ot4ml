...
A minimal but useful example is obtained by learning a bilinear cost on
$\RR^d$,
...
coupling look optimal. This is a simple finite-dimensional setting where
learning a cost is just learning a linear parameter. Since
...
bilinear cost with $A=-I$, up to the marginal-only terms $\norm{x}^2$ and
$\norm{y}^2$. The opposite choice $A=+I$ reverses this preference and favors
anti-aligned pairs. Rank-one choices such as $A=-e_1e_1^\top$ or
$A=-e_2e_2^\top$ keep only one coordinate of this correlation score, and
therefore learn only horizontal or vertical alignment.
...
uniform assignment problem with $c_A(x,y)=\dotp{Ax}{y}$. The two rank-one
matrices keep only the horizontal or vertical coordinate, $A=-I$ recovers the
quadratic $\Wass_2$ assignment up to marginal-only terms, and $A=+I$ gives the
corresponding anti-Wasserstein matching. The idea of drawing a logo through OT
maps is inspired by the POT logo {cite:p}`flamary2021pot`.*
:::
