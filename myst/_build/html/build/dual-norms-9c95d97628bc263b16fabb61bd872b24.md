...
As recalled in Definition labeled `defn-total-variation` and Proposition labeled `prop-tv-dual-measure`, total variation is the dual norm associated with the unit ball of continuous functions
...
$\Wass_1$, as defined in labeled `eq-w1-cont`, is the dual norm labeled `eq-dual-norm-cont` associated with
...
The Kullback--Leibler divergence $\KL \eqdef \Divergm_{\phi_{\KL}}$, also known as the relative entropy, was already introduced in labeled `eq-defn-rel-entropy` and labeled `eq-kl-defn`. It is the divergence associated to the Shannon--Boltzmann entropy function $\phi_{\KL}$, given by
...
If $\al$ is discrete as in labeled `eq-div-disc-meas`, then the TV norm is the $\ell^1$ norm of vectors in $\RR^n$, $\norm{\al}_{\TV}=\sum_i |\a_i| = \norm{\a}_{\ell^1}$.
:::
