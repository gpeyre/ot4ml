...
metric contraction mechanism, and finally turns to Gaussian closed forms,
sample-complexity consequences, and a non-variational M-function viewpoint.
...
Proposition {ref}`prop-phi-double-soft-transform-monotone` shows that the generalized double soft transform
...
is also topical for any admissible extremal selection, and in particular
whenever the scalar updates are unique. Equivalently, the double transform
commutes with translations,
$\mathcal A_\phi(f+\lambda)=\mathcal A_\phi(f)+\lambda$, and preserves the
pointwise order. Hence
...
This is only a nonexpansiveness statement. To obtain a linear convergence rate
for the fixed-point iteration, one needs a genuine contraction estimate with a
constant strictly smaller than one in a suitable projective metric. For KL
regularization, the Hilbert-metric analysis in Section
{ref}`sec-sinkhorn-hilbert` supplies such a contraction under strict positivity
assumptions on the Gibbs kernel. For general non-KL $\phi$-regularizers, an
analogous broadly applicable contraction principle seems, to the best of our
knowledge, open.
:::
...
## Monotone Non-Variational Scaling

The previous convergence mechanisms mostly used variational structure: Sinkhorn
is alternating KL projection, or coordinate ascent on the entropic dual. There
is also a broader order-theoretic mechanism. In Galichon's equilibrium-flow
viewpoint, and in related work on substitutability and inverse isotonicity, one
studies nonlinear market-clearing equations that need not be gradients of a
convex potential {cite:p}`GalichonJacquet2024Substitutability,GalichonSamuelsonVernet2022Monotone`.
This is the nonlinear counterpart of the classical theory of M-matrices and
M-functions {cite:p}`MoreRheinboldt1973PSFunctions,Plemmons1977MMatrix`.
...
The coordinate-clearing update solves

```{math}
Q_\ell(z_\ell^+,z_{-\ell})=0,
\qquad 1\leq \ell\leq n+m.
```

For the scaling examples below, these scalar equations decouple inside each
block, so the update is again a row rescaling followed by a column rescaling.
The same construction extends to any finite number of blocks.

(def-mfunctions)=
:::{admonition} Definition: Z-functions and M-functions
:class: important
Let $D\subset\RR^N$ be an order interval. A map $Q:D\to\RR^N$ is diagonally
isotone if $Q_\ell(z_\ell,z_{-\ell})$ is nondecreasing in $z_\ell$ for each
...
and that own effects dominate them enough to prevent a global reversal of
order. A degenerate $M_0$-function keeps the same order structure but allows a
null gauge direction; this is what happens for balanced Sinkhorn before one
normalizes the potentials.

(thm-mfunction-jacobi-convergence)=
:::{admonition} Theorem: Monotone Convergence of Coordinate Clearing
:class: important
Assume $Q:D\to\RR^N$ is continuous, diagonally isotone and an M-function.
Assume every scalar clearing equation $Q_\ell(\xi,z_{-\ell})=0$ has a unique
solution in $\xi$, and that there exist a subsolution $\underline z$ and a
supersolution $\overline z$. Then $Q(z)=0$ has a unique solution $z^\star$.
Jacobi coordinate-clearing iterates starting from any subsolution increase
monotonically to $z^\star$, while those starting from any supersolution
decrease monotonically to $z^\star$.
:::

:::{dropdown} Proof
If $z$ is a subsolution, then
$Q_\ell(z_\ell,z_{-\ell})\leq0=Q_\ell(T_\ell(z),z_{-\ell})$, where $T$ is the
coordinate-clearing map. Diagonal isotonicity gives $z\leq T(z)$. Since $Q$ is
a Z-function,
...
so $T(z)$ is again a subsolution. The supersolution case is analogous. Every
subsolution lies below every supersolution because inverse isotonicity applied
to $Q(\underline z)\leq0\leq Q(\overline z)$ gives
$\underline z\leq\overline z$. The monotone sequences are therefore trapped in
one order interval. Passing to the limit in
$Q_\ell(z_\ell^{k+1},z_{-\ell}^k)=0$ gives a zero. Two zeros must coincide by
inverse isotonicity.
:::

### Sinkhorn as an $M_0$-system
...
:::{admonition} Example: Lossy Sinkhorn Clearing
:class: ot4ml-example
Let $0<\eta_{ij}\leq1$, $\sigma_i>0$, $\tau_j>0$, and set
$P_{ij}=r_iK_{ij}s_j$, $r_i=e^{u_i}$, $s_j=e^{-v_j}$. Consider
...
Source mass can leave through an outside sink, while target mass can be served
by effective arrivals or by a local outside option. The update remains
Sinkhorn-like:

```{math}
r_i^+=\frac{a_i}{\sigma_i+\sum_jK_{ij}s_j},
\qquad
s_j^+=\frac{b_j}{\tau_j+\sum_i\eta_{ij}K_{ij}r_i}.
```

On the invariant domain $r_i\leq a_i/\sigma_i$, the sufficient condition

```{math}
\tau_j>\sum_i(1-\eta_{ij})K_{ij}\frac{a_i}{\sigma_i}
```

gives the weighted diagonal-dominance certificate $\mathbf 1^\top DQ\gg0$, so
the system is an M-function there. It is generally non-variational: equality of
cross-partials would force $-P_{ij}=-\eta_{ij}P_{ij}$ on each active arc, hence
$\eta_{ij}=1$.
:::
...
$\tau$. The first row colors the source log-scaling $\log r$, and the second
row colors the target log-scaling $\log s$. The left panel uses uniform outside
coefficients; the right panel uses spatially varying outside coefficients and
directional losses. The updates are Sinkhorn-like row and column clearings, but
$\eta_{ij}\neq1$ breaks the cross-partial symmetry required by a convex
potential.*
:::
