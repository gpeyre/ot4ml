...
**Initialize:** Set
\(K_{ij}=e^{-\C_{ij}/\epsilon}\a_i\b_j, \qquad \rho=\frac{\tau}{\tau+\epsilon}, \qquad v^{(0)}\in\RR_{+,*}^m.\)
...
>
> **Project and sort:**
> \(s_i^\ell=\dotp{\theta_\ell}{x_i}, \qquad t_i^\ell=\dotp{\theta_\ell}{y_i}, \qquad s_{(1)}^\ell\leq\cdots\leq s_{(n)}^\ell, \quad t_{(1)}^\ell\leq\cdots\leq t_{(n)}^\ell .\)
>
> **Compute**
