...
H_\beta(\theta)
\eqdef
\mathcal E_\beta\big((f_\theta)_\sharp\alpha\big)
=
\mathcal D\big((f_\theta)_\sharp\alpha,\beta\big).
```
...
```{math}
\alpha_t\eqdef(f_{\theta_t})_\sharp\alpha,
\qquad
X_t=f_{\theta_t}(Z),
\qquad
Z\sim\alpha .
...
Let $\eta_{t,x}$ be the disintegration of the latent law $\alpha$ with respect
to the map $f_{\theta_t}$, so that $\eta_{t,x}$ is supported on the fiber
$\{z:f_{\theta_t}(z)=x\}$ and
...
where $v_t$ agrees with $-\nabla\delta_\alpha\mathcal E_\beta(\alpha_t)$, or
with its $L^2(\alpha_t)$ projection on the tangent cone generated by the model
class.

In machine learning, however, $\beta$ is accessed through minibatches. If

```{math}
\hat\beta_n=\frac1n\sum_{i=1}^n\delta_{Y_i},
\qquad
