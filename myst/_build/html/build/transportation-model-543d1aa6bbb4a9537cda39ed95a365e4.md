...
objective appears in the Gaussian covariance catalogue of
...
particle trajectories colored from red to blue. The six panels on the right
...
```{math}
\mathcal R_t^{\mathrm{drift}}(\alpha|\alpha_t)
=
\epsilon
\int
\log
\frac{\int K_\epsilon(x,y)\,\d\alpha_t(y)}
     {\int K_\epsilon(x,y)\,\d\beta(y)}
\,\d\alpha(x)
+\mathrm{constant}.
```

It is "semi-relaxed" because the current model $\alpha_t$ is used to build the potential, but it is not varied inside the denominator when computing the first variation in $\alpha$.
:::
...
A general regressed field $b_t$ is not necessarily a Wasserstein gradient, since Wasserstein tangent vectors are represented by gradient fields modulo $L^2(\alpha_t)$-null directions. The gradient component is obtained by the weighted projection

```{math}
\nabla\phi_t
=
\uargmin{\nabla\phi}
\int \norm{\nabla\phi(x)-b_t(x)}^2\,\d\alpha_t(x).
```

One may first normalize $b_t$ pointwise, for instance by $b_t/(\norm{b_t}+\eta)$, or globally by $\norm{b_t}_{L^2(\alpha_t)}$, before this projection. Proposition {ref}`prop-drifting-semi-relaxed-gradient` then applies to the projected field. Non-gradient components can still be useful in a parametric model, but they are not descent directions of a scalar functional for the $\Wass_2$ Riemannian metric.
:::
...
:class: important
Let $\gamma=\Gaussian(0,\Id)$ and let $\alpha_t=\Gaussian(0,C_t)$ with $C_t\succ0$. For the normalizations displayed below, the Wasserstein descent constrained to the centered Gaussian manifold satisfies $\dot C_t=h(C_t)$, with $$\begin{array}{rcl}
        \KL(\al|\gamma) &:& h(C)=2(\Id-C), \\[.35em]
        \frac12\,\mathcal I(\al|\gamma) &:& h(C)=2(C^{-1}-C), \\[.35em]
        \Wass_2^2(\al,\gamma) &:& h(C)=4(C^{1/2}-C), \\[.35em]
        \MMD_k^2(\al,\gamma),\quad k(x,y)=\dotp{x}{y}^2
            &:& h(C)=8(C-C^2), \\[.35em]
        S_\epsilon(\al,\gamma)
            &:& h(C)=
    ...
    \end{array}$$ where $S_\epsilon$ is the debiased Sinkhorn divergence for the quadratic cost $\norm{x-y}^2$ and KL regularization strength $\epsilon$, and $$V(C)=
        ...
        \theta\theta^\top \,\d\sigma(\theta)$$ for the normalized spherical measure $\sigma$. Here $$\mathcal I(\al|\gamma)=\int \left|\nabla\log\rho(x)+x\right|^2\rho(x)\,\d x
        \qquad(\al=\rho\,\d x).$$ Thus the unhalved Fisher divergence has right-hand side $4(C^{-1}-C)$. Multiplying any of these energies by a constant simply rescales the corresponding right-hand side.
:::
...
For $\Wass_2^2(\cdot,\gamma)$, the Brenier map from $\Gaussian(0,C)$ to $\gamma$ is $C^{-1/2}x$, so the descent velocity for the unhalved squared distance is $2(C^{-1/2}-\Id)x$, giving $4(C^{1/2}-C)$. For the polynomial MMD row, centered Gaussians satisfy $\MMD_k^2(\al,\gamma)=\norm{C-\Id}_{\mathrm{F}}^2$; the first variation is quadratic and its descent velocity is $4(\Id-C)x$, giving $8(C-C^2)$.
...
Let the target be $\beta=\Gaussian(\bar\mean,\bar\cov)$ and assume $\alpha_t=\Gaussian(\mean_t,\cov_t)$. For the Gaussian kernel
...
```{math}
B_\epsilon[\alpha_t](x)
=
-\epsilon(\cov_t+\epsilon\Id)^{-1}(x-\mean_t).
```

Indeed the smoothed density $x\mapsto\int K_\epsilon(x,y)\d\alpha_t(y)$ is proportional to the Gaussian density with mean $\mean_t$ and covariance $\cov_t+\epsilon\Id$. Thus $B_\epsilon[\alpha_t]$ is the mean-shift vector of a Gaussian density: it points linearly toward the Gaussian mode, with strength set by the bandwidth. The drifting velocity {eq}`eq-cross-minus-self-drift` is therefore the difference of two affine mean-shift fields; it is affine and preserves Gaussianity. With
