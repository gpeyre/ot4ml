...
but the velocity $v_\alpha$ is now the PMO direction for the geometry induced
by $d$, not necessarily the classical $\Wass_2$ direction. Unbalanced variants
...
section only uses the local cost in the PMO. For a local velocity action
...
keeping a continuity equation. For a smooth density $\rho_t$ and an energy $F$,
the formal gradient flow associated with the pointwise momentum action
$J_\theta(a,m)=|m|^2/\theta(a)$ is
