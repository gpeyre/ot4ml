...
For $p=2$, Bobkov and Ledoux gave a complementary representation involving only
...
This identity is Theorem 2.11 of Bobkov and Ledoux
...
This cumulative formula has recently been used to design data-parallel
estimators of sliced Wasserstein distances {cite:p}`VauthierMerigotKorba2026CDFSW`.
For each projection direction $\theta$, sliced OT requires the one-dimensional
cost between $(P_\theta)_\sharp\al$ and $(P_\theta)_\sharp\be$. Rather than
collecting all projected samples in order to invert a global empirical CDF, one
can estimate projected CDF values on a grid, aggregate those values across
workers, and apply {eq}`eq-bobkov-ledoux-cdf-w2-web` before averaging over
directions. This is especially natural for mixture models and federated
settings, where CDF summaries are easier to communicate than full projected
samples.
...
the term $\big(\sum_{e\in[x,y]}\ell_e\big)^p$ couples the edges along a path, so
the simple sum of absolute subtree imbalances no longer gives the optimal
value, although the tree structure can still be exploited algorithmically in
discrete problems.
