...
recovers $v_\alpha=-\Wgrad f(\alpha)$.
...
Let $\alpha=\rho\Leb$ and let $u=\nabla(\delta F/\delta\rho)$. For the velocity
action $\mathbb A_{\theta,\Leb}$, the PMO introduced in
...
\operatorname{PMO}_{\mathbb A_{\theta,\Leb},\rho\Leb}(u)
=
-\frac{\theta(\rho)}{\rho}u.
```
