...
the Monge constraint may be empty and splitting couplings are essential.(alg:north-west-corner)=
...
Apply Algorithm {ref}`alg:north-west-corner` to the sorted weights $(\a_i)_i$ and $(\b_j)_j$. Return the resulting monotone coupling, i.e. the plan that always sends the leftmost remaining source mass to the leftmost remaining target mass.
:::

:::{admonition} Example: Unbounded convex sets may have no extreme point
:class: ot4ml-example

Compactness cannot be dropped from the corresponding proposition. For instance, the closed convex set $\enscond{(x,y)\in\RR_+^2}{xy\geq1}$ is unbounded and has no extreme point.
:::
...
coupling gives the same value.(alg:plan-displacement-interpolation)=
...
:::
(alg:wow-gaussian-mixture-interpolation)=
