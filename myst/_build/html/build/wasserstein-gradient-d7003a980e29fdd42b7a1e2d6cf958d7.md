...
selected by $d$. For $\Wass_p$, for instance,
...
only the direction and is the object used by normalized algorithms. Along
smooth solutions, the same $2$-homogeneity gives
...
geometries and nonlocal logarithmic-mean geometries are also local actions, but
in edge-flux or jump-flux variables rather than in vector fields on $\RR^d$.
Thus $q_\alpha$ is the common notation, while $Q_\alpha$ is available only in
Hilbertian subcases.
...
$q_{\gamma,\alpha}(v)=\mathcal N_{\gamma,\alpha}(v)^2$. Its descent ray is the LMO
{eq}`eq-local-action-lmo`: after rescaling, it minimizes
