...
the local quadratic, or restricted Riemannian, case. Assume first that nearby
measures are generated by maps and that the squared discrepancy has a
second-order expansion along smooth vector fields. Define
...
on the admissible velocity space by a positive self-adjoint operator
$Q_\alpha$,
...
$v_\alpha=-\Wgrad f(\alpha)$. Concave-mobility quadratic actions also fit this
template: if $\alpha=\rho\d x$ and $A_\theta(\rho,m)=|m|^2/\theta(\rho)$, then
the tangent norm of the advective velocity $v=m/\rho$ is

```{math}
\int \frac{\rho(x)}{\theta(\rho(x))}\norm{v(x)}^2\,\d\alpha(x),
```

so $Q_\alpha$ is multiplication by $\rho/\theta(\rho)$. WFR fits a closely
related product Riemannian structure, but with tangent variables $(v,g)$ rather
than only $v$, and block metric
$\int(\norm{v}^2+\kappa^2g^2)\d\alpha$. Discrete Markov-chain and nonlocal
logarithmic-mean geometries are also quadratic after replacing vector fields by
edge or jump fluxes. By contrast, Wasserstein-$p$ actions with $p\neq2$, or
more general one-homogeneous and Finsler-type gauges, do not admit a linear
operator $Q_\alpha$ of this form; their steepest-descent law involves a
nonlinear duality map instead of the preconditioner $Q_\alpha^{-1}$.
...
energy functional can be the same, while the tangent norm, mobility, reaction
term, graph structure, spectral normalization or jump kernel changes the PDE
selected by the minimizing movement. This section mirrors the dynamic-distance
taxonomy: each subsection takes
