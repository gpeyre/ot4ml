...
<span id="prop-basic-geodesic-convexity"></span>
...
<span id="prop-gaussian-log-sobolev-ot"></span>
...
$k(x,y)=-\norm{x-y}$, so that $f(\mu)=\MMD_k^2(\mu,\beta)$ is the squared
energy distance between $\mu$ and a target $\beta$. The first-order flow is
compared with the undamped Newton lift $\ddot x=v_{\mu_t}$, initialized with
zero velocity.
...
*First-order and inertial Wasserstein particle flows for the squared MMD
energy with energy-distance kernel. The transported measure starts as a single
Gaussian on the left and moves toward a two-Gaussian target mixture. The
Newton row uses the same Wasserstein force as an acceleration, which produces
overshoot and oscillatory collective motion rather than overdamped relaxation.*
:::
...
top row solves the confined entropy Wasserstein gradient flow. The two lower
rows solve the undamped Newton lift on a grid in $(x,s)$: its spatial marginal
is shown in color and the full phase-space density below in grayscale.*
:::
...
steepest-descent direction by a norm-constrained linear minimization problem
{cite:p}`PethickXieAntonakopoulosEtAl2025`; this is useful for normalized SGD
{cite:p}`MurraySwensonKar2019,CutkoskyMehta2020`, tensor-aware optimizers such
as Shampoo {cite:p}`GuptaKorenSinger2018`, and Muon-type spectral
...
monotone spectral gauges of {ref}`sec-spectral-subspace-wasserstein`: the
