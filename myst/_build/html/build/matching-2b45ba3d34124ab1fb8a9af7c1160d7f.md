...
Choose $s^\star\in\argmin_s E_s$ and return $x_{(k)}\mapsto y_{(k+s^\star)}$. Equivalently, open the circle at a gap compatible with $s^\star$, lift all points to the unfolded interval, and apply Algorithm {ref}`alg:one-dimensional-sorting`.
:::
