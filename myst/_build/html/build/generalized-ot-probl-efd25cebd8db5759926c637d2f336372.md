...
$\alpha$. For Gaussian measures with the same mean, this reduces to the
Loewner order on covariance matrices:
...
discrete profile in the displacement variable, so the coupling
$P_{ij}=a_iK_i(y_j)$ satisfies $\sum_j y_jK_i(y_j)=x_i$ and hence has
barycentric projection $\bar T_P(x_i)=x_i$. The blue marginal
