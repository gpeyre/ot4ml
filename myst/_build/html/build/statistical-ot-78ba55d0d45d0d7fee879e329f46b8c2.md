...
Gaussian scale mixtures. This is the Schoenberg--Bernstein mechanism behind
the positive definiteness of radial stable kernels
...
Set $a=p/2\in(0,1]$. There exists a probability measure
$\nu_{a,\epsilon}$ on $[0,+\infty)$ such that
...
:::{dropdown} Proof
The function $s\mapsto e^{-s^a/\epsilon}$ is completely monotone for
$0<a\leq1$. Bernstein's theorem therefore gives the displayed Laplace-transform
representation by a probability measure $\nu_{a,\epsilon}$. Applying this
...
precisely in this range. For $p>2$, the generalized Gaussian kernel is not
positive definite in general, so one should not expect a positive-feature
factorization of the above type. Constants in the cost, such as using
...
$\epsilon$. Proposition {ref}`prop-gaussian-positive-features` gives a positive
