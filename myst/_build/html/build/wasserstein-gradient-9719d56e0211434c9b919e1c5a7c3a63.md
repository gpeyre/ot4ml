...
Linear spectral gauges $\gamma(M)=\operatorname{tr}(AM)$, with $A\succeq0$,
again give a quadratic tangent form. By contrast, Wasserstein-$p$ actions with
$p\neq2$, nonlinear spectral gauges such as $\gamma(M)=\lambda_{\max}(M)$,
and more general non-Hilbertian gauges do not admit a single linear operator
$Q_\alpha$ of this form; their steepest-descent law involves a nonlinear
duality map, or a subgradient of the tangent norm, instead of the preconditioner
$Q_\alpha^{-1}$.
