...
| $\Pp_{2,\mathbf m}(\RR^d;\RR_+^p)$ | Positive multi-species measures with fixed component masses. | [Eq. eq-multispecies-space](wasserstein-gradient-flows#eq-multispecies-space) |
| $\Wass_{2,\oplus}$ | Product Wasserstein distance for independent species transport. | [Eq. eq-multispecies-product-metric](wasserstein-gradient-flows#eq-multispecies-product-metric) |
