...
:::{dropdown} Proof
Fix a coupling $\pi$ of two admissible laws $\mu$ and $\nu$, and write
...
is positive semidefinite. For positive definite $\Sigma$ and $\Lambda$, this
implies $K=\Sigma^{1/2}R\Lambda^{1/2}$ for a contraction $R$, and hence

```{math}
\tr(K)
\leq
\sup_{\norm{R}\leq1}\tr(\Sigma^{1/2}R\Lambda^{1/2})
=
...
The same bound for singular matrices follows by regularization with
$\eta I$ and by letting $\eta\downarrow0$. Hence every admissible coupling has
cost at least $\Bb(\Sigma,\Lambda)^2$. Conversely, the centered Gaussian laws
$\Gaussian(0,\Sigma)$ and $\Gaussian(0,\Lambda)$ satisfy the prescribed
