...
For a fixed matrix $A$, the corresponding forward map is the selection of an
optimal coupling
...
Although $A\mapsto C(A)$ is linear, the optimizer $A\mapsto P_A$ is
polyhedral and piecewise constant: changing $A$ changes the direction in which
the transport polytope is probed. The figure below illustrates this map on the
OT4ML point clouds. The construction follows the visual idea of the Python
Optimal Transport logo {cite:p}`flamary2021pot`: red source atoms, blue target
atoms and straight segments show the selected optimal bijection. The rank-one
matrices $A=-e_1e_1^\top$ and $A=-e_2e_2^\top$ only score horizontal or
vertical correlations. The matrix $A=-I$ gives the usual quadratic $\Wass_2$
assignment, up to the marginal-only terms discussed below, while $A=+I$
reverses the correlation and produces an anti-$\Wass_2$ matching.
...
*Forward map $A\mapsto P_A$ for the bilinear cost
$c_A(x,y)=\dotp{Ax}{y}$ on the OT4ML logo point clouds. Each panel solves the
equal-weight assignment problem with a different matrix $A$; the source atoms
are red, the target atoms are blue, and the gray segments give the selected
optimal bijection.*
:::
...
generated it. The next figure generates an observed bijection $\widehat P$
from this cost on two empirical mixtures of Gaussians, and then evaluates
...
so that $t=0$ recovers the matrix that generated the observed matching.
...
optimizer as the quadratic $\Wass_2$ cost; the faint ellipses indicate the
sampling components. Panels (b,c): the unregularized Fenchel--Young Kantorovich gap
$\mathcal L_{\mathrm{iOT}}(C(A_t);\widehat P)$ along
$A_t=-\diag(1+t,1-t)$ for $n=20$ and $n=100$, using the same vertical scale.
