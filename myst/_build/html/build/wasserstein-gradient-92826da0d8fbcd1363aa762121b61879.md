...
$|\dot\alpha_t|_{\Wass_2}$ of an absolutely continuous curve. In the smooth
formal setting, a Wasserstein gradient flow with velocity
$v_t=-\Wgrad f(\alpha_t)$ satisfies the energy-dissipation identity
...
for $0\leq s<t$. In the abstract metric theory, the same formula is expressed
as an energy-dissipation identity for curves of maximal slope, as developed in
the metric-gradient-flow framework of Ambrosio--Gigli--Savaré and in the
optimal-transport presentation of Santambrogio
{cite:p}`ambrosio2006gradient,SantambrogioBook`; if one uses only the weaker
inequality convention, the estimate below remains true up to the inessential
constant $\sqrt2$. In the smooth setting considered here, Cauchy's inequality
yields
...
This estimate is weaker than a convergence rate: it does not identify a
minimizer and it gives no decay law. It does, however, rule out escape to
infinity in finite metric time whenever the energy is bounded below on finite
time intervals. Indeed the trajectory segment remains in the $\Wass_2$-ball
centered at $\alpha_s$ with radius given by
