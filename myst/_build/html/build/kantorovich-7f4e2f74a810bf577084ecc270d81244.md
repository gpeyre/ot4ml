...
Compactness cannot be dropped from Proposition labeled `prop-extreme-point-existence`. For instance, the closed convex set $\enscond{(x,y)\in\RR_+^2}{xy\geq1}$ is unbounded and has no extreme point.
:::
...
The Memoli profile lower bound in Proposition labeled `prop-memoli-gw-profile-lower-bound` is precisely a Wasserstein-over-Wasserstein comparison of these laws of local profiles. It replaces the full pairwise distortion by an ordinary OT problem whose ground cost is itself a one-dimensional Wasserstein distance.

Note that there exist alternative distances which also metricize weak convergence. The simplest ones are Hilbertian kernel norms, which are detailed in Section labeled `sec-dual-norms`.
:::
