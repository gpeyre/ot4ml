...
$h_\alpha(\lambda x)=\lambda^2h_\alpha(x)$ for $\lambda>0$. In particular,
stationarity forces $h_\alpha$ to vanish on
...
$h_\alpha(x)=r^2h_\alpha(\omega)=0$. Consequently
...
$\int h_\alpha\,\d\beta<0$. Hence there exists a nonzero point $x=r\omega$
with $r>0$, $\|\omega\|=1$, and $h_\alpha(x)<0$. Equivalently
...
same radial-derivative contradiction then rules out non-optimal stationary
limits.
:::
