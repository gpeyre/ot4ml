...
The relation with the usual Wasserstein gradient can be read formally from the
tangent metric. Suppose that, for smooth vector fields $w$ and for $t\to0$,
...
where $Q_\alpha$ is a positive self-adjoint operator on the admissible tangent
velocity space. If $f$ has Wasserstein first variation $\Wgrad f(\alpha_t)$,
then expanding {eq}`eq-generalized-jko-step` with
...
$v_\alpha=-\Wgrad f(\alpha)$. The examples below can be read as different ways
of changing this infinitesimal metric tensor: modifying the mobility, adding
reaction terms, replacing the state space by a graph, normalizing directions
spectrally, or allowing nonlocal jumps.
