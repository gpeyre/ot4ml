...
{eq}`eq-dynamic-unbalanced-ot` and used for flows below in
{ref}`def-wfr-jko-gradient-flow`, has an analogous product Riemannian
...
Finsler norm, mobility, reaction term, graph structure, spectral gauge or jump
kernel changes the PDE selected by the minimizing movement. This section
mirrors the dynamic-distance taxonomy: each subsection takes
...
This is a slight departure from the purely advective picture used in the
previous paragraph: the tangent variable is no longer only a vector field $v$
transporting the measure, but a pair $(v,g)$ where the scalar field $g$ locally
modulates the amount of mass. The dynamic unbalanced distance introduced in
{eq}`eq-dynamic-unbalanced-ot` gives exactly this geometry.
