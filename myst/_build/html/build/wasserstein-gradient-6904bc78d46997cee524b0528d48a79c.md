...
where $\Omega\subset\RR^d$ is convex. The indicator term is not represented
by an ordinary first variation; it contributes a normal-cone term. The JKO
step becomes the constrained minimization
...
:::{dropdown} Proof
For smooth positive densities, let $\alpha_i=\rho_i\d x$ with
...
so $\rho_t\leq\kappa$. The general case follows by approximation, stability
of optimal maps, and weak-star closedness of the constraint $\rho\leq\kappa$
in $L^\infty$.
:::

Formally, the constrained flow contains a pressure field $p_t$, the Lagrange
multiplier of the density cap. With no-flux boundary conditions this gives
...
$\rho_t=\kappa$. For nonsmooth solutions these conditions are read in the
variational sense associated with the normal cone to $\mathcal K_\kappa$.
Thus even when the driving potential $\int h\d\alpha$ is linear, the
constraint couples distant particles through the pressure field.
