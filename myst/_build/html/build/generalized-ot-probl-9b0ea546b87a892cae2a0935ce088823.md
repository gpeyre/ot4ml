...
9. Update $R_3=Z\oslash P$ on entries where $P>0$.
10. Set
...
For the empirical self-coupling in {numref}`fig:capacity-constrained-ot-2d`, it
is more informative to express the cap on the conditional kernel $P_{ij}/a_i$
rather than directly on the mass $P_{ij}$. Indeed, with uniform weights
$a_i=1/n$, a literal bound of order $k/n$ on $P_{ij}$ would be inactive for
$k\geq1$, whereas the conditional bound $P_{ij}/a_i\leq k/n$ enforces a genuine
local degree constraint.
...
plan. The three panels use the conditional caps $P_{ij}/a_i\leq 2/n$, $3/n$ and
$5/n$, equivalently $P_{ij}\leq k/n^2$ because $a_i=1/n$.*
:::
