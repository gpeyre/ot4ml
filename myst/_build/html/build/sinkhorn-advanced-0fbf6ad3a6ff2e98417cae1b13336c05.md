...
$(x,\alpha,f)$ and $(y,\beta,g)$. If $\Phi$ is differentiable and
$\rho_\phi=\Phi'$, the scalar minimizers satisfy
...
domains. In the nonsmooth case, $\rho_\phi$ is replaced by a measurable
selection in $\partial\Phi$.

(prop-phi-double-soft-transform-monotone)=
:::{admonition} Proposition: Monotonicity of Generalized Double Soft Transforms
:class: important
Assume that the scalar block updates defining $g^{\bar c,\epsilon,\phi}$ and
$f^{c,\epsilon,\phi}$ are single-valued. Then each generalized soft
$c$-transform is order reversing and additively anti-homogeneous:
...
$\Phi$ is convex and nondecreasing. In the differentiable case,
$\rho_\phi=\Phi'$ is nondecreasing. For fixed $x$, define

```{math}
M_g(x,u)
\eqdef
\int_{\Yy}
\rho_\phi\!\left(\frac{u+g(y)-c(x,y)}{\epsilon}\right)
\d\beta(y).
```

The scalar update $u=g^{\bar c,\epsilon,\phi}(x)$ is characterized by
$M_g(x,u)=1$. If $g\leq g'$, then $M_g(x,u)\leq M_{g'}(x,u)$ for every $u$.
Since $u\mapsto M_g(x,u)$ is also nondecreasing, the unique solution of
$M_{g'}(x,u')=1$ satisfies $u'\leq u$. Thus
$g\mapsto g^{\bar c,\epsilon,\phi}$ is order reversing. Moreover,

```{math}
M_{g+\lambda}(x,u)=M_g(x,u+\lambda),
```

so the solution is shifted from $u$ to $u-\lambda$. The proof for
$f\mapsto f^{c,\epsilon,\phi}$ is identical. Composing the two
order-reversing anti-homogeneous maps gives an order-preserving additively
homogeneous map. For nonsmooth $\Phi$, use the monotonicity of
$\partial\Phi$, or smooth $\Phi$ and pass to the limit along monotone
selections of the scalar optimality condition.
:::
