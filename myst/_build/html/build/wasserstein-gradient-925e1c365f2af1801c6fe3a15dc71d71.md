...
:::{admonition} Remark: Two interpretations of the heat equation
:class: ot4ml-remark
The same heat equation can be generated by two different gradient-flow structures. Write $\alpha=\rho\d x$ and impose, for instance, periodic or no-flux boundary conditions. In the Hilbert geometry of densities, with
...
the first variation is $\delta\mathcal D/\delta\rho=-\Delta\rho$, and the $L^2$ gradient flow $\partial_t\rho=-\delta\mathcal D/\delta\rho$ is $\partial_t\rho=\Delta\rho$. This interpretation says that the evolution decreases the Dirichlet energy and regularizes the density. In the Wasserstein geometry, the same equation is instead the $\Wass_2$ gradient flow of the Shannon entropy $\int \rho\log\rho\d x$. Thus the observed vector field, or PDE, does not by itself determine a unique explanatory pair ``energy plus metric'': the same dynamics may arise from distinct choices of $(f,d)$.
:::
