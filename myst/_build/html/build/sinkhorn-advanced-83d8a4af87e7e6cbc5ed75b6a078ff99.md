...
gap whose constants depend on the cost range and potential oscillation.(alg:cyclic-bregman-projections)=
...
remaining dual gap, up to the bounded-radius constant.(alg:certified-entropic-ot-accuracy)=
...
Corollary {ref}`cor-sinkhorn-dual-complexity` then gives
...
approaches a deterministic Brenier map.(alg:gaussian-sinkhorn-closed-form)=
...
The entropic value is then obtained by inserting the same singular values into Proposition {ref}`prop-gaussian-sinkhorn-closed-form`.
:::
