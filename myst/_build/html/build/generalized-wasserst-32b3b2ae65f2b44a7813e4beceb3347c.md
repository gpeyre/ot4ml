...
this by measuring transport after optimizing over a group action. This
viewpoint connects OT with shape spaces, metamorphosis models and
...
write $g_\sharp\alpha$ for the push-forward of a measure $\alpha$ by
$g\in\mathcal G$. The quotient $p$-Wasserstein distance between the orbits of
...
If $\mathcal G$ acts by isometries, then $\Wass_{p,\mathcal G}$ is a
pseudo-distance on $\Pp_p(\Xx)$ and a distance on the quotient by orbit
closures. If the orbits are closed, for instance in the usual compact-group
settings, it is a distance on the orbit space $\Pp_p(\Xx)/\mathcal G$.
:::
...
$g_1,g_2$, gives the result. Zero distance means that the two orbit closures
meet; if the orbits are closed, this is equivalent to equality of orbits.
:::

### Rigid Motions and Wasserstein-Procrustes

The most common example is the Euclidean group. For measures on $\RR^d$,
...
correspondences by a mass-preserving OT plan, which better accommodates
non-uniform sampling and ambiguous correspondences; this idea also appears in
Wasserstein Procrustes alignment of embeddings {cite:p}`grave2018unsupervised`.
...
rank deficient. If one imposes $R\in\mathrm{SO}(d)$, replace the last singular
direction by the standard determinant correction.
:::
...
$R=UV^\top$; the special-orthogonal constraint is handled by flipping the last
singular vector when $\det(UV^\top)<0$.
:::
...
**Input:** Weighted point clouds $(x_i,\a_i)_{i=1}^n$, $(y_j,\b_j)_{j=1}^m$, initial rigid motion $(R^{(0)},t^{(0)})$, tolerance $\mathrm{tol}$.

**Output:** Coupling $\P$ and rigid motion $(R,t)$.

**Set** $k=0$ and $\eta_0=+\infty$.
...
>
> **Set** $R^{(k+1)}=UV^\top$ and $t^{(k+1)}=\bar y-R^{(k+1)}\bar x$.
>
...
*Wasserstein-Procrustes alignment of two bunny silhouettes. The target
silhouette is shown in black. The moving source silhouette is sampled by
farthest-point sampling and colored from red to blue along the first
iterations. Each step solves an equal-weight OT assignment, then updates the
rigid motion by the closed-form Procrustes formula. The displayed motion is
damped only to make the early iterations visible; the underlying update is the
block-coordinate method above.*
:::
