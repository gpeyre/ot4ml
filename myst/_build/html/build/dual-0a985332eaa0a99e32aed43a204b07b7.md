...
**Output:** Assignment of each source to one target.

**Initialize:** Set prices $p_j=0$; mark all sources and targets as unassigned.
...
**If** target $j_i$ has owner $i'$, mark $i'$ as unassigned.

**Assign** target $j_i$ to source $i$.

**End while**; **return** the final assignment.
:::
...
**Set** the source closure:

```{math}
\tilde f=g^{\bar c}=f^{c\bar c}.
```

**Set** the closed target potential:
