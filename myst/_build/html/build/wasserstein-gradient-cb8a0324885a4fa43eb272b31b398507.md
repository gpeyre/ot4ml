...
as an energy-dissipation identity for curves of maximal slope; if one uses only
the weaker inequality convention, the estimate below remains true up to the
inessential constant $\sqrt2$. In the smooth setting considered here, Cauchy's
inequality yields
...
The Poincaré inequality plays the same role after linearizing the Wasserstein
geometry at equilibrium. Let $\beta=Z^{-1}e^{-V}\d x$, write
$\alpha=h\beta$, and let $L=\Delta-\dotp{\nabla V}{\nabla}$ be the reversible
generator. The logarithmic Sobolev inequality linearizes around $h=1$: if
$h_\epsilon=1+\epsilon g$ with $\int g\d\beta=0$, then
...
Thus log-Sobolev implies the Poincaré, or spectral-gap, inequality
...
This is not quite the usual $\Wass_2$-PL inequality for the chi-square energy
...
The exact PL interpretation is obtained by freezing the mobility at
equilibrium, i.e. by using the $H^{-1}(\beta)$ metric, the linearization of
$\Wass_2$ at $\beta$.

(prop-poincare-linearized-wasserstein-pl)=
:::{admonition} Proposition: Poincaré as Linearized Wasserstein-PL
:class: important
For a mean-zero perturbation $u$, define
...
Equivalently, the associated dynamic metric is obtained from
...
For $E_2(h)=\frac12\int(h-1)^2\d\beta$, its squared slope in this linearized
...
The squared dual norm of this differential is therefore
$\int\norm{\nabla h}^2\d\beta$, which proves the slope identity. The Poincaré
inequality is precisely $2\lambda E_2\leq|\partial E_2|_{-1,\beta}^2$. The
gradient field is $-Lh$, hence the gradient flow is $\partial_t h_t=Lh_t$.
Along this flow,
