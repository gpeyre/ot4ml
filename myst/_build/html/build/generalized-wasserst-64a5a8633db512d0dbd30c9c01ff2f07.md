...
Slicing can also be understood as applying a Radon transform before measuring
discrepancy. For a probability measure $\alpha$ on $\RR^d$, define its
measure-valued Radon transform by

```{math}
\mathfrak R\alpha(\theta)\eqdef (P_\theta)_\sharp\alpha,
\qquad \theta\in\mathbb S^{d-1}.
```

Thus $\mathfrak R\alpha$ is a collection of one-dimensional projected measures
indexed by directions. If $\alpha=\rho(x)\d x$ has a density, then
$\mathfrak R\alpha(\theta)$ has density given by the classical Radon transform
...
With this notation, {eq}`eq-sliced-wasserstein` is the pull-back, through
$\mathfrak R$, of the $L^p(\mathbb S^{d-1};\Wass_p)$ distance on Radon data.
The full all-direction transform is injective on probability measures by the
Cramer--Wold theorem, which is why sliced Wasserstein separates measures. The
warning is numerical and variational: finitely sampled Radon data are not
injective, and independently constructed one-dimensional Radon data need not
satisfy the range conditions of an actual image. This is precisely the issue
behind sliced and Radon barycenter reconstructions discussed in Section
{ref}`sec-barycenters`.
:::
...
Conversely, if $d\geq2$ and $\alpha,\beta\in\mathcal P_1(\RR^d)$ are
supported in $B_R=\{x\in\RR^d:\norm{x}\leq R\}$, then there is a constant
$C_d$ depending only on $d$ such that

```{math}
\Wass_1(\alpha,\beta)
\leq
C_d R^{\frac{d}{d+1}}\operatorname{SW}_1(\alpha,\beta)^{\frac{1}{d+1}}.
```
...
\Wass_1(\alpha,\beta)
\leq
c_d R^{\frac{d-1}{d}}\operatorname{SW}_1(\alpha,\beta)^{\frac1d}.
```
:::
...
perturbations. A small gradient displacement is already optimal for the
quadratic cost, and slicing simply tests its one-dimensional projections.

(prop-sliced-first-order-tangent)=
:::{admonition} Proposition: First-Order Comparison Along Brenier Perturbations
:class: theorem
...
(rem-sliced-length-not-wasserstein)=
:::{admonition} Remark: Sliced length and geodesic completion
:class: ot4ml-remark

There are two geometries associated with $\SW_2$. The first is the chordal
metric obtained by comparing all one-dimensional projections. The second is the
intrinsic length metric generated by this chordal metric. If
$(\alpha_t)_{t\in[0,1]}$ is continuous in $(\Pp_2(\RR^d),\SW_2)$, set

```{math}
\mathsf L_{\SW_2}((\alpha_t)_{t\in[0,1]})
=
\sup_{0=t_0<\cdots<t_N=1}
\sum_{i=0}^{N-1}\SW_2(\alpha_{t_i},\alpha_{t_{i+1}})
```

and define $\SW_2^{\mathrm{len}}(\alpha,\beta)$ as the infimum of this length
over all continuous curves joining $\alpha$ to $\beta$. This length distance is
always larger than $\SW_2$, but it should not be confused with $\Wass_2$.
Park and Slepčev study this sliced length geometry and show that sliced
Wasserstein space is not generally a length space {cite:p}`ParkSlepcev2023SlicedGeometry`.

The preceding estimate is only an upper comparison of tangent speeds. Along
$\alpha_t=(\Id+t\nabla\varphi)_\sharp\alpha$, the $\Wass_2$ metric derivative
is the full norm $\|\nabla\varphi\|_{L^2(\alpha)}$. The sliced derivative first
projects and then minimizes on each one-dimensional slice. Under standard
smoothness assumptions, with $\alpha_\theta=(P_\theta)_\sharp\alpha$,
...
\abs{
\EE\left[\dotp{\theta}{\nabla\varphi(X)}
\,\middle|\,\dotp{\theta}{X}=s\right]}^2
\d\alpha_\theta(s)\d\sigma(\theta),
\qquad X\sim\alpha .
```

Jensen's inequality gives the bound in Proposition
{ref}`prop-sliced-first-order-tangent`, and the inequality is generally strict.
Thus slicing preserves enough projected information to metrize the same topology
as $\Wass_2$, but it does not preserve the same Riemannian norm.
:::

(prop-sliced-length-translation)=
:::{admonition} Proposition: Translations separate sliced length from $\Wass_2$
:class: theorem

Let $\tau_h(x)=x+h$. For any $\alpha\in\Pp_2(\RR^d)$,

```{math}
\SW_2^{\mathrm{len}}\big(\alpha,(\tau_h)_\sharp\alpha\big)
=
\SW_2\big(\alpha,(\tau_h)_\sharp\alpha\big)
=
\frac{\norm{h}}{\sqrt d},
\qquad
...
Consequently, even the intrinsic length metric generated by $\SW_2$ is not the
usual $\Wass_2$ metric when $d>1$.
:::

:::{admonition} Proof
:class: proof

For each direction $\theta$, the projected measure
$(P_\theta)_\sharp(\tau_h)_\sharp\alpha$ is the translate of
$(P_\theta)_\sharp\alpha$ by the scalar $\dotp{\theta}{h}$. Hence the
one-dimensional squared distance is $\abs{\dotp{\theta}{h}}^2$, and integrating
over directions gives $\SW_2^2=\norm{h}^2/d$.

For the translation path $\alpha_s=(\tau_{sh})_\sharp\alpha$, the same
computation gives $\SW_2(\alpha_s,\alpha_t)=\abs{s-t}\norm{h}/\sqrt d$, so its
sliced length is $\norm{h}/\sqrt d$. The reverse inequality follows because
every length dominates the endpoint distance. Finally,
$\tau_h=\nabla(\frac12\norm{x}^2+\dotp{h}{x})$ is cyclically monotone, hence
optimal for the quadratic cost, so the $\Wass_2$ distance is $\norm{h}$.
:::

The proposition is the useful sanity check: the geodesic completion suggested
by slicing is a projected sliced geometry, not the usual displacement geometry.
Straight chords in the projected-quantile Hilbert embedding below need not
satisfy the consistency constraints required to come from actual measures on
$\RR^d$.
...
The compact reverse estimates in Proposition {ref}`prop-sliced-wasserstein-metric`
explain why sliced distances metrize the same topology on bounded sets, while
the dimension-dependent powers prevent a dimension-free bi-Lipschitz comparison
with $\Wass_p$.
:::
...
reference law changes the coordinates used to represent $\al$. Vector
...
Section {ref}`sec-wasserstein-flows-mlp`, the depth of a residual network. The
