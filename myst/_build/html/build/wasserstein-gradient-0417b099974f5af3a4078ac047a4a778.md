...
null directions. For ordinary $\Wass_2$, $Q_\alpha=\Id$ and one recovers
null directions. For ordinary $\Wass_2$, $Q_\alpha=\Id$ on $T_\alpha$ and one
recovers $v_\alpha=-\Wgrad f(\alpha)$. Concave-mobility quadratic actions also
fit this template: if $\alpha=\rho\d x$ and
...
admissible velocities. WFR has an analogous product Riemannian structure, but
its tangent variable is $(v,g)$ rather than only $v$; the block metric is
$\int(\norm{v}^2+\kappa^2g^2)\d\alpha$, so the growth component is weighted by
$\kappa^2$. Discrete Markov-chain and nonlocal logarithmic-mean geometries are
also quadratic, but in edge-flux or jump-flux variables rather than in vector
fields on $\RR^d$. Linear spectral gauges $\gamma(M)=\operatorname{tr}(AM)$,
with $A\succeq0$, again give a quadratic tangent form. By contrast,
...
law involves a nonlinear duality map, or a subgradient of the tangent norm,
instead of the preconditioner $Q_\alpha^{-1}$.
