...
$\epsilon\to0$, up to an additive constant. The same heat equation also linearizes the nonlinear PDEs. With the same temperature normalization as above, the change of variables $u=e^{-\phi/\epsilon}$ converts

```{math}
\partial_t\phi+\frac12\norm{\nabla\phi}^2=\frac{\epsilon}{2}\Delta\phi,
```

into $\partial_tu=(\epsilon/2)\Delta u$. Conversely, a positive heat solution gives $\phi=-\epsilon\log u$, and
$v=\nabla\phi=-\epsilon\nabla\log u$ solves the gradient viscous Burgers equation
$\partial_t v+(v\cdot\nabla)v=(\epsilon/2)\Delta v$. In one dimension this is the classical Cole--Hopf transform for Burgers; in higher dimension this scalar reduction applies to irrotational velocity fields.
