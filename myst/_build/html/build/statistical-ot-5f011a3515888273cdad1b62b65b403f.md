...
nonnegative factorization $K=BB^\top$. Thus total positivity in this sense is
precisely the kernel-level condition behind positive random features. For
...
Cuturi {cite:p}`ScetbonCuturi2020PositiveFeatures`.
...
rank-$40$ positive-feature sketch remains visually close to the dense plan,
whereas rank-$10$ and rank-$3$ sketches still enforce the prescribed marginals
after Sinkhorn scaling but progressively lose the geometric concentration of
the true coupling.
