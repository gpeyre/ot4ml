...
(prop-strassen-stochastic-orders)=
:::{admonition} Proposition: Strassen's Coupling Theorems
:class: important
Let $\alpha,\beta$ be probability measures.

If $\alpha,\beta\in\Pp(\RR)$, define the classical stochastic order by
...
If $\alpha,\beta\in\Pp_1(\RR^d)$, define the convex order by
...
Then $\alpha\preceq_{\mathrm{cx}}\beta$ if and only if
$\Couplings_{\mathrm{mart}}(\alpha,\beta)$ is nonempty.
:::

:::{dropdown} Proof
For the stochastic order, a coupling supported on $x\leq y$ immediately gives
the integral inequality for every increasing $\varphi$. Conversely, testing
approximate indicators of intervals $(t,+\infty)$ gives
...
For the convex order, any martingale coupling satisfies the inequality by
Jensen's inequality:
...
$\int\varphi\d\alpha\leq\int\varphi\d\beta$. The converse is the martingale
part of Strassen's theorem. In the compactly supported case, one proves it by
separating $\beta$ from the closed convex set of all second marginals reachable
from $\alpha$ by martingale kernels. A separating test function $\psi$ would
satisfy
...
Proposition {ref}`prop-strassen-stochastic-orders` is used here as a feasibility
test. The first part says that an order relation between laws can be realized
by a pathwise order constraint on a coupling. The second part says that the
martingale constraint is feasible exactly when $\beta$ is larger than $\alpha$
in convex order; otherwise the martingale OT value is simply $+\infty$. Testing
affine functions in the definition of $\preceq_{\mathrm{cx}}$ gives equality of
means, while convex test functions say that $\beta$ is more spread out than
$\alpha$.
