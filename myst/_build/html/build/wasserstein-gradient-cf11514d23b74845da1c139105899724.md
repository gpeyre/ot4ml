...
The metric ingredient needed below is the local action $q_\alpha$ introduced in
{ref}`sec-generalized-dynamic-wasserstein-distances`. It is the second-order
tangent cost in {eq}`eq-generalized-distance-tangent-form`: $\Wass_p$ gives the
Finsler action $q_\alpha(w)=(\int\norm{w}^p\d\alpha)^{2/p}$, spectral
Wasserstein distances give the square of a velocity-covariance gauge, and local
Riemannian geometries give the quadratic expansion
{eq}`eq-generalized-distance-tangent-expansion`. This section keeps the same
notation but uses it for steepest descent rather than for defining distances.

Given a first-order direction $g\in T_\alpha$, with $T_\alpha$ the admissible
tangent space used in {eq}`eq-generalized-distance-tangent-form`, and usually
$g=\Wgrad f(\alpha)$, the local steepest-descent velocity associated with the
action $q_\alpha$ is formally
...
{eq}`eq-generalized-distance-tangent-expansion`,
{eq}`eq-local-action-steepest-descent` is a linear preconditioning rule:
...
Several geometries below can now be read from the local actions defined in
{ref}`sec-generalized-dynamic-wasserstein-distances`.
...
$\RR^d$. Thus $q_\alpha$ is the common notation between the distance and flow
chapters, while $Q_\alpha$ is available only in Hilbertian vector-field
subcases.
