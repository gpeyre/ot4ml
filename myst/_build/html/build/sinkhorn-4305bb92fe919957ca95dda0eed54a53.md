...
The Hopf--Lax formula is the Hamilton--Jacobi counterpart of the $c$-transform.
...
the Hopf--Lax infimum for an initial datum $h$ is simply

```{math}
(-h)^{\bar c}(x)=\inf_y\left\{h(y)+\frac12\norm{x-y}^2\right\}.
```

The sign comes from the convention of {ref}`def-c-transform`. This is the
unit-time version of the classical Hopf--Lax semigroup; the time parameter can
be absorbed by rescaling the cost, while here the smoothing parameter is the
entropic temperature $\epsilon$ {cite:p}`evans2010pde,Villani09`.

The continuous soft $c$-transform of {ref}`def-continuous-soft-c-transform`
replaces this infimum by a log-sum-exp soft minimum. For Lebesgue measure on
$\RR^d$,
...
Laplace's principle gives $(-h)^{\bar c,\epsilon}\to(-h)^{\bar c}$ as
$\epsilon\to0$ under the usual coercivity assumptions. If
...
Thus the soft quadratic $c$-transform is a Gaussian convolution followed by a
logarithm, up to an explicit additive constant independent of $x$.

(prop-soft-legendre-convolution)=
:::{admonition} Proposition: Soft Quadratic $c$-Transform and Legendre Approximation
:class: important
Let $h:\RR^d\to\RR\cup\{+\infty\}$ be such that the integrals below are finite,
and set $\mathsf S h(y)=h(y)-\norm{y}^2/2$. For $\epsilon>0$, define

```{math}
:label: eq-soft-legendre-definition-web
h_\epsilon^*(p)
=\frac12\norm{p}^2-\big(-\mathsf S h\big)^{\bar c,\epsilon}(p).
```
...
\exp\!\left(\frac{\dotp{p}{y}-h(y)}{\epsilon}\right)\,dy,
```
...
h_\epsilon^*(p)
=\frac12\norm{p}^2
+\epsilon\log\big(G_\epsilon\ast e^{-\mathsf S h/\epsilon}\big)(p)
+\frac{\epsilon d}{2}\log(2\pi\epsilon).
```

If $y\mapsto\dotp{p}{y}-h(y)$ satisfies the standard tightness assumptions of
Laplace's principle, then $h_\epsilon^*(p)\to h^*(p)$ as $\epsilon\to0$.
:::

The proof is just completion of squares. Since

```{math}
\inf_y\left\{\mathsf S h(y)+\frac12\norm{p-y}^2\right\}
=\frac12\norm{p}^2-h^*(p),
```

replacing the hard infimum by the soft $c$-transform gives the definition of
$h_\epsilon^*$. Expanding the square yields the log-sum-exp formula, and the
Gaussian-convolution expression follows from the normalized kernel
$G_\epsilon$.

:::{admonition} Fast Soft Legendre--Fenchel Transforms
:class: tip
On a periodic, or sufficiently padded, grid, the term
$G_\epsilon\ast e^{-\mathsf S h/\epsilon}$ is a Gaussian convolution. It can
...
*Hopf--Cole numerics in one dimension: for three values of $\epsilon$, the upper curves show the viscous Hamilton--Jacobi potential $\phi_t$ and the lower curves show the Burgers velocity $v_t=\partial_x\phi_t$, with time colored from red to blue.*
:::
