...
continuum theory when the number of codepoints tends to infinity. In one
dimension, Caglioti, Golse and Iacobelli embed the ordered particle
...
developed in {cite:p}`caglioti2018quantization2d`. The Eulerian variable in
this limit is the density $f$ of codepoints, not the target density $\rho$. For
$p$-quantization in dimension $d$, set $r=p/d$. The asymptotic quantization
...
up to the universal cell-shape constant. Its $\mathcal W_2$-gradient flow is the
weighted ultrafast diffusion equation
...
with periodic or no-flux boundary conditions. Iacobelli, Patacchini and
Santambrogio use the JKO scheme and Wasserstein-gradient-flow tools to prove
well-posedness, regularity estimates and convergence to equilibrium for this
equation {cite:p}`iacobelli2019weighted`. With $m=\rho^{1/(r+1)}$ and
$u=f/m$, the same equation becomes

```{math}
\partial_t u
=
-\frac{r+1}{m}\operatorname{div}\!\left(m\nabla(u^{-r})\right),
```
