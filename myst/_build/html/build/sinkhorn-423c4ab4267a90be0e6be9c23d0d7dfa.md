...
:::{admonition} Procedure: Sinkhorn scaling
:class: ot4ml-procedure

Set $\K_{ij}=e^{-\C_{ij}/\epsilon}$ and initialize $\vD^{(0)}\in\RR_{+,*}^m$. For $k=0,1,\ldots$, alternate the row and column normalizations
...
:::{admonition} Procedure: Endpoint-to-path Schr\"odinger lift
:class: ot4ml-procedure

Solve the static entropic endpoint problem
...
For each transported pair $(x,y)$ sampled from $\pi_\epsilon^\star$, draw a reference bridge path

```{math}
\omega\sim\Rr^{\epsilon,x,y}.
```

The resulting path law is the mixture

```{math}
M_\epsilon^\star=\int \Rr^{\epsilon,x,y}\,\d\pi_\epsilon^\star(x,y).
```

For a Brownian reference, this means that Sinkhorn chooses the endpoint coupling and Brownian bridges fill in the random trajectories between the paired endpoints.
:::
...
:::{admonition} Procedure: Log-domain Sinkhorn by soft transforms
:class: ot4ml-procedure

Initialize a potential $\gD^{(0)}\in\RR^m$. For $k=0,1,\ldots$, alternate stabilized soft $c$-transforms
...
where each log-sum-exp is evaluated after subtracting the relevant maximum. Return
