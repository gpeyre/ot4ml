...
Initialize prices $p_j=0$ and no assignments. While some source $i$ is unassigned, compute
...
Update the winning target price and ownership by
...
If $j_i$ had a previous owner, mark that source as unassigned. Stop when every source owns one target.
:::
...
Start from a feasible or arbitrary source potential $f$ on $\X$. Compute the target best response
...
Then compute the source closure

```{math}
\tilde f=g^{\bar c}=f^{c\bar c}.
```

Finally update the target potential once more:

```{math}
\tilde g=\tilde f^c=f^{c\bar c c}=f^c.
```

Return the closed pair $(\tilde f,\tilde g)=(f^{c\bar c},f^c)$. Further exact alternating $c$-transforms leave this pair unchanged up to the usual additive gauge.
:::
