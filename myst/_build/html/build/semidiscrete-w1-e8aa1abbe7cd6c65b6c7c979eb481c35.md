...
decreasing $g_j$ shrinks it. Semi-dual ascent is therefore a mass-balancing
procedure on a power diagram.
...
problem. The same blue target sites and the same red Gaussian source density
are used in all panels. Only the highlighted violet weight is changed. If the
highlighted cell has too little source mass, then
