...
| $\mathcal I_0(\al,\be)$ | Integrated Fisher information along the quadratic Wasserstein geodesic. | [Proposition prop-small-epsilon-expansion](sinkhorn#prop-small-epsilon-expansion-web) |
...
| $\be_s,\la_s$ | Input measures and weights in barycenter problems. | [Section sec-barycenters](generalized-ot-problems#ot-barycenters) |
...
| $m_t=\rho_t v_t$ | Momentum/flux variable in convex dynamic formulations. | [Eq. eq:benamou-brenier-convex](dynamic-ot#eq-benamou-brenier-convex) |
...
| $A(\rho,m)$ | Local convex action density for generalized dynamic distances. | [Section sec-generalized-dynamic-wasserstein-distances](dynamic-ot#sec-generalized-dynamic-wasserstein-distances) |
| $\mathcal A_A,\mathsf D_A$ | Integrated dynamic action generated by $A$ and its homogeneous distance. | [Proposition prop-homogeneous-dynamic-action-distance](dynamic-ot#prop-homogeneous-dynamic-action-distance) |
| $A_p(\rho,m)$ | Perspective action $\norm{m}^p/\rho^{p-1}$ for $\Wass_p$. | [Section sec-generalized-dynamic-wasserstein-distances](dynamic-ot#sec-generalized-dynamic-wasserstein-distances) |
| $\theta(\rho),A_\theta,\mathsf W_\theta$ | Concave mobility, its action, and the associated mobility distance. | [Section sec-generalized-dynamic-wasserstein-distances](dynamic-ot#sec-generalized-dynamic-wasserstein-distances) |
...
| $\mathcal N_{\gamma,\alpha}$ | Spectral tangent norm induced by the gauge $\gamma$. | [Eq. eq-normalized-spectral-tangent-norm](dynamic-ot#eq-normalized-spectral-tangent-norm) |
| $\mathsf W_{\gamma,\mathrm{dyn}}$ | Dynamic path-length version of spectral Wasserstein geometry. | [Eq. eq-dynamic-spectral-wasserstein](dynamic-ot#eq-dynamic-spectral-wasserstein) |
...
| $\theta(a,b)$ | Logarithmic mean used as Markov/nonlocal mobility. | [Section sec-discrete-wasserstein-markov](dynamic-ot#sec-discrete-wasserstein-markov) |
...
| $\mathcal A(\rho,\psi),\mathcal W_K$ | Markov-chain action and associated discrete Wasserstein distance. | [Eqs. eq-discrete-markov-action](dynamic-ot#eq-discrete-markov-action), [eq-discrete-markov-distance](dynamic-ot#eq-discrete-markov-distance) |
| $\mathfrak m,K(x,\d y),\mathsf J$ | Reference measure, reversible jump kernel and symmetric jump measure. | [Section sec-generalized-dynamic-wasserstein-distances](dynamic-ot#sec-generalized-dynamic-wasserstein-distances) |
...
| $\mathcal A_K,\mathcal W_K$ | Nonlocal jump action and associated Wasserstein distance. | [Eq. eq-nonlocal-wasserstein-distance](dynamic-ot#eq-nonlocal-wasserstein-distance) |
...
| $\Pp_{2,\lambda}(S\times\Theta)$ | Conditional probability laws with fixed condition marginal $\lambda$ and finite parameter second moment. | [Chapter sec-wasserstein-gradient-flows](wasserstein-gradient-flows#sec-wasserstein-gradient-flows) |
| $\Couplings_\lambda(\alpha,\beta)$ | Conditional couplings that keep the condition variable fixed. | [Eq. eq-conditional-ot-general](wasserstein-gradient-flows#eq-conditional-ot-general) |
| $\MK_c^\lambda,\Wass_{2,\lambda}$ | Conditional OT value and conditional Wasserstein distance. | [Eqs. eq-conditional-ot-general](wasserstein-gradient-flows#eq-conditional-ot-general), [eq-conditional-wasserstein-distance](wasserstein-gradient-flows#eq-conditional-wasserstein-distance) |
| $\mathcal J_{\gamma}^{\alpha}(g)$ | Spectral steepest-descent direction for normalized flows. | [Definition def-normalized-spectral-descent](wasserstein-gradient-flows#def-normalized-spectral-descent) |
| $S_\alpha(g),A_\alpha^\star$ | Gradient covariance and active polar preconditioner in spectral flow. | [Proposition prop-normalized-spectral-lmo-polar](wasserstein-gradient-flows#prop-normalized-spectral-lmo-polar) |
