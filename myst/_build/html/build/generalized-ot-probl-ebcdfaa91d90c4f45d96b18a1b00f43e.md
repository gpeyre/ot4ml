...
bilinear cost with $A=-I$, up to these marginal-only terms and an irrelevant
positive factor. The opposite choice $A=+I$ reverses this preference and
favors anti-aligned pairs. Rank-one choices such as $A=-e_1e_1^\top$ or
$A=-e_2e_2^\top$ retain only the horizontal or vertical coordinate in the
correlation score.
...
costs see only one coordinate, $A=-I$ gives the same optimizer as the
quadratic $\Wass_2$ assignment, and $A=+I$ gives the corresponding
anti-Wasserstein matching. The idea of drawing a logo through OT maps is
inspired by the POT logo {cite:p}`flamary2021pot`.*
:::
