...
The quantile construction becomes a deterministic Monge map when the source has
no atoms, because the cumulative distribution function can then be used as a
genuine change of variable. This gives a complete one-dimensional Monge
solution. The more general case, where $\al$ may have atoms and mass splitting
can be unavoidable, belongs naturally to the Kantorovich relaxation and is
treated in Section {ref}`sec-kantorovich-continuous`, paragraph
...
Let $S$ be any admissible map and set $\pi_S=(\Id,S)_\sharp\al$. The quantile
coupling $\pi^\star=(q_\al,q_\be)_\sharp\mathrm{Leb}_{[0,1]}$ is optimal among
all couplings by Theorem {ref}`prop-1d-kantorovich-quantile-coupling`. Since
$\al$ has no atoms, $(\Id,\T)_\sharp\al=\pi^\star$, and therefore
