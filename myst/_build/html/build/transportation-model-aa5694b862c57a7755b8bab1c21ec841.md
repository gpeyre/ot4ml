...
The name "stochastic interpolant" is broader than the first construction used below. We start with a path that is deterministic once the latent variable has been sampled: the randomness is in $u\sim\pi$, while $t\mapsto P_t(u)$ is a differentiable curve. This already covers the flow-matching and rectified-flow regressions used below, because the induced law is a push-forward curve and therefore obeys an ordinary continuity equation. Genuine temporal noise, where the path itself has Brownian fluctuations, is discussed after Proposition {ref}`prop-flow-matching-vector-field`.

We assume first that $\alpha_t$ is defined via a "projection" (in a loose sense) of a latent distribution $\pi \in \Pp(\RR^{d'})$, using an operator $P_t : \RR^{d'} \to \RR^d$ where $d' \gg d$, i.e. $$\label{eq-interp-coupling}
    \forall t \in [0,1], \quad \alpha_t := (P_t)_\sharp \pi.$$ A common case is $d'=2d$. We write $(x,y) \in \RR^{d'} = \RR^d \times \RR^d$ and assume $P_0(x,y)=x$ and $P_1(x,y)=y$, so that $\pi$ is a probabilistic coupling between $\alpha_0$ and $\alpha_1$, i.e. $\pi$ has marginals $(\alpha_0, \alpha_1)$. For instance, one can use $\pi = \alpha_0 \otimes \alpha_1$, the trivial coupling. An even more special case is the linear interpolation $P_t(x,y)=(1-t)x+ty$. With this linear path and an arbitrary coupling $\pi$, the regression below is the common core of flow matching and rectified flow: Lipman et al. emphasize conditional probability paths and simulation-free training of continuous normalizing flows, while rectified flow emphasizes straight couplings, reflow, and the possibility of reducing transport costs and discretization error {cite:p}`Lipman2022FlowMatching,Liu2023RectifiedFlow`. More complex constructions are possible when sampling from $\pi$ remains simple; stochastic interpolants add latent variables or noise, connecting deterministic flows, probability-flow ODEs and diffusion SDEs {cite:p}`Albergo2025StochasticInterpolants`.
...
:::{admonition} Remark: Noisy Stochastic Interpolants
:class: ot4ml-remark
The deterministic push-forward formalism above also covers random latent perturbations such as $P_t(u,\xi)$ with a time-independent auxiliary noise variable $\xi$: one simply enlarges the latent variable $u$. The equation changes only when the interpolation contains genuine temporal noise. For instance, let $Z_t$ solve the Itô equation
$$
\d Z_t = r_t(U,Z_t)\d t + \Sigma_t(U,Z_t)\d B_t,
\qquad \alpha_t=\operatorname{Law}(Z_t),
$$
where $U\sim\pi$ is static and $B_t$ is Brownian motion. Define
$$
...
```
Thus the natural noisy analogue of {eq}`eq-flow-matching` regresses the conditional drift,
$$
\min_w \EE\bigl[\norm{w_t(Z_t)-r_t(U,Z_t)}^2\bigr],
$$
and enforces the Fokker--Planck equation {eq}`eq-noisy-interpolant-fokker-planck`. It is not a pure continuity equation unless the diffusion tensor vanishes. If one wants an ODE with the same marginals, one can use the probability-flow velocity
$$
\bar v_t = v_t - \frac{1}{2\alpha_t}\operatorname{div}(D_t\alpha_t),
$$
where the divergence of the matrix-valued measure is taken row-wise. In the scalar constant case \(D_t=\sigma_t^2\Id\), this gives \(\bar v_t=v_t-\frac{\sigma_t^2}{2}\nabla\log\alpha_t\), the familiar score correction.
:::
