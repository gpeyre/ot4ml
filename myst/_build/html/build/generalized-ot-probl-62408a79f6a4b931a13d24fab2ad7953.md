...
irrelevant positive factor. The figure below generates an observed bijection
$\widehat P$ from this cost on two empirical mixtures of
Gaussians, and then evaluates
...
Equivalently, if $\widehat\sigma\in\Perm(n)$ is the observed assignment and
$\widehat P=n^{-1}P_{\widehat\sigma}$, then the plotted loss is

```{math}
\mathcal L_{\mathrm{iOT}}(C(A_t);\widehat P)
=
\frac1n\sum_{i=1}^n \dotp{A_t x_i}{y_{\widehat\sigma(i)}}
-
\min_{\sigma\in\Perm(n)}
\frac1n\sum_{i=1}^n \dotp{A_t x_i}{y_{\sigma(i)}} .
```

Because $t\mapsto C(A_t)$ is affine and the optimal assignment value is a
minimum of affine functions, this one-dimensional gap is convex and piecewise
affine. Its zero set can contain an interval for a small sample, reflecting
the fact that the same assignment remains optimal for a cone of nearby costs.
...
*Inverse-OT gap loss for a bilinear cost. Left: two empirical mixtures of two
Gaussians are matched with the cost $c_{A_\star}(x,y)=\dotp{A_\star x}{y}$ for
$A_\star=-I$, which gives the same assignment as the quadratic $\Wass_2$ cost.
Middle and right: the unregularized Fenchel--Young assignment gap
