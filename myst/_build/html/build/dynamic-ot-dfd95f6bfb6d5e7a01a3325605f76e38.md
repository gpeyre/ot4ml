...
In the local notation above, $\ell_{A_p,\alpha}(v)=\int |v|^p\d\alpha$, while
the squared tangent cost associated with the distance $\Wass_p$ is
$q_{p,\alpha}(v)=\ell_{A_p,\alpha}(v)^{2/p}$.
:::
...
for $\alpha=\rho\d x$. Hence this is a local Riemannian case whenever the
multiplier $\rho/\theta(\rho)$ is finite and positive.
:::
