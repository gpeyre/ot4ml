...
**Sort** the source and target points:
...
**Sort** the combined red-blue sequence on the line.
...
**Open** the circle at a gap compatible with $s^\star$.
...
**Initialize:** Choose dual feasible labels $(u,v)$; set $M=\emptyset$.

**While** $M$ is not perfect **do**:

**Build** the equality graph:

```{math}
E(u,v)=\{(i,j):u_i+v_j=C_{i,j}\}.
```

**Choose** an unmatched source.
