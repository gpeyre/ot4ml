...
The first extension changes only the kinetic exponent. For measures with finite $p$-th moments and $1<p<+\infty$, the Benamou--Brenier principle gives
...
under the linear constraint $\partial_t\rho_t+\nabla\!\cdot m_t=0$. The $p$-th root of the value is exactly the Wasserstein distance $\Wass_p$; in particular this construction defines a distance because it is another representation of the static Kantorovich distance. The case $p=2$ is the quadratic action used above, while the endpoint case $p=1$ loses strict convexity and leads to the Beckmann formulation of $\Wass_1$.
