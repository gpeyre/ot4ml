...
transports through an intermediate measure
$\eta=\sum_k g_k\delta_{z_k}$ with only $r$ Dirac masses.
...
regularization $\epsilon>0$, maximum number of iterations $L$, tolerance $\mathrm{tol}$.

**Output:** factored coupling $P=Q\operatorname{diag}(g)^{-1}R^\top$.

Initialize $R^{(0)}=b\otimes g$ and $J^{(0)}=+\infty$.
