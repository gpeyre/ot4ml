...
tangent cost in {eq}`eq-generalized-distance-tangent-form`. For action
densities $A(\rho,m)$, the instantaneous action $\ell_{A,\alpha}$ is defined in
{eq}`eq-local-action-density-induced`; if $A$ is $r$-homogeneous in the momentum
and the induced distance is $\mathcal A_A^{1/r}$, then
$q_\alpha(v)=\ell_{A,\alpha}(v)^{2/r}$. Thus $\Wass_p$ gives the Finsler action
$q_{p,\alpha}(v)=(\int\norm{v}^p\d\alpha)^{2/p}$, while quadratic geometries
give $q_\alpha=\ell_{A,\alpha}$. This section keeps the same notation but uses
it for steepest descent rather than for defining distances.
...
The examples below are organized by the same dictionary. Concave-mobility
actions use $q_{\theta,\alpha}$ from {ref}`rem-generalized-bb`; spectral flows
use $q_{\gamma,\alpha}(v)=\mathcal N_{\gamma,\alpha}(v)^2$ from
{eq}`eq-normalized-spectral-tangent-norm`; kernelized Benamou--Brenier flows
...
studied separately in {ref}`sec-dynamic-unbalanced-wfr-flows`, is another quadratic
geometry, but its tangent variable is a pair $(v,g)$ combining displacement and
mass modulation. Thus $q_\alpha$ is the common notation between the distance
and flow chapters, whereas $Q_\alpha$ is reserved for Hilbertian vector-field
subcases where a linear preconditioner actually represents the tangent geometry.
...
Thus  {eq}`eq-normalized-spectral-lmo` is, up to the harmless factor $1/n$, the finite-dimensional matrix LMO
