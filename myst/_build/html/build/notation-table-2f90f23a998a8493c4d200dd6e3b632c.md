...
| $\mathbb J_{A,\lambda}(\alpha,\mu)$ | Measure-level momentum action associated with $A$ relative to a reference measure $\lambda$; written $\mathbb J_A$ only in intrinsic cases. | [Eq. eq-general-measure-momentum-action](dynamic-ot#eq-general-measure-momentum-action) |
