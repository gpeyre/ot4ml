...
For KL marginal penalties of strength $\tau$, set
...
Initialize $v^{(0)}\in\RR_{+,*}^m$. For $k=0,1,\ldots$, perform the damped normalizations
...
Return $\P^{(k)}=\diag(u^{(k)})K\diag(v^{(k)})$, whose marginals are generally only approximately equal to $\a$ and $\b$.
:::
...
Conversely, on compact sets, $\Wass_p$ can be bounded by a dimension-dependent power of $\SW_p$; such inequalities are weaker than the direct bound of Proposition labeled `prop-sliced-wasserstein-metric` and explain why sliced distances metrize the same topology without being bi-Lipschitz equivalent to $\Wass_p$ in high dimension {cite:p}`bonnotte2013unidimensional,nadjahi2019asymptotic`.
:::
...
For empirical measures $\alpha=n^{-1}\sum_i\delta_{x_i}$ and $\beta=n^{-1}\sum_i\delta_{y_i}$ with equal weights, sample directions $\theta_\ell\sim\sigma$ on $\Sphere^{d-1}$ for $\ell=1,\ldots,L$. For each direction, project and sort
...
\qquad
s_{(1)}^\ell\leq\cdots\leq s_{(n)}^\ell,\quad
...
```{math}
\widehat{\SW}_p^p(\alpha,\beta)
=
\frac1L\sum_{\ell=1}^L
\frac1n\sum_{i=1}^n
\abs{s_{(i)}^\ell-t_{(i)}^\ell}^p.
```

For non-uniform or continuous measures, replace the sorted samples by the corresponding one-dimensional quantile computation.
:::
(alg:lifted-min-sliced-matching)=
:::{admonition} Algorithm: Lifted min-sliced matching
:class: ot4ml-algorithm

Given equal-weight point clouds $(x_i)_{i=1}^n$ and $(y_i)_{i=1}^n$, choose a finite set of directions $\Theta\subset\Sphere^{d-1}$. For each $\theta\in\Theta$, sort the projected values $\dotp{\theta}{x_i}$ and $\dotp{\theta}{y_j}$ and match equal ranks, obtaining a permutation $\sigma_\theta$. Evaluate the full-dimensional cost

```{math}
E(\theta)=\frac1n\sum_{i=1}^n\norm{x_i-y_{\sigma_\theta(i)}}^2.
```

Return the lifted coupling

```{math}
\pi_{\theta^\star}=\frac1n\sum_i\delta_{(x_i,y_{\sigma_{\theta^\star}(i)})},
\qquad
\theta^\star\in\argmin_{\theta\in\Theta}E(\theta).
```

This gives a feasible high-dimensional coupling, not the exact sliced distance.
:::
...
Several constructions in this text embed measures into Hilbert spaces, but they encode different geometries. Kernel mean embeddings send $\alpha$ to $\int k(x,\cdot)\d\alpha(x)$ in an RKHS and lead to MMD distances; see Section labeled `sec-rkhs-mmd`. Quadratic sliced Wasserstein sends a measure to the collection of one-dimensional quantile functions of its projections, viewed in $L^2(\Sphere^{d-1}\times[0,1])$; see Section labeled `sec-sliced-wasserstein`. Linear OT sends $\alpha$ to the displacement field $T_\alpha-\Id$ from a fixed reference $\rho$ in $L^2(\rho;\RR^d)$. The first construction is linear in the measure and depends on the kernel, the second is nonlinear but reduces OT to projected one-dimensional quantiles, and the third is a tangent approximation to the full Wasserstein geometry around a chosen reference.
:::
