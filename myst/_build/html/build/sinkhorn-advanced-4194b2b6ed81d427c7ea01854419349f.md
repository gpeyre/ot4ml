...
Recent work also clarifies weak limits beyond smooth costs and second-order
null limits for Sinkhorn divergences
{cite:p}`gonzalezSanzHundrieser2023weak,goldfeld2022limit`. These
...
and in particular $n^{-1}$ for $\Wass_2^2$. Thus the exponent $p/d'$ should be
read as the high-dimensional matching scale, not as a universal formula
...
$n^{-1/2}$ central limit theorem. Thus ``slow OT statistics'' can mean a large
empirical bias, a non-smooth directional limit, or the need to remove a
regularization bias $T_\epsilon(\alpha,\beta)-T_0(\alpha,\beta)$; these
...
The independent two-sample version has the same directional form. If
...
The empirical histogram can have a zero entry, but since $a_i>0$ for all $i$,
this event has probability at most $\sum_i(1-a_i)^N$, hence is exponentially
small and does not affect the first-order limit or the $N^{-1}$ bias. On the
complementary event, Taylor's formula applies in the relative interior: the
first-order term has zero expectation and the Hessian is bounded locally, so
the plug-in bias is $O(N^{-1})$. The variance convergence follows from the same
expansion and bounded moments of the multinomial fluctuation. The two-sample
formula follows by applying the same argument independently to the two
marginals.
:::
...
is another example where the asymptotic law depends not only on the rate, but
also on the local geometry of the functional.
...
population weights are positive. The regularization bias vanishes only when
...
for fixed $\epsilon$, even under quite weak assumptions on the cost, but its
statistical bias relative to exact OT must still be sent to zero if the target
quantity is unregularized. Thus the sample-complexity section gives
dimension-dependent magnitudes, while the present section identifies the local
asymptotic constants and covariance formulas that support inference.
