...
viewpoint was developed in closely related forms by Mei, Montanari and
Nguyen {cite:p}`MeiMontanariNguyen2018MeanFieldNN`, Rotskoff and
Vanden-Eijnden {cite:p}`RotskoffVandenEijnden2018Trainability`, and Chizat
and Bach {cite:p}`ChizatBach2018OverparameterizedOT`. These works identify
wide-network training with an interacting-particle system and derive the
limiting nonlinear PDE for the neuron law.
...
noise adds a Laplacian, which smooths the neuron law and can create a density
even from a singular initialization. Rotskoff and Vanden-Eijnden emphasize
the interacting-particle interpretation, long-time convergence and asymptotic
error scaling with the network width
