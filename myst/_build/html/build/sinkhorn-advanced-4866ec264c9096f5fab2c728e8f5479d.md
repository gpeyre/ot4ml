...
view of Bigot--Cazelles--Papadakis, Klatt--Tameling--Munk and Mena--Weed
{cite:p}`bigot2017central,klatt2020empirical,mena2019statistical`. These
...
$\mathbb E\Wass_p^p(\hat\alpha_n,\alpha)$ is the leading term. In the
high-dimensional regime where the support has intrinsic dimension $d'>2p$, this
bias has order $n^{-p/d'}$; this is the curse of dimensionality already visible
in {ref}`prop-empirical-ot-rate`. Low-dimensional and smooth-density regimes can
have different critical logarithmic or parametric-looking powers, so the
