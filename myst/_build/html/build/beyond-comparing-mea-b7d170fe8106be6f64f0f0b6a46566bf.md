...
**Solve** the entropic OT subproblem:
...
**If** $\norm{\P^{(k+1)}-\P^{(k)}}\leq\mathrm{tol}$, **return** $\P^{(k+1)}$.
:::
...
**Solve** the $A$-projection equation:
...
**Solve** the $B$-projection equation:
...
**If** both partial-trace residuals are at most $\mathrm{tol}$, **return** $T_e(F^{(k+1)},G^{(k+1)})$.
:::
