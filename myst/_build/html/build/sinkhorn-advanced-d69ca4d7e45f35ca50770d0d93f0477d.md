...
supported on a compact set with regular $d'$-dimensional volume growth, for
instance on a smooth compact $d'$-dimensional submanifold with densities
bounded above and below with respect to its volume measure, the same estimate
holds with $d$ replaced by $d'$ when $d'>2p$.
:::
