...
smooth diffeomorphisms that locally compress a uniform source grid around one,
three, and five centers. The pushed density is rendered by flat cells with
quantized opacity values, without level sets; darker blue cells correspond to
smaller Jacobian determinant and hence larger density. The deformed grid reaches
the boundary of each crop, so the reader sees the local warp rather than an
artificial square support. This illustrates the factor $|\det \T'(x)|^{-1}$ in
