...
:class: important
Let $\beta=e^{-V}\d x/Z$ be a smooth probability measure on $\RR^d$, and assume
that $\nabla^2 V\succeq \lambda \operatorname{Id}$ for some $\lambda\in\RR$.
For $\alpha=\rho\d x$, define the relative Fisher information

```{math}
\mathcal I_\beta(\alpha)
\eqdef
\int \norm{\nabla\log\frac{\rho}{\d\beta/\d x}}^2\,\d\alpha,
```
...
:::{dropdown} Proof
Assume first that $\alpha$ has a smooth positive density and finite Fisher
information. Let $T$ be the Brenier map from $\alpha$ to $\beta$, set
$v=T-\operatorname{Id}$, and let
...
f'(0)
=
\int \dotp{\nabla\log\frac{\rho}{\d\beta/\d x}}{v}\,\d\alpha.
```
...
\KL(\alpha|\beta)
\leq
-\int \dotp{\nabla\log\frac{\rho}{\d\beta/\d x}}{T-\operatorname{Id}}\,\d\alpha
-\frac{\lambda}{2}\Wass_2^2(\alpha,\beta).
```
