...
:::{dropdown} Proof
Assume first that $A$ and $B$ have smooth positive densities after a harmless
regularization. Let $\alpha$ and $\beta$ be the uniform probability measures on
$A$ and $B$, and let $T=\nabla\phi$ be the Brenier map with
$T_\sharp\alpha=\beta$. The interpolating map $T_t=(1-t)\operatorname{Id}+tT$
sends $\alpha$ to a measure $\alpha_t$ supported in $(1-t)A+tB$. At points
where $T$ is differentiable,
...
Because $\alpha_t$ is a probability measure supported in $(1-t)A+tB$,
integration gives the displayed Brunn-Minkowski inequality. General bounded
Borel sets follow by approximation.
