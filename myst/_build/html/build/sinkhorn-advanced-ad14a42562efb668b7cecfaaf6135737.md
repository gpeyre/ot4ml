...
The previous section gave non-asymptotic sample-complexity bounds: it
controlled how many samples are needed for an empirical OT quantity to be
accurate, typically up to universal constants. We now change viewpoint.
Instead of asking only for a magnitude, we ask for the first statistical
expansion of the plug-in estimator. Given empirical laws $\hat\alpha_n$ and
$\hat\beta_m$, and an OT functional $T$, the two basic objects are the leading
statistical bias
...
is different from the central limit theorem of
{ref}`sec-law-large-numbers-clt`, which describes the random signed measure
$\sqrt n(\hat\alpha_n-\alpha)$ itself. Here this empirical process is pushed
through a nonlinear, often nonsmooth, transport value. The answer is therefore
controlled by the local differentiability of the OT functional and by the
geometry of its optimal dual potentials.
...
{cite:p}`tameling2017empirical,hundrieser2022empirical`, show that the correct
limit is a directional delta-method limit, often the support function of a
...
Entropic regularization changes the picture: for fixed $\epsilon>0$, the value
is smooth in the marginals, and the limiting variance is read directly from
the entropic dual potentials. This is the point of view developed by
...
{ref}`sec-sample-complexity`; they refine them by identifying the local
constants, the limiting laws, and the mechanisms behind bias.
...
$n^{-1/2}$ central limit theorem. A ``slow OT statistic'' can therefore mean
three different things: a large empirical bias, a nonsmooth directional limit,
...
The dual formulation writes $\Phi$ as the supremum, over the feasible dual
...
growth in the multinomial fluctuation. Uniform integrability then gives
convergence of the first two moments, and hence the bias and variance
...
formula applies in the relative interior: the first-order term has zero
expectation and the Hessian is locally bounded, so the plug-in bias is
...
instead the corresponding difference between cross and self entropic
potentials. At the null
...
is a useful warning: the asymptotic law is determined not only by the rate, but
...
This identity is usually read after choosing $\epsilon=\epsilon_n$. For fixed
...
The finite-dimensional formulas above are best viewed as a template. Whenever
an OT value is Hadamard differentiable and admits a unique sufficiently regular
dual potential $f^\star$, the empirical-process CLT and the delta method give a
first-order Gaussian limit with one-sample variance
...
on the cost. If the target is unregularized OT, however, the statistical
