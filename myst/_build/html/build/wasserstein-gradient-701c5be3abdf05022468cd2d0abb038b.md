...
Before asking for rates, the energy-dissipation identity already gives a useful
global control on the path. For the classical gradient flow of a smooth function
$h:\RR^d\to\RR$, $\dot x_t=-\nabla h(x_t)$, one has
...
Hence, for every $T>0$, Cauchy's inequality gives

```{math}
:label: eq-euclidean-gradient-flow-length
\int_0^T \norm{\dot x_t}\,\d t
\leq
\sqrt{T}\left(\int_0^T\norm{\dot x_t}^2\d t\right)^{1/2}
=
\sqrt{T}\,\bigl(h(x_0)-h(x_T)\bigr)^{1/2}.
```

The same statement holds in Wasserstein geometry with the metric derivative
$|\dot\alpha_t|_{\Wass_2}$. A smooth Wasserstein gradient flow with velocity
$v_t=-\Wgrad f(\alpha_t)$ satisfies

```{math}
f(\alpha_0)-f(\alpha_T)
=
\int_0^T |\dot\alpha_t|_{\Wass_2}^2\d t
=
...
\int_0^T |\dot\alpha_t|_{\Wass_2}\,\d t
\leq
\sqrt{T}\,\bigl(f(\alpha_0)-f(\alpha_T)\bigr)^{1/2}.
```

This bound does not by itself identify a minimizer or give a rate. It
nevertheless rules out finite-time escape by metric length: whenever the energy
drop is finite, the whole segment $(\alpha_t)_{0\leq t\leq T}$ remains in the
$\Wass_2$-ball of radius given by {eq}`eq-wasserstein-gradient-flow-length`
around $\alpha_0$. Thus finite-time blow-up by travelling to infinity is
impossible for a genuine energy-dissipating trajectory, even when the objective
is not coercive. The PL and convexity estimates below add the missing
