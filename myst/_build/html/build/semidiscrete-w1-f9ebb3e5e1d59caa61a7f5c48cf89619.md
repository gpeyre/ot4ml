...
:::{admonition} Procedure: Semi-discrete Laguerre descent
:class: ot4ml-procedure

Initialize dual weights $\gD^{(0)}\in\RR^m$. At iteration $k$, compute the Laguerre cells
...
and their source masses

```{math}
m_j^{(k)}=\int_{\Laguerre_j(\gD^{(k)})}\d\al .
```

Update the concave semi-dual by
...
or use this gradient in a quasi-Newton ascent. Stop when $m_j^{(k)}\simeq \b_j$ for all $j$.
:::

:::{admonition} Procedure: Stochastic semi-discrete ascent
:class: ot4ml-procedure

Initialize $\gD^{(0)}=0$. For $\ell=0,1,\ldots$, draw $x_\ell\sim\al$ and find the active Laguerre cell

```{math}
j_\ell\in\argmin_j\bigl(c(x_\ell,y_j)-\gD_j^{(\ell)}\bigr).
```

With a decreasing step size $\tau_\ell$, update every coordinate by
...
:::{admonition} Procedure: Lloyd quantization
:class: ot4ml-procedure

Initialize codepoints $Y^{(0)}=(y_j^{(0)})_{j=1}^m$. At iteration $k$, form the Voronoi cells
...
For the squared Euclidean cost, update each nonempty cell by its centroid:
...
Keep or reseed empty cells according to the chosen implementation.
:::
...
:::{admonition} Procedure: Graph Beckmann network-simplex pivot
:class: ot4ml-procedure

Replace each undirected edge by two directed arcs with cost $\ell_e$, and impose node balances

```{math}
\sum_j u_{ij}-\sum_j u_{ji}=r_i .
```

Maintain a spanning-tree basis, a feasible tree flow, and node potentials. At each pivot, price non-tree arcs by their reduced costs and choose an entering arc with negative reduced cost. Adding it to the tree creates a unique cycle; send the largest admissible amount of flow around this cycle until one arc becomes tight, remove that leaving arc, and update the tree, potentials and reduced costs. Stop when all reduced costs are nonnegative; the resulting transshipment flow solves the graph Beckmann problem.
:::
