...
iteration itself: one lets the grid resolution $k$ and the inverse entropic
temperature grow together, $\epsilon=k^{-1}$, and observes the Sinkhorn iterates
at macroscopic time $t=m/k$. In this high-resolution scaling, Sinkhorn becomes
a flow of potentials whose stationary equation is the Monge--Ampere equation
for the optimal map. This viewpoint connects matrix scaling with the geometric
theory of real and complex Monge--Ampere flows.
...
densities on the flat torus $\mathbb T^d$. A gauge-fixed potential
...
Thus $u_m^{(k)}\to u_t$ with $t=m/k$ leads to the continuous flow above,
after the gauge normalization. The equation is a real parabolic
Monge--Ampere equation. Its Kahler analogue is the parabolic complex
Monge--Ampere equation, which explains the link with geometric
Monge--Ampere theory.
...
Since both measures have mass one and $T$ is a diffeomorphism of the torus,
integration and change of variables give $e^c=1$. The identity is then exactly
the Jacobian equation for $T_{\#}(e^{-F}dx)=e^{-G}dx$. The converse follows by
...
Monge--Ampere equation obtained from Berman's high-resolution Sinkhorn scaling.
Time is encoded from red to blue; the faint bottom silhouettes show the source
density in red and the target density in blue.*
:::
