...
**Output:** Optimal concave-cost matching.
...
**If** an indicator certifies an adjacent source-target pair, record this pair and remove both points.

**End for.**

**Recompute** only chains affected by removals.

**End while**; **return** the recorded matching.
:::
...
**Initialize:** Choose dual feasible labels $(u,v)$ and set $M=\emptyset$.
...
**Choose** an unmatched source; grow an alternating tree in $E(u,v)$.

**If** the tree reaches an unmatched target, augment $M$ along the alternating path.

**Else** set $S=$ reached sources and $T=$ reached targets, compute
