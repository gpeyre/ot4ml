...
The Gaussian kernel used by Sinkhorn is also a heat kernel. This viewpoint
explains how entropic OT can be accelerated on grids and surfaces, and it also
connects soft minima with Hopf--Cole formulas.
...
heat kernel with $t=\epsilon/4$. A related family is produced by the
shifted-Laplacian resolvent. On the line, the kernel of $(I-\tau\Delta)^{-1}$
is exactly proportional to $\exp(-|x-y|/\sqrt{\tau})$; in higher dimension it
is a Yukawa--Bessel kernel with the same exponential tail. Thus one resolvent
step gives a Laplace-type kernel, while the heat semigroup gives the Gaussian
...
Writing $L=-\Delta_M$, one has

```{math}
e^{-tL}=\lim_{q\to\infty}\left(I+\frac{t}{q}L\right)^{-q}.
```

A single inverse shifted Laplacian gives a Laplace-type smoothing, while
repeated resolvent steps approximate the Gaussian heat kernel. On a mesh or
grid, the limit is delicate: the regularization length scale $\sqrt t$ must be
resolved by the mesh. If it goes to zero faster than the grid spacing,
metrication artifacts and inconsistent schemes can dominate the geodesic limit.
...
Completing the square gives $Q_t f(x)=\norm{x}^2/(2t)-(f+\norm{\cdot}^2/(2t))^*(x/t)$. The entropic version replaces this Legendre transform by the Gaussian log-sum-exp formula, equivalently a heat-kernel convolution at time $\epsilon t/2$.
...
Laplace's principle gives $Q_t^\epsilon f\to Q_t f$ as $\epsilon\to0$, up to an
additive constant. The Hopf--Cole transform is the PDE version of the same
idea: if
...
so that the generator is $(\epsilon/2)\Delta$. A path law $M$ with the same
initial marginal and $M\ll R^\epsilon$ can be represented, in the weak sense,
by an adapted drift $u_t$ of finite energy,
...
Conversely, such a finite-energy drift defines a law absolutely continuous
with respect to $R^\epsilon$. Girsanov's formula then gives
...
the endpoint constraint. Hence the Schrodinger bridge can be read as
