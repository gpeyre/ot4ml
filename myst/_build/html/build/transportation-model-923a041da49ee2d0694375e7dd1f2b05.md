...
$$
and enforces the Fokker--Planck equation {eq}`eq-noisy-interpolant-fokker-planck`, not a pure continuity equation unless $D_t=0$. If one wants an ODE with the same marginals, one can use the probability-flow velocity
$$
\bar v_t = v_t - \frac{1}{2\alpha_t}\operatorname{div}(D_t\alpha_t),
$$
where the divergence of the matrix-valued measure is taken row-wise. When $D_t=\sigma_t^2\Id$, this gives $\bar v_t=v_t-\frac{\sigma_t^2}{2}\nabla\log\alpha_t$, the familiar score correction.
:::
