...
the potential evolve in continuous time. In a smooth high-resolution scaling,
Sinkhorn becomes a flow of potentials whose stationary equation is the
...
With the quadratic cost convention, $u_t$ is a signed representative of the
source potential, while the companion target potential $g_t$ is recovered by
the corresponding soft $c$-transform. The parameter "epsilon" in the name
refers to the entropic temperature used in the discrete Sinkhorn scaling; after
the parabolic rescaling of {cite:p}`berman2017sinkhorn`, the continuum limit
takes the normalized form above. The equation is real parabolic
Monge--Ampere. Its Kahler analogue is the parabolic complex Monge--Ampere
equation, which explains the link with geometric Monge--Ampere theory.
...
Monge--Ampere equation. Time is encoded from red to blue; the faint bottom
silhouettes show the source density in red and the target density in blue.*
:::
