...
To fix constants, suppose that $R^\epsilon$ is Brownian motion on $\RR^d$ with
diffusion coefficient $\sqrt{\epsilon}$ and initial law $\alpha$. A controlled
law $M$ with the same initial law can be represented, under the usual
non-degeneracy assumptions, by
...
where $u_t$ is an adapted drift. Girsanov's formula gives
...
Thus the bridge is the least energetic change of drift that steers the prior
Brownian dynamics from $\alpha$ to $\beta$. The optimizer may be chosen
Markovian; if $h_t$ denotes the positive space-time harmonic Schrodinger
factor, then the optimal feedback drift has the Doob-transform form
