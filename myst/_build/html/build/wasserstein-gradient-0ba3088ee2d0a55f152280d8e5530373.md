...
The JKO construction is not tied to the quadratic Wasserstein distance. Given a
metric, or more generally a sufficiently well behaved discrepancy $d$, one can
define a minimizing movement by the implicit Euler step
...
{cite:p}`ambrosio2006gradient,gigli2011user`. As $\tau\to0$, and under
compactness and lower-semicontinuity assumptions adapted to $d$, these steps
converge to a continuous curve $t\mapsto\alpha_t$. In the local mass-preserving
cases considered below, the limiting curve is still represented by a continuity
equation of the type introduced in {eq}`eq-eulerian-advection` and used for
Wasserstein flows in {eq}`eq-wassflow-pde`,
...
discrepancy $d$, not necessarily for $\Wass_2$. Unbalanced and nonlocal variants
replace this equation by the corresponding balance or nonlocal continuity
equation.

The formal tangent relation is easy to read from a Taylor expansion. Suppose
that, for smooth vector fields $w$,
...
where $Q_\alpha$ is a positive self-adjoint operator on the tangent velocity
space. Expanding {eq}`eq-generalized-jko-step` with
$\alpha=(\Id+\tau v)_\sharp\alpha_t$ gives, up to $o(\tau)$,
...
Thus the formal descent velocity is
...
with the inverse understood on the range of $Q_\alpha$, or as a pseudo-inverse
when constraints create null directions. For ordinary $\Wass_2$, $Q_\alpha=\Id$
and one recovers $v_\alpha=-\Wgrad f(\alpha)$. The examples below are different
ways of changing this infinitesimal operator: by modifying the mobility, adding
