...
stochastic dynamics, the natural motion is instead nonlocal: mass can jump
...
{cite:p}`Maas2011,MielkeCVPDE`, replaces the local gradient by a discrete
difference over pairs $(x,y)$. Subsequent work clarified metric and
asymptotic properties of such nonlocal transport distances
{cite:p}`SlepcevWarren2022NonlocalWasserstein`, and related gradient-flow
structures for nonlocal diffusion equations
{cite:p}`Warren2024NonlocalDiffusionGradientFlow`. This produces a transport
...
fluxes through neighboring points. The nonlocal version keeps the same idea
...
Markov jump kernel reversible with respect to $\mathfrak m$. Equivalently,
...
$\theta(a,a)=a$. A curve $\alpha_t=\rho_t\mathfrak m$ driven by an
...
:::{admonition} Proposition: Entropy Geometry of Reversible Jump Kernels
:class: important
...
and the Markov semigroup generated by
...
The nonlocal gradient of the first variation $\log\rho+1$ therefore turns the
metric flux in {eq}`eq-nonlocal-continuity-weak` into the linear difference
$\rho(y)-\rho(x)$. This gives exactly the reversible jump generator $L$. The
metric properties and geodesic existence require compactness and
lower-semicontinuity estimates for the action and are the main content of
{cite:t}`Erbar2012JumpEntropy`.
:::
...
kernels. Choosing a heavy-tailed jump kernel turns the entropy flow into a
...
More generally, if $\beta=\rho_\beta\mathfrak m$ is a desired invariant
measure, one can choose a reversible target-dependent jump kernel $K_\beta$
and apply the same construction to $r_t=\d\alpha_t/\d\beta$; the gradient flow
of $\KL(\alpha|\beta)$ is then $\partial_t r_t=L_\beta r_t$. This is the
nonlocal analogue of the Fokker-Planck gradient flow of relative entropy, and
it is one of the motivations behind more recent nonlocal diffusion
gradient-flow frameworks {cite:p}`Warren2024NonlocalDiffusionGradientFlow`.

### Lévy SDE Viewpoint

The same operators appear probabilistically as generators of jump processes.
This gives a complementary interpretation of the nonlocal PDE as the law of a
stochastic process with heavy-tailed jumps.
...
describe the same nonlocal mechanism: relaxation is driven not only by local
...
A coarse continuous-time model is then
...
Wasserstein geometries a natural language for linking heavy-tailed training
