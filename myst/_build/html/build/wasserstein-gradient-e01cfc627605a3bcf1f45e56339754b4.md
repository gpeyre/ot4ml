...
$\Theta\subset\RR^q$ be a Euclidean parameter space. Denote by
$\Pp_{2,\lambda}(S\times\Theta)$ the set of probability measures whose first
marginal is $\lambda$ and whose conditional second moment
$\int_S\int_\Theta\norm{\theta}^2\d\alpha_s(\theta)\d\lambda(s)$ is finite.
For $\alpha,\beta\in\Pp_{2,\lambda}(S\times\Theta)$, write their
disintegrations as

```{math}
\alpha(\d s,\d\theta)=\alpha_s(\d\theta)\lambda(\d s),
\qquad
\beta(\d s,\d\theta)=\beta_s(\d\theta)\lambda(\d s).
```

A conditional coupling between $\alpha$ and $\beta$ is a probability measure

```{math}
\Pi(\d s,\d\theta,\d\theta')
=
\pi_s(\d\theta,\d\theta')\lambda(\d s)
```

on $S\times\Theta\times\Theta$ such that
$\pi_s\in\Couplings(\alpha_s,\beta_s)$ for $\lambda$-a.e. $s$; denote the set
of such couplings by $\Couplings_\lambda(\alpha,\beta)$. Given a Borel family
of lower-semicontinuous costs $c_s(\theta,\theta')$, bounded from below, the
...
\int_S\int_{\Theta\times\Theta}
c_s(\theta,\theta')\d\pi_s(\theta,\theta')\d\lambda(s) \\
&=
\int_S
\inf_{\pi_s\in\Couplings(\alpha_s,\beta_s)}
\int_{\Theta\times\Theta} c_s(\theta,\theta')\d\pi_s(\theta,\theta')
\d\lambda(s).
\end{aligned}
```

The equality follows by measurable selection of fiberwise optimal plans, or by
measurable near-optimal selections when the minimum is not attained. Thus
conditional OT is a direct integral of ordinary OT problems.

For $c_s(\theta,\theta')=\norm{\theta-\theta'}^2$, this gives the conditional
...
=
\int_S \Wass_2^2(\alpha_s,\beta_s)\d\lambda(s).
```

Equivalently, one may view a conditional coupling as a coupling on
$(S\times\Theta)^2$ concentrated on pairs with the same condition. Hence this is
not the unconstrained Wasserstein distance on the product space: mass can move
in the parameter variable $\theta$, but transport between two different
conditions is forbidden, or assigned infinite cost.

For ResNets, the conditioning variable is the continuous depth $s\in[0,1]$ and
$\lambda(\d s)=\d s$ is the normalized Lebesgue probability measure. A mean-field infinite-depth and infinite-width residual
network is described by a conditional family $s\mapsto\alpha_s\in\Pp_2(\Theta)$,
where each slice is the neuron law of a two-layer mean-field block. We now write
...
training by gradient flow for $\Wass_{2,\lambda}$ formally gives the fiberwise
continuity equation
...
v_{t,s}(\theta)
=
-\nabla_\theta\frac{\delta f_{\mathrm{Res}}}{\delta\alpha_s}(\alpha_t)(\theta),
```

for a.e. depth $s$, whenever the first variation is differentiable in the
parameter variable. This is a layerwise Wasserstein flow: for each fixed depth,
parameters move inside their own neuron space, while the different fibers are
coupled through the forward ODE and the adjoint equations appearing in the
variation of $f_{\mathrm{Res}}$.

A finite ResNet discretizes $s$ into layers and each conditional law $\alpha_s$
into finitely many neurons. If
$\alpha_\ell=n^{-1}\sum_i\delta_{\theta_{\ell,i}}$ and
$\beta_\ell=n^{-1}\sum_i\delta_{\theta'_{\ell,i}}$ are two empirical neuron laws
at layer $\ell$, the discrete analogue of {eq}`eq-conditional-wasserstein-distance`
is

```{math}
\frac1L\sum_{\ell=1}^L \Wass_2^2(\alpha_\ell,\beta_\ell).
```

Keeping neuron labels fixed gives the usual rescaled Euclidean parameter metric
as an upper bound; quotienting by the permutation symmetry within each layer
gives exactly the Wasserstein matching metric above. In this sense, the shallow
mean-field flow is the one-fiber case, while infinitely deep ResNets require a
continuum of coupled Wasserstein flows indexed by depth.
