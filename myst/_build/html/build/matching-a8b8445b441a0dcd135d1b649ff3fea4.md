...
:::{admonition} Procedure: One-dimensional sorting assignment
:class: ot4ml-procedure

For equal weights and a convex one-dimensional cost $h(x-y)$:

```{math}
\text{sort }x_{\sigma_X(1)}\leq\cdots\leq x_{\sigma_X(n)},
\qquad
\text{sort }y_{\sigma_Y(1)}\leq\cdots\leq y_{\sigma_Y(n)}.
```

Return the monotone assignment

```{math}
x_{\sigma_X(k)}\longmapsto y_{\sigma_Y(k)},
\qquad
\sigma=\sigma_Y\circ\sigma_X^{-1}.
```
:::

:::{admonition} Procedure: Concave line matching by local indicators
:class: ot4ml-procedure

For a strictly concave distance cost on the line and unit masses, sort the combined source-target sequence and decompose it into maximal alternating chains. On each chain, compute the local matching indicators of Delon, Salomon and Sobolevski {cite:p}`delon-concave`. Whenever an indicator certifies that an adjacent source-target pair must be matched, record this pair and remove the two points from the ordered sequence. Recompute the affected chains and repeat until no point remains. The resulting matching is optimal for the one-dimensional concave assignment problem.
:::

:::{admonition} Procedure: Circle assignment by cutting
:class: ot4ml-procedure

Choose cyclic orderings $(x_{(k)})_{k=1}^n$ and $(y_{(k)})_{k=1}^n$. For each shift $s\in\{0,\ldots,n-1\}$, compute
...
Choose $s^\star\in\argmin_s E_s$ and return $x_{(k)}\mapsto y_{(k+s^\star)}$. Equivalently, open the circle at a gap compatible with $s^\star$, lift all points to the unfolded interval, and apply the one-dimensional sorting procedure.
:::
...
:::{admonition} Procedure: Hungarian primal-dual augmentation
:class: ot4ml-procedure

Initialize a dual feasible pair $(u,v)$ and an empty matching $M\subset E(u,v)$. While $M$ is not perfect, choose an unmatched source and grow an alternating tree in the equality graph

```{math}
E(u,v)=\{(i,j):u_i+v_j=C_{i,j}\}.
```

If the tree reaches an unmatched target, augment $M$ along the alternating path. Otherwise, with $S$ the reached sources and $T$ the reached targets, compute
...
Continue until a perfect matching contained in $E(u,v)$ is obtained.
:::
