...
of histograms, posterior distributions or shapes. Consistency of Wasserstein
barycenters and related statistical constructions is developed in
...
:::{dropdown} Proof
Since $\Omega$ is compact metric, $\mathcal P(\Omega)$ is compact metric for
weak convergence, and so is $\mathcal P(\mathcal P(\Omega))$. The empirical law
$\widehat{\mathfrak A}_p$ therefore satisfies the ordinary strong law of large
numbers on the compact space $\mathcal P(\Omega)$: for every continuous
$\Phi:\mathcal P(\Omega)\to\RR$,
...
$(\beta,\alpha)\mapsto\mathcal L_c(\beta,\alpha)$ is continuous on the compact
space $\mathcal P(\Omega)^2$. Therefore the family of functions

```{math}
\alpha\mapsto\mathcal L_c(\beta,\alpha),
\qquad \beta\in\mathcal P(\Omega),
```

is compact in $C(\mathcal P(\Omega))$. The weak convergence of
$\widehat{\mathfrak A}_p$ to $\mathfrak A$ is thus uniform over this compact
family, hence the empirical objectives
...
converge uniformly on $\mathcal P(\Omega)$ to
...
subsequence $\beta_{p_k}\rightharpoonup\beta$. For any
$\gamma\in\mathcal P(\Omega)$,
