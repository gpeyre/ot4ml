...
A common abstraction is to keep the continuity equation but replace the quadratic cost $|m|^2/\rho$ by a convex local action. Let $I\subset[0,+\infty)$ be the admissible range of densities and let

```{math}
A:I\times\RR^d\to[0,+\infty]
```

be a lower-semicontinuous convex function of $(\rho,m)$, even in the momentum variable, with $A(\rho,0)=0$. For densities $\rho_0,\rho_1$ with the same total mass, define the dynamic value
