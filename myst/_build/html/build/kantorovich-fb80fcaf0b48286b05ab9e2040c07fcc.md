...
:::{admonition} Procedure: North-west corner coupling
:class: ot4ml-procedure

Set $\P=0$, residuals $r=\a$, $s=\b$, and indices $(i,j)=(1,1)$. While $i\leq n$ and $j\leq m$, set
...
\P_{ij}\leftarrow \eta,
\qquad
r_i\leftarrow r_i-\eta,\quad s_j\leftarrow s_j-\eta.
```

If $r_i=0$, increase $i$; if $s_j=0$, increase $j$. Return the sparse feasible coupling $\P$.
:::

:::{admonition} Procedure: Weighted one-dimensional sweep
:class: ot4ml-procedure

For one-dimensional inputs and a convex cost, first sort the atoms:
...
Apply the weighted one-dimensional sweep to the sorted weights $(\a_i)_i$ and $(\b_j)_j$. Return the resulting monotone coupling, i.e. the plan that always sends the leftmost remaining source mass to the leftmost remaining target mass.
:::

:::{admonition} Example: Unbounded convex sets may have no extreme point
:class: ot4ml-example

Compactness cannot be dropped from the extreme-point proposition. For instance, the closed convex set $\enscond{(x,y)\in\RR_+^2}{xy\geq1}$ is unbounded and has no extreme point.
:::

:::{admonition} Procedure: Birkhoff--von Neumann decomposition
:class: ot4ml-procedure

Given $P\in\mathcal B_n$, initialize the residual $R=P$ and an empty list. While $R\neq0$, find a permutation $\sigma$ supported by the positive entries of $R$,
...
and append the term $\lambda P_\sigma$ to the list. Update

```{math}
R\leftarrow R-\lambda P_\sigma .
```

The residual keeps equal row and column sums, and at each step at least one positive entry vanishes; hence the procedure terminates and returns

```{math}
P=\sum_r \lambda_r P_{\sigma_r},
\qquad
\sum_r\lambda_r=1.
```
:::
...
:::{admonition} Procedure: Displacement interpolation from a transport plan
:class: ot4ml-procedure

Solve the quadratic Kantorovich problem and choose an optimal plan $\pi^\star$. For each time $t\in[0,1]$, push this plan forward by

```{math}
e_t(x,y)=(1-t)x+t y.
```

The interpolated measure is

```{math}
\al_t=(e_t)_\sharp\pi^\star.
```

In the discrete case $\pi^\star=\sum_{i,j}P^\star_{ij}\delta_{(x_i,y_j)}$, this is simply

```{math}
\al_t=\sum_{i,j}P^\star_{ij}
\delta_{(1-t)x_i+t y_j}.
```

Thus each positive entry of the optimal matrix becomes one moving atom, and mass splitting in the plan becomes branching in the interpolation.
:::
...
The Memoli profile lower bound is precisely a Wasserstein-over-Wasserstein comparison of these laws of local profiles. It replaces the full pairwise distortion by an ordinary OT problem whose ground cost is itself a one-dimensional Wasserstein distance.

Note that there exist alternative distances which also metricize weak convergence. The simplest ones are Hilbertian kernel norms, detailed in the dual-norms chapter.
:::

:::{admonition} Procedure: Component-level interpolation of Gaussian mixtures
:class: ot4ml-procedure

Represent two Gaussian mixtures as discrete measures on Gaussian components,
...
Build the component cost

```{math}
C_{ij}=\norm{m_i-n_j}^2+\Bb(\Sigma_i,\Lambda_j)^2
```

and solve the discrete OT problem between the weights $a$ and $b$, obtaining $\Pi^\star$. For each active pair $(i,j)$, let $A_{ij}$ be the Gaussian Brenier linear part from $\Sigma_i$ to $\Lambda_j$ and set

```{math}
m_{ij,t}=(1-t)m_i+t n_j,
\qquad
\Sigma_{ij,t}=\big((1-t)\Id+tA_{ij}\big)\Sigma_i\big((1-t)\Id+tA_{ij}\big).
```

The component-level interpolant is the collapsed mixture
...
This follows the Wasserstein geometry of components, not necessarily the true $\Wass_2$ geodesic between the collapsed mixture densities.
:::
