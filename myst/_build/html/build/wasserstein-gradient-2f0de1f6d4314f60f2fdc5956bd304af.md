...
displacement convex exactly when the associated integrand satisfies McCann's
condition. If
$\d\beta=Z^{-1}e^{-V}\d x$, then
...
so $\KL(\alpha|\beta)$ is $\lambda$-geodesically convex when $V$ is
...
so it is the sum of the $0$-geodesically convex entropy and the
$\lambda_V$-geodesically convex linear potential generated by $V$.
:::
...
plus a linear potential. The KL divergence is the canonical exception with this
