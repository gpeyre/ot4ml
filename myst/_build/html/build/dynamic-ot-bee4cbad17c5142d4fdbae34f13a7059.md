...
an admissible velocity representative. It generates the length-space value
...
by an arbitrary discrepancy. Different choices of $\mathbb A$ change the
resulting geometry; {ref}`sec-generalized-dynamic-wasserstein-flows` later
reuses these choices when dynamics are introduced.

### Quadratic, or Riemannian, Tangent Actions

A particularly transparent case occurs when $w\mapsto\mathbb A(\alpha,w)$ is
quadratic. If its polarization is represented on the tangent space by a
positive self-adjoint operator $Q_\alpha:T_\alpha\to T_\alpha$,
...
then the infinitesimal expansion of the associated length distance reads

```{math}
:label: eq-generalized-distance-tangent-expansion
\mathsf D_Q^2\big((\Id+t w)_\sharp\alpha,\alpha\big)
=
...
The usual $\Wass_2$ geometry corresponds to $Q_\alpha=\Id$ on the
Wasserstein tangent space. Thus $Q_\alpha$ records how the chosen geometry
deforms the Euclidean $L^2(\alpha)$ tangent norm: no deformation for
...
### Local Velocity Actions and Momentum Variables
...
The same action can be written in momentum variables; it becomes a convex
formulation only when the perspective below is jointly convex. Set
$\mu=\alpha w$, so that $\mu$ is a vector-valued measure. At the measure level,
define
...
:::{admonition} Proposition: Convexity of the Momentum Perspective
:class: important
Let $I\subset[0,+\infty)$ be a convex interval and let
$A:I\times\RR^d\to[0,+\infty]$ be an extended-valued velocity action. Define
$J_A(a,m)=A(a,m/a)$ for $a>0$, and extend it to $a=0$ by lower semicontinuity.
Then $J_A$ is convex if and only if, for all $a_0,a_1>0$,
$w_0,w_1\in\RR^d$, and $\zeta\in[0,1]$, writing

```{math}
a_\zeta=(1-\zeta)a_0+\zeta a_1,
\qquad
w_\zeta=
\frac{(1-\zeta)a_0w_0+\zeta a_1w_1}{a_\zeta},
```
...
A(a_\zeta,w_\zeta)
\leq
(1-\zeta)A(a_0,w_0)+\zeta A(a_1,w_1).
```

In particular, $J_A$ is convex for $A(a,w)=aL(w)$ with $L$ convex, and for
$A(a,w)=a^2\norm w^2/\theta(a)$ with $\theta\geq0$ concave.
:::

:::{dropdown} Proof
Convexity of $J_A$ applied to $(a_i,m_i)=(a_i,a_iw_i)$ gives exactly the
mass-weighted inequality above. Conversely, given $(a_i,m_i)$ with $a_i>0$, set
$w_i=m_i/a_i$; the same inequality gives convexity of $J_A$ in the interior,
and lower semicontinuity gives the boundary cases. The two examples are the
usual perspective of a convex function and the concave-mobility perspective.
:::

(rem-generalized-bb)=
### Homogeneous Momentum Actions

The momentum formulation is often the cleanest way to prove metric properties,
because both the balance equation and the feasible set become convex. Let
$I\subset[0,+\infty)$ be a convex interval of admissible densities and let

```{math}
J:I\times\RR^d\to[0,+\infty]
```

be an extended-valued lower-semicontinuous convex function of $(a,m)$, even in
the momentum variable, with $J(a,0)=0$. For smooth densities $\rho_0,\rho_1$
with the same total mass, define

```{math}
:label: eq-homogeneous-momentum-action-value
\mathfrak A_J(\rho_0,\rho_1)
\eqdef
\inf_{\substack{\partial_t\rho_t+\nabla\!\cdot m_t=0\\
\rho_{t=0}=\rho_0,\ \rho_{t=1}=\rho_1}}
\int_0^1\!\int J(\rho_t(x),m_t(x))\,\d x\,\d t .
```

When $J=J_A$ is the perspective of a velocity action, this is the
momentum-variable form of the length construction above. The next proposition
isolates the assumptions under which the value defines a path metric.

(prop-homogeneous-dynamic-action-distance)=
:::{admonition} Proposition: Homogeneous Dynamic Actions Define Distances
:class: important
Assume, in addition to the hypotheses above, that for some $r>1$

```{math}
J(a,\xi m)=|\xi|^rJ(a,m)
\qquad
\text{for all }\xi\in\RR,
```

and that $J(a,m)=0$ if and only if $m=0$. Set

```{math}
\mathsf D_J(\rho_0,\rho_1)
\eqdef
\mathfrak A_J(\rho_0,\rho_1)^{1/r}.
```

On every fixed-mass class, and after the usual lower-semicontinuous relaxation
assuming that zero-action values are attained, $\mathsf D_J$ is an extended
distance on each finite-action component: it is symmetric, satisfies the
triangle inequality, and $\mathsf D_J(\rho_0,\rho_1)=0$ only when
$\rho_0=\rho_1$.
:::

:::{dropdown} Proof
Zero self-distance is obtained by the constant curve. Conversely, zero relaxed
action forces $J(\rho_t,m_t)=0$ a.e., hence $m_t=0$ a.e.; the continuity equation
then gives $\rho_0=\rho_1$. Symmetry follows by time reversal and evenness in
$m$. For the triangle inequality, concatenate two almost optimal curves with
...
Without homogeneity or nondegeneracy, $\mathfrak A_J$ remains a useful action
principle, but its $r$-th root need not define a distance.
...
and, for the squared local action associated with $d=\Wass_p$,
...
Using $J=J_p$ in {eq}`eq-homogeneous-momentum-action-value`, Proposition
{ref}`prop-homogeneous-dynamic-action-distance` gives the usual identity
$\mathfrak A_{J_p}^{1/p}=\Wass_p$. The endpoint $p=1$ can be treated separately:
$J_1(a,m)=\norm m$, and the dynamic problem collapses to Beckmann's formulation
of $\Wass_1$ {cite:p}`Beckmann52`.
:::
...
The crucial convexity is a perspective argument: since
$(s,m)\mapsto\norm m^2/s$ is convex and nonincreasing in $s>0$, the concavity of
$\theta$ makes $J_\theta(a,m)=\norm m^2/\theta(a)$ convex in $(a,m)$. The
associated value

```{math}
\mathsf W_\theta^2(\rho_0\d x,\rho_1\d x)
\eqdef
\mathfrak A_{J_\theta}(\rho_0,\rho_1)
```

is the quadratic case $r=2$ of Proposition
{ref}`prop-homogeneous-dynamic-action-distance`. The choice $\theta(\rho)=\rho$
recovers $\Wass_2$; choices such as $\theta(\rho)=\rho(1-\rho/M)$ encode
volume-filling effects. The induced squared tangent action is

```{math}
\mathbb A_\theta(\alpha,v)
=
...
$\rho/\theta(\rho)$ is finite and positive.
...
tangent action $\mathbb A_\gamma(\alpha,\cdot)$.
:::
...
$\Wass_\gamma(\alpha,(\Id+\tau v)_\sharp\alpha)^2=\tau^2\mathbb A_\gamma(\alpha,v)+o(\tau^2)$. The use of this geometry for normalized flows, including the operator-gauge connection with Muon-type normalization, is developed in {ref}`sec-normalized-spectral-wasserstein-dynamics`.
...
for $\alpha=\rho\mathfrak m$. This is the nonlocal analogue of a tangent action; here $v$ is not a vector field on $X$ but an
...
$v_t\in\RKHS_k^d$. This type of Stein geometry was introduced in the analysis
of SVGD by Liu and Wang {cite:p}`LiuWang2016SVGD,Liu2017SVGDGradientFlow`
...
is the mobility selected so that the entropy calculus later recovers exactly the
