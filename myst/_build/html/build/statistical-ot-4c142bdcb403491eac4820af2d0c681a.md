...
temperature; the computational question is how to run it when the empirical
supports are very large. A Sinkhorn iteration is cheap in arithmetic but
expensive in memory if one materializes the whole Gibbs kernel. Sketching
replaces this kernel by a positive low-rank factorization, so that each
Sinkhorn sweep costs linear time in the number of samples, up to the sketch
dimension.

### From a Kernel Matrix to a Nonnegative Factorization
...
The Sinkhorn scaling equations are
...
where divisions are componentwise. If $K$ admits a positive rank-$r$
approximation

```{math}
K\simeq \widetilde K=\Phi_X\Phi_Y^\top,
\qquad
\Phi_X\in\RR_+^{n\times r},
\quad
\Phi_Y\in\RR_+^{m\times r},
```

then the two matrix-vector products are evaluated as
...
Each scaling sweep therefore costs $O((n+m)r)$ operations and stores only
$O((n+m)r)$ numbers. The corresponding entropic plan is never formed as a
dense matrix; it is represented by the nonnegative factorization
...
LR^\top,
\qquad
L=\diag(u)\Phi_X,\quad R=\diag(v)\Phi_Y .
```

This kernel-sketching route to linear-time Sinkhorn iterations is
complementary to worst-case near-linear Sinkhorn analyses
{cite:p}`altschuler2017near`, to low-rank Gaussian-kernel approximations for
quadratic transport {cite:p}`AltschulerBachRudiWeed2018QuadraticTransport`,
and to factored-coupling models in which the coupling itself is constrained to
have low rank {cite:p}`scetbon2021lowrank`.

### Why Positivity and Logarithmic Accuracy Matter

A generic low-rank approximation is not enough. Sinkhorn repeatedly divides by
$Kv$ and, in dual form, applies soft transforms of the form
$-\epsilon\log(Kv)$. Negative entries are therefore forbidden, and tiny
negative artifacts from a signed sketch can destroy the iteration. Moreover,
the relevant precision is not only an additive Frobenius error on $K$. Since
the kernel encodes the cost through $c=-\epsilon\log k_\epsilon$, a
multiplicative approximation

```{math}
e^{-\delta}k_\epsilon(x,y)
\leq
...
corresponds to an additive cost perturbation bounded by $\epsilon\delta$.
Thus Sinkhorn is naturally sensitive to errors in $\log k_\epsilon(x,y)$, or
equivalently to relative errors on the positive kernel. This is the motivation
for positive random features in the linear-time Sinkhorn divergences of
Scetbon and Cuturi {cite:p}`ScetbonCuturi2020PositiveFeatures`.

### Positive Sketches for the Gaussian Kernel

For the quadratic cost $c(x,y)=\norm{x-y}^2/2$, the Gibbs kernel is the
Gaussian kernel
...
Draw independent frequencies
$\omega_1,\ldots,\omega_r\sim\mathcal N(0,\epsilon^{-1}\Id_d)$, and define
positive features
...
\eqdef
\frac1{\sqrt r}
\exp\!\left(\dotp{\omega_\ell}{x}-\frac{\norm{x}^2}{\epsilon}\right),
\qquad
\Phi(x)=(\varphi_1(x),\ldots,\varphi_r(x))\in\RR_+^r .
```

Then

```{math}
\mathbb E_\omega\big[\varphi_\omega(x)\varphi_\omega(y)\big]
=
\exp\!\left(-\frac{\norm{x-y}^2}{2\epsilon}\right),
\qquad
\widetilde k_\epsilon(x,y)
...
is a positive Monte-Carlo sketch of $k_\epsilon$. The factorization matrices
are simply
...
The positivity is exact for every draw, not only in expectation, and this is
what makes the sketch compatible with Sinkhorn divisions. In practice,
variance reduction, orthogonalized features and stabilization of log-domain
updates are important, especially when $\epsilon$ is small or the data have
large diameter.
...
relevant to entropic potentials. The analogy is useful because both settings
ask for scalable positive kernel algebra, but Sinkhorn is more fragile: each
iteration uses the approximate kernel inside a nonlinear normalization, not
just once inside an averaging operator.
