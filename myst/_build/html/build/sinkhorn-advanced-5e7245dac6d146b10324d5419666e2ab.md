...
diagnostic. Given empirical laws $\hat\alpha_n$ and $\hat\beta_m$, and an OT
functional $T$, one wants to identify the leading statistical bias
...
describes the random signed measure $\sqrt n(\hat\alpha_n-\alpha)$. Here the empirical
...
weights to transport cost is convex and piecewise affine. Sommerfeld and Munk
...
view of Bigot--Cazelles--Papadakis, Klatt--Tameling--Munk,
Hundrieser--Klatt--Munk and Mena--Weed
{cite:p}`bigot2017central,klatt2020empirical,hundrieser2021limit,mena2019statistical`. These
asymptotic results refine, rather than replace, the dimension-dependent bounds
of {ref}`sec-sample-complexity`.
...
$d'$-dimensional support, the canonical empirical-measure regimes are
high-dimensional order $n^{-p/d'}$ when $d'>2p$, a critical logarithmic
correction when $d'=2p$, and a faster low-dimensional regime when $d'<2p$; in
one dimension, for instance, smooth positive densities give the familiar
quantile-process asymptotics for $\Wass_2^2$ at order $n^{-1}$. Thus the
exponent $p/d'$ should be read as the high-dimensional matching scale, not as a
universal formula {cite:p}`dudley1969speed,fournier2015rate,weed2017sharp`. By
contrast, when $T(\alpha,\beta)$ is nonzero and the OT functional is
...
mean a large empirical bias, a non-smooth directional limit, or the need to
remove a regularization bias $T_\epsilon(\alpha,\beta)-T_0(\alpha,\beta)$; these
...
yields the displayed distributional limit. Since the feasible dual polyhedron
may be intersected with a bounded gauge slice, the support functions above have
at most linear growth in the multinomial fluctuation. Uniform integrability
therefore gives convergence of the first two moments, hence the bias and
variance statements. If $f^\star$ is unique modulo constants, the support
function reduces to $\dotp{f^\star}{G_a}$. Its expectation is zero, and the
...
are unique, the first-order bias is negligible and the asymptotic variance of
the unscaled estimator is
...
a Gaussian process over that face and need not itself be Gaussian; the leading
bias is then the expectation of this supremum divided by the square-root sample
size. This non-smoothness is one reason why inference for exact OT can be
delicate even on a finite space.
...
$(a,b)$ in the relative interiors of the simplices.
...
Since the first-order term has zero expectation and the Hessian is bounded
locally, Taylor's formula gives an $O(N^{-1})$ plug-in bias. The variance
convergence follows from the same expansion and bounded moments of the
multinomial fluctuation. The two-sample formula follows by applying the same
argument independently to the two marginals.
:::
...
$T_\epsilon$, the error has three conceptually distinct pieces:
...
This identity is often analyzed after choosing $\epsilon=\epsilon_n$. For fixed
$\epsilon$, the statistical bias is typically $O(n^{-1}+m^{-1})$ in finite
dimension, and the centered fluctuation is
$O_{\mathbb P}(n^{-1/2}+m^{-1/2})$. The regularization bias vanishes only when
$\epsilon\to0$, while the variance constants usually deteriorate as
$\epsilon\downarrow0$. This is the asymptotic version of the bias--variance
...
sufficiently regular dual potential $f^\star$, the empirical-process CLT and
the delta method give a first-order Gaussian limit whose one-sample variance is
$\operatorname{Var}_\alpha(f^\star(X))$. When differentiability fails, as can happen
for exact OT because the dual optimizer is not unique or because one studies
the degenerate self-distance $T(\alpha,\alpha)=0$, the limit may be non-Gaussian or
