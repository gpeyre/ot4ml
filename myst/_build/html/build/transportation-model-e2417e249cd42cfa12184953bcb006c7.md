...
Its original motivation is Bayesian sampling: given a target density
$\beta\propto e^{-V}$ known through its score $\nabla\log\beta=-\nabla V$,
evolve particles so that their empirical law approaches $\beta$ without
estimating the score of the current empirical law. Geometrically, this replaces
...
For a smooth vector field $v$ and the perturbation
$\alpha_\epsilon=(\Id+\epsilon v)_\sharp\alpha$, integration by parts gives

```{math}
\left.\frac{\d}{\d\epsilon}\KL(\alpha_\epsilon|\beta)\right|_{\epsilon=0}
=
-\int
\big(\dotp{\nabla\log\beta(x)}{v(x)}+\operatorname{div} v(x)\big)
\d\alpha(x).
```

Restricting $v$ to the unit ball of $\RKHS_k^d$ gives the representer formula
...
\Big(k(X_j^\ell,X_i^\ell)\nabla\log\beta(X_j^\ell)
+
\nabla_{X_j}k(X_j^\ell,X_i^\ell)\Big).
```

The first term attracts particles toward regions where the target score points
upward; the second term is a kernel repulsion that prevents immediate collapse.
