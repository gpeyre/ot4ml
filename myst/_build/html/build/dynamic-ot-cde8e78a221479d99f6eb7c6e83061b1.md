...
This is the same RKHS norm as in {ref}`sec-rkhs-mmd`, but applied to vector
fields instead of scalar test functions. The kernelized dynamic action is
...
and developed geometrically in {cite:p}`DuncanNueskenSzpruch2019SVGDGeometry,NueskenRenger2021SVGDAsymptotics`.

One should read this as an extended distance on finite-action components, not
as a replacement for $\Wass_2$ on all of $\Pp_2(\RR^d)$. If $k$ is smooth enough
