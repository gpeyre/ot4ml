...
distribution, are rarely geodesically convex in the transport geometry. This
is unfortunate for generative AI and generative modeling more broadly:
training a distribution to match data by decreasing
$\Wass_2^2(\alpha,\beta)$, $\SW_2^2(\alpha,\beta)$, or similar discrepancies
should not be expected to inherit the convex behavior of Euclidean least
squares.

The squared Wasserstein loss has the opposite one-sided curvature: it is
$2$-geodesically semiconcave along $\Wass_2$ geodesics. More precisely, if
$(\alpha_t)_{t\in[0,1]}$ is a constant-speed $\Wass_2$ geodesic, then
...
This is the standard $2$-geodesic semiconcavity convention for squared distances, often informally called "$2$-concavity": the graph may fall below the chord, but only by the quadratic correction $t(1-t)\Wass_2^2(\alpha_0,\alpha_1)$. Equivalently,
$-\Wass_2^2(\cdot,\beta)$ is $(-2)$-geodesically convex in the convention of
{ref}`def-geodesic-convexity`.

The sliced loss has the same flavor, but one must keep track of which geometry
is used. The projection of a $\Wass_2$ geodesic need not be the monotone
one-dimensional geodesic between the projected endpoint measures. The
preceding proof nevertheless applies after projection, using the projected
coupling induced by the optimal plan generating the $\Wass_2$ geodesic. With
the normalization of {ref}`def-sliced-wasserstein`, this gives, along the same
$\Wass_2$ geodesic,
...
$\int_{\Sphere^{d-1}}|\langle\theta,x_0-x_1\rangle|^2\d\sigma(\theta)
=\|x_0-x_1\|^2/d$ and the underlying coupling is $\Wass_2$-optimal. Thus this
is a semiconcavity estimate for the sliced discrepancy along $\Wass_2$
geodesics, with a $\Wass_2$-controlled correction. If one instead works in the
idealized Hilbert embedding by projected quantile functions, straight sliced
chords satisfy the exact identity with the last term
$\SW_2^2(\alpha_0,\alpha_1)$. The caveat, discussed in
{ref}`rem-sliced-length-not-wasserstein`, is that such straight chords need not
be realized by actual measures on $\RR^d$.
:::
...
but the velocity $v_\alpha$ is now the steepest-descent direction for the
geometry induced by $d$, not necessarily for $\Wass_2$. Unbalanced variants
replace this continuity equation by a balance law, while nonlocal variants
replace vector fields by jump fluxes.

The common infinitesimal object is a **local action**. Let $T_\alpha$ denote
the admissible tangent space at $\alpha$, for instance the $L^2(\alpha)$-closure
of gradients for the usual $\Wass_2$ geometry. Assume first that nearby
measures are generated by maps $(\Id+t w)_\sharp\alpha$, with
$w\in T_\alpha$, and that the squared discrepancy admits the expansion
...
$q_\alpha(w)=(\int\norm{w}^p\d\alpha)^{2/p}$; for spectral Wasserstein
distances it is the square of the spectral tangent norm; for local quadratic
geometries it is represented by an operator $Q_\alpha$.

Given a first-order direction $g\in T_\alpha$, usually
$g=\Wgrad f(\alpha)$, the local steepest-descent velocity associated with
$q_\alpha$ is formally
...
This is the infinitesimal minimization obtained by inserting
$\alpha=(\Id+\tau w)_\sharp\alpha_t$ in {eq}`eq-generalized-jko-step` and
keeping the terms of order $\tau$. The associated formal gradient-flow PDE is
...
When $q_\alpha$ is convex and $2$-homogeneous, $\mathcal J_\alpha(g)$ can be chosen on
the same descent ray as the normalized linear-minimization oracle

```{math}
:label: eq-local-action-lmo
...
The penalized map $\mathcal J_\alpha$ fixes the time scale, while the LMO fixes
only the direction and is the object used by normalized algorithms. If
$q_\alpha$ has zero-action directions, this statement is understood on the
quotient tangent space, or after projecting onto finite-action directions
modulo the kernel of $q_\alpha$. Along smooth solutions of
{eq}`eq-local-action-gradient-flow-pde`, whenever the minimizer in
{eq}`eq-local-action-steepest-descent` is attained, the same $2$-homogeneity
...
The restricted Riemannian case is obtained when $q_\alpha$ is quadratic. This
is the setting introduced in {ref}`sec-generalized-dynamic-wasserstein-distances`
around the quadratic tangent action {eq}`eq-general-quadratic-tangent-action`.
If the polarization of $q_\alpha$ is represented on $T_\alpha$ by a positive
...
Then {eq}`eq-local-action-steepest-descent` is a linear preconditioning rule:
...
Several geometries below can now be read from their local actions.
Concave-mobility quadratic actions, defined in the generalized mobility
construction of {ref}`rem-generalized-bb`, give, for $\alpha=\rho\d x$,
...
so $Q_\alpha$ is multiplication by $\rho/\theta(\rho)$, restricted to
admissible velocities. Dynamic spectral Wasserstein flows, based on
{eq}`eq-dynamic-spectral-wasserstein`, use
...
=
\gamma\!\left(\int vv^\top\d\alpha\right).
```

They fit the $Q_\alpha$-framework only for linear gauges
$\gamma(M)=\operatorname{tr}(AM)$, with $A\succeq0$. For nonlinear gauges, such
as $\gamma(M)=\lambda_{\max}(M)$, the flow is still described by
$q_{\gamma,\alpha}$ and its LMO, but no single linear preconditioner
$Q_\alpha$ represents the tangent geometry. WFR, studied separately in
{ref}`sec-wfr-gradient-flows`, has an analogous product local action, but its
tangent variable is $(v,g)$ rather than only $v$. Discrete Markov-chain
geometries and nonlocal logarithmic-mean geometries are also quadratic actions,
but on edge-flux or jump-flux variables rather than on vector fields in
$\RR^d$. Thus $q_\alpha$ is the common notation, while $Q_\alpha$ is available
only in Hilbertian vector-field subcases.

The generalized distances of {ref}`sec-generalized-dynamic-wasserstein-distances`
are useful precisely because they change what *steepest descent* means. The
energy functional can be the same, while the local quadratic metric tensor,
Finsler norm, mobility, graph structure, spectral gauge or jump kernel changes
the PDE selected by the minimizing movement. This section mirrors the
mass-preserving part of that dynamic-distance taxonomy: each subsection takes
one distance defined in {ref}`sec-dynamic-optimal-transport` and records the
corresponding formal gradient-flow equation or algorithmic interpretation. The
transport--reaction case is separated in {ref}`sec-wfr-gradient-flows`, because
there the tangent variable contains both a velocity and a mass-modulation field.
...
The spectral tangent norm {eq}`eq-normalized-spectral-tangent-norm`, and the
...
$g=\Wgrad f(\alpha)=\nabla\delta f(\alpha)$. This is the general local-action
rule {eq}`eq-local-action-steepest-descent` applied to
$q_{\gamma,\alpha}(v)=\mathcal N_{\gamma,\alpha}(v)^2$. When this direction is
nonzero, its descent ray is the LMO {eq}`eq-local-action-lmo`: after rescaling, it minimizes
$\int\dotp{g}{w}\d\alpha$ over $\mathcal N_{\gamma,\alpha}(w)\leq1$. In the
trace case, $\mathcal N_{\tr,\alpha}(v)^2=\int\norm{v}^2\d\alpha$ and
...
the same local action $q_{\gamma,\alpha}$ is obtained either from the dynamic
...
small-step limit of {eq}`eq-normalized-spectral-jko`, equivalently
{eq}`eq-local-action-gradient-flow-pde` with
$q_\alpha=q_{\gamma,\alpha}$, solves
...
```{math}
\ddot x_i(t)=v_{\alpha_t}(x_i(t)).
```

This is Newton's law with acceleration field $v_{\alpha_t}$. Thus first-order Wasserstein gradient flow reads the force as a velocity, while second-order momentum flow reads the same force as an acceleration. The price is that the state is no longer just the spatial measure $\alpha_t$; it is the phase-space law of position and velocity. In this limit the flow is Hamiltonian rather than dissipative whenever the force comes from a smooth interaction energy and $\gamma=0$.
:::
