...
A first, Riemannian-like, way to generalize the Benamou--Brenier formula is to
keep the continuity equation but change the infinitesimal norm on velocities.
Instead of measuring a tangent velocity $v$ at $\alpha$ by

```{math}
\int |v(x)|^2\,\d\alpha(x),
```

one prescribes an admissible tangent space $T_\alpha\subset
L^2(\alpha;\RR^d)$ and a positive self-adjoint operator
$Q_\alpha:T_\alpha\to T_\alpha$. This leads to the formal geodesic value
...
flows: changing $Q_\alpha$ changes the meaning of steepest descent. When
