...
### PCA in Linear OT Coordinates

Once the measures have been embedded in the Hilbert space
$L^2(\rho;\RR^d)$, standard Euclidean statistical tools can be applied to the
displacement fields. Given training measures $(\alpha_i)_{i=1}^N$, set
...
The empirical covariance operator on $L^2(\rho;\RR^d)$ is
...
and its leading eigenvectors $e_k$ define principal linear OT modes. The
score of $\alpha_i$ along mode $e_k$ is
$\dotp{z_i-\bar z}{e_k}_{L^2(\rho)}$, and a low-rank reconstruction with
coefficients $a=(a_1,\ldots,a_m)$ is obtained by pushing the reference
through
...
For small excursions around the mean displacement, this is a practical
...
coefficients, the map $T_a$ need not remain a Brenier map, so the resulting
curve should be interpreted as a linearized visualization rather than an
intrinsic Wasserstein geodesic. This LOT-PCA viewpoint was introduced for
image variability analysis in {cite:p}`wang2013linear` and developed in
transport-based signal analysis
{cite:p}`thorpe2017transportation,kolouri2017optimal`; it is complementary
to intrinsic Wasserstein principal-geodesic or geodesic-PCA approaches,
which optimize directly in the curved Wasserstein space
