...
the entropy limit. For a flat probability reference
$\beta=b\,\mathbf 1_\Omega\,\d x$ on a bounded convex domain, a
$\phi$-divergence reduces to an internal energy after rescaling and is
displacement convex whenever the associated integrand satisfies McCann's
condition. For nonuniform targets, no such conclusion holds for an arbitrary
$\phi$. The KL divergence is the main exception, because the logarithm separates
the density ratio into an entropy part and a linear potential part.
...
:::{admonition} Proposition: Examples of $\lambda$-Geodesic Convexity
:class: important
The following standard energies are $\lambda$-geodesically convex for the
indicated constants.

1. If $h$ is $\lambda_h$-strongly convex, then
   $f(\alpha)=\int h\d\alpha$ is $\lambda_h$-geodesically convex. The merely
   convex case corresponds to $\lambda_h=0$.
2. If $W$ is $\lambda_W$-strongly convex on $\RR^d\times\RR^d$ for the product
   Euclidean norm, then
   $f(\alpha)=\frac12\iint W(x,y)\d\alpha(x)\d\alpha(y)$ is
   $\lambda_W$-geodesically convex. If the energy is written with a
   nonsymmetric kernel, apply this statement to its symmetrized part.
3. Shannon entropy and all internal energies satisfying
   {ref}`thm-mccann-internal-energy` are $0$-geodesically convex on $\RR^d$.
4. Let $\Omega\subset\RR^d$ be a bounded convex domain and let
   $\beta=b\,\mathbf 1_\Omega\,\d x$, with $b=|\Omega|^{-1}$. If
   $g_b(r)=b\,\phi(r/b)$ satisfies McCann's criterion, then the
   $\phi$-divergence

   ```{math}
   \Divergm_\phi(\alpha|\beta)
   =
   \int_\Omega \phi\left(\frac{\rho}{b}\right)b\,\d x,
   \qquad
   \alpha=\rho\,\d x,
   ```

   is $0$-geodesically convex on the absolutely continuous finite-value class
   in $\Pp_2(\Omega)$.
5. If $\beta=Z^{-1}e^{-V}\d x$ and $V$ is $\lambda_V$-strongly convex, then
   $\KL(\alpha|\beta)$ is $\lambda_V$-geodesically convex. Its flow is the
   Fokker--Planck equation with invariant law $\beta$.
:::
...
Strong convexity of $W$ gives the additional term

```{math}
-\frac{\lambda_W}{2}t(1-t)
\left(\norm{X_1-X_0}^2+\norm{X_1'-X_0'}^2\right).
```

After integration over the product coupling and multiplication by the prefactor
$1/2$, this becomes
$-\frac{\lambda_W}{2}t(1-t)\Wass_2^2(\alpha_0,\alpha_1)$.

The internal-energy claim is {ref}`thm-mccann-internal-energy`. For a
flat-reference $\phi$-divergence, one simply rewrites

```{math}
\Divergm_\phi(\alpha|\beta)
=
\int_\Omega b\,\phi\left(\frac{\rho(x)}{b}\right)\d x
=
\int_\Omega g_b(\rho(x))\,\d x.
```

The convexity of $\Omega$ ensures that McCann interpolations between measures
supported in $\Omega$ remain supported in $\Omega$, so
{ref}`thm-mccann-internal-energy` applies to $g_b$. If $\phi$ has finite
recession slope, the usual lower-semicontinuous extension of the divergence
adds the matching singular recession term, so the same statement extends to the
corresponding finite-value class.

The nonflat KL case is Proposition
{ref}`prop-kl-gibbs-geodesic-convexity`.
:::

The $\phi$-divergence extension above is therefore automatic for flat
references, because the reference density is constant and the density ratio
only changes the internal-energy integrand. For a nonuniform reference
$\beta=q\,\d x$, the expression $\int q(x)\phi(\rho(x)/q(x))\d x$ usually
couples $x$ and $\rho$ nonlinearly and does not split into an internal energy
plus a linear potential. Proposition
{ref}`prop-kl-gibbs-geodesic-convexity` explains why the KL divergence is
the canonical exception with this simple decomposition.
...
viewpoint of Section {ref}`sec-geodesic-convexity`, the inequalities that
compare an energy gap with the squared Wasserstein slope are exactly
Wasserstein-PL inequalities, and therefore imply rates through Theorem
{ref}`thm-wasserstein-pl-convergence`. This section records a few representative
examples. To keep the presentation readable, the proofs are
...
space of probability measures is equipped with a metric, an extended distance
or a discrepancy $d$ compatible with the relevant topology, one can define a
...
interpolants of {eq}`eq-generalized-jko-step` converge, as $\tau\to0$, to a
curve of maximal slope. In the local mass-preserving examples considered below,
this limiting curve is again represented by a continuity equation of the type
...
For an energy $f$, the formal small-step expansion of the generalized JKO step
{eq}`eq-generalized-jko-step`, when the distance is generated by the
squared-speed action $\mathbb A$, selects the PMO direction
...
The static/dynamic equality for spectral Wasserstein distances implies that
the same local action $\mathbb A_\gamma$ is obtained either from the dynamic
path-length geometry or from the small-step expansion of the static distance
$\Wass_\gamma$. Thus the normalized dynamics can be obtained formally from the
general JKO scheme {eq}`eq-generalized-jko-step` with $d=\Wass_\gamma$.

For a small displacement $\alpha=(\Id+\tau v)_\sharp\alpha^k$, the local-action
expansion is
$\Wass_\gamma(\alpha^k,(\Id+\tau v)_\sharp\alpha^k)^2
=\tau^2\mathbb A_\gamma(\alpha^k,v)+o(\tau^2)$. Combining this with the
first-variation expansion of $f$ yields the local problem
{eq}`eq-local-action-steepest-descent`, specialized here to the spectral action
$\mathbb A_\gamma$.
...
small-step limit of the general JKO scheme {eq}`eq-generalized-jko-step` with
$d=\Wass_\gamma$, equivalently {eq}`eq-local-action-gradient-flow-pde` with
$\mathbb A=\mathbb A_\gamma$, solves
...
$v=\operatorname{PMO}_{\mathbb A_\gamma,\alpha_t}(\Wgrad f(\alpha_t))$, and the transport
...
$\psi:\Theta\times\RR^d\to\RR^d$, a data law $\rho$ on input-label
...
generally, let $F$ be a differentiable functional on measures
...
in the parameter variable. In the ResNet case one takes $F=f_{\mathrm{Res}}$.
