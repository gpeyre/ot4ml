...
The preceding non-example is not an accident. Losses of the form
...
is unfortunate for generative modeling: training a distribution to match data
by decreasing $\Wass_2^2(\alpha,\beta)$, $\SW_2^2(\alpha,\beta)$, or similar
discrepancies should not be expected to inherit the convex behavior of
Euclidean least squares.

The squared Wasserstein loss is instead $2$-geodesically semiconcave. More
precisely, if $(\alpha_t)_{t\in[0,1]}$ is a constant-speed $\Wass_2$
geodesic, then
...
This is the standard $2$-geodesic semiconcavity convention for squared distances, often informally called "$2$-concavity": the graph is allowed to bend upward, but it stays above the chord minus the quadratic correction $t(1-t)\Wass_2^2(\alpha_0,\alpha_1)$. Equivalently,
$-\Wass_2^2(\cdot,\beta)$ is $(-2)$-geodesically convex in the convention of
{ref}`def-geodesic-convexity`.

The sliced loss has the same flavor. With the normalization of
{ref}`def-sliced-wasserstein`, applying the same one-dimensional argument
after each projection $P_\theta(x)=\langle\theta,x\rangle$ yields, along the
same $\Wass_2$ geodesic,
...
because
$\int_{\Sphere^{d-1}}|\langle\theta,v\rangle|^2\d\sigma(\theta)=\|v\|^2/d$.
In the idealized Hilbert embedding by projected quantile functions, straight
sliced chords satisfy the exact analogue with the last term
$\SW_2^2(\alpha_0,\alpha_1)$. The caveat, discussed in
{ref}`rem-sliced-length-not-wasserstein`, is that such straight chords need
not be realized by actual measures on $\RR^d$.
:::

:::{dropdown} Proof of the Wasserstein estimate
Write
$\alpha_t=((1-t)P_0+tP_1)_\sharp\pi^\star$ with $\pi^\star$ optimal between
$\alpha_0$ and $\alpha_1$. Fix $t$, set $z=(1-t)x_0+tx_1$, take an optimal
coupling between $\alpha_t$ and $\beta$, and glue it with a disintegration of
$\pi^\star$ over $z$. This gives random variables $(X_0,X_1,Y)$ such that
$Z=(1-t)X_0+tX_1$ satisfies $(Z,Y)$ optimal between $\alpha_t$ and $\beta$,
while $(X_0,X_1)$ is optimal between $\alpha_0$ and $\alpha_1$. Since
...
repeating the same one-dimensional argument after projection and integrating
over directions.
:::
