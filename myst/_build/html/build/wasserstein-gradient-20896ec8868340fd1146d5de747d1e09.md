...
uses $(v,g)=(-\nabla\phi,-\phi/\kappa^2)$, which gives the PDE.
:::

The reaction term makes additive constants in the first variation meaningful:
adding a constant to $\phi$ changes the global growth rate. Thus unbalanced
flows are naturally written for energies on finite measures, not only on the
probability simplex. Rigorous constructions of Kantorovich--Fisher--Rao and
WFR gradient flows are developed in
{cite:p}`gallouet2017jko,2017-chizat-focm,2015-chizat-unbalanced`. Chizat's
measure-space gradient framework relates such unbalanced flows to mirror-type
descent on positive measures {cite:p}`Chizat2021GradientMethodsMeasures`. In
...
Then $\delta F(\alpha)=\log(\rho/b)$. The balanced $\Wass_2$ flow is the
Fokker--Planck equation
