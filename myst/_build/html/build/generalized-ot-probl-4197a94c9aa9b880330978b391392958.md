...
segments show the selected optimal bijection. The rank-one matrices
$A=-e_1e_1^\top$ and $A=-e_2e_2^\top$ only score horizontal or vertical
correlations. The matrix $A=-I$ gives the usual quadratic $\Wass_2$ assignment,
up to the marginal-only terms discussed below, while $A=+I$ reverses the
correlation and produces an anti-$\Wass_2$ matching.
...
different matrix $A$; the source atoms are red, the target atoms are blue, and
the gray segments give one deterministic optimal bijection.*
:::
