...
only on the two marginals and their KL-proximal maps are simple.
...
sense associated with $s\log s-s+1$; on probability couplings it coincides with
...
In finite dimension, with positive reference weights $\a\in\RR_+^n$,
$\b\in\RR_+^m$, and convex functions
...
:class: important
Assume $a_i,b_j>0$ and $\epsilon>0$. The Fenchel dual of the discrete
...
For the scaling form, fix $\mathbf v$ and set $z=\K\mathbf v$. Optimizing the
row block is equivalent, by the chain rule for KL, to minimizing
