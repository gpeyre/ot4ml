...
respect to the input law and to the ground cost. These two derivatives are the
two certificates of optimality: a Kantorovich potential and an optimal
coupling. The main caveat is non-uniqueness. Unregularized OT gives one-sided
directional derivatives, or equivalently subgradients in the measure variable
and supergradients in the cost variable. Entropic regularization removes the
plan non-uniqueness and, in finite dimension with positive marginals, gives
...
$\{\dotp{\ones}{\chi}=0\}$ and $\nabla_C\mathcal L_C(a,b)=P^\star$. Without
uniqueness, the exact directional derivative with respect to $C$ is the
minimum pairing with the perturbation over all optimal plans.
...
directional derivative is obtained by minimizing this pairing over the optimal
face, while any selected optimal plan gives a valid supergradient with respect
to the cost. This is the calculus behind ground-metric learning, which was
