...
panel shows its bipartite positive-support graph: red nodes are rows, blue
nodes are columns, thin purple edges correspond to $0<P_{ij}<1$, and bold black
edges correspond to isolated entries $P_{ij}=1$. The orange halo marks the
minimal $2\times2$ fractional cycle along which one can add and subtract mass
while preserving all row and column sums.*
:::
