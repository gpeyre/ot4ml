...
the entropy limit. For a flat reference $\beta$, a $\varphi$-divergence reduces
to an internal energy after rescaling and is displacement convex exactly when
the associated integrand satisfies McCann's condition. If
...
4. If $\beta=b\,\d x$ is a constant-density reference on a convex domain
   $\Omega$, and if $g_b(r)=b\,\phi(r/b)$ satisfies McCann's criterion, then the
   $\phi$-divergence
   ...
   is $0$-geodesically convex among probability measures supported in $\Omega$.
...
{ref}`thm-mccann-internal-energy` applies to $g_b$.
...
references, because it is an internal energy in disguise. For nonuniform
references, an arbitrary $\phi$-divergence does not usually split into an
internal energy plus a linear potential; the KL divergence is the canonical
exception with this simple decomposition.
