...
The metric ingredient needed below is the action $\mathbb A(\alpha,w)$ introduced in
...
below. For a local velocity action
$A(a,w)$, the corresponding $\mathbb A$ and momentum perspective $J_A$ are fixed
by {eq}`eq-local-velocity-action` and {eq}`eq-general-momentum-perspective`.
Thus ordinary $\Wass_2$ has $\mathbb A(\alpha,w)=\int\norm w^2\d\alpha$,
$\Wass_p$ uses the squared Finsler speed
$\mathbb A_p(\alpha,w)=(\int\norm w^p\d\alpha)^{2/p}$, and spectral geometries
use $\mathbb A_\gamma$ from {eq}`eq-spectral-tangent-action`.
...
Let $\mathbb A(\alpha,\cdot)$ be the local action at $\alpha$. For a vector
field $u\in L^2(\alpha;\RR^d)$, usually $u=\Wgrad f(\alpha)$, the penalized
minimization oracle associated with this action is
...
{eq}`eq-generalized-jko-step` selects the PMO direction
...
$1<p<+\infty$, the tangent action associated with the squared distance is
...
use the action $\mathbb A_\theta$ from {ref}`rem-generalized-bb`; spectral flows
...
space. Discrete Markov-chain geometries and nonlocal logarithmic-mean
geometries also have quadratic actions, but their tangent variables are edge
potentials or jump velocities rather than vector fields in $\RR^d$. WFR,
studied separately in {ref}`sec-dynamic-unbalanced-wfr-flows`, is another
quadratic geometry, but its tangent variable is a pair $(v,g)$ combining
displacement and mass modulation. The notation $Q_\alpha$ is reserved for
Hilbertian vector-field subcases where a linear preconditioner represents the
tangent geometry.

The generalized distances of {ref}`sec-generalized-dynamic-wasserstein-distances`
are useful precisely because they change what *descent* means. The
energy functional can be the same, while the local metric tensor, Finsler norm,
mobility, graph structure, spectral gauge, RKHS constraint or jump kernel
changes the PDE selected by the minimizing movement. The rest of this section
mirrors the mass-preserving part of that dynamic-distance taxonomy. The
transport--reaction case is separated in {ref}`sec-dynamic-unbalanced-wfr-flows`, because
there the tangent space is enlarged by a scalar growth direction.
...
:class: important
For $g\in L^2(\alpha;\RR^d)$, the spectral steepest-descent direction is the
PMO direction for the spectral action:

```{math}
:label: eq-normalized-spectral-steepest
\mathcal J_{\gamma}^{\alpha}(g)
\eqdef
\operatorname{PMO}_{\mathbb A_\gamma,\alpha}(g).
```
:::
...
algorithms keep the same descent ray and only change the scalar normalization;
equivalently, the rescaled field minimizes $\int\dotp{g}{w}\d\alpha$ over
...
(prop-normalized-spectral-steepest-polar)=
...
```{math}
:label: eq-normalized-spectral-steepest-explicit
...
{eq}`eq-local-action-steepest-descent`, specialized here to
{eq}`eq-normalized-spectral-steepest`.
...
Thus {eq}`eq-normalized-spectral-steepest` is, up to the harmless factor $1/n$, the finite-dimensional matrix constrained linear problem
