...
We assume that $\alpha_t$ is defined via a "projection" (in a loose sense) of a latent distribution $\pi \in \Pp(\RR^{d'})$, using an operator $P_t : \RR^{d'} \to \RR^d$ where $d' \gg d$, i.e. $$\label{eq-interp-coupling}
    \forall t \in [0,1], \quad \alpha_t := (P_t)_\sharp \pi.$$ A common case is $d'=2d$. We write $(x,y) \in \RR^{d'} = \RR^d \times \RR^d$ and assume $P_0(x,y)=x$ and $P_1(x,y)=y$, so that $\pi$ is a probabilistic coupling between $\alpha_0$ and $\alpha_1$, i.e. $\pi$ has marginals $(\alpha_0, \alpha_1)$. For instance, one can use $\pi = \alpha_0 \otimes \alpha_1$, the trivial coupling. An even more special case is the linear interpolation $P_t(x,y)=(1-t)x+ty$. With this linear path and an arbitrary coupling $\pi$, the regression below is the common core of flow matching and rectified flow: Lipman et al. emphasize conditional probability paths and simulation-free training of continuous normalizing flows, while rectified flow emphasizes straight couplings, reflow, and the possibility of reducing transport costs and discretization error {cite:p}`Lipman2022FlowMatching,Liu2023RectifiedFlow`. More complex constructions are also possible, provided that sampling from $\pi$ remains simple; stochastic interpolants allow additional latent variables and stochastic perturbations, thereby unifying deterministic flows, probability-flow ODEs and diffusion SDEs {cite:p}`Albergo2025StochasticInterpolants`.
...
```{code-cell} ipython3
:tags: [remove-input]
show_book_figure("generative-diffusion-versus-ot-2d-paths", width=760)
```
...
Choose a parametrized time-dependent field $v_\theta(t,z)$. During training, sample
...
and take a stochastic-gradient step on

```{math}
\norm{v_\theta(t,z)-w}^2.
```

After training, draw $X_0\sim\alpha_0$ and integrate the learned ODE
...
to obtain the generated sample $X_1$.
:::

:::{admonition} Remark: Changing the bridge speed does not restore optimality
:class: ot4ml-remark

The same terminal map labeled `eq-gaussian-product-fm-terminal-map` is obtained for any scalar schedule $Z_t=a_tX_0+b_tX_1$ with the same endpoints, because after whitening the covariance path remains $a_t^2\Id+b_t^2C$. Thus changing the speed of a scalar Gaussian bridge, for instance by using an OU schedule, cannot repair the non-optimality created by non-commuting covariances.
...
Let the data law be a Gaussian mixture and choose a scalar noising schedule
...
At each time, evaluate the mixture density $p_t$ of $Z_t$ and its score $s_t=\nabla\log p_t$ in closed form. The forward probability-flow velocity is
...
whenever $a_t\neq0$. To sample, draw terminal particles from the Gaussian endpoint and integrate the same ODE backward in time, using a small positive cutoff near singular data endpoints when necessary.
:::
...
At training step $k$, sample latent variables $z$ and generate particles $x=G_{\theta_k}(z)$. Estimate the current descent field

```{math}
w_k(x)=-\nabla\delta_\alpha\mathcal E_\beta(\alpha_{\theta_k})(x)
```

using the chosen discrepancy to the data law $\beta$. Fit a residual field $U_{\eta_k}$ by minimizing
...
Update the generator by composition with the residual step,
...
After training, sampling uses only the final generator evaluation $G_{\theta_K}(z)$.
:::
(alg:self-corrected-drifting-particles)=
:::{admonition} Algorithm: Self-corrected drifting particle update
:class: ot4ml-algorithm

Given particles $x_i^k$ representing $\mu_k$ and data samples from $\beta$, estimate
...
with the normalized kernel formula labeled `eq-normalized-kernel-drift`. Set
...
The data term attracts particles toward the target, while the self term repels them from their current smoothed concentration.
:::

:::{admonition} Example: Kernel drifting as a semi-relaxed divergence
:class: ot4ml-example

For the Gaussian-kernel drift labeled `eq-cross-minus-self-drift`, set
...
Then $u_t=\nabla\phi_t$, so Proposition labeled `prop-drifting-semi-relaxed-gradient` shows that kernel drifting is the Wasserstein gradient descent of
...
One may first normalize $b_t$ pointwise, for instance by $b_t/(\norm{b_t}+\eta)$, or globally by $\norm{b_t}_{L^2(\mu_t)}$, before this projection. Proposition labeled `prop-drifting-semi-relaxed-gradient` then applies to the projected field. Non-gradient components can still be useful in a parametric model, but they are not descent directions of a scalar functional for the $\Wass_2$ Riemannian metric.
:::
...
Represent a layer-$k$ context by the empirical token measure

```{math}
\alpha_k=\frac1n\sum_{i=1}^n\delta_{x_i^k}.
```

Compute the context-dependent attention velocity
...
With depth step $\tau=1/T$, update all tokens by
...
\alpha_{k+1}
=
(\Id+\tau\Gamma_{\theta_k}[\alpha_k])_\sharp\alpha_k.
```

As $T\to\infty$, this is the explicit-Euler discretization of the token-measure transport PDE.
:::
...
Proposition labeled `prop-gaussian-gradient-bullet-list` turns many standard energies into explicit affine fields:
...
Therefore a Gaussian law of neurons remains Gaussian. Its mean and covariance follow Proposition labeled `prop-gaussian-affine-closure`, with a matrix depending only on the current cross moment $Q_t$, a raw second-moment subblock that becomes a covariance subblock when the neuron law is centered. This exact closure is special to the linear activation; for nonlinear activations, Gaussian closures are usually projections rather than invariant families.
:::

:::{admonition} Example: Flow matching and diffusion paths between Gaussians
:class: ot4ml-example

Consider a prescribed Gaussian interpolation $\alpha_t=\Gaussian(\mean_t,\cov_t)$. Proposition labeled `prop-gaussian-affine-closure` shows that an exact flow-matching velocity can be taken affine:
...
the normalized field labeled `eq-normalized-kernel-drift` satisfies
...
Thus the drifting velocity labeled `eq-cross-minus-self-drift` is affine and preserves Gaussianity. With
...
then $\mathcal R\be_s=\be_s$, and Theorem labeled `thm-gelbrich-projection` gives $F(\mathcal R\mu)\leq F(\mu)$. Thus the moment-matched Gaussian projection of any barycenter is again a barycenter; when the barycenter is unique, it must itself be Gaussian. This is the contraction viewpoint behind Corollary labeled `cor-gaussian-discrete-barycenters`.
:::
