...
| $\simplex_n$ | Probability simplex of histograms of length $n$. | [Definition def-probability-simplex](monge#histograms) |
| $\de_x$ | Dirac mass at $x$. | [Definition def-discrete-measure](monge#discrete-and-empirical-measures) |
| $\al,\be,\ga$ | Source, target and auxiliary probability measures. | [Eq. eq-monge-continuous](monge#eq-monge-continuous-web) |
| $\density{\al}$ | Density of $\al$ with respect to a reference measure. | [Definition def-relative-density](monge#relative-densities) |
...
| $P_\sigma,\mathcal P_n^{\mathrm{perm}}$ | Permutation matrix and the set of all such matrices. | [Definition def-permutation-matrices](kantorovich#permutation-matrices-as-couplings) |
| $\mathcal B_n$ | Birkhoff polytope of bistochastic matrices. | [Definition def-birkhoff-polytope](kantorovich#permutation-matrices-as-couplings) |
...
| $\distD$ | Ground distance matrix for discrete Wasserstein distances. | [Definition def-discrete-wasserstein-distance](kantorovich#metric-properties-wasserstein-distances) |
| $\WassD_p(\a,\b)$ | Discrete $p$-Wasserstein distance. | [Definition def-discrete-wasserstein-distance](kantorovich#metric-properties-wasserstein-distances) |
...
| $\Bb(\cov_\al,\cov_\be)$ | Bures covariance distance. | [Definition def-bures-metric](monge#eq-bure-defn-web) |
...
| $\Couplings(\al,\be)$ | Set of couplings between $\al$ and $\be$. | [Eq. eq-coupling-generic](kantorovich#eq-coupling-generic-web) |
| $\MK_\c(\al,\be)$ | Kantorovich optimal value with ground cost $\c$. | [Eq. eq-mk-generic](kantorovich#eq-mk-generic-web) |
| $\dist$ | Ground distance on the underlying metric space. | [Eq. eq-defn-wass-dist](kantorovich#eq-defn-wass-dist-web) |
| $\Wass_p(\al,\be)$ | $p$-Wasserstein distance. | [Definition def-wasserstein-distance](kantorovich#metric-properties-wasserstein-distances) |
| $\Wass_\infty(\al,\be)$ | Worst-displacement Wasserstein distance. | [Eq. eq-wass-infty](kantorovich#eq-wass-infty-web) |
| $\mathfrak A,\mathfrak B$ | Probability laws over probability measures. | [Eq. eq-wow-parametric-law](kantorovich#wasserstein-over-wasserstein) |
| $\bar\alpha_{\mathfrak A}$ | Collapsed mixture associated with a law over measures. | [Definition def-collapsed-barycentric-mixture](kantorovich#wasserstein-over-wasserstein) |
| $\mathbb W_2$ | Wasserstein distance on the Wasserstein space. | [Eq. eq-wow-distance](kantorovich#wasserstein-over-wasserstein) |
...
| Notation | Meaning | First reference |
| --- | --- | --- |
| $\fD,\gD$ | Discrete dual potentials. | [Eq. eq-dual](dual#eq-dual-discrete-web) |
| $\f,\g$ | Continuous dual potentials. | [Eq. eq-dual-generic](dual#eq-dual-generic-web) |
| $\PotentialsD(\a,\b)$ | Feasible set of discrete dual potentials. | [Eq. eq-feasible-potential](dual#eq-feasible-potential-web) |
...
| $\Lip(f)$ | Lipschitz constant of $f$. | [Eq. eq-lip-constant](semidiscrete-w1#eq-lip-constant-web) |
