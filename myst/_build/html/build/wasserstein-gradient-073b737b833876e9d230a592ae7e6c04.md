...
### Metric Derivative and Maximal Slope

The same idea can be expressed without any differentiable structure. If
$(\mathcal X,d)$ is a metric space and $x_t$ is an absolutely continuous curve,
...
whenever this limit exists. For absolutely continuous curves it exists for
a.e. $t$ and is the smallest $L^1_{\rm loc}$ function $g$ such that
$d(x_s,x_t)\leq\int_s^t g(r)\d r$. For a lower semicontinuous functional $f$,
the metric slope is
...
A curve of maximal slope for $f$ is then an absolutely continuous curve that
satisfies the energy-dissipation inequality
...
{cite:p}`ambrosio2006gradient`. In $\mathcal X=\Pp_2(\RR^d)$ with
$d=\Wass_2$, smooth solutions of {eq}`eq-wassflow-pde` satisfy
$|\dot\alpha_t|_{\Wass_2}=\norm{v_t}_{L^2(\alpha_t)}$ and
$|\partial f|(\alpha_t)=\norm{\Wgrad f(\alpha_t)}_{L^2(\alpha_t)}$, so the
maximal-slope definition reduces to the velocity-field equation above. The
energy consequences of this viewpoint are used below in Section
