...
### Local Tangent Actions

The common infinitesimal object behind these dynamic distances is a local
action on tangent directions. Let $T_\alpha$ denote the admissible tangent
space at $\alpha$, for instance the $L^2(\alpha)$-closure of gradients for the
usual $\Wass_2$ geometry. Assume first that nearby measures are generated by
maps $(\Id+t w)_\sharp\alpha$, with $w\in T_\alpha$, and that a distance or
discrepancy $d$ has the second-order tangent expansion

```{math}
:label: eq-generalized-distance-tangent-form
q_\alpha(w)
\eqdef
\lim_{t\to0}
...
whenever this limit exists. The notation $q_\alpha$ is broader than a quadratic
form: it is the squared local norm, or more generally the squared local action,
selected by $d$. For instance, for $\Wass_p$ with $1\leq p<+\infty$, one has
$q_\alpha(w)=(\int\norm{w}^p\d\alpha)^{2/p}$; for the spectral distances
studied below it is the square of the spectral tangent norm; and for local
quadratic geometries it is represented by an operator $Q_\alpha$.

The Riemannian, or Hilbertian, special case occurs when $q_\alpha$ is
quadratic. If its polarization is represented on $T_\alpha$ by a positive
self-adjoint operator $Q_\alpha:T_\alpha\to T_\alpha$,
...
```{math}
:label: eq-generalized-distance-tangent-expansion
...
This is the metric counterpart of a Riemannian tensor on the space of measures.
Section {ref}`sec-generalized-dynamic-wasserstein-flows` uses the same
$q_\alpha$ to define steepest-descent directions and the corresponding
gradient-flow PDEs.

### Geodesic Distances from Quadratic Tangent Norms

The quadratic case of the local action can be integrated along curves to define
a geodesic distance. Keeping the continuity equation but replacing the usual
infinitesimal norm $\int\norm{v}^2\d\alpha$ by
$\langle Q_\alpha v,v\rangle_{L^2(\alpha)}$ leads to the formal geodesic value
...
The usual $\Wass_2$ geometry corresponds to $T_\alpha$ being the Wasserstein
tangent space and $Q_\alpha=\Id$. This viewpoint is useful later for gradient
flows: changing $Q_\alpha$ changes the meaning of steepest descent, as made
explicit in {eq}`eq-generalized-gradient-q-preconditioner`. When
$\alpha=\rho\d x$, $m=\rho v$, and $Q_\alpha$ acts by pointwise multiplication
with a symmetric positive matrix field $B_\rho(x)$, the integrand becomes
...
Thus local quadratic Riemannian metrics are a special case of the convex
action-density formalism below, with

```{math}
A_Q(\rho,m,x)=\frac{m^\top B_\rho(x)m}{\rho}.
```

For $B_\rho=\Id$, this reduces to the Benamou--Brenier density
$|m|^2/\rho$. The next paragraph uses the $(\rho,m)$ variables as the primitive
objects. This is less geometric but more robust analytically: convexity and
lower-semicontinuous relaxation become transparent, and nonquadratic, hence
non-Riemannian, Finsler-type actions are included as well.

(rem-generalized-bb)=
### General Mobilities and Convex Action Densities

The local action formalism keeps the continuity equation but replaces the
quadratic cost $|m|^2/\rho$ by a convex function of density and momentum. It
contains the quadratic $Q_\alpha$-construction above whenever $Q_\alpha$ is a
local multiplication operator, and it also covers infinitesimal lengths that
are not induced by a Hilbert norm. Let $I\subset[0,+\infty)$ be a convex
interval of admissible densities and let
...
For a smooth state $\alpha=\rho\d x$ and a velocity $v$, the associated
instantaneous action is

```{math}
:label: eq-local-action-density-induced
\ell_{A,\alpha}(v)
\eqdef
\int A(\rho(x),\rho(x)v(x))\,\d x .
```

If $A$ is $r$-homogeneous in the momentum and the distance is
$\mathsf D_A=\mathcal A_A^{1/r}$, then the squared tangent cost in the notation
of {eq}`eq-generalized-distance-tangent-form` is
$\ell_{A,\alpha}(v)^{2/r}$. Thus $\ell_{A,\alpha}$ and $q_\alpha$ coincide in
the quadratic case $r=2$, but not for a general $\Wass_p$-type Finsler action.
This convention keeps the metric definition here aligned with the
steepest-descent notation used later in
{ref}`sec-generalized-dynamic-wasserstein-flows`.
...
In the local-action notation of
{eq}`eq-generalized-distance-tangent-form`, this means
...
velocity variance. The associated dynamic action distance is
...
This functional is convex in $(\rho,m)$ by the matrix perspective, together
with the monotonicity and convexity of $\gamma$. It is nevertheless not, in
general, a pointwise action density of the form
$\int A(\rho(x),m(x))\,\d x$, because the covariance is computed globally
before applying $\gamma$. It becomes local only for linear spectral gauges. For
instance, if $\gamma(M)=\operatorname{tr}(GM)$ with $G\succeq0$, then the
spectral action is the integral of the quadratic density
...
Equivalently, the static spectral distance is the distance generated by the
tangent norm $\mathcal N_{\gamma,\alpha}$.
:::
...
The mechanism is the same as in the finite-state Markov-chain construction recalled at the end of this section. A graph
...
This construction therefore does not fit the pointwise local action-density
formalism $A(\rho(x),m(x))$ used above. Its action is still quadratic, but the
tangent variable is an antisymmetric jump velocity $v(x,y)$, and the mobility
depends simultaneously on the two endpoint densities $\rho(x)$ and $\rho(y)$.
It is best viewed as a convex action on the pair space $X\times X$, rather than
as an integral of independent costs attached to single base points $x$.
...
which is the nonlocal squared tangent action $q_{K,\alpha}(v)$ for
$\alpha=\rho\mathfrak m$. Here $v$ is not a vector field on $X$ but an
...
Nonnegativity is immediate from the definition of $\mathcal A_K$. If
...
$\alpha_2$ with action $A_1$. For $0<\lambda<1$, concatenate the two curves by
...
after time rescaling. Since $\mathcal A_K$ is quadratic in $v$, the concatenated
action is
...
Optimizing in $\lambda$, for instance taking
$\lambda=\sqrt{A_0}/(\sqrt{A_0}+\sqrt{A_1})$ when both actions are positive,
...
The nonlocal entropy-flow consequences and fractional PDE examples are developed
in {ref}`sec-nonlocal-wasserstein-pdes`.
:::
...
in {ref}`sec-rkhs-mmd`. The kernelized dynamic action between two probabilities is defined by
...
with endpoint constraints $\alpha_{t=0}=\alpha_0$ and
$\alpha_{t=1}=\alpha_1$. This type of Stein geometry was introduced in the
analysis of SVGD by Liu and Wang {cite:p}`LiuWang2016SVGD,Liu2017SVGDGradientFlow`
...
$q_{k,\alpha}(v)=\norm{v}_{\RKHS_k^d}^2$, with the important caveat that the
admissible tangent space is the smooth RKHS class, not the whole Wasserstein
tangent space.

One should read the square root of {eq}`eq-kernelized-bb-distance` as an
extended distance on finite-action components, not as a replacement for
$\Wass_2$ on all of $\Pp_2(\RR^d)$. If $k$ is smooth enough
that $\RKHS_k^d$ embeds into Lipschitz vector fields, finite-action curves are
induced by regular flows. Atomic measures remain atomic with the same number of
atoms, so a Dirac mass cannot be transported at finite kernelized action to a
measure with a density. This lack of splitting is precisely what makes the
geometry useful for deterministic particle methods, and also what makes it a
nontrivial extended object rather than a full probability metric.
...
This is the finite-state squared tangent action $q_{K,\rho}(\psi)$; the tangent
