...
The Monge--Ampere operator is a nonlinear, degenerate elliptic analogue of the
Laplacian: its linearization gives the weighted Poisson equation of
Proposition {ref}`prop-linearized-monge-ampere`, but the full operator
$\phi\mapsto\det(\nabla^2\phi)$ is elliptic only on convex potentials. This is
the central numerical difficulty. One cannot discretize the determinant of the
Hessian by arbitrary centered differences and expect convergence; the scheme
must encode convexity, the second boundary condition, and a consistent
degenerate-elliptic approximation of the operator. Different approaches include
Newton solvers for the transport boundary condition, monotone wide-stencil
finite differences, variational discretizations and power-diagram-inspired
schemes. Representative references are
{cite:p}`Loeper:2005fn,Benamou:2014jw,froese2011convergent,benamou2016monotone,mirebeau2015discretization,sulman2011efficient`.
We do not develop these schemes here, but the point to remember is that the
main obstacle is the construction of a consistent discretization of this
nonlinear Laplacian-like operator.
:::
