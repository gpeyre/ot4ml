...
measurable near-optimal selections when the minimum is not attained.
...
Equivalently, this is the product-space transport problem in which the first
coordinate is copied rather than transported: mass can move in the parameter
variable $\theta$, but transport between two different conditions is forbidden,
or assigned infinite cost.
...
where each slice is the neuron law of a two-layer mean-field block with the
feature $\psi(\theta,z)$ defined above. The forward pass is the controlled ODE
...
variation of $f_{\mathrm{Res}}$. A finite ResNet discretizes $s$ into layers
and each conditional law $\alpha_s$ into finitely many neurons; the conditional
metric is the continuum limit of the Euclidean training metric that moves
parameters inside each layer but does not permute mass between different
depths. In this sense, the shallow mean-field flow above is the one-fiber case,
while infinitely deep ResNets require a continuum of coupled Wasserstein flows
indexed by depth.
