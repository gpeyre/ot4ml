...
The preceding non-example is a useful warning. Losses of the form
...
{eq}`eq-wasserstein-gradient-flow-length`. Non-coercivity of the objective
means that sublevel sets need not be compact, but an energy-dissipating curve
still cannot travel an infinite $\Wass_2$-distance in finite time without an
infinite energy drop. The PL and convexity estimates below add the missing
quantitative information that forces convergence toward minimizers.
