...
$\Q=\K\operatorname{diag}(\mathbf v)$ and $z=\Q\ones_m=\K\mathbf v$. Updating
the row scaling means looking for $\P=\operatorname{diag}(\mathbf u)\Q$, so its
row marginal is $r=\P\ones_m=\mathbf u\odot z$. The row-wise chain rule gives

```{math}
\KLD(\operatorname{diag}(\mathbf u)\Q|\Q)
=
