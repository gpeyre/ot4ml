...
heat kernel with $t=\epsilon/4$. A related Laplace-type kernel is obtained from
the shifted-Laplacian resolvent: informally, the kernel of $(I+\tau L)^{-1}$
decays like $\exp(-d(x,y)/\sqrt{\tau})$, whereas the heat kernel decays like
$\exp(-d(x,y)^2/(4t))$. On a Riemannian manifold or surface $M$, Varadhan's
formula gives
...
A single inverse shifted Laplacian gives a rough Laplace-type kernel approximation,
while repeated resolvent steps approximate the Gaussian heat kernel. On a mesh
or grid, the limit is delicate: if the regularization scale goes to zero faster
than the grid size, metrication artifacts and inconsistent schemes can dominate
the geodesic limit.
...
*Geodesics-in-heat approximation of the distance to three source points. Larger
heat times stabilize the linear solve but visibly round and merge level-set
features.*
:::
...
The entropic version is the Gaussian log-sum-exp formula
