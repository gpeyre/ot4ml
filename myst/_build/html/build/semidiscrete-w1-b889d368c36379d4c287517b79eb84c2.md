...
**If** $\max_j\abs{m_j^{(k)}-\b_j}\leq\mathrm{tol}$, **return** $\gD^{(k+1)}$ and the cells.
:::
...
**End for**; **return** the last iterate or an averaged iterate.
:::
...
**If** a cell is empty, keep or reseed its codepoint.
...
**Add** it to the tree; identify the unique cycle.

**Send** the largest admissible flow around the cycle.

**Remove** the leaving arc that becomes tight.

**Update** the tree, potentials, and reduced costs.

**End while**; **return** the final transshipment flow $u$.
:::
