...
**If** $r_i=0$, set $i\leftarrow i+1$.

**If** $s_j=0$, set $j\leftarrow j+1$.
...
**Return** the resulting monotone coupling $\P$.
:::
...
**Initialize:** Set $R=P$ and start an empty list $\mathcal L$.
...
**Solve** the quadratic Kantorovich problem; choose $\pi^\star$.

**Set** the interpolation map:
...
**If** $\pi^\star=\sum_{i,j}P^\star_{ij}\delta_{(x_i,y_j)}$, compute equivalently
...
**Input:** Gaussian mixtures $\sum_i a_i\Gaussian(m_i,\Sigma_i)$ and $\sum_j b_j\Gaussian(n_j,\Lambda_j)$; time $t\in[0,1]$.

**Output:** Component-level interpolated mixture $\bar\alpha_t$.
...
**Solve** the discrete OT problem between weights $a$ and $b$; store $\Pi^\star$.

**For** each active pair $(i,j)$ **do**:

**Compute** the Gaussian Brenier linear part $A_{ij}$ from $\Sigma_i$ to $\Lambda_j$.
