...
If the masses $\mathbf m=(m_1,\ldots,m_p)$ are fixed, a convenient notation is

```{math}
:label: eq-multispecies-space
\Pp_{2,\mathbf m}(\RR^d;\RR_+^p)
=
\left\{
...
The simplest geometry is the product Wasserstein metric

```{math}
:label: eq-multispecies-product-metric
\Wass_{2,\oplus}^2(\alpha,\beta)
=
\sum_{i=1}^p\Wass_2^2(\alpha_i,\beta_i),
```

which transports each species independently but allows the energy to couple
them. The corresponding JKO step for a functional $F$ is

```{math}
:label: eq-multispecies-jko
\alpha^{k+1}
\in
\underset{\alpha\in\Pp_{2,\mathbf m}(\RR^d;\RR_+^p)}{\operatorname{argmin}}
...
enters through the first variations. Multi-species Wasserstein systems and
related displacement properties are studied for instance in
{cite:p}`CarlierChizatLabordeDisplacementSmoothness`; congestion and
shared-density variants are closely related to crowd and traffic models
...
density is prescribed. The constrained JKO step is obtained from
{eq}`eq-multispecies-jko` by restricting the minimization to
$\mathcal C_\beta$. In the smooth-density regime on a domain $\Omega$, the
formal constrained flow has a common pressure $\lambda_t$:
...
Lagrange multiplier enforcing $\sum_i\rho_i=b$ at all times. For the separable
Shannon entropy
...
=
\int\left(\rho\log\frac{\rho}{b}-\rho+b\right)\d x,
\qquad
\alpha=\rho\d x.
```

This is the Csiszár divergence on positive measures; it is nonnegative and is
minimized, without imposing a mass constraint, at $\alpha=\beta$. Its first
variation is $\delta F(\alpha)=\log(\rho/b)$. The balanced $\Wass_2$ flow,
when the total mass is fixed, is the Fokker--Planck equation
...
=
\Delta\rho_t-\operatorname{div}(\rho_t\nabla\log b),
```
...
The first two terms transport and diffuse mass, while the last term kills mass
where $\rho_t>b$ and creates it where $\rho_t<b$.
:::
