...
velocity variance. The associated dynamic distance is
...
{cite:p}`peyre2026muon`, shows that this path-length definition is not only
infinitesimal: it coincides with the static displacement-covariance formulation.
...
$\alpha_0,\alpha_1\in\mathcal P_2(\RR^d)$ be compactly supported. Then
...
Equivalently, the static spectral distance is the length distance generated by
the tangent norm $\mathcal N_{\gamma,\alpha}$.
:::
...
Conversely, let $(\alpha_t,v_t)$ be a finite-action competitor. By the
superposition principle, or by the classical flow in the smooth case, it is
represented by absolutely continuous paths satisfying
$\dot\gamma_t=v_t(\gamma_t)$. For the endpoint coupling
$\pi=(\gamma_0,\gamma_1)_\sharp M$, Jensen along each path gives

```{math}
(\gamma_1-\gamma_0)(\gamma_1-\gamma_0)^\top
\preceq
\int_0^1\dot\gamma_t\dot\gamma_t^\top\d t .
```

After integration over the path law, $M_\pi\preceq\int_0^1 C_t\d t$. Therefore
monotonicity, convexity and $1$-homogeneity of $\gamma$ imply
