...
and that boundary terms vanish. With the no-$1/2$ convention of
{eq}`eq-benamou-brenier-convex`, the convex dynamic value has the dual
formulation
...
with the usual perspective convention. Thus every dual feasible potential
gives a lower bound on every primal feasible pair. Conversely, introducing
$\phi$ as a Lagrange multiplier for
$\partial_t\rho+\operatorname{div}m=0$ and minimizing pointwise over $m$
gives $m=\rho\nabla\phi/2$ and the finite-dual condition
$\partial_t\phi+\norm{\nabla\phi}^2/4\leq0$. Fenchel--Rockafellar duality
then gives no duality gap, and equality in the two pointwise inequalities
gives {eq}`eq-bb-primal-dual-relation`.
:::

The inequality in {eq}`eq-benamou-brenier-dual` is the Hamilton--Jacobi
equation associated with the Hamiltonian $H(p)=\norm{p}^2/4$. On the
transported mass, equality holds and the characteristics satisfy
...
density, the notebook checks $m_t=\rho_t\partial_x\phi_t/2$ and
