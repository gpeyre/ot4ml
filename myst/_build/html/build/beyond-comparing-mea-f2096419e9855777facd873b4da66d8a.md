...
:::{admonition} Procedure: Entropic Gromov--Wasserstein linearization
:class: ot4ml-procedure

Initialize $\P^{(0)}\in\CouplingsD(\a,\b)$. For $k=0,1,\ldots$, form the linearized cost
...
Then solve the entropic OT subproblem
...
using Sinkhorn scaling. Iterate until the coupling or GW objective stabilizes.
:::
...
:::{admonition} Procedure: Exact quantum Bregman projections
:class: ot4ml-procedure

Initialize Hermitian potentials $F^{(0)}$ and $G^{(0)}$, and set
...
At an $A$-projection step, solve the Hermitian equation

```{math}
\operatorname{Tr}_B T_e(F^+,G^{(k)})=A
```

and set $F^{(k+1)}=F^+$. At a $B$-projection step, solve

```{math}
\operatorname{Tr}_A T_e(F^{(k+1)},G^+)=B
```

and set $G^{(k+1)}=G^+$. Repeat these two exact block projections until both partial traces match, then return $T_e(F^{(k)},G^{(k)})$.
:::

:::{admonition} Procedure: Gurvits/operator scaling for quantum Sinkhorn
:class: ot4ml-procedure

Initialize $U,V\succ0$. Alternately normalize the two operator marginals:
...
Return the symmetrically scaled coupling $T_s=(U\otimes V)K(U\otimes V)$.
:::
