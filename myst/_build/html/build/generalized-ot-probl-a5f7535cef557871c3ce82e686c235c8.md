...
Low-rank OT reduces the size of a transport plan by forcing it to pass through a
small latent measure. This is useful when the coupling is expected to be
organized by a few hidden clusters or prototypes, and it gives a different
approximation principle from low-rank kernel approximations inside Sinkhorn
iterations. The idea was introduced statistically through factored couplings by
Forrow, Hütter, Nitzan, Rigollet, Schiebinger and Weed
{cite:p}`forrow2019factored`, and developed algorithmically for arbitrary costs
by Scetbon, Cuturi and Peyré {cite:p}`scetbon2021lowrank`.
...
where zero columns are discarded before applying the formula.
:::
...
given $Z$. In geometric language, OT is replaced by a succession of two
transports through an intermediate measure with only $r$ Dirac masses.
...
therefore the minimum, over triples satisfying Definition
{ref}`def-low-rank-couplings`, of
...
common entropic variant adds entropy to the two sub-couplings,
...
simplest way to see the mechanism and is the one used in Figure
...
6. Stop when the objective change is below $\mathrm{tol}$.
:::
