...
<iframe class="ot4ml-live-frame" title="Sinkhorn convergence controls" src="../live/sinkhorn-advanced-convergence.html" loading="lazy" style="width:100%;height:500px;border:0;display:block;"></iframe>
...
$\alpha\otimes\beta$. This is the standard $\Gamma$-convergence mechanism for
entropic OT {cite:p}`leonard2012schrodinger,2017-carlier-SIMA`; the
finite-entropy approximation hypothesis holds, for instance, for
Lebesgue-density marginals on compact Euclidean domains after smoothing and
marginal correction.

(prop-continuous-convergence-epsilon-web)=
...
perturbation of the product coupling. Let $r=\alpha\otimes\beta$ and define
...
With the gauge $\int g_\epsilon\,\d\beta=0$, the corresponding potentials
satisfy

```{math}
f_\epsilon(x)=\bar c_\X(x)+O(\epsilon^{-1}),
\qquad
g_\epsilon(y)=\bar c_\Y(y)-\bar c+O(\epsilon^{-1}).
```
:::

The coefficient $c_0$ is the part of the cost with zero conditional means. It
is precisely the component that cannot be hidden inside row and column
potentials.
...
and a Fisher-information term along the McCann interpolation
{cite:p}`ConfortiTamanini2021EntropicDerivative,ChizatRoussillonLegerVialardPeyre2020Sinkhorn`.

(prop-small-epsilon-expansion-web)=
...
$\int g_\epsilon\,\d\beta=\int g_0\,\d\beta=0$, then under the same
non-degenerate Laplace assumptions
...
locally uniformly, with $O(\epsilon)$ errors away from degeneracies.
:::
