...
$(\mathbf f_n^\epsilon,\mathbf g_n^\epsilon)\in\RR^n\times\RR^m$, with
...
$P^\epsilon=(P_{ij}^\epsilon)$ be the empirical entropic coupling and let
$\mathbf g_n^\epsilon$ be the target-side dual potential. Define
...
\sum_j b_j
\exp\left(
\frac{\mathbf g_{n,j}^\epsilon-c_2(x,Y_j)}{\epsilon}
\right),
\qquad
T_n^\epsilon(x)=x-\nabla u_n^\epsilon(x).
```

Equivalently, after subtracting the quadratic marginal terms, one works with
the bilinear cost $c_{\rm ip}(x,y)=-\langle x,y\rangle$, and the map is the
gradient of a soft convex conjugate. The explicit soft barycentric form is

```{math}
T_n^\epsilon(x)=\sum_j\omega_j^\epsilon(x)Y_j,
\qquad
\omega_j^\epsilon(x)=
\frac{b_j e^{(\mathbf g_{n,j}^\epsilon-c_2(x,Y_j))/\epsilon}}
{\sum_k b_k e^{(\mathbf g_{n,k}^\epsilon-c_2(x,Y_k))/\epsilon}} .
```

At a sampled point $X_i$, Sinkhorn normalization gives

```{math}
T_n^\epsilon(X_i)=\frac1{a_i}\sum_jP_{ij}^\epsilon Y_j,
```
...
optimal unregularized empirical plan.
...
map from $\alpha$ to $\beta$. If uniform empirical measures
...
inequality applied to each conditional law turns convergence of plans into
convergence of barycentric projections in empirical $L^2$.
:::
...
with $\bar s=s\wedge3$ {cite:p}`pooladian2021entropicOTMaps`. Fixed
$\epsilon$ gives a parametric empirical fluctuation for the entropic map,
whereas $\epsilon\simeq n^{-1/(d+\bar s+1)}$ gives a convergent estimator of
the Brenier map.
