...
<span id="prop-discrete-product-coupling-degenerate"></span>
...
<span id="alg-north-west-corner"></span>
...
<span id="prop-extreme-point-existence"></span>
...
<span id="prop-linear-program-extreme-minimizer"></span>
...
<span id="thm-birkhoff-von-neumann"></span>
...
panel shows its bipartite positive-support graph, with the column nodes ordered
for readability: red nodes are rows, blue nodes are columns, thin purple edges
correspond to $0<P_{ij}<1$, and bold black edges correspond to isolated entries
$P_{ij}=1$. The orange halo marks the minimal $2\times2$ fractional cycle along
...
<span id="cor-kantorovich-matching"></span>
...
<span id="cor-monge-kantorovich-brenier"></span>
...
<span id="def-discrete-wasserstein-distance"></span>
...
<span id="def-wasserstein-distance"></span>
...
(prop-metric-measure)=
<span id="prop-metric-measure"></span>
...
<span id="prop-kantorovich-relaxation-monge"></span>
...
<span id="prop-wass-metrizes-weak-compact"></span>
...
<span id="prop-wasserstein-space-polish"></span>
...
how fast. As recalled in Remark {ref}`rem-clt`, the central limit theorem first
appears as a qualitative weak$^*$ convergence of the rescaled convolution laws
toward a Gaussian.
The next result is its quantitative counterpart: the same convergence becomes a
rate estimate in $\Wass_1$, which controls the error of all $1$-Lipschitz
observables of the normalized sum.
