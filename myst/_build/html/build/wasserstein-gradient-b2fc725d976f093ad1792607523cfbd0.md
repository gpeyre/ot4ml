...
geometry induced by $d$, not necessarily for $\Wass_2$. Unbalanced and nonlocal
variants replace this continuity equation by the corresponding balance law or
nonlocal continuity equation.

The following comparison with the usual Wasserstein gradient applies only to
the local quadratic, or restricted Riemannian-type, case introduced in
{ref}`sec-generalized-dynamic-wasserstein-distances` around the quadratic
tangent action {eq}`eq-general-quadratic-tangent-action`. The point is
infinitesimal: the discrepancy $d$ must have, along advective perturbations,
the same kind of second-order expansion as the $Q_\alpha$-metric. This is more
restrictive than the metric construction {eq}`eq-generalized-jko-step`: many
dynamic distances below are Finsler-like or use non-vectorial tangent
variables. In the advective case, let $T_\alpha$ denote the admissible tangent
space at $\alpha$, for instance the $L^2(\alpha)$-closure of gradients for the
usual $\Wass_2$ geometry. Assume that nearby measures are generated by maps
$(\Id+t w)_\sharp\alpha$, with $w\in T_\alpha$, and that the squared
discrepancy has a second-order expansion. Define
...
whenever this limit exists. If $q_\alpha$ is a quadratic form, its polarization
defines a bilinear form $q_\alpha(w,z)$. When this bilinear form is represented
on $T_\alpha$ by a positive self-adjoint operator
$Q_\alpha:T_\alpha\to T_\alpha$,
...
If $f$ has Wasserstein first variation $\Wgrad f(\alpha_t)$, then expanding
{eq}`eq-generalized-jko-step` with
$\alpha=(\Id+\tau v)_\sharp\alpha_t$ gives, after removing the constant term
and dividing by $\tau$,

```{math}
\min_v
\frac12\left\langle Q_{\alpha_t}v,v\right\rangle_{L^2(\alpha_t)}
+
\left\langle \Wgrad f(\alpha_t),v\right\rangle_{L^2(\alpha_t)}.
```

The Euler equation of this quadratic problem gives the formal descent velocity
...
recovers $v_\alpha=-\Wgrad f(\alpha)$. Concave-mobility quadratic actions,
defined in the generalized mobility construction of {ref}`rem-generalized-bb`,
also fit this template: if $\alpha=\rho\d x$ and
$A_\theta(\rho,m)=|m|^2/\theta(\rho)$, then the tangent norm of the advective
velocity $v=m/\rho$ is
...
admissible velocities. WFR, defined by the dynamic unbalanced formulation
{eq}`eq-dynamic-unbalanced-ot` and studied separately in
{ref}`sec-wfr-gradient-flows`, has an analogous product Riemannian
structure, but its tangent variable is $(v,g)$ rather than only $v$; the block
metric is $\int(\norm{v}^2+\kappa^2g^2)\d\alpha$, so the growth component is
weighted by $\kappa^2$. Discrete Markov-chain geometries, defined in
{ref}`sec-discrete-wasserstein-markov` and studied as flows in
{ref}`prop-discrete-markov-entropy-gradient`, and nonlocal logarithmic-mean
geometries, defined by {eq}`eq-nonlocal-wasserstein-distance` and studied as
flows in {ref}`sec-nonlocal-wasserstein-pdes`, are also quadratic, but in
edge-flux or jump-flux variables rather than in vector fields on $\RR^d$.
Dynamic spectral Wasserstein flows, based on the distance
{eq}`eq-dynamic-spectral-wasserstein` and developed in
{ref}`sec-normalized-spectral-wasserstein-dynamics`, fit the same
$Q_\alpha$-framework exactly for linear gauges
$\gamma(M)=\operatorname{tr}(AM)$, with $A\succeq0$: then $Q_\alpha$ is the
constant matrix $A$ acting on velocities and
$\mathcal N_{\gamma,\alpha}(v)^2=\int v^\top A v\,\d\alpha$. By contrast,
Wasserstein-$p$ actions with $p\neq2$, nonlinear spectral gauges such as
$\gamma(M)=\lambda_{\max}(M)$, and more general non-Hilbertian gauges do not
admit a single linear operator $Q_\alpha$ of this form; their steepest-descent
law involves a nonlinear duality map, a spectral linear-minimization oracle as
in {ref}`prop-normalized-spectral-gradient-flow`, or a subgradient of the
tangent norm, instead of the preconditioner $Q_\alpha^{-1}$.
