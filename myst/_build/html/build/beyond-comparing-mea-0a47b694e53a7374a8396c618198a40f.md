...
The previous estimate can be optimized over rigid alignments. If
...
Indeed, applying rigid motions to either measure does not change its intrinsic
metric-measure isometry class, because all pairwise distances inside each space
are preserved. For any rigid motions $g,h$, this invariance and
{ref}`prop-gw-controlled-by-wasserstein` give
...
and taking the infimum over $g,h$ gives the displayed inequality.
Wasserstein-Procrustes is therefore an extrinsic upper certificate for the
intrinsic GW distance. This complements the Mémoli profile lower bound below,
which gives an intrinsic lower certificate before solving the full non-convex
GW problem.
...
In the Euclidean setting, combining the profile lower bound with the
Procrustes upper certificate gives the sandwich
