...
**Integrate** $\dot X_t=v_\theta(t,X_t)$ for $t\in[0,1]$.
...
$$
v_t(x)=\EE[\partial_t I_t(X_0,X_1,Z)\mid X_t=x],
$$
...
:class: ot4ml-remark
One can also represent an interpolating marginal curve by Brownian-in-time dynamics. This is a different construction from the static-noise bridge of Remark {ref}`rem-static-noise-stochastic-interpolants`. Let $Z_t$ solve the Itô equation
$$
...
$$
\partial_t\rho_t+\operatorname{div}(\rho_t\bar v_t)=0,
\qquad
\bar v_t = v_t - \frac{1}{2\rho_t}\operatorname{div}(\rho_tD_t),
$$
where the divergence of the matrix field $\rho_tD_t$ is taken row-wise. In the scalar spatially homogeneous case \(D_t=\sigma_t^2\Id\), this gives \(\bar v_t=v_t-\frac{\sigma_t^2}{2}\nabla\log\rho_t\), the familiar score correction.
:::
...
Moment measures give another way to make a whole distribution from one convex potential. Instead of first choosing a simple reference law and then learning a transport map, one asks for a convex function whose log-concave density and gradient map are tied together. This produces a rigid but mathematically rich factorization of a target law, and it exposes a hidden convexity closely related to the displacement-convexity principles used earlier for Wasserstein gradient flows.
...
The normalization removes additive constants in $u$, while translations of the argument also leave the moment measure unchanged: $u(\cdot-a)$ has the same moment measure as $u$. A first constraint is immediate. Formally, if $u$ is smooth and decays sufficiently fast, then
...
This theorem is due to Cordero--Erausquin and Klartag {cite:p}`CorderoErausquinKlartag2015MomentMeasures`. It is a functional analogue of a Minkowski-type problem: the target measure prescribes how the gradient image of a log-concave density should be distributed.

### Optimal-transport variational formulation.

Santambrogio {cite:p}`Santambrogio2015MomentMeasures` reformulates the moment-measure problem as a minimization over probability densities. For a centered $\mu\in\Pp_1(\RR^d)$, define the maximal-correlation transport functional
...
:::{admonition} Proposition: Hidden convexity of the moment-measure functional
:class: important

Let $\mu\in\Pp_1(\RR^d)$ be centered and not supported on a hyperplane. The functional $\rho\mapsto\mathcal H(\rho)+\mathcal C_\mu(\rho)$ is convex along generalized Wasserstein geodesics with base measure $\mu$ whenever the terms are finite. Its minimizers solve the moment-measure equation
...
for a convex function $u$. Conversely, such a pair is a global minimizer. The minimizer is unique up to translations of $\rho$.
:::

:::{dropdown} Proof
The entropy term is displacement convex by McCann's theorem. Santambrogio proves that the maximal-correlation term $\mathcal C_\mu$ is convex along generalized geodesics with base $\mu$. When second moments are finite, identity {eq}`eq-moment-correlation-w2` makes this compatible with the usual quadratic transport geometry; the general $\Pp_1$ case follows by approximation and lower semicontinuity {cite:p}`Santambrogio2015MomentMeasures`.

Let $\rho$ be a minimizer and let $u$ be a convex minimizer in the dual formula {eq}`eq-moment-max-correlation-dual`. Varying $\rho$ in {eq}`eq-moment-measure-variational` with $u$ fixed gives
...
so $\rho=Z_u^{-1}e^{-u}\d x$. The optimality condition for the scalar-product transport problem says that an optimal coupling is supported on $\{(x,y):y\in\partial u(x)\}$. Since $\rho$ is absolutely continuous and $u$ is convex, $\partial u(x)=\{\nabla u(x)\}$ for $\rho$-almost every $x$, hence $\mu=(\nabla u)_\sharp\rho$. Conversely, if this relation holds, the first variation at $\rho$ is nonnegative in every displacement direction, and the generalized displacement convexity turns this stationarity condition into global minimality. Strict displacement convexity of entropy gives uniqueness, except for translations; translations do not change $\mathcal C_\mu$ because $\mu$ is centered.
:::
