...
If, for instance, $h$ is proper, lower semicontinuous and superlinear, then
$h_\epsilon^*(p)\to h^*(p)$ for every $p$ as $\epsilon\to0$.
:::
...
definition of $h_\epsilon^*$. Expanding the square yields the log-sum-exp
