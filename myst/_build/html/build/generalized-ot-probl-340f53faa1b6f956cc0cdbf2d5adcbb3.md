...
```{math}
\inf_{\pi\in\Couplings_{\mathrm{mart}}(\alpha,\beta)}
\int c(x,y)\d\pi(x,y),
```

with value $+\infty$ when the admissible set is empty.
:::
...
barycentric constraint on the conditional laws. It is central in robust
finance, where one transports today prices to tomorrow prices without
...
For martingale OT the relevant order is the convex order,

```{math}
\alpha\preceq_{\mathrm{cx}}\beta
\quad\Longleftrightarrow\quad
\int\varphi\,\d\alpha\leq\int\varphi\,\d\beta
\quad\text{for every convex }\varphi ,
```

assuming finite first moments. Testing affine functions gives equality of
means, while convex test functions say that $\beta$ is more spread out than
$\alpha$. For centered Gaussian measures, this reduces to the Loewner order
...
centered convolutions. The source $\alpha$ is a red Gaussian mixture on a
grid. Each row kernel $K_i$ is symmetric around the source location $x_i$, so
the coupling $P_{ij}=a_iK_{ij}$ satisfies $\sum_j y_jK_{ij}=x_i$ and hence
has barycentric projection $\bar T_P(x_i)=x_i$. The blue marginal
$\beta_j=\sum_iP_{ij}$ is more spread out, illustrating the convex-order
