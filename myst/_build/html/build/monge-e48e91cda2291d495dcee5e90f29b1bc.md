...
probability measures over the real line is therefore Hilbertian. This makes
one-dimensional OT much simpler than higher-dimensional OT, where the
Wasserstein geometry is not globally Hilbertian.
:::
...
primitives. This norm property is not specific to the line: in any dimension,
and more generally on metric spaces after restricting to zero-mass signed
measures for which the Kantorovich--Rubinstein supremum is finite, duality
defines the norm studied in Section {ref}`sec-W1`. The one-dimensional formula
is special only because the norm has the explicit primitive
representation $\|\cumul{\al}-\cumul{\be}\|_{L^1(\RR)}$.
:::
