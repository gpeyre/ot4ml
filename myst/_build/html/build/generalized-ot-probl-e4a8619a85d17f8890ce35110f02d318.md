...
ratio. Large $\kappa$ recovers the usual Kantorovich value, while $\kappa$
close to $1$ forces a diffuse plan close to the independent reference.
...
Equivalently, up to constants, it is the KL projection of $K$ onto the
...
Initialize $P=K$ and correction matrices $R_1=R_2=R_3=\ones_{n\times m}$.

While the row, column or capacity residual is larger than $\mathrm{tol}$:
...
Gaussian mixture. The source and target are the same uniform empirical measure,
but the diagonal is removed to avoid the trivial identity plan. The three
panels use the entry caps $U_{ij}=2/n^2$, $5/n^2$ and $10/n^2$ off the
diagonal.*
:::
