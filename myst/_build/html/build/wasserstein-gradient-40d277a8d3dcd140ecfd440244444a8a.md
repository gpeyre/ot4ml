...
nonempty. The indicator term is not represented by an ordinary first
variation; its Wasserstein subdifferential contains the normal cone to
$\mathcal K_\kappa$. The JKO step becomes the constrained minimization
...
so $\rho_t\leq\kappa$. The general case follows by approximation and
stability of optimal plans, or equivalently by the same $L^\infty$
density-bound argument in McCann's displacement-convexity theory.
:::
...
$\rho_t\nabla(h+p_t)\cdot n=0$ on $\partial\Omega$, this gives the
...
$h(x)=\|x\|^2/2$ in one dimension, computed in quantile variables by
projected Lagrangian steps. Each panel stacks five times vertically, colored
