...
instead let the transported marginals be optimized, while penalizing or
constraining them separately. This is the viewpoint of the generalized scaling
algorithms of Chizat, Peyré, Schmitzer and Vialard {cite:p}`2016-chizat-sinkhorn`:
entropic OT remains a diagonal-scaling problem as soon as the extra terms depend
only on the two marginals and their proximal maps are simple.

Let $\mathcal F$ and $\mathcal G$ be convex lower semicontinuous functionals on
nonnegative measures on $\Xx$ and $\Yy$. The unregularized marginal-dependent
...
where $\pi_1,\pi_2$ are the two marginals of $\pi$. Entropic regularization
turns this problem into the scaling-friendly form
...
where $\alpha$ and $\beta$ are reference measures.
...
entropic JKO step is obtained by fixing the first marginal to the previous
iterate and putting the energy on the second marginal, namely
...
In finite dimension, with reference weights $\a\in\RR_+^n$, $\b\in\RR_+^m$,
and convex functions $\mathsf f:\RR_+^n\to\RR\cup\{+\infty\}$,
...
$\C$ can be absorbed into the Gibbs reference and the problem is, up to an
additive constant,
...
Fenchel duality gives the smooth concave dual
...
terms are $\langle\mathbf f,\a\rangle+\langle\mathbf g,\b\rangle$, and one
recovers the usual entropic dual.

Define the KL-proximal map
...
With entrywise divisions, one generalized Sinkhorn cycle is
...
In potential variables, this is the same as
$\mathbf f=\epsilon\log\mathbf u$ and
$\mathbf g=\epsilon\log\mathbf v$, so the update is dual ascent written in
multiplicative form. The classical Sinkhorn update is the special case in which
the KL proximal maps return the prescribed marginals $\a$ and $\b$.
...
- **KL marginal relaxation.** If $\mathsf f(r)=\tau\KLD(r|\a)$, then
