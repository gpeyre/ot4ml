...
Residual networks
introduced the idea of learning small residual updates around the identity
{cite:p}`HeZhangRenSun2016ResNet`. This naturally connects depth with time:
very deep residual architectures can be viewed as discretizations of
differential equations, a viewpoint developed in stable architectures, neural
ODEs and mean-field optimal-control models of deep learning
...
limit to this depth-continuous picture. The goal of this section is to make explicit the three nested descriptions: a finite ResNet is a labelled particle system arranged by layers, the infinite-width limit turns each layer into a neuron law, and the infinite-depth limit turns the collection of layers into a conditional measure indexed by depth. Conditional Wasserstein geometry is the natural metric for this last object because it transports neurons inside each depth fiber while keeping the layer variable fixed.

### Finite-Depth Finite-Width ResNets

The starting point is the ordinary architecture: depth is discrete, width is finite, and each layer carries a labelled collection of neurons. A classical finite-depth and finite-width ResNet is a composition of residual
blocks. With $L$ layers, step size $\tau=1/L$, widths $n_r$, and parameters
$\theta_{r,i}\in\Theta$, one may write
...
The first mean-field limit removes the labels inside each layer while keeping the residual depth grid fixed. It sends the widths to infinity while keeping the depth grid finite.
Each residual block is then the mean-field two-layer MLP described above: one
replaces the empirical law $\alpha_r$ by an arbitrary probability measure in
$\Pp_2(\Theta)$ and defines
...
The second limit turns the layer index into a conditioning variable, so the model is no longer a single neuron law but a family of neuron laws indexed by depth. The state is now a
...
defined in Section {ref}`sec-conditional-wasserstein-distances`. In the ResNet
setting, the conditioning variable is the continuous depth $s\in[0,1]$,
$\lambda(\d s)=\d s$, and the ground space is the neuron parameter space
$\Theta\subset\RR^q$. For two conditional neuron laws
...
With continuous depth in place, the forward pass becomes a controlled ODE whose control is the conditional neuron law. A mean-field infinite-depth and infinite-width residual network is
described by a conditional family $s\mapsto\alpha_s\in\Pp_2(\Theta)$, where
each slice is the neuron law of a two-layer mean-field block. We write
$\theta$ for the neuron parameter $x=(u,v)$ used above, so that the feature is
$\psi(\theta,z)$. The forward pass is the controlled ODE
...
This defines a functional on the conditional Wasserstein space.

### Conditional Wasserstein Gradient Flow

Training now means descending an objective on conditional laws. More generally, let $F$ be a differentiable functional on conditional laws
$\alpha(\d s,\d\theta)=\alpha_s(\d\theta)\lambda(\d s)$. We use the fiberwise
...
for signed perturbations
$\xi(\d s,\d\theta)=\xi_s(\d\theta)\lambda(\d s)$ with zero fiber masses. The
conditional Wasserstein gradient flow is then defined fiberwise by
...
This is a layerwise Wasserstein flow: each depth evolves by a Wasserstein
steepest descent in its own neuron space, while the different fibers remain
coupled through the forward ODE and the adjoint equations entering the variation
of $f_{\mathrm{Res}}$.

### Particle Discretization and Layerwise Matching

The finite network is recovered by discretizing both the condition variable and the fiber measures. The finite-depth and finite-width ResNet is the particle discretization of this
construction with the discrete depth marginal $\lambda_L$. If, for simplicity,
two such networks have the same widths and empirical layer laws
...
which is one feasible coupling and therefore an upper bound on the squared
conditional Wasserstein distance. Optimizing over permutations within each
equal-weight layer gives exactly the Wasserstein matching term above. In this
sense, the shallow mean-field flow is the one-fiber case, while infinitely deep
ResNets require a continuum of coupled Wasserstein flows indexed by depth.
