...
{ref}`sec-nonlocal-wasserstein-flows`. WFR, studied separately in
...
{ref}`sec-nonlocal-wasserstein-flows`, and the transport--reaction case is
...
section records the corresponding gradient-flow interpretation. In the
continuum jump-kernel case, entropy descent gives nonlocal diffusion and,
for power-law kernels, fractional PDEs. On a finite state space, the same
logarithmic-mean edge calculus makes a reversible Markov chain exactly the
entropy gradient flow of the discrete Wasserstein distance. Both examples
are mass-preserving, but they are not local Benamou--Brenier flows: their
tangent variables live on pairs or graph edges rather than at individual
points.

### Continuum Jump-Kernel Flows
