...
| $\mathbb A(\alpha,w)$ | Instantaneous action on a measure--velocity pair. | [Eq. eq-generalized-action-length-distance](dynamic-ot#eq-generalized-action-length-distance) |
| $\mathsf E_{\mathbb A}$ | Dynamic endpoint value obtained by integrating $\mathbb A$ along admissible paths; it is not automatically a metric power. | [Eq. eq-generalized-action-length-distance](dynamic-ot#eq-generalized-action-length-distance) |
...
| $\mathbb A_K(\alpha,\nu),\mathcal W_K$ | Relaxed nonlocal measure--flux action and associated Wasserstein distance. | [Eqs. eq-nonlocal-relaxed-action](dynamic-ot#eq-nonlocal-relaxed-action), [eq-nonlocal-wasserstein-distance](dynamic-ot#eq-nonlocal-wasserstein-distance) |
| $\mathcal C_G(\alpha)$ | Closed congestion functional, including the recession cost of singular mass. | [Eq. eq-mfg-congestion-functional](dynamic-ot#eq-mfg-congestion-functional) |
