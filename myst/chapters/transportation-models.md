...
Then the pair $(\alpha_t,v_t)$ satisfies the continuity equation in Eulerian form.
:::

:::{dropdown} Proof
We first recall the two equivalent ways of writing the interpolated measure. Formally, one may write $$\alpha_t(z)=\int_{\RR^{d'}}\delta(z-P_t(u))\,\d\pi(u),$$ while the rigorous meaning is that, for every smooth test function $\varphi$,
...
Let $\Sigma_0,\Sigma_1\succ0$ and let $X_0\sim\Gaussian(0,\Sigma_0)$ and $X_1\sim\Gaussian(0,\Sigma_1)$ be independent. Consider the linear flow-matching interpolation $$Z_t=(1-t)X_0+tX_1,
        \qquad
        \alpha_t=\operatorname{Law}(Z_t)=\Gaussian(0,\Sigma_t),$$ where
...
```
 ...
 Instead of integrating {eq}`eq-one-step-wgf` at inference time, one fits a parametric residual field $U_\eta$ along the current model distribution:
...
precisely, when the projected transports are well defined, if $T_\theta$
...
{ref}`sec-kernelized-bb-distance`, whose velocities live in an RKHS.
...
Drifting methods need not start from an exact Wasserstein gradient. They often prescribe an attraction-minus-repulsion field and then regress this field in $L^2(\alpha_t)$. A simple continuous version uses a positive kernel $K_\epsilon(x,y)$ and defines, for any measure $\nu$,
...
:class: important
Let $\alpha_t$ be a smooth curve of positive densities and let $u_t=\nabla\phi_t$ be a smooth time-dependent gradient field. Define the semi-relaxed functional

```{math}
:label: eq-semi-relaxed-drift-functional

\mathcal R_t(\alpha|\alpha_t)
        \eqdef
        -\int \phi_t(x)\,\d\alpha(x)
        +\int \phi_t(x)\,\d\alpha_t(x).
```

 Here $\alpha_t$ and $\phi_t$ are frozen when taking the first variation with respect to the first argument $\alpha$. Then the continuity equation $$\partial_t\alpha_t+\operatorname{div}(\alpha_t u_t)=0$$ is the formal Wasserstein gradient descent of the time-dependent functional $\alpha\mapsto\mathcal R_t(\alpha|\alpha_t)$.
:::

:::{dropdown} Proof
Since $\alpha_t$ and $\phi_t$ are fixed in the variation with respect to $\alpha$, the first variation is $$\delta_\alpha \mathcal R_t(\alpha|\alpha_t)(x)=-\phi_t(x).$$ By the Wasserstein-gradient formula, $$\Wgrad \mathcal R_t(\alpha|\alpha_t)
        =
        \nabla\delta_\alpha \mathcal R_t(\alpha|\alpha_t)
        =
        -\nabla\phi_t
        =
...
It is "semi-relaxed" because the current model $\al_t$ is used to build the potential, but it is not varied inside the denominator when computing the first variation in $\alpha$.
:::
...
A general regressed field $b_t$ is not necessarily a Wasserstein gradient, since Wasserstein tangent vectors are represented by gradient fields modulo $L^2(\al_t)$-null directions. The gradient component is obtained by the weighted projection
...
same \(u\) define both the log-concave source density \(\rho_u\) and the
...
Santambrogio {cite:p}`Santambrogio2015MomentMeasures` reformulates the moment-measure problem as a minimization over probability densities. For a centered $\al\in\Pp_1(\RR^d)$, define the maximal-correlation transport functional, with values in $\RR\cup\{+\infty\}$,

```{math}
:label: eq-moment-max-correlation
\mathcal C_\al(\rho)
\eqdef
\sup_{\pi\in\Couplings(\rho,\al)}
...
\left\{
\int v(x)\,\d\rho(x)+\int v^*(y)\,\d\al(y)
\right\},
```

where $v^*$ is the Legendre transform. If $\rho,\al\in\Pp_2(\RR^d)$, then
...
\left\{
\mathcal H(\rho)+\mathcal C_\al(\rho)
\right\},
\qquad
...
The centering of $\al$ makes this functional invariant under translations of $\rho$, since translating $\rho$ by a vector $a$ changes $\int x\cdot y\,\d\pi$ by $a\cdot\int y\,\d\al(y)=0$.

(prop-moment-hidden-convexity)=
:::{admonition} Proposition: Variational characterization of moment measures
:class: important

Let $\al\in\Pp_1(\RR^d)$ be centered and not supported on a hyperplane. Then the minimization problem {eq}`eq-moment-measure-variational` admits a solution, unique up to translations. Every minimizer is a log-concave density of the form
...
Conversely, any essentially continuous convex $u$ satisfying this equation yields a global minimizer. In the finite-second-moment case $\al\in\Pp_2(\RR^d)$, the objective $\rho\mapsto\mathcal H(\rho)+\mathcal C_\al(\rho)$ is displacement convex along $\Wass_2$ geodesics on $\Pp_2(\RR^d)$ whenever the terms are finite. For general $\al\in\Pp_1(\RR^d)$, the same statement is obtained by approximation and lower semicontinuity.
:::

:::{dropdown} Proof
The existence proof has two ingredients. First, since $\al$ is centered, translating $\rho$ does not change either $\mathcal H(\rho)$ or $\mathcal C_\al(\rho)$, so one can center a minimizing sequence. Second, the assumption that $\al$ is not supported on a hyperplane gives a coercive lower bound of the form $\mathcal C_\al(\rho)\geq c_\al\int\norm{x}\,\d\rho(x)$ for centered absolutely continuous $\rho$, with $c_\al>0$. Together with the lower semicontinuity estimates for entropy and maximal correlation, this yields a minimizer {cite:p}`Santambrogio2015MomentMeasures`.
...
```{math}
\frac{\d}{\d t}\mathcal C_\al(\rho_t)\Big|_{t=0^+}
\geq
\int \dotp{T(x)-x}{\nabla u(x)}\,\d\rho(x),
```

because $(\Id,\nabla u)_\sharp\rho$ is optimal for the scalar-product problem. The first-order terms cancel, hence the one-sided derivative of $\mathcal H+\mathcal C_\al$ at $\rho$ in every such direction is nonnegative. Displacement convexity then implies global minimality, and approximation removes the smooth compact-support restriction. Strict displacement convexity of entropy gives uniqueness, except for translations; translations do not change $\mathcal C_\al$ because $\al$ is centered.

It remains to justify the convexity assertion. The entropy term is displacement convex by McCann's theorem. If $\al$ has finite second moment, identity {eq}`eq-moment-correlation-w2` writes $\mathcal C_\al$ as the sum of the $1$-convex moment term $\rho\mapsto\frac12\int\norm{x}^2\,\d\rho$ and the $(-1)$-convex term $\rho\mapsto-\frac12\Wass_2^2(\rho,\al)$, hence $\mathcal C_\al$ is displacement convex. For merely finite first moment $\al$, the same convexity and the variational characterization follow by the approximation and semicontinuity arguments of Santambrogio.
:::
...
Then, as long as this matrix ODE is defined, $\Sigma_t\succ0$ and $\alpha_t=\Gaussian(m_t,\Sigma_t)$ is the solution of the continuity equation
...
For the converse, fix $\alpha=\Gaussian(m,\Sigma)$ and denote by $\rho$ its density. If the velocity is tangent to the Gaussian manifold, then the infinitesimal density variation has the form

```{math}
-\diverg(\rho V)=q\rho,
```

where $q$ is a polynomial of degree at most two with zero $\alpha$-mean. Since $V$ is the Wasserstein representative, $V=\nabla\phi$ in $L^2(\alpha)$. After the affine whitening $y=\Sigma^{-1/2}(x-m)$, this equation becomes, for the standard Gaussian density,

```{math}
y\cdot\nabla\psi(y)-\Delta\psi(y)=\tilde q(y),
```

with $\tilde q$ again of degree at most two. The Hermite polynomials diagonalize the Ornstein--Uhlenbeck operator $y\cdot\nabla-\Delta$: constants are the only kernel, and degree-$k$ Hermite modes have eigenvalue $k$. Therefore the unique finite-energy solution modulo constants has $\psi$ of degree at most two. Thus $\nabla\psi$ is affine, and transforming back gives $V(x)=b+A(x-m)$ with $A=A^\top$ because $V$ is a gradient field.
...
The debiased Sinkhorn row uses the notation of {ref}`cor-gaussian-sinkhorn-divergence`: for Gaussian inputs,
...
\right)\theta\theta^\top\,\d\sigma(\theta).
```
...
This is an affine field, hence the full Wasserstein descent is exact and Proposition {ref}`prop-gaussian-affine-closure` gives the displayed moment equations. The remaining rows are exact for the same structural reason: their ambient first variations are quadratic, or their Wasserstein velocities are affine, on Gaussian inputs.

For the quadratic potential and interaction rows, the ambient first variations are respectively
...
which yield affine descent velocities on Gaussian inputs.

For the KL row, the ambient first variation is $\log(\rho/\gamma)$ up to constants. If $\alpha=\Gaussian(m,\Sigma)$, then

```{math}
\nabla\log(\rho/\gamma)(x)
=
...
Similarly, if $u=\log(\rho/\gamma)$, the first variation of $\frac12\mathcal I$ is, up to constants,

```{math}
-\Delta u-\frac12|\nabla u|^2-\dotp{\nabla u}{\nabla\log\gamma},
```

which is quadratic when both $\rho$ and $\gamma$ are Gaussian. Multiplying by two for $\mathcal I$ gives the descent velocity

```{math}
v(x)=-2A^2\delta_m+2(\Sigma^{-2}-A^2)(x-m).
```

The Wasserstein row follows from the affine Gaussian Brenier map. For the polynomial MMD row, $k(x,y)=\dotp{x}{y}^2$ makes the discrepancy a squared distance between raw second moments, so the ambient first variation is $2x^\top R x$ and the descent velocity is $-4Rx$.

For Sinkhorn, the first variation of the entropic value with respect to the first marginal is a Sinkhorn dual potential. For Gaussian marginals, the cross and self dual potentials entering the debiased divergence are quadratic, hence their spatial gradient is affine. Since the restriction to Gaussians is
...
```{math}
2\int_{\Sphere^{d-1}}
\bigl(T_\theta(P_\theta x)-P_\theta x\bigr)\theta\,\d\sigma(\theta).
```
...
:class: important
For $\al\in\Pp_2(\RR^d)$, let $$\mathcal R\al\eqdef \Gaussian(\mean_\al,\cov_\al)$$ be the Gaussian with the same mean and covariance as $\al$. Then $$\Wass_2^2(\mathcal R\al,\mathcal R\nu)
        =
        \norm{\mean_\al-\mean_\nu}^2+\Bb^2(\cov_\al,\cov_\nu)
        \leq
        \Wass_2^2(\al,\nu).$$
:::

:::{dropdown} Proof
Take any coupling $(X,Y)$ of $\al$ and $\nu$, center the variables, and write $C=\EE[(X-\mean_\al)(Y-\mean_\nu)^\top]$. In the positive definite case, positivity of the block covariance matrix implies the factorization $C=\cov_\al^{1/2}K\cov_\nu^{1/2}$ with $\norm{K}_{\mathrm{op}}\leq1$, and therefore, by operator/nuclear norm duality, $$\tr C\leq \tr\left((\cov_\al^{1/2}\cov_\nu\cov_\al^{1/2})^{1/2}\right).$$ The semidefinite case follows by adding $\eta\Id$ to both covariance matrices and letting $\eta\downarrow0$. Expanding $\EE\norm{X-Y}^2$ gives the lower bound $$\EE\norm{X-Y}^2
        \geq
...
Let the target be $\beta=\Gaussian(\bar\mean,\bar\cov)$ and assume $\al_t=\Gaussian(\mean_t,\cov_t)$. For the Gaussian kernel
...
Indeed the smoothed density $x\mapsto\int K_\epsilon(x,y)\d\al_t(y)$ is proportional to the Gaussian density with mean $\mean_t$ and covariance $\cov_t+\epsilon\Id$. Thus $B_\epsilon[\al_t]$ is the mean-shift vector of a Gaussian density: it points linearly toward the Gaussian mode, with strength set by the bandwidth. The drifting velocity {eq}`eq-cross-minus-self-drift` is therefore the difference of two affine mean-shift fields; it is affine and preserves Gaussianity. With
...
```{math}
F(\al)=\sum_s\la_s\Wass_2^2(\al,\be_s),
```

then $\mathcal R\be_s=\be_s$, and Theorem {ref}`thm-gelbrich-projection` gives $F(\mathcal R\al)\leq F(\al)$. Thus the moment-matched Gaussian projection of any barycenter is again a barycenter; when the barycenter is unique, it must itself be Gaussian. This is the contraction viewpoint behind Corollary {ref}`cor-gaussian-discrete-barycenters`.
:::
