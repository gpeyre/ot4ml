...
(par-diffusion-model-connection)=
...
```{math}
:label: eq-distribution-free-mean-covariance-closure
...
(prop-scalar-moment-closure)=
:::{admonition} Proposition: Scalar moment closure and eikonal characterization
:class: ot4ml-theorem
...
```{math}
:label: eq-scalar-moment-affine-carre
...
Moreover, on every connected open set $U\subset\{x:\nabla\varphi(x)\neq0\}$, the restriction of {eq}`eq-scalar-moment-affine-carre` to $U$ is equivalent to the existence of an eikonal coordinate $r\in C^2(U)$ and constants $c,\kappa,\lambda\in\RR$ such that

```{math}
:label: eq-scalar-moment-eikonal-form
\norm{\nabla r}=1,
\qquad
\varphi=c+\kappa r+\frac{\lambda}{2}r^2.
```

Under {eq}`eq-scalar-moment-affine-carre`, one may take $\lambda=b/2$.
:::

:::{dropdown} Proof
The first variation is $\delta f_g(\alpha)(x)=g'(m_\varphi(\alpha))\varphi(x)$, so the continuity equation gives the displayed evolution of $m_t$. Condition {eq}`eq-scalar-moment-affine-carre` then proves sufficiency.
...
Hence $q$ is affine, which is precisely {eq}`eq-scalar-moment-affine-carre`.

On a regular set $U$, the right-hand side of {eq}`eq-scalar-moment-affine-carre` is positive. Fix $s_0\in\varphi(U)$ and define

```{math}
r(x)=\int_{s_0}^{\varphi(x)}\frac{\d s}{\sqrt{a+bs}}.
```

Then $\norm{\nabla r}=1$. Inverting this change of variables gives {eq}`eq-scalar-moment-eikonal-form` with $c=s_0$, $\kappa=\sqrt{a+bs_0}$, and $\lambda=b/2$. Conversely, {eq}`eq-scalar-moment-eikonal-form` gives
$\norm{\nabla\varphi}^2=\kappa^2-2\lambda c+2\lambda\varphi$.
:::

:::{admonition} Example: A closed signed-distance moment
:class: ot4ml-example
