...
With this notation,  {eq}`eq-sliced-wasserstein` says that $\SW_p$ is the pull-back, through $\mathfrak R$, of the $L^p(\Sphere^{d-1};\Wass_p)$ distance on Radon data. The full all-direction transform is injective on probability measures by the Cramer--Wold theorem, which is why sliced Wasserstein separates measures. The warning is numerical and variational: finitely sampled Radon data are not injective, and independently constructed one-dimensional Radon data need not satisfy the range conditions of an actual image. This is precisely the issue behind sliced and Radon barycenter reconstructions discussed in Section {ref}`sec-barycenters` and illustrated in Figure {ref}`fig:sliced-radon-barycenter`.
:::
...
There are two different geometries associated with $\SW_2$. The first is the chordal metric of Definition {ref}`def-sliced-wasserstein`, obtained by comparing all one-dimensional projections. The second is the intrinsic length metric generated by this chordal metric. If $(\alpha_t)_{t\in[0,1]}$ is a continuous curve in $(\Pp_2(\RR^d),\SW_2)$, define
...
```{math}
\SW_2^{\mathrm{len}}(\alpha,\beta)
\eqdef
\inf_{\substack{\alpha_0=\alpha,\ \alpha_1=\beta\\
t\mapsto\alpha_t\ \text{continuous}}}
\mathsf L_{\SW_2}((\alpha_t)_{t\in[0,1]}).
```
...
Jensen's inequality gives the upper bound of Proposition {ref}`prop-sliced-first-order-tangent`, and the inequality is generally strict. Thus slicing preserves enough projected information to metrize the same topology as $\Wass_2$, but it does not preserve the same Riemannian norm.
:::
