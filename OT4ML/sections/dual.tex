...
The range of the $c$-transform has more structure than Definition~\ref{def-c-concave-functions} alone suggests. Denote its finite-valued part by
\[
...
The class is always stable under finite \emph{tropical barycenters}: if $u_r=f_r^c\in\operatorname{Conc}_c(\Y)$ and $a_r\in\RR$, then
\eql{\label{eq-c-concave-tropical-barycenter}
...
\index{neural optimal transport}% strictindex
	...
	\]
	$c$-concave by~\eqref{eq-c-concave-tropical-barycenter}. At differentiability points they generate the intrinsic map
