...
\textbf{Input:} Finite profit matrix $A=(a_{ij})\in\RR^{n\times n}$ with $n\geq2$, bid increment $\epsilon>0$.

\textbf{Output:} Assignment map $\sigma$ and target prices $p$ satisfying $\epsilon$-complementary slackness.
...
\end{algblock}
\algreturnskip
\textbf{Return} $\sigma(i)=j$ iff $o(j)=i$, together with $p$.
\end{alg}
