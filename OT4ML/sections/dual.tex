...
\end{tabular}
\caption{\emph{Discrete $c$-transform as a lower envelope for costs $c_p(x,y)=|x-y|^p$.} The red circles are four source atoms $x_i$ with potential values $\fD_i$; the gray curves are the translated functions $y\mapsto c_p(x_i,y)-\fD_i$; the colored curve is their lower envelope $\fD^c(y)=\min_i c_p(x_i,y)-\fD_i$. This is the semi-discrete situation where $\X$ is finite, equivalently the source measure $\al$ is discrete; Chapter~\ref{sec-semidiscr-w1} studies the complementary case where the eliminated potential is supported on finitely many target atoms.}
...
\[
	...
	\frac12\norm{x}^2-\phi^{**}(x).
\]
On the full Euclidean space, the $c$-closed functions are therefore those for which
$x\mapsto\frac12\norm{x}^2-f(x)$ is lower-semicontinuous and convex; equivalently, $f$ is $1$-semiconcave. The double transform is the smallest $\bar c$-concave majorant of $f$. On restricted compact domains, the same statement holds with the additional restriction that the supporting slopes belong to the opposite domain.
\index{convex!function}% strictindex
\index{c-transform}% strictindex

\begin{rem}[Proof of Brenier's theorem]\label{rem-proof-brenier}
\index{Brenier!theorem}% strictindex
	Let $\al,\be\in\Pp_2(\RR^d)$ and assume that $\al$ has a density. For $c(x,y)=\frac12\norm{x-y}^2$, let $(u,v)$ be a closed optimal dual pair and set
	\[
		\phi(x)\eqdef\frac12\norm{x}^2-u(x),
		\qquad
		\psi(y)\eqdef\frac12\norm{y}^2-v(y).
	\]
	The constraint $u(x)+v(y)\leq c(x,y)$ becomes
