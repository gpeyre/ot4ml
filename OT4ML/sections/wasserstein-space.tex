...
\index{metric!triangle inequality}% strictindex
	The proof of Proposition~\ref{prop-metric-measure} is the prototype for many metric constructions in the book. The only genuinely transport-specific step is the gluing lemma, Lemma~\ref{lem-gluing-general}: two couplings sharing a marginal can be lifted to a joint law, and the ordinary triangle inequality is then applied pointwise before integrating. The same pattern already appeared in the discrete proof of Proposition~\ref{prop-metric-histo}, where the glued tensor is written explicitly in \eqref{eq-glued-discr}. It reappears directly in the Gromov--Wasserstein metric theorem, Theorem~\ref{thm-gw-metric}, where couplings are glued while controlling pairwise distortions. Quotient Wasserstein and conditional Wasserstein inherit the same mechanism through Proposition~\ref{prop-quotient-wasserstein-metric} and Proposition~\ref{prop-conditional-wasserstein-distance}. By contrast, the homogenization step for unbalanced OT in Proposition~\ref{prop-homogeneous-unbalanced} relies on disintegration and conditional Jensen rather than on a new gluing argument. In this sense, gluing is the metric engine behind many extensions of \(\Wass_p\), not merely a technical lemma for \eqref{eq-defn-wass-dist}.
\end{rem}
...
\index{Monge!problem}% strictindex
	...
	This distance $\Wass_p$ defined through the Kantorovich problem~\eqref{eq-defn-wass-dist} should be contrasted with the directed distance $\tilde\Wass_p$ obtained using Monge's problem~\eqref{eq-monge-distance}. The Kantorovich feasible set is never empty, since it contains the product coupling, although the $p$-cost may still be infinite without moment assumptions on non-compact spaces. By contrast, Monge's constraint set $\enscond{T}{T_\sharp \al=\be}$ can be empty. Every admissible Monge map induces the feasible graph coupling $(\Id,T)_\sharp\al$, and therefore $\Wass_p(\al,\be)\leq\tilde\Wass_p(\al,\be)$. Existence of a Monge minimizer alone does not imply equality, because its graph coupling need not be optimal among all Kantorovich plans. Equality holds when a Kantorovich optimizer is induced by a map; the next proposition gives equality of the infimal values under an atomlessness assumption even when no optimal map exists.
...
The following proposition identifies the TV norm with a Kantorovich value for the measurable $0/1$ cost. It is a genuine Wasserstein distance when this cost defines a Polish ground metric, notably on countable discrete spaces.
\index{Wasserstein!distance}% strictindex
...
\index{total variation}% strictindex
		...
		\eq{
			\MK_{d_0}(\al,\be)
			=
			\inf_{\pi\in\Couplings(\al,\be)}\int d_0(x,y)\d\pi(x,y)
			=
			\frac{1}{2}\norm{\al-\be}_{\TV}.
		}
...
On a finite metric space, the strong and weak topologies coincide. If the space is a singleton, all probability measures coincide and every distance below vanishes. The following quantitative comparison handles the nontrivial case.
\index{Wasserstein!distance}% strictindex
\index{topology!weak}% strictindex
...
The basic computational reason for the popularity of Wasserstein DRO is a one-dimensional dual reformulation. The following positive-radius statement separates strong duality from the additional assumptions needed for a worst-case distribution or pointwise maximizers to exist.

\begin{prop}[Duality for empirical Wasserstein DRO]\label{prop-empirical-wasserstein-dro-duality}
\index{distributional robustness!duality}% strictindex
	...
	\[
		\ell_\theta(z)-\ell_\theta(z_0)
		\leq A d(z,z_0)^p+B
		\qquad\text{for every }z\in\Zz.
	\]
...
The positive-radius hypothesis is deliberate. At $\rho=0$, the primal value is $n^{-1}\sum_i\ell_\theta(z_i)$. If $\ell_\theta$ is upper semicontinuous and satisfies the same growth bound, then~\eqref{eq-dro-dual-envelope} remains valid with the minimum replaced by an infimum: each penalized envelope decreases to $\ell_\theta(z_i)$ as $\lambda\to+\infty$. The infimum need not be attained. For a merely measurable loss, even equality can fail. Indeed, on $\Zz=\RR$ with $n=1$, $z_1=0$, $p=1$, $\ell(0)=0$, and $\ell(z)=1$ for $z\neq0$, the primal value is zero whereas every finite-$\lambda$ envelope has supremum one. The inner suprema in~\eqref{eq-dro-dual-envelope} are penalized envelopes. Only when such a supremum is attained does one of its maximizers define a worst penalized perturbation of the corresponding sample; existence of a worst-case distribution is a separate issue. For $p=1$ and an $L_\theta$-Lipschitz loss, the Kantorovich--Rubinstein dual gives the transparent upper bound
\[
...
Figure~\ref{fig:kantorovich-dro-ambiguity} gives a geometric illustration for nonlinear binary classification. The score belongs to the reproducing-kernel Hilbert space of the Laplacian kernel $k_h(x,z)=e^{-\norm{x-z}/h}$. The two classes form noisy interlocking crescents whose opposing tips overlap locally. The robust panels are produced by a projected deterministic adversarial-training heuristic with one perturbed location per observation, alternating perturbation and classifier updates. The displayed pointwise coupling has root-mean-square cost $\rho$, hence certifies that the adversarial empirical law lies in the $\Wass_2$ ball; it need not be an optimal coupling, and the heuristic is not a solution certificate for the unrestricted inner DRO problem.
...
{\small $\rho=0.11$}
\end{tabular}
\caption{\emph{A deterministic adversarial-training heuristic reshapes the separator between two noisy interlocking moons.} Filled dots are the observed classes, the black curve is the learned zero-score boundary, and the background encodes the logistic probability. The opposing tips of the red and blue crescents overlap locally, forcing the nominal classifier to wind through a narrow nonlinear channel. In the robust panels, the dashed gray curve recalls this nominal boundary; hollow dots show all adversarial locations, while violet segments display a representative half of their transports. As $\rho$ increases, the separator moves away from the most fragile overlap regions and trades clean fit for wider margins. Under the displayed pointwise coupling, the root-mean-square displacement equals $\rho$, and therefore the empirical $\Wass_2$ distance is at most $\rho$.}
...
The endpoint inherits the metric structure and is the large-exponent limit of finite-order Wasserstein distances.
\begin{prop}[Metric and endpoint properties of $\Wass_\infty$]\label{prop-wasserstein-infinity-metric}
\index{extended metric}% strictindex
	For every $\al,\be\in\Pp(\Xx)$ and every $r\geq0$,
	\[
...
This limit concerns the distances, not the linear objectives defining $\Wass_p^p$. The endpoint minimizes the worst displacement rather than an average displacement. Although the essential-supremum objective is not linear, Proposition~\ref{prop-wasserstein-infinity-metric} shows that each sublevel test is a convex feasibility problem.
