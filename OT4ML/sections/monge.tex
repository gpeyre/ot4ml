...
The second-moment quotient has an exact finite-dimensional counterpart.
Writing a positive semidefinite matrix as $\Sigma=AA^\top$ realizes it as a
row-Gram matrix, while right multiplication $A\mapsto AR$ by an orthogonal
matrix leaves $\Sigma$ unchanged. The Bures metric is the quotient of the
Frobenius distance by this orthogonal ambiguity.
...
\begin{proof}
	...
	\[
		\norm{A_0^\top B_0}_*
		=
		\tr\!\left((\Sigma^{1/2}\Lambda\Sigma^{1/2})^{1/2}\right).
	\]
