...
Figure~\ref{fig:monge-sphere-mccann-interpolation} makes this intrinsic interpolation explicit on the closed upper hemisphere
\(M=\Sphere^2_+\eqdef\{x\in\RR^3:\norm{x}=1,\ x_3\geq0\}\). Consider two equal-weight empirical measures
\index{upper hemisphere}% strictindex
\(\al=n^{-1}\sum_i\delta_{x_i}\) and \(\be=n^{-1}\sum_j\delta_{y_j}\), and solve the discrete Kantorovich problem for
\[
	\C_{ij}=\frac12 d_{\Sphere^2}(x_i,y_j)^2
	=\frac12\arccos\!\bigl(\dotp{x_i}{y_j}\bigr)^2.
\]
By the Birkhoff--von Neumann theorem, Theorem~\ref{thm-birkhoff-von-neumann}, an optimal coupling can be chosen as a permutation coupling
\(\P^\star_{i,\sigma(i)}=1/n\). Assuming that no matched pair is antipodal, put
\[
	\vartheta_i=\arccos\!\bigl(\dotp{x_i}{y_{\sigma(i)}}\bigr),
	\qquad
	\gamma_i(t)
	=
...
\]
Each \(\gamma_i\) is the constant-speed minimizing spherical geodesic joining its matched endpoints, and
\(\al_t=n^{-1}\sum_i\delta_{\gamma_i(t)}\) is the corresponding intrinsic McCann interpolation. Since both sine coefficients are nonnegative on \([0,1]\), endpoints in \(\Sphere^2_+\) generate paths that remain in the hemisphere.
...
\begin{figure}[ht]
	...
	\caption{\emph{Intrinsic McCann interpolation on the upper hemisphere.} The thin violet great-circle arcs show the fixed optimal permutation coupling between the hollow red and blue endpoint atoms; the filled atoms follow these arcs at \(t=0,1/4,1/2,3/4,1\), with color interpolated from red to blue.}
	\label{fig:monge-sphere-mccann-interpolation}
\end{figure}

\paragraph{Poincar\'e disk.}

Negative curvature gives a complementary intrinsic picture. The Poincar\'e disk is
\[
	\mathbb D^2\eqdef\enscond{x\in\RR^2}{\norm{x}<1},
	\qquad
	g_x=\frac{4}{(1-\norm{x}^2)^2}\Id_2,
\]
a complete Riemannian manifold of constant curvature \(-1\). Its geodesic distance has the closed form
\[
	d_{\mathbb D}(x,y)
	=
	...
	\right).
\]
For equal-weight empirical measures with the same number of atoms, the intrinsic quadratic cost
\(\C_{ij}=d_{\mathbb D}(x_i,y_j)^2/2\) again admits an optimal permutation coupling; denote its permutation by \(\sigma\).

The constant-speed interpolation is explicit through the hyperboloid lift and Lorentz product
\[
	\iota(x)
	\eqdef
	\frac{(1+\norm{x}^2,\,2x)}{1-\norm{x}^2}
	...
	\dotp{X}{Y}_L
	\eqdef
	-X_0Y_0+\dotp{X_{1:2}}{Y_{1:2}}.
\]
Put \(X_i=\iota(x_i)\), \(Y_i=\iota(y_{\sigma(i)})\), and
\(\eta_i=d_{\mathbb D}(x_i,y_{\sigma(i)})\). Then
\[
	Z_i(t)
	=
	...
	\gamma_i(t)
	=
	\frac{Z_{i,\mathrm{sp}}(t)}{Z_{i,0}(t)+1},
\]
where \(Z_i=(Z_{i,0},Z_{i,\mathrm{sp}})\); the coincident-endpoint case is obtained by continuity. The curve \(\gamma_i\) has constant hyperbolic speed \(\eta_i\), and in disk coordinates its trace is a Euclidean circle orthogonal to \(\partial\mathbb D^2\), with diameters as the limiting case. Thus
\(\al_t=n^{-1}\sum_i\delta_{\gamma_i(t)}\) is the hyperbolic McCann interpolation shown in Figure~\ref{fig:monge-hyperbolic-mccann-interpolation}, where reversed mode populations force part of the optimal coupling to cross between modes. The \(\sinh\) coefficients are the negative-curvature counterparts of the \(\sin\) coefficients in the spherical formula above.
...
\begin{figure}[H]
	...
	\caption{\emph{Intrinsic McCann interpolation in the Poincar\'e disk.} The black circle is the ideal boundary, the faint grid consists of radial geodesics and equally spaced hyperbolic-radius circles, and the thin violet arcs are the fixed optimal geodesic coupling. The source places \(11/16\) of its mass in the lower mode, whereas the target places \(11/16\) in the upper mode, forcing six atoms to cross between modes. Filled atoms move from the hollow red source to the hollow blue target at \(t=0,1/4,1/2,3/4,1\).}
