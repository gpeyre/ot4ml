...
Formulation~\eqref{eq-w1-metric} is a special case of a dual norm. This viewpoint provides a way to design ``weak'' discrepancies by testing signed differences of measures against a controlled class of functions.
...
On zero-mass signed measures with finite first moment, the Kantorovich--Rubinstein norm underlying $\Wass_1$, as defined in~\eqref{eq-w1-metric}, is the extended dual seminorm~\eqref{eq-dual-norm-cont} associated with
\eq{
...
On a convex Euclidean domain, $\Lip(f)=\norm{\nabla f}_\infty$ for continuously differentiable $f$; on a nonconvex domain, the gradient controls Lipschitz continuity for the intrinsic path metric rather than necessarily for the ambient Euclidean metric.
\end{example}

Figure~\ref{fig:dualnorms-ipm-witnesses} compares representative witness shapes associated with several integral probability metrics for the same pair of one-dimensional densities.
...
\end{tabular}
\caption{\emph{Display-normalized witness shapes for integral probability metrics.} The red and blue filled profiles are two one-dimensional probability densities, and the violet curves show the qualitative shapes of corresponding witnesses. For visibility, each density and each witness is normalized independently by its own peak absolute value; the displayed curves are therefore not certified unit-ball optimizers of~\eqref{eq-dual-norm-cont}. $\Wass_1$ restricts slopes through Kantorovich--Rubinstein duality~\eqref{eq-w1-metric}, MMD restricts the RKHS norm as in Proposition~\ref{prop-kernel-rkhs-dual}, and total variation admits measurable sign witnesses that react sharply to signed density differences.}
...
\end{proof}
The endpoint $\Wass_\infty$ studied in Section~\ref{sec-dro-wasserstein-infinity} controls the largest displacement and does not metrize weak convergence. For instance, on $[0,1]$, the measures $\al_n=(1-1/n)\de_0+n^{-1}\de_1$ converge weakly to $\de_0$, whereas $\Wass_\infty(\al_n,\de_0)=1$ for every $n$.
...
\index{conditionally positive definite kernel}% strictindex
...
The conditional notions are the right ones for probability distances, because one applies quadratic forms to signed measures $\xi=\al-\be$ of total mass zero. Adding a term of the form $a(x)+a(y)$ to a kernel does not change $\iint \Krkhs(x,y)\d\xi(x)\d\xi(y)$ on such measures. Many natural distance kernels are conditionally negative definite, while their negatives are the conditionally positive kernels used to define MMDs.
\index{signed!measure}% strictindex
...
\index{kernel!Matern@Mat\'ern}% strictindex
	On $\RR^d$, translation-invariant kernels are most transparent in Fourier variables. The Riesz family associated with $(-\Delta)^{-s}$ has multiplier $\norm{\om}^{-2s}$ and defines a nonnegative quadratic form on zero-mass measures for which the corresponding Fourier energy is finite; this is the kernel counterpart of classical Riesz potentials~\cite{berg84harmonic}. The energy distance corresponds to the conditionally positive kernel $\Krkhs(x,y)=-\norm{x-y}$, whose Fourier multiplier is proportional to $\norm{\om}^{-(d+1)}$. If $\al$ and $\be$ have finite first moments and $\xi=\al-\be$, then
...
\index{kernel!positive}% strictindex
	$-\iint\norm{x-y}\d\xi(x)\d\xi(y)$ is exactly the customary squared energy distance
...
Shifted kernels replace $(-\Delta)^{-s}$ by $(-\Delta+\lambda\Id)^{-s}$ with $\lambda>0$. The spectral density $(\norm{\om}^2+\lambda)^{-s}$ defines a finite Bochner measure, and hence an ordinary bounded stationary kernel, exactly when $s>d/2$. Writing $\nu=s-d/2>0$ gives the Mat\'ern family; in closed form these kernels are radial and involve a modified Bessel function~\cite{wendland2005scattered}. For $s\leq d/2$, the inverse Fourier object is singular on the diagonal, with a distributional interpretation where needed, and hence is not a finite real-valued kernel in the sense of Definition~\ref{def-positive-kernels}. The Laplacian kernel $e^{-\norm{x-y}/\sigma}$ is the Mat\'ern case $\nu=1/2$, while the Gaussian kernel $e^{-\norm{x-y}^2/(2\sigma^2)}$ is the infinite-smoothness limit after the usual rescaling of $\nu$.
...
Finite Gram positivity does not by itself specify a domain for the quadratic form on arbitrary measures. For a positive-definite kernel, set $\Krkhs^\circ=\Krkhs$. For a conditionally positive-definite kernel, fix $x_0\in\Xx$ and set
\[
	\Krkhs^\circ(x,y)
	\eqdef
	\Krkhs(x,y)-\Krkhs(x,x_0)-\Krkhs(x_0,y)+\Krkhs(x_0,x_0).
\]
The kernel $\Krkhs^\circ$ is positive definite: apply conditional positivity to the points $x_1,\ldots,x_n,x_0$ with coefficients $r_1,\ldots,r_n,-\sum_i r_i$. Denote its RKHS by $\RKHS^\circ$ and its canonical feature map by $\Phi^\circ(x)=\Krkhs^\circ(x,\cdot)$.
...
\index{signed!measure}% strictindex
	...
	For two probability measures such that $\al-\be$ is $\Krkhs$-admissible, the maximum mean discrepancy associated with $\Krkhs$ is
\index{probability measure}% strictindex
\index{maximum mean discrepancy}% strictindex
	\[
...
The RKHS Cauchy--Schwarz inequality gives $|\Krkhs^\circ(x,y)|\leq\sqrt{\Krkhs^\circ(x,x)\Krkhs^\circ(y,y)}$, so the double integral in~\eqref{eq-kernel-dual} is absolutely convergent. In the conditional case its value is independent of $x_0$: anchoring instead at $x_1$ replaces the feature map by $\Phi^\circ(x)-\Phi^\circ(x_1)$, whose integral against $\xi$ is unchanged because $\xi(\Xx)=0$. Whenever the original $\Krkhs$-energy is absolutely convergent, it equals~\eqref{eq-kernel-dual}. For bounded continuous positive-definite kernels on separable metric spaces, every finite signed measure is admissible. These seminorms are usually called maximum mean discrepancies in statistics and machine learning~\cite{gretton2012kernel,muandet2017kernel}, and kernel norms in shape analysis~\cite{glaunes2004diffeomorphic,vaillant2005surface}. If $X,X'$ are independent with law $\al$, then \(\norm{\al}_{\Krkhs}^2=\EE_{X,X'}(\Krkhs(X,X'))\) whenever $\al$ is admissible for a positive-definite kernel.
\index{kernel!norm}% strictindex
\index{RKHS}% strictindex
\index{maximum mean discrepancy}% strictindex
...
KL and total variation are two very different $\phi$-divergences: the former is smooth and sensitive to density ratios, whereas the latter is a nonsmooth norm. Pinsker's fundamental inequality nevertheless controls the square of the latter by the former~\cite{pinsker1964information}.

\begin{thm}[Pinsker inequality]\label{thm-pinsker}
\index{Pinsker inequality}% strictindex
\index{total variation}% strictindex
\index{Kullback-Leibler@Kullback--Leibler!divergence}% strictindex
	Let $\al,\be\in\Mm_+^1(\Xx)$ be probability measures on a measurable space. With the full-variation convention~\eqref{eq-defn-tv},
	\[
		\TV(\al|\be)^2
		=
		\norm{\al-\be}_{\TV}^2
...
\begin{proof}
	...
	\[
		...
		\frac12\norm{\al-\be}_{\TV}.
	\]
	Applying Jensen's inequality separately on $A$ and $A^c$, or equivalently coarse-graining onto the partition $(A,A^c)$, gives
	\[
		\KL(\al|\be)
		\geq
		a\log\frac{a}{b}
	...
	\]
	For fixed $b\in(0,1)$, subtract $2(a-b)^2$ from the binary relative entropy on the right. The resulting function of $a$ has value and first derivative zero at $a=b$, while its second derivative is
	\[
		\frac1a+\frac1{1-a}-4\geq0.
	\]
...
Several classical divergences fit in the same template. For $s>0$, the power-divergence family
\[
...
\]
It interpolates, up to conventional multiplicative normalizations, between Pearson's $\chi^2$ divergence at $\gamma=2$, Hellinger behavior at $\gamma=1/2$, KL as $\gamma\to1$, and reverse KL (or Burg entropy) $\phi_0(s)=-\log s+s-1$ as $\gamma\to0$. The latter has $\phi_0(0)=+\infty$ and $\phi'_{0,\infty}=1$. The Hellinger divergence is often written separately with $\phi_H(s)=(\sqrt s-1)^2$, for which $\phi_H(0)=\phi'_{H,\infty}=1$. If $\al=\rho_\al\lambda$ and $\be=\rho_\be\lambda$ relative to any common dominating measure $\lambda$, then
\[
	\Hellinger(\al,\be)
	\eqdef\Divergm_{\phi_H}(\al|\be)^{1/2}
	=\norm{\sqrt{\rho_\al}-\sqrt{\rho_\be}}_{L^2(\lambda)}.
\]
For probability measures, the Jensen--Shannon distance~\cite{endres2003new,osterreicher2003new} is the square root of the symmetrized KL-to-the-mixture divergence
\index{Pearson divergence}% strictindex
\index{Kullback-Leibler@Kullback--Leibler!divergence!reverse}% strictindex
\index{Burg entropy}% strictindex
\index{Jensen-Shannon@Jensen--Shannon divergence}% strictindex
...
\]
Here $\phi_{\JS}(0)=\phi'_{\JS,\infty}=\frac12\log 2$; the unscaled generator $2\phi_{\JS}$ used below therefore has recession slope $\log 2$.
...
\begin{proof}
	...
	\[
		\int f\d\al-\Divergm_\phi(\al|\be)
		=
		\int\bigl(f\rho-\phi(\rho)\bigr)\d\be
		+
		\int(f-L)\d\eta.
	\]
...
Under the compact-space hypotheses of Proposition~\ref{prop-phi-div-dual}, any $\phi$-divergence can be written in adversarial form through~\eqref{eq-dual-div}~\cite{NowozinCsekeTomioka2016fGAN}. Writing $L=\phi'_\infty$ gives
\index{phi-divergence}% strictindex
\index{dual!formula}% strictindex
\eq{
...
\[
	\inf_\theta\sup_{\xi:\,f_\xi\leq L}
	\int_\Zz f_\xi(g_\theta(z))\d\zeta(z)
	-
	\frac1m\sum_{j=1}^m \phi^*(f_\xi(y_j)).
\]
For fixed $\theta$, this admissible restriction gives a lower bound on the exact divergence unless the discriminator class is rich enough to attain the supremum. Mapping the critic output into $\dom\phi^*$ is a convenient stronger condition. When $L<+\infty$ and the endpoint is excluded from $\dom\phi^*$, a strict output activation can approach, but need not attain, the value that prices generated mass singular to the data. Without a global cap, the empirical data sum alone does not control critic values on such generated-only support and the displayed objective need not be a divergence lower bound. This distinction is essential for empirical data: if $\be$ is discrete and $\al_\theta$ is non-atomic, a superlinear divergence is $+\infty$, whereas its neural restricted objective can remain finite. The original vanilla GAN~\cite{GAN} corresponds, up to an additive constant and the usual reparametrization by a discriminator taking values in $(0,1)$, to the unscaled Jensen--Shannon generator $\widehat\phi_{\JS}=2\phi_{\JS}$,
\[
	\widehat\phi_{\JS}(s)=s\log s-(s+1)\log\frac{s+1}{2},
	\qquad
	\widehat\phi_{\JS}^*(u)=-\log(2-e^u),\quad u<\log 2.
\]
Thus $\Divergm_{\widehat\phi_{\JS}}=2\JS^2$, consistently with the normalization above. In practice the min--max problem is solved by alternating stochastic gradient descent/ascent on $(\theta,\xi)$. Although the unrestricted maximization is concave in the function $f$, neural parametrization generally destroys concavity in $\xi$; the generator problem is likewise non-convex in $\theta$. These facts contribute to instability and mode collapse. Moreover, density-ratio losses are informative when the measures overlap but can saturate when model and data are mutually singular: $\JS^2$ reaches its maximum $\log 2$ on disjoint supports.
...
\index{weight clipping}% strictindex
	Wasserstein GANs originally used a tunable clipping parameter $c>0$, constraining each critic parameter to $[-c,c]$, equivalently $\norm{\xi}_\infty\leq c$, as a proxy for enforcing a Lipschitz bound~\cite{WassersteinGAN}. The resulting Lipschitz constant depends on the architecture and on $c$ and is not canonically one. The parameter box is convex, but its image through a neural-network parametrization is neither the full unit Lipschitz ball nor generally a convex class of functions, and optimization in the parameters remains non-concave. Clipping is therefore a practical heuristic rather than a faithful implementation of the Kantorovich--Rubinstein constraint.
\end{rem}
