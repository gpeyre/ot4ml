...
The quadratic Benamou--Brenier formula is only one instance of a broader fixed-mass dynamic language. By changing the local action, the mobility, the state graph, the velocity class or the allowed jumps, one obtains transport geometries adapted to constraints, Markov chains, normalized directions, kernelized particle motion or nonlocal motion. The focus here is deliberately metric: we define distances whose tangent variables still preserve total mass. Their use as gradient-flow geometries is gathered later in Section~\ref{sec-generalized-dynamic-wasserstein-flows}; the reaction--transport case, where mass may vary, is separated in Section~\ref{sec-unbalanced-ot}.
\index{Wasserstein!distance}% strictindex
\index{perspective function}% strictindex
...
\end{equation}
whenever this limit exists. The notation \(q_\alpha\) is intentionally broader than a quadratic form: it is the squared local norm, or more generally the squared local action, selected by \(d\). For instance, for \(\Wass_p\) with \(1\leq p<+\infty\), one has \(q_\alpha(w)=(\int\norm{w}^p\d\alpha)^{2/p}\); for the spectral distances studied below it is the square of the spectral tangent norm. Section~\ref{sec-generalized-dynamic-wasserstein-flows} uses the same \(q_\alpha\) to define steepest-descent directions and the corresponding gradient-flow PDEs.
...
Here \(q_\alpha\) is assumed quadratic and represented by an operator \(Q_\alpha\). The Hilbertian, or Riemannian, special case occurs when \(q_\alpha\) is quadratic. If its polarization is represented on \(T_\alpha\) by a positive self-adjoint operator \(Q_\alpha:T_\alpha\to T_\alpha\),
\[
	q_\alpha(w,z)
	=
...
This quadratic local action can be integrated along curves to define a geodesic distance. Keeping the continuity equation but replacing the usual infinitesimal norm \(\int\norm{v}^2\d\alpha\) by \(\langle Q_\alpha v,v\rangle_{L^2(\alpha)}\) leads to the formal geodesic value
...
For a smooth state \(\alpha=\rho\d x\) and a velocity \(v\), assume that \(A\) is \(r\)-homogeneous in the momentum and that the induced distance is \(\mathsf D_A=\mathcal A_A^{1/r}\). The associated local squared action is then written with the same \(q\)-notation as in \eqref{eq-generalized-distance-tangent-form}:
\begin{equation}\label{eq-local-action-density-induced}
	...
	\right)^{2/r}.
\end{equation}
For quadratic actions, \(r=2\), this is simply the instantaneous action \(\int A(\rho,\rho v)\d x\). For nonquadratic Finsler actions, such as \(\Wass_p\) with \(p\neq2\), the raw action and the local squared distance differ by the exponent \(2/r\). This convention keeps the metric definition in this section aligned with the steepest-descent notation used later in Section~\ref{sec-generalized-dynamic-wasserstein-flows}.
...
\index{finite-action component}% strictindex
	...
	\[
		A(\rho,\lambda m)=|\lambda|^rA(\rho,m)
		\qquad
...
\begin{proof}
	...
	\[
		...
		\end{cases}
	\]
	The rescaled curve is admissible from \(\rho_0\) to \(\rho_2\). By \(r\)-homogeneity in \(m\), its action is
	\[
		\tau^{1-r}a+(1-\tau)^{1-r}b.
	\]
	Writing \(X=a^{1/r}\) and \(Y=b^{1/r}\), the optimal allocation is \(\tau=X/(X+Y)\), with the evident limiting convention if \(X+Y=0\), and gives
	\[
		\inf_{\tau\in(0,1)}
		\left\{\tau^{1-r}a+(1-\tau)^{1-r}b\right\}
		=
		(X+Y)^r.
	\]
	Hence \(\mathcal A_A(\rho_0,\rho_2)^{1/r}\leq a^{1/r}+b^{1/r}\). Taking \(a\) and \(b\) arbitrarily close to the two optimal values yields
	\[
...
Without homogeneity or nondegeneracy, \(\mathcal A_A\) remains a useful action principle, but its \(r\)-th root need not define a distance.
...
\index{Wasserstein-p action}% strictindex
	...
	\[
		...
		\end{cases}
	\]
	under the linear constraint $\partial_t\rho_t+\nabla\!\cdot m_t=0$. This is precisely Proposition~\ref{prop-homogeneous-dynamic-action-distance} with \(r=p\): the $p$-th root of the value is $\Wass_p$. The endpoint $p=1$ can be treated separately: the perspective becomes $A_1(\rho,m)=|m|$, and the dynamic problem collapses to Beckmann's formulation of $\Wass_1$~\cite{Beckmann52}.
	In the local notation above, the squared tangent cost associated with the distance \(\Wass_p\) is
	\[
		q_{p,\alpha}(v)=q_{A_p,\alpha}(v)=
		\left(\int |v|^p\d\alpha\right)^{2/p}.
	\]
\index{Wasserstein!distance}% strictindex
