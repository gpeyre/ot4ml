...
Figure~\ref{fig:statistical-berry-esseen-w1} confronts these equivalents with exact one-dimensional computations. For a centered, unit-variance input law \(\alpha\), define
\[%%%
...
\begin{proof}
Suppose first that \(k\) has a positive-feature representation. For \(x_1,\ldots,x_n\), let \(v_q(z):=(\phi_q(x_i,z))_{i=1}^n\geq0\). Their Gram matrix is
\[
