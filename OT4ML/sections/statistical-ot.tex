...
\begin{proof}
	...
	\[%%%
		...
		C\eta^2 h^d,
	\]
	because the two densities differ only on \(Q_k\), the perturbation has size \(O(\eta)\), and the densities are uniformly bounded away from zero. For \(N\) independent samples the divergence is multiplied by \(N\). Choose \(m\) so that \(m^d\simeq N\). Then \(N h^d\) is bounded, and, by taking \(\eta\) sufficiently small, Theorem~\ref{thm-pinsker} gives a uniform bound
	\[%%%
