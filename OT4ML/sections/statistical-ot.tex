...
\begin{equation}\label{eq-empirical-wasserstein-scale}
	...
	\end{cases}
\end{equation}
Indeed, if \(\widehat\alpha_n=n^{-1}\sum_{i=1}^n\de_{X_i}\) for i.i.d.\ samples \(X_i\sim\alpha\) and \(\Xx\subset\RR^d\) is compact, then
\begin{equation}\label{eq-empirical-wasserstein-moment-scale}
	...
	C_{\Xx,p,d}\,r_{n,p,d}.
\end{equation}
These worst-case rates follow from empirical Wasserstein moment estimates~\cite{dereich2013constructive,fournier2015rate}. They can improve under additional regularity or lower-dimensional support assumptions, as discussed below.
...
\begin{proof}
	...
	\]
	The dual formulation writes the map \(\a'\mapsto\MKD_\C(\a',\b)\) as the supremum, over the feasible polyhedron \(f_i+g_j\leq \C_{i,j}\), of the affine functions \(\a'\mapsto \dotp{\fD}{\a'}+\dotp{\gD}{\b}\). Its directional derivative at \(\a\), in any tangent direction \(h\) with \(\dotp{\ones}{h}=0\), is therefore
	\[
		D_{\a}\MKD_\C(\a,\b)[h]
		=
...
\[
	...
	\qquad
	K_{i,j}=k_\epsilon(x_i,y_j)\eqdef e^{-c(x_i,y_j)/\epsilon}.
\]
Assume here that the weights are positive and that \(K_{i,j}>0\). The Sinkhorn scaling equations are
\[
	u=\frac{a}{Kv},
	\qquad
	v=\frac{b}{K^\top u},
...
Let \(\epsilon>0\) and \(K_{i,j}=\EE h_{i,j}(Z)>0\) for \(1\leq i\leq n\), \(1\leq j\leq m\), where \(Z\) has law \(\xi\) and \(|h_{i,j}(Z)|\leq M\) almost surely. Set \(c_{i,j}=-\epsilon\log K_{i,j}\), and let
\[
	\widetilde K_{i,j}=\frac1r\sum_{\ell=1}^r h_{i,j}(Z_\ell),
	\qquad
	Z_1,\ldots,Z_r\sim\xi.
\]
Then, for every \(\eta>0\),
\[
	\PP\pa{\max_{i,j}\abs{\widetilde K_{i,j}-K_{i,j}}>\eta}
	\leq
	2nm\exp\pa{-\frac{r\eta^2}{2M^2}}.
\]
In particular, if \(\kappa\eqdef\min_{i,j}K_{i,j}>0\), \(\eta<\kappa\), and the event above does not occur, then \(\widetilde K\) is entrywise positive. If moreover \(\eta\leq\kappa/2\), then the sketched cost \(\widetilde c_{i,j}=-\epsilon\log \widetilde K_{i,j}\) satisfies
\[
	\max_{i,j}\abs{\widetilde c_{i,j}-c_{i,j}}
	\leq
	\frac{2\epsilon\eta}{\kappa}.
\]
If, more sharply, the relative variables \(\zeta_{i,j}(Z)\eqdef h_{i,j}(Z)/K_{i,j}\) satisfy \(|\zeta_{i,j}(Z)|\leq\psi\) almost surely, then for every \(0<\delta\leq1/2\),
\[
	\PP\pa{
...
The available worst-case bounds retain an explicit dimension dependence. For the quadratic cost on points contained in a radius-\(R\) ball of \(\RR^d\), the Nystr\"om analysis of Altschuler, Bach, Rudi and Niles-Weed~\cite{AltschulerBachRudiNilesWeed2019NystromSinkhorn} studies \(K_{i,j}=\exp(-\eta\norm{x_i-x_j}^2)\) and bounds the effective rank by a quantity of the form
\[
