...
\]
If singular component covariances are allowed, the Bures cost~\eqref{eq-bures-defn} remains valid, but the deterministic map $A_{i,j}$ need not exist. In that case, choose minimizing factors $R_{i,j},S_{i,j}\in\RR^{d\times d}$ in the Procrustes formula~\eqref{eq-bures-procrustes}, so that $R_{i,j}R_{i,j}^{\top}=\Sigma_i$ and $S_{i,j}S_{i,j}^{\top}=\Lambda_j$. If $Z\sim\Gaussian(0,\Id_d)$, then $X=m_i+R_{i,j}Z$ and $Y=n_j+S_{i,j}Z$ form an optimal jointly Gaussian coupling $\gamma_{i,j}$ of the two components. Define their interpolation as the law of $(1-t)X+tY$. This is still a Gaussian $\Wass_2$ geodesic, although singular endpoints can admit several optimal couplings and hence several geodesics.
...
\index{Gromov-Wasserstein@Gromov--Wasserstein!distance}% strictindex
		...
		\end{align}
\end{defn}
This size is the natural domain condition: GW from $\XX$ to the one-point space equals $\mathsf S_{\De,p}(\XX)$. More generally, the triangle inequality for $\De$ and Minkowski's inequality bound the $p$th root of every feasible objective in~\eqref{eq-gw-generic} by $\mathsf S_{\De,p}(\XX)+\mathsf S_{\De,p}(\YY)$. The objective is lower semicontinuous under weak convergence because $\pi\mapsto\pi\otimes\pi$ is weakly continuous and the distortion integrand is nonnegative and lower semicontinuous. Since the coupling set is weakly compact, the displayed minimum is finite and attained. For the standard choice $\De(r,s)=|r-s|$, this size reduces to the ordinary condition $\int\dist_\X(x,x')^p\d\al(x)\d\al(x')<+\infty$.
...
Figure~\ref{fig:gromov-nonisometric-distortion} complements the global GW objective with local diagnostics. Its generator uses squared Euclidean dissimilarities, normalizes each space by its own maximum squared distance $M_\X$ and $M_\Y$, and displays
\[
	r_{i,i'}
	=
	\left|
...
\small normalized residual $r$
\end{tabular}
\caption{\emph{Local distortion in a mildly non-isometric GW match.} The left panel colors transport segments by the row average of the normalized residual $r$ defined above. The right panel displays $r$ in white-to-black scale, with darker entries marking larger disagreement between the separately normalized squared-Euclidean dissimilarities. Its entrywise square, rather than $r$ itself, is the local square-loss integrand of the discrete GW objective.}
...
Continuous feature embeddings make this cost lower semicontinuous; on compact input spaces they also make the integrability condition below automatic.
...
Discrete DTW depends on the sampling density because every visited cell contributes once. A direct continuous registration minimizes $\int_0^1 c(x(t),y(\gamma(t)))\d t$ over nondecreasing endpoint-fixing maps $\gamma$, but this one-clock formulation privileges the parameterization of~$x$. Continuous DTW instead traverses both clocks and measures mismatch per unit length in the parameter square~\cite{BuchinNusserWong2022CDTW}. For normalized clocks $x,y:[0,1]\to\mathcal Z$, this gives the following symmetric problem.
...
Fix $\epsilon>0$. The hard minimum in~\eqref{eq-dtw-recurrence} is nonsmooth when several paths tie. Soft-DTW replaces it with the log-sum-exp soft minimum~\cite{CuturiBlondel2017SoftDTW},
