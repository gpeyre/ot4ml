...
\textbf{Input:} Symmetric metric matrices $\distD,\distD'$, weights $\a,\b$, regularization $\epsilon>0$, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.
...
\textbf{Set} $T^{(k+1)}=T_e(F^{(k+1)},G^{(k+1)})$ and
\(r_{k+1}=\max\{\norm{\operatorname{Tr}_B T^{(k+1)}-A}_{\mathrm F},\norm{\operatorname{Tr}_A T^{(k+1)}-B}_{\mathrm F}\}.\)

\textbf{If} $r_{k+1}\leq\mathrm{tol}$ \textbf{then}:
\begin{algblock}
\textbf{Return} $T^{(k+1)}$ and $r_{k+1}$.
\end{algblock}
\end{algblock}
\algreturnskip
\textbf{Return} $T^{(L)}$ and $r_L$.
\end{alg}
