...
\[
	...
	\dotp{\C(\P^{(\ell)})}{\P}-\frac{\epsilon}{2}\HD(\P).
\]
...
\begin{algblock}
\(\C^{(\ell)} = \distD^{\odot2}\a\,\ones_m^\top + \ones_n(\distD'^{\odot2}\b)^\top - 2\distD\,\P^{(\ell)}\,\transp{\distD'}\).

\textbf{Solve} entropic OT subproblem:
\(\P^{(\ell+1)} = \uargmin{\P\in\CouplingsD(\a,\b)} \dotp{\P}{\C^{(\ell)}}-(\epsilon/2)\HD(\P)\).

\textbf{If} $\norm{\P^{(\ell+1)}-\P^{(\ell)}}_{\mathrm F}\leq\mathrm{tol}$ \textbf{then}:
\begin{algblock}
...
The set-based analogues use worst-case rather than averaged distortion.
