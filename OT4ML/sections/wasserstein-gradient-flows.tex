...
\textbf{Input:} Particles $X^0=(x_1^0,\ldots,x_n^0)$, functional $f$, step size $h>0$, tolerance $\mathrm{tol}\geq0$, integer maximum iteration count $K\geq1$.
...
\index{maximum mean discrepancy}% strictindex
\textbf{Input:} Initial particles $(x_i^0)_{i=1}^n$, teacher law $\beta$ or teacher samples $(y_b)_{b=1}^B$, kernel $k$, step size $h>0$, integer number of steps $K\geq1$.
