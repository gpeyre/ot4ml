...
\index{JKO scheme}% strictindex
\textbf{Input:} Energy $f$, initial measure $\alpha^0$, time step $\tau>0$, integer number of steps $K\geq1$.
...
\left\{\frac{1}{2\tau}\Wass_2^2(\alpha^k,\alpha)+f(\alpha)\right\}\)
...
\textbf{Input:} Particles $X^0=(x_1^0,\ldots,x_n^0)$, functional $f$, step size $h>0$, tolerance $\mathrm{tol}\geq0$, iteration budget $K\geq1$.

\textbf{Output:} Particle trajectory $(X^k)_{k=0}^{k_\star}$ and empirical measures $\alpha^k=\frac1n\sum_i\delta_{x_i^k}$, with $1\leq k_\star\leq K$.
...
\begin{algblock}
\textbf{Return} $(X^\ell)_{\ell=0}^{k+1}$ and $(\alpha^\ell)_{\ell=0}^{k+1}$.
\end{algblock}
\end{algblock}
\algreturnskip
\textbf{Return} $(X^k)_{k=0}^{K}$ and $(\alpha^k)_{k=0}^{K}$.
\end{alg}
...
\index{maximum mean discrepancy}% strictindex
\textbf{Input:} Initial particles $(x_i^0)_{i=1}^n$, teacher law $\beta$ or teacher samples $(y_b)_{b=1}^B$, differentiable symmetric kernel $k$, step size $h>0$, integer number of steps $K\geq1$.

\textbf{Output:} Particle trajectory $(x_i^\ell)_{1\leq i\leq n,\,0\leq\ell\leq K}$ targeting $\beta$.

\textbf{For} $\ell=0,\ldots,K-1$ \textbf{do}:
\begin{algblock}

\textbf{For} $i=1,\ldots,n$ \textbf{do}:
\begin{algblock}
\textbf{Set} self-interaction \(r_i^\ell=-\frac{2}{n}\sum_{j=1}^n\nabla_x k(x_i^\ell,x_j^\ell)\)

\textbf{If} $\beta$ is available analytically \textbf{then}:
\begin{algblock}

\textbf{Set} teacher attraction \(a_i^\ell=2\int\nabla_x k(x_i^\ell,y)\d\beta(y)\)

\end{algblock}
\textbf{Else}:
\begin{algblock}

\textbf{Set} \(a_i^\ell=\frac{2}{B}\sum_{b=1}^B\nabla_x k(x_i^\ell,y_b)\)

\end{algblock}
\textbf{Set} velocity \(v_i^\ell=r_i^\ell+a_i^\ell\)

\textbf{Update}
\(x_i^{\ell+1}=x_i^\ell+h\,v_i^\ell\)
\end{algblock}
\end{algblock}
\algreturnskip
\textbf{Return} $(x_i^\ell)_{i,\ell}$.
\end{alg}
...
Figures~\ref{fig:gradflow-interaction-particles} and~\ref{fig:gradflow-particle-objective-geometries} complement this target-matching example: the first isolates three self-interaction regimes, while the second compares the trajectory geometries produced by several discrepancies.
...
\index{generative model}% strictindex
	...
	\(\int_{\Sphere^{d-1}}\abs{\dotp{\theta}{x_0-x_1}}^2\d\sigma(\theta)=\norm{x_0-x_1}^2/d\) and \(\pi_{01}\) is \(\Wass_2\)-optimal. Thus this is a semiconcavity estimate for the sliced discrepancy along \(\Wass_2\) geodesics, with a \(\Wass_2\)-controlled correction. If one instead works in the idealized Hilbert embedding by projected quantile functions, straight sliced chords satisfy the exact identity with the last term \(\SW_2^2(\alpha_0,\alpha_1)\). The caveat, discussed in Remark~\ref{rem-sliced-length-not-wasserstein}, is that such straight chords need not be realized by actual measures on \(\RR^d\).
\end{rem}
...
The logarithmic Sobolev inequality, introduced by Gross~\cite{Gross1975LogSobolev}, turns entropy into a quantitative relaxation principle by controlling it with its dissipation. Let \(\beta=\rho_\beta\d x\) be a reference probability measure. For \(\alpha=h\beta\), define the relative Fisher information by
\begin{equation}\label{eq-relative-fisher-information}
	...
	\int \frac{\norm{\nabla h}^2}{h}\,\d\beta,
\end{equation}
with the lower-semicontinuous convention \(\mathcal I(\alpha|\beta)=+\infty\) when \(\alpha\not\ll\beta\) or when the displayed integral is not finite. One says that \(\beta\) satisfies a logarithmic Sobolev inequality with constant \(\lambda>0\) if
