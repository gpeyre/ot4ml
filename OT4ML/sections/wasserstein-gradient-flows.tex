...
\begin{equation}
    \alpha^{k+1} \in \uargmin{\alpha\in\Pp_2(\RR^d)}
    \left\{\frac{1}{2 \tau} \Wass_2(\alpha^k, \alpha)^2 + f(\alpha)\right\}. \label{eq:jko-discr}
\end{equation}
\end{defn}
The piecewise-constant interpolation $\alpha_\tau(t)=\alpha^k$ for $t\in[k\tau,(k+1)\tau)$ is called a minimizing movement.
...
As $\tau \to 0$, under certain conditions on $f$, \eqref{eq:jko-discr} defines a continuous evolution $t \mapsto \alpha_t$. As discussed earlier, this evolution can be described as a Lagrangian evolution \eqref{eq:lagrangian-advection}. Its differential is represented by a scalar first variation.
\index{first variation}% strictindex
\index{signed!measure}% strictindex
\begin{defn}[First variation]\label{def-first-variation}
Assume that $f(\alpha)<+\infty$. A measurable function $\delta f(\alpha)$ is a first variation of $f$ at $\alpha$ if, for every $\beta\in\Pp(\RR^d)$ such that $\delta f(\alpha)$ is integrable with respect to $|\beta-\alpha|$ and $f((1-\tau)\alpha+\tau\beta)<+\infty$ for all sufficiently small $\tau>0$, setting $\eta=\beta-\alpha$ gives
\begin{equation*}
    f((1-\tau)\alpha + \tau \beta) = f(\alpha + \tau \eta) = f(\alpha) + \tau \int [\delta f(\alpha)](x) \d \eta(x) + o(\tau)
    \qquad(\tau\downarrow0).
\end{equation*}
\end{defn}
It is defined only up to an additive constant because $\eta$ has zero total mass.
...
\begin{example}[Homogeneous Wasserstein--KL examples]\label{ex-homogeneous-wasserstein-kl}
...
\index{interaction energy}% strictindex
	...
	\[
		|\partial f_q|^2(\alpha)
		=
		q^2\int |x|^{2q-2}\,\d\alpha(x)
		\geq
		q^2 f_q(\alpha)^{2(q-1)/q},
	\]
	...
	The same homogeneity appears for powers of distances. On a geodesic metric space, the function \(x\mapsto d(x,x_\star)^q\) has metric slope \(q d(x,x_\star)^{q-1}\) away from the minimizer. Consequently, in the matching \(\Wass_q\) geometry, the functional \(\alpha\mapsto\Wass_q^q(\alpha,\beta)\) has formal metric slope \(q\Wass_q(\alpha,\beta)^{q-1}=q f(\alpha)^{(q-1)/q}\). In the present \(\Wass_2\) geometry, this applies directly to powers \(\Wass_2^q(\alpha,\beta)\); the notation \(\Wass_q^q\) should be read as the analogous statement for the \(q\)-Wasserstein metric.
	...
	\[
		\frac{\delta f_q^{\rm int}}{\delta\alpha}(x)
		=
		\int |x-y|^q\,\d\alpha(y),
	\]
	...
	\[
		G_\alpha(x)
		=
		q\int |x-y|^{q-2}(x-y)\,\d\alpha(y).
	\]
	Writing \(\bar x_\alpha=\int x\,\d\alpha(x)\), one has \(\int G_\alpha\,\d\alpha=0\) and, by symmetrization,
	\[
		\int G_\alpha(x)\cdot(x-\bar x_\alpha)\,\d\alpha(x)
		=
		q f_q^{\rm int}(\alpha).
	\]
	Moreover, Jensen gives
	\[
		\int |x-\bar x_\alpha|^q\,\d\alpha(x)
		\leq
		\iint |x-y|^q\,\d\alpha(x)\d\alpha(y)
		=
		2 f_q^{\rm int}(\alpha).
	\]
	Combining this estimate with H{\"o}lder and Cauchy's inequality yields the non-sharp but useful bound
	\[
		|\partial f_q^{\rm int}|(\alpha)
		=
		\left(\int |G_\alpha|^2\,\d\alpha\right)^{1/2}
		\geq
		q\,2^{-1/q} f_q^{\rm int}(\alpha)^{(q-1)/q}.
	\]
...
\begin{equation}\label{eq-generalized-jko-step}
	...
	\left\{\frac{1}{2\tau}d^2(\alpha^k,\alpha)+f(\alpha)\right\}.
\end{equation}
...
=-\mathcal K_{\rho^{k+1}}\log\rho^{k+1}+o(1)
=K\rho^{k+1}+o(1).
\]
