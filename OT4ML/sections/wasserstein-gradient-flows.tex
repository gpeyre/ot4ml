...
\]
with an additional factor \(\sqrt2\) under the energy-dissipation inequality. Thus an energy-dissipating trajectory is \(1/2\)-H\"older continuous as a curve in Wasserstein space on every time interval where the energy remains bounded from below. Non-coercivity of the objective means that sublevel sets need not be compact, but an energy-dissipating curve still cannot travel an infinite \(\Wass_2\)-distance in finite time without an infinite energy drop. The PL and convexity estimates below add the missing quantitative information that forces convergence toward minimizers.
...
The exponential decay of the energy gap is the direct metric-gradient-flow analogue of the classical PL argument. Similar slope-based statements are standard in the theory of curves of maximal slope~\cite{ambrosio2006gradient} and in Wasserstein gradient flows under functional inequalities such as logarithmic Sobolev and HWI inequalities, discussed below. We are not aware of a reference that states the final \(\Wass_2\)-distance-to-\(\operatorname{Argmin}f\) estimate in exactly the form of Theorem~\ref{thm-wasserstein-pl-convergence}; the tail-length refinement used above was communicated to us by Rapha{\"e}l Barboni.
...
\begin{prop}[Convex examples covered by the theory]\label{prop-convex-flow-examples}
	...
	\begin{enumerate}
		\item For the linear energy $f(\alpha)=\int h\d\alpha$, geodesic convexity holds when $h$ is convex. If $h$ is $\lambda$-strongly convex, then $f$ is $\lambda$-geodesically convex; Proposition~\ref{prop-strong-geodesic-convexity-implies-pl} and Theorem~\ref{thm-wasserstein-pl-convergence} give the exponential rate.
\index{convexity!geodesic}% strictindex
...
\index{Brenier map}% strictindex
	...
	\]
	so it is the sum of the displacement-convex entropy and the $\lambda$-geodesically convex linear potential generated by $V$. Proposition~\ref{prop-convex-wass-flow-rate} gives dissipation and the convex \(O(1/t)\) estimate in these cases, while the PL theorem upgrades positively curved cases to exponential convergence.
\end{proof}
...
\[
	...
	\qquad a\in\RR^d,
\]
one has \(f_a\gamma_d=\mathcal N(a,\Id)\). A direct computation gives
\[
	\int f_a\log f_a\,\d\gamma_d
	=
	\frac12\norm{a}^2,
	\qquad
	\frac12\int\frac{\norm{\nabla f_a}^2}{f_a}\,\d\gamma_d
	=
	\frac12\norm{a}^2.
\]
...
The Poincar\'e inequality is the linearized form of the logarithmic Sobolev inequality at equilibrium. Let \(\beta=\rho_\beta\d x=Z^{-1}e^{-V}\d x\), write \(\alpha=h\beta\), and let \(L\varphi=\Delta\varphi-\dotp{\nabla V}{\nabla\varphi}=\rho_\beta^{-1}\nabla\cdot(\rho_\beta\nabla\varphi)\) be the reversible generator acting on test functions. If a logarithmic Sobolev inequality holds with constant \(\lambda\), then its expansion around \(h=1\) gives Poincar\'e with the same constant. Indeed, for \(h_\epsilon=1+\epsilon g\) with \(\int g\d\beta=0\),
	\[
	...
	\]
The second-order terms give the Poincar\'e, or spectral-gap, inequality
	\[
		...
		\int g\d\beta=0.
	\]
This linearization is important. Poincar\'e is not, in general, the usual \(\Wass_2\)-PL inequality for the chi-square energy \(E_2(h)=\frac12\int(h-1)^2\d\beta\), because the \(\Wass_2\) slope contains the moving mobility \(h\),
	\[
		|\partial E_2|_{\Wass_2}^2(h\beta)
		=
		\int h\norm{\nabla h}^2\d\beta .
	\]
The exact linear PL interpretation is obtained by freezing the mobility at equilibrium. This gives the \(H^{-1}(\beta)\) metric, which is precisely the linearization of \(\Wass_2\) at \(\beta\).
...
\index{Wasserstein-PL inequality}% strictindex
	...
	\]
	For a smooth density ratio \(h\) with \(\int h\d\beta=1\), set \(E_2(h)=\frac12\int(h-1)^2\d\beta\). Its squared slope in this linearized Wasserstein metric is
	\[
		|\partial E_2|_{-1,\beta}^2(h)
		=
		\int\norm{\nabla h}^2\d\beta .
	\]
	Consequently, the Poincar\'e inequality with constant \(\lambda>0\) is exactly the PL inequality
	\[
		2\lambda E_2(h)
		\leq
		|\partial E_2|_{-1,\beta}^2(h).
	\]
	The corresponding gradient flow is the linear Fokker--Planck equation \(\partial_t h_t=Lh_t\), and it satisfies
	\[
		\chi^2(h_t\beta|\beta)
		\leq
...
\]
but the velocity \(v_{\alpha}\) is now the steepest-descent direction for the geometry induced by \(d\), not necessarily for \(\Wass_2\). Unbalanced variants replace this continuity equation by a balance law, while nonlocal variants replace vector fields by jump fluxes.

The metric ingredient needed below is the local squared action \(q_\alpha\) introduced in Section~\ref{sec-generalized-dynamic-wasserstein-distances}. It is the second-order tangent cost in \eqref{eq-generalized-distance-tangent-form}. For action densities \(A(\rho,m)\), the induced object is \(q_{A,\alpha}\) in \eqref{eq-local-action-density-induced}; if \(A\) is quadratic in the momentum, this is exactly the instantaneous action \(\int A(\rho,\rho v)\d x\), while for \(r\)-homogeneous Finsler actions it is the raw action raised to the power \(2/r\). Thus \(\Wass_p\) gives \(q_{p,\alpha}(v)=(\int\norm{v}^p\d\alpha)^{2/p}\), and quadratic geometries give \(q_{A,\alpha}(v)=\int A(\rho,\rho v)\d x\). This section keeps the same \(q_\alpha\)-notation but uses it for steepest descent rather than for defining distances.
...
\begin{equation}\label{eq-local-action-steepest-descent}
	...
	\right\}.
\end{equation}
This is the infinitesimal minimization obtained by inserting \(\alpha=(\Id+\tau w)_\sharp\alpha_t\) in \eqref{eq-generalized-jko-step} and keeping the terms of order \(\tau\). The associated formal gradient-flow PDE is therefore
\begin{equation}\label{eq-local-action-gradient-flow-pde}
	...
	\right)=0.
\end{equation}
When \(q_\alpha\) is convex and \(2\)-homogeneous, \(\mathcal J_\alpha(g)\) can be chosen on the same descent ray as the normalized linear-minimization oracle
...
\end{equation}
The penalized map \(\mathcal J_\alpha\) fixes the time scale, while the LMO fixes only the direction and is the object used by normalized algorithms. If \(q_\alpha\) has zero-action directions, this statement is understood on the quotient tangent space, or after projecting onto finite-action directions modulo the kernel of \(q_\alpha\). Along smooth solutions of \eqref{eq-local-action-gradient-flow-pde}, whenever the minimizer in \eqref{eq-local-action-steepest-descent} is attained, the same \(2\)-homogeneity gives the dissipation identity
\[
	\frac{\d}{\d t}f(\alpha_t)
	=
...
In the restricted Riemannian case of \eqref{eq-generalized-distance-tangent-expansion}, \eqref{eq-local-action-steepest-descent} is a linear preconditioning rule:
\begin{equation}\label{eq-generalized-gradient-q-preconditioner}
	v_{\alpha}
	=
	-\,Q_\alpha^{-1}\Wgrad f(\alpha),
\end{equation}
with the inverse understood on the range of \(Q_\alpha\), or as a Moore--Penrose inverse after projecting onto admissible directions when constraints create null directions. For ordinary \(\Wass_2\), \(Q_\alpha=\Id\) on \(T_\alpha\) and one recovers \(v_\alpha=-\Wgrad f(\alpha)\).

The examples below are organized by the same dictionary. Concave-mobility actions use \(q_{\theta,\alpha}\) from Section~\ref{rem-generalized-bb}; spectral flows use \(q_{\gamma,\alpha}(v)=\mathcal N_{\gamma,\alpha}(v)^2\) from \eqref{eq-normalized-spectral-tangent-norm}; kernelized Benamou--Brenier flows use \(q_{k,\alpha}(v)=\norm{v}_{\RKHS_k^d}^2\) with a restricted RKHS tangent space. Discrete Markov-chain geometries and nonlocal logarithmic-mean geometries also have quadratic actions, but their tangent variables are edge potentials or jump velocities rather than vector fields in \(\RR^d\). WFR, defined by \eqref{eq-dynamic-unbalanced-ot} and studied separately in Section~\ref{sec-wfr-gradient-flows}, is another quadratic geometry, but its tangent variable is a pair \((v,g)\) combining displacement and mass modulation. Thus \(q_\alpha\) is the common notation between the distance and flow chapters, whereas \(Q_\alpha\) is reserved for Hilbertian vector-field subcases where a linear preconditioner actually represents the tangent geometry.

The generalized distances of Section~\ref{sec-generalized-dynamic-wasserstein-distances} are useful precisely because they change what \emph{steepest descent} means. The energy functional can be the same, while the local metric tensor, Finsler norm, mobility, graph structure, spectral gauge, RKHS constraint or jump kernel changes the PDE selected by the minimizing movement. The rest of this section mirrors the mass-preserving part of that dynamic-distance taxonomy. The transport--reaction case is separated in Section~\ref{sec-wfr-gradient-flows}, because there the tangent space is enlarged by a scalar growth direction.

\paragraph{General mobility flows.}
\index{mobility}% strictindex
\index{concave mobility}% strictindex
The concave-mobility distances of Section~\ref{rem-generalized-bb} replace the linear mobility \(\rho\) by \(\theta(\rho)\). This changes the Onsager operator of the gradient flow while keeping a continuity equation.

For a smooth density \(\rho_t\) and an energy \(F\), the formal gradient flow associated with the action \(A_\theta(\rho,m)=|m|^2/\theta(\rho)\) is
...
\]
Combining this with the first-variation expansion of $f$ yields the general local problem \eqref{eq-local-action-steepest-descent}, specialized here to \eqref{eq-normalized-spectral-lmo}.
...
\index{matrix!linear minimization oracle}% strictindex
...
A metric gradient flow can be read as the continuous-time limit of the implicit Euler, or JKO, step
\begin{equation}
