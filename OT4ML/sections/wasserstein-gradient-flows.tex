...
Once $\Wass_2$ is understood as a dynamic metric, one can run gradient descent directly on the space of measures. This chapter derives the Wasserstein gradient from the JKO minimizing-movement scheme, develops convergence through geodesic convexity and functional inequalities, and applies the calculus to interacting particles and mean-field neural networks. It then changes the underlying geometry to obtain mobility, spectral, nonlocal, unbalanced, conditional, and inertial flows.
...
\end{equation}
\end{defn}
The piecewise-constant interpolation $\alpha_\tau(t)=\alpha^{(\ell)}$ for $t\in[\ell\tau,(\ell+1)\tau)$ is called a minimizing movement.
...
\begin{defn}[First variation]\label{def-first-variation}
	...
	\]
\end{defn}
Because \(\beta-\alpha\) has zero mass, \(\delta f(\alpha)\) is determined only up to an additive constant. It represents the directional linear form
\[
	Df(\alpha)[\eta]
	\eqdef
...
Higher derivatives are mostly unnecessary for the first-order flows studied in this chapter, but they provide the appropriate notion of smoothness for functionals of measures. For this brief account, fix a compact set \(\Xx\subset\RR^d\) and denote by \(\Mm_0(\Xx)\) the finite signed Borel measures on \(\Xx\) with zero total mass.
\begin{defn}[Higher-order vertical derivatives]\label{def-higher-order-vertical-derivatives}
\index{vertical derivative!higher order}% strictindex
	Set \(D^0f=f\). The functional \(f\) admits vertical derivatives through order \(r\) if there are symmetric multilinear forms
	\[
		D^j f(\alpha):\Mm_0(\Xx)^j\longrightarrow\RR,
		\qquad 1\leq j\leq r,
	\]
	such that, along every mixture segment in \(\Pp(\Xx)\) and for fixed \(\eta_1,\ldots,\eta_{j-1}\in\Mm_0(\Xx)\),
	\[
		\frac{\d}{\d\vartheta}
		D^{j-1}f(\alpha_\vartheta)[\eta_1,\ldots,\eta_{j-1}]
		=
		D^j f(\alpha_\vartheta)
		[\beta-\alpha,\eta_1,\ldots,\eta_{j-1}].
	\]
	At the endpoints, derivatives are understood one-sided.
\end{defn}
When these multilinear forms have integral kernels, the notation
\[
	D^j f(\alpha)[\eta_1,\ldots,\eta_j]
	=
	\int_{\Xx^j}
	\frac{\delta^j f}{\delta\alpha^j}
	(\alpha,x_1,\ldots,x_j)
	\d\eta_1(x_1)\cdots\d\eta_j(x_j)
\]
is customary. Because the \(\eta_i\) have zero mass, these kernels have additive gauge freedoms; one can fix them by requiring zero mean in each spatial variable. This calculus and its relation to differential calculus on Wasserstein space are developed in~\cite{CarmonaDelarue2018,ChassagneuxSzpruchTse2022}.

\begin{rem}[Higher smoothness and faster particle-polynomial approximation]\label{rem-higher-order-particle-polynomial}
\index{particle polynomial}% strictindex
\index{Wasserstein!Taylor regularity}% strictindex
\index{Richardson--Romberg extrapolation}% strictindex
	Section~\ref{sec-measure-to-vector-maps} approximates \(f\) by the particle polynomial \(\mathsf B_n f(\alpha)=\EE f(\widehat\alpha_n)\) from~\eqref{eq-empirical-particle-polynomial}. Proposition~\ref{prop-holder-particle-polynomial} uses only metric H\"older regularity of order \(0<\eta\leq1\). On a geodesic Wasserstein space, a global metric H\"older exponent larger than one would force \(f\) to be constant, so higher smoothness must instead be measured after subtracting a Taylor polynomial.

	Let \(s>1\), set \(r=\lceil s\rceil-1\), and define
	\[
		\operatorname{Tay}_\alpha^r f(\beta)
		\eqdef
		f(\alpha)
		+\sum_{j=1}^r\frac1{j!}
		D^j f(\alpha)[(\beta-\alpha)^{\otimes j}].
	\]
	We say that \(f\) has Wasserstein--Taylor regularity of order \(s\) when its vertical derivatives exist through order \(r\) and
	\begin{equation}\label{eq-higher-order-wasserstein-holder}
		\left|f(\beta)-\operatorname{Tay}_\alpha^r f(\beta)\right|
		\leq
		L\Wass_p(\alpha,\beta)^s.
	\end{equation}
	The convention \(r=\lceil s\rceil-1\) treats an integer \(s\) as the Lipschitz endpoint \(C^{s-1,1}\).

	Combining the weak empirical expansion of~\cite{ChassagneuxSzpruchTse2022} with~\eqref{eq-higher-order-wasserstein-holder} gives, when the centered derivative kernels are uniformly bounded and with \(r_{n,p,d}\) from~\eqref{eq-empirical-wasserstein-scale},
	\[
		...
		\end{cases}
	\]
	The reason is simple: \(\EE(\widehat\alpha_n-\alpha)=0\), so the linear Taylor term vanishes in expectation, while the quadratic term generally produces a bias of order \(n^{-1}\).

	A signed Richardson--Romberg combination
	\[
		\mathsf B_n^{[J]}f
		=
		\sum_{j=1}^J a_j\mathsf B_{m_jn}f,
		\qquad
		\sum_{j=1}^J a_j=1,
		\qquad
		\sum_{j=1}^J a_jm_j^{-k}=0
		\quad (1\leq k<J),
	\]
	for distinct positive integers \(m_j\), cancels the first \(J-1\) bias coefficients. If \(s>2J-2\) and the derivative kernels through order \(2J-2\) are uniformly bounded, then
	\[
		\norm{\mathsf B_n^{[J]}f-f}_\infty
		\lesssim
		n^{-J}+r_{n,p,d}^{\,s}.
	\]
	Thus higher vertical smoothness can be converted into faster polynomial approximation, although none of this machinery is needed for the first-order PDEs below.
\end{rem}
...
The vertical derivative asks how \(f\) reacts when mass is reweighted. Wasserstein geometry asks how it reacts when mass is moved through a Lagrangian displacement \((\Id+\vartheta v)_\sharp\alpha\). When the first variation is differentiable in \(x\), its spatial gradient represents this horizontal differential.
...
\index{Wasserstein!gradient}% strictindex
	Section~\ref{sec-measure-to-vector-maps} introduced Wasserstein--Lipschitz regularity as the endpoint of Wasserstein--H\"older regularity; see Definition~\ref{def-wasserstein-holder-functional}. An \(L\)-Lipschitz functional has descending metric slope at most \(L\). Conversely, on a geodesic metric space, if the slope is a strong upper gradient uniformly bounded by \(L\), integrating it along geodesics shows that the functional is \(L\)-Lipschitz. In the smooth Otto calculus, under the chain-rule assumptions used in Proposition~\ref{prop-formal-wass-gradient}, this slope is
	\[
...
give the first-order problem
	\[
		...
		+o(\tau).
	\]
Its pointwise minimizer is $v=-\Wgrad f(\alpha_t)$, which gives the velocity in the continuity equation. A rigorous passage from the discrete scheme to a curve of maximal slope uses compactness and lower-semicontinuity rather than this formal parametrization; see~\cite{JDB-JKO,ambrosio2006gradient}.
\index{continuity equation}% strictindex
...
For lower-semicontinuous $f:\mathcal X\to(-\infty,+\infty]$, define
		...
		\end{cases}
		\]
It is a strong upper gradient if $|f(y_t)-f(y_s)|\leq \int_s^t |\partial f|(y_r)|\dot y_r|\d r$ along every absolutely continuous $y$. When this property holds, a curve $x_t$ is a curve of maximal slope for $f$ if
...
\centering
\begin{tabular}{@{}ccc@{}}
\small small cap, $\rho_{\max}=.32$ & \small medium cap, $\rho_{\max}=.72$ & \small no cap, $\rho_{\max}=+\infty$ \\[-.15em]
...
\end{tabular}
\caption{\emph{Density-constrained gradient flow under a quadratic attraction.} The one-dimensional flow is computed in quantile variables by explicit Lagrangian descent followed by the isotonic projection enforcing $\partial_s q\geq1/\rho_{\max}$. Each panel stacks five times vertically, colored from red to blue, using the same vertical density scale in all three panels. The dashed gray line marks the maximal density $\rho_{\max}$ for the constrained runs. A small cap forces the mass to form a wide saturated block, a medium cap allows a narrower congested region, and the unconstrained flow concentrates freely near the minimizer of $h$.}
\index{density!constraint}% strictindex
\index{crowd motion}% strictindex
\index{pressure field}% strictindex
...
\end{equation}
This is the diagonal, independent-channel geometry; cross-effects may still enter through the energy or through constraints. In the dynamic language of Section~\ref{sec-vector-matrix-valued-measures}, it corresponds to the diagonal mobility~\eqref{eq-diagonal-vector-mobility}. The corresponding JKO step is
...
A second useful model imposes a pointwise composition constraint. Given a fixed reference measure $\beta=\density{\beta}\d x$ with total mass $\sum_i m_i$, set
...
\index{convexity!geodesic}% strictindex
	...
	\begin{enumerate}
	\item If \(\Omega\subset\RR^d\) is convex, \(\beta=\density{\beta}\d x\) is uniform on \(\Omega\), and \(m\geq1-1/d\), then \(\alpha\mapsto\Divergm_{\phi_m}(\alpha|\beta)\) is geodesically convex on \(\Pp_2(\Omega)\).
...
\begin{proof}
	...
	\[
		...
		\geq\rho_{\max}^{-1/d}.
	\]
	Hence $\rho_t\leq\rho_{\max}$ for all $t$. The general case follows by approximation and lower semicontinuity of the $L^\infty$ bound, as in McCann's displacement-convexity theory~\cite{mccann1997convexity}.
\index{Brenier!map}% strictindex
...
\index{generative model}% strictindex
	...
	\(\alpha_t=((1-t)P_0+tP_1)_\sharp\pi_{0,1}\), where \(\pi_{0,1}\) is optimal between \(\alpha_0\) and \(\alpha_1\). Fix \(t\), put \(Z=(1-t)X_0+tX_1\), take an optimal coupling between \(\alpha_t\) and \(\beta\), and glue it with a disintegration of \(\pi_{0,1}\) over \(Z\). This gives random variables \((X_0,X_1,Y)\) such that \((Z,Y)\) is optimal between \(\alpha_t\) and \(\beta\), while \((X_0,X_1)\) is optimal between \(\alpha_0\) and \(\alpha_1\). Since \((X_i,Y)\) are admissible couplings between \(\alpha_i\) and \(\beta\),
	\[
	...
	The sliced loss has the same flavor, but one must keep track of which geometry is used. The projection of a \(\Wass_2\) geodesic need not be the monotone one-dimensional geodesic between the projected endpoint measures. Fix a direction $\theta$ and project $\pi_{0,1}$ onto the pair $(\dotp{\theta}{X_0},\dotp{\theta}{X_1})$. This projected endpoint coupling generates the projected curve, although it need not be optimal. At time $t$, couple the projected $\alpha_t$ optimally to the projected $\beta$, then glue this coupling with a disintegration of the projected endpoint coupling over $(1-t)\dotp{\theta}{X_0}+t\dotp{\theta}{X_1}$. The same Euclidean identity therefore applies, with correction cost $\EE|\dotp{\theta}{X_0-X_1}|^2$. Integrating in $\theta$ with the normalization of Definition~\ref{def-sliced-wasserstein} gives, along the original \(\Wass_2\) geodesic,
	\[
		\SW_2^2(\alpha_t,\beta)
		\geq
	...
	\(\int_{\Sphere^{d-1}}\abs{\dotp{\theta}{x_0-x_1}}^2\d\sigma(\theta)=\norm{x_0-x_1}^2/d\) and \(\pi_{0,1}\) is \(\Wass_2\)-optimal. Thus this is a semiconcavity estimate for the sliced discrepancy along \(\Wass_2\) geodesics, with a \(\Wass_2\)-controlled correction. If one instead works in the idealized Hilbert embedding by projected quantile functions, straight sliced chords satisfy the exact identity with the last term \(\SW_2^2(\alpha_0,\alpha_1)\). The caveat, discussed in the paragraph on intrinsic sliced length in Section~\ref{sec-sliced-wasserstein}, is that such straight chords need not be realized by actual measures on \(\RR^d\).
\end{rem}
...
The PL condition is the quadratic case of a broader slope--energy principle. The classical starting point is the gradient inequality of {\L}ojasiewicz for real-analytic functions~\cite{Lojasiewicz1963GradientInequality}, later extended by Kurdyka to functions definable in o-minimal structures~\cite{Kurdyka1998GradientsDefinable}. Modern nonsmooth optimization uses the same idea as the Kurdyka--{\L}ojasiewicz property to prove convergence of descent algorithms and subgradient flows; see, for instance, Bolte, Daniilidis, Ley and Mazet~\cite{BolteDaniilidisLeyMazet2010Lojasiewicz} and Attouch, Bolte, Redont and Soubeyran~\cite{attouch-2010}. For displacement-convex functionals, Bolte and Blanchet~\cite{BolteBlanchet2018LojasiewiczDisplacementConvex} develop a family of {\L}ojasiewicz-type functional inequalities in Wasserstein space and relate them to convergence of the associated gradient dynamics. In the metric-space setting relevant here, the Euclidean gradient norm is replaced by the metric slope, as in the general theory of curves of maximal slope and recent KL/PL convergence results for metric gradient flows~\cite{HauerMazon2019KLSMetric,DelloSchiavoMaasPedrotti2023LocalConditions}.
...
Theorem~\ref{thm-wasserstein-kl-sublinear} is the Wasserstein analogue of the standard KL-rate argument for curves of maximal slope in metric spaces; see Attouch, Bolte and Svaiter~\cite{attouch-2010}, Hauer and Maz{\'o}n~\cite{HauerMazon2019KLSMetric}, and Dello Schiavo, Maas and Pedrotti~\cite{DelloSchiavoMaasPedrotti2023LocalConditions}. The endpoint \(\theta=1/2\) is the PL theorem above. Exponents \(\theta>1/2\) correspond to flatter landscapes and slower polynomial rates; formally, \(\theta<1/2\) would instead lead to finite-time convergence in the scalar comparison inequality.
...
\index{moment energy}% strictindex
	Let \(V\) be smooth and bounded below, set \(V_\star=\inf V\), and define \(f_V(\alpha)=\int V\,\d\alpha\), whose formal slope is \(|\partial f_V|^2(\alpha)=\int|\nabla V|^2\,\d\alpha\). For \(\theta\in[1/2,1)\),
	\[
		|\nabla V(x)|\geq c(V(x)-V_\star)^\theta
		\quad\Longleftrightarrow\quad
		|\partial f_V|(\alpha)\geq c(f_V(\alpha)-V_\star)^\theta
	\]
	for every probability measure \(\alpha\). Jensen proves the forward implication, and testing \(\alpha=\delta_x\) proves the converse.
...
Optimal transport proves inequalities by turning comparison into geometry. One transports a density by a monotone or Brenier map, interpolates along the resulting rays, and converts concavity of a Jacobian determinant or convexity of an energy into an integral estimate. The first two examples below illustrate this geometric mechanism. The remainder concerns entropy, where a functional inequality comparing the energy gap with its squared Wasserstein slope is exactly a Wasserstein--PL inequality and therefore yields convergence rates through Theorem~\ref{thm-wasserstein-pl-convergence}. We work in the smooth Euclidean setting so that the main calculation remains visible; the standard approximation arguments recover the usual Borel and Sobolev formulations. Systematic accounts include McCann's displacement-convexity principle, the Riemannian interpolation inequalities of Cordero-Erausquin, McCann and Schmuckenschl\"ager, and Villani's treatment of concentration and functional inequalities~\cite{mccann1997convexity,cordero2001riemannian,Villani09}.
\index{Brenier!map}% strictindex
\index{displacement convexity}% strictindex
\index{Wasserstein!PL inequality}% strictindex

\paragraph{Brunn--Minkowski and isoperimetry.}
\index{Brunn-Minkowski@Brunn--Minkowski inequality}% strictindex
\index{isoperimetric inequality}% strictindex
...
\begin{proof}
	...
	\[
		\rho_t(T_t(x))^{-1/d}
		=
		\mathrm{Vol}(A)^{1/d}\det(DT_t(x))^{1/d}
		\geq
		c_t,
	\]
...
The logarithmic Sobolev inequality, introduced by Gross~\cite{Gross1975LogSobolev}, turns entropy into a quantitative relaxation principle by controlling it with its dissipation. The two quantities are fixed by the following definition.
...
\end{equation}
\end{defn}
This is a coercivity property of the reference measure, not a universal inequality. A standard sufficient condition is positive curvature of its potential, in the sense of the Bakry--Emery criterion~\cite{BakryEmery1985Diffusions,BakryGentilLedoux2014Markov}. The transport proof below uses the HWI inequality stated later in this section.
...
\index{relative!Fisher information}% strictindex
	Under the usual smoothness assumptions, for \(f(\alpha)=\KL(\alpha|\beta)\) the Wasserstein gradient is
	\[
		\Wgrad f(\alpha)
		=
...
\begin{proof}
	Let \(\alpha=\rho\gamma_d\), and let \(T=\nabla\phi\) be the Brenier map with \(T_\sharp\alpha=\gamma_d\). Write \(T(x)=x+\xi(x)\). The Monge--Amp\`ere equation reads
	\[
		\rho(x)e^{-\norm{x}^2/2}
		=
	...
	\[
		\mathrm{Ent}_{\gamma_d}(\rho)
		\leq
		\int \rho(\diverg\xi-\dotp{x}{\xi})\,\d\gamma_d
		-\frac12\int \rho\norm{\xi}^2\,\d\gamma_d.
	\]
...
\[
	...
	\qquad a\in\RR^d,
\]
one has \(\rho_a\gamma_d=\Gaussian(a,\Id)\). A direct computation gives
\[
...
\index{Ornstein-Uhlenbeck@Ornstein--Uhlenbeck flow}% strictindex


\paragraph{Poincar\'e as a linearized Wasserstein--PL inequality.}
\index{Poincare inequality@Poincar\'e inequality}% strictindex
\index{linearized Wasserstein metric}% strictindex
Many quadratic functional inequalities are the infinitesimal form of nonlinear slope inequalities near equilibrium. Assume that \(\beta\) minimizes an energy \(f\), and let \(\xi\) be a smooth bounded density modulation with \(\int \xi\,\d\beta=0\). For \(|\epsilon|\) small enough, set
\[
\beta_\epsilon=(1+\epsilon\xi)\beta,
\qquad
\beta_0=\beta.
\]
...
\index{Wasserstein!PL inequality}% strictindex
\index{functional inequality}% strictindex
	Let \(\beta\) be a minimizer of an energy \(f\), and assume that \(f\) satisfies
	\[
		|\partial f|^2(\alpha)
		\geq
		2\kappa\bigl(f(\alpha)-f(\beta)\bigr)
	\]
...
\begin{proof}
	Apply the PL inequality to \(\beta_\epsilon\):
	\[
		|\partial f|^2(\beta_\epsilon)
		\geq
		2\kappa\bigl(f(\beta_\epsilon)-f(\beta)\bigr).
	\]
	Divide by \(\epsilon^2\) and let \(\epsilon\to0\). The definitions of \(\mathsf H^f_\beta\) and \(\mathsf D^f_\beta\) give the claim.
\end{proof}

The two forms above can be evaluated explicitly under the indicated smoothness and integrability assumptions. Density divergences provide the local model.

\begin{example}[Linearized \(\phi\)-divergences]\label{ex-linearized-discrepancy-forms}\label{ex-linearized-phi-divergence-forms}
\index{phi-divergence}% strictindex
	Suppose that \(\beta\) has a smooth positive density and that \(\phi\) is twice continuously differentiable near \(1\), with \(\phi(1)=0\) and \(\phi''(1)>0\). For the Csisz\'ar divergence \(f(\alpha)=\Divergm_\phi(\alpha|\beta)\), one has
	\[
		\mathsf H^f_\beta(\xi)
		=
		\phi''(1)\int \xi^2\,\d\beta,
		\qquad
		\mathsf D^f_\beta(\xi)
		=
		\phi''(1)^2\int \norm{\nabla\xi}^2\,\d\beta.
	\]
	The affine part of \(\phi\) is immaterial on probability measures, so only the curvature \(\phi''(1)\) enters the linearization. For KL, \(\phi(r)=r\log r-r+1\), hence \(\phi''(1)=1\), and the linearized PL inequality becomes the Poincar\'e inequality.
\end{example}

Kernel discrepancies instead produce nonlocal quadratic forms.

\begin{example}[Linearized maximum mean discrepancy]\label{ex-linearized-mmd-forms}
\index{maximum mean discrepancy}% strictindex
	For \(f(\alpha)=\MMD_k^2(\alpha,\beta)\), use the convention
	\[
		\MMD_k^2(\alpha,\beta)
		=
		\iint k\,\d(\alpha-\beta)\d(\alpha-\beta).
	\]
	Assume that \(k\) is symmetric, differentiable in its first variable, and positive semidefinite on signed measures of zero total mass, and that the displayed integrals are finite. Then
	\[
		\mathsf H^f_\beta(\xi)
		=
		2\iint k(x,y)\xi(x)\xi(y)\,\d\beta(x)\d\beta(y),
	\]
	whereas
	\[
		\mathsf D^f_\beta(\xi)
		=
		4\int
		\norm{\int \nabla_x k(x,y)\xi(y)\,\d\beta(y)}^2
		\d\beta(x).
	\]
	Thus the energy Hessian is the kernel quadratic form, whereas the slope form differentiates the kernel witness spatially.
\end{example}

The transport discrepancy reveals the negative Sobolev geometry obtained by linearizing \(\Wass_2\).

\begin{example}[Linearized squared Wasserstein distance]\label{ex-linearized-wasserstein-forms}
\index{negative Sobolev norm}% strictindex
...
\index{spectral!gap}% strictindex
	...
	\[
		\lambda\int\xi^2\,\d\beta
		\leq
		\int\norm{\nabla\xi}^2\,\d\beta.
	\]
	In particular, if \(\d\beta=Z^{-1}e^{-V}\d x\) and \(\nabla^2 V\succeq \lambda\Id\), so that \(\beta\) is \(\lambda\)-strongly log-concave, then the conclusion holds by the Bakry--Emery criterion~\cite{BakryEmery1985Diffusions,BakryGentilLedoux2014Markov}.
\end{prop}

\begin{proof}
	For \(h_\epsilon=1+\epsilon\xi\), with \(|\epsilon|\) small enough, one has \(\beta_\epsilon=h_\epsilon\beta\) and
	\[
		\KL(\beta_\epsilon|\beta)
		=
...
\[
	\int\norm{\nabla\xi}^2\,\d\beta
	=
	\langle \xi,-L_\beta\xi\rangle_{L^2(\beta)}.
\]
Thus Poincar\'e says that the bottom of the spectrum of \(-L_\beta\) on mean-zero functions is at least \(\lambda\). When the resolvent is compact, this is the first nonzero eigenvalue; on a noncompact space the spectral gap need not be attained by an eigenfunction. This interpretation, classical for reversible Markov diffusion generators~\cite{Ledoux2001Concentration,BakryGentilLedoux2014Markov}, is the infinitesimal shadow of the nonlinear Wasserstein--PL/log-Sobolev inequality.
\index{weighted!Laplacian}% strictindex
\index{spectral!gap}% strictindex

This interpretation can be made exact by freezing the Wasserstein mobility at equilibrium. For a mean-zero density perturbation \(\dot h\), define
\[
	...
	\right\}.
\]
For \(E_2(h)=\frac12\chi^2(h\beta|\beta)=\frac12\int(h-1)^2\d\beta\), integration by parts shows that its squared slope in this \(H^{-1}(\beta)\) geometry is \(\int\norm{\nabla h}^2\d\beta\). Poincar\'e is therefore exactly the PL inequality for \(E_2\) in the linearized Wasserstein geometry. It is not the full \(\Wass_2\)-PL inequality for the same energy: the nonlinear Wasserstein mobility is \(h\beta\), and the corresponding squared slope is \(\int h\norm{\nabla h}^2\d\beta\).
...
The distance estimate in Theorem~\ref{thm-wasserstein-pl-convergence} already reveals the general principle: when \(\KL(\cdot|\beta)\) satisfies logarithmic Sobolev with constant \(\lambda\), the entropy flow yields the transport-entropy bound \(\Wass_2^2(\alpha,\beta)\leq2\KL(\alpha|\beta)/\lambda\). This is the Otto--Villani implication from logarithmic Sobolev to Talagrand's \(T_2\) inequality~\cite{OttoVillani2000Generalization}. For the standard Gaussian, the same Jacobian estimate used above gives a direct sharp proof of Talagrand's original result~\cite{Talagrand1996Transport,CorderoErausquin2002GaussianMassTransport,Villani09}.
\index{logarithmic Sobolev inequality}% strictindex

\begin{prop}[Gaussian \(T_2\) inequality]\label{prop-gaussian-talagrand-t2}
	For every $\alpha\in\Pp_2(\RR^d)$,
	\[
		\frac12\Wass_2^2(\alpha,\gamma_d)
		\leq
		\KL(\alpha|\gamma_d),
	\]
...
\begin{proof}
	...
	\[
		...
		\log\det(\Id+D\xi(x)).
	\]
	Using again $\log\det(\Id+D\xi)\leq\diverg\xi$ and integrating with respect to $\gamma_d$,
	\[
		\KL(\alpha|\gamma_d)
		\geq
		\frac12\int\norm{\xi}^2\,\d\gamma_d
		+
		\int(\dotp{x}{\xi}-\diverg\xi)\,\d\gamma_d.
	\]
...
\[
	...
	\frac{1}{2\lambda}\mathcal I(\alpha|\beta).
\]
Since \(\mathcal I(\cdot|\beta)=|\partial\KL(\cdot|\beta)|^2\), this is the Wasserstein--PL inequality with constant \(\lambda\). This is precisely the curvature-to-logarithmic-Sobolev implication used in Proposition~\ref{prop-curvature-log-sobolev}; Proposition~\ref{prop-lsi-entropy-decay} then converts the same estimate into exponential convergence of the Fokker--Planck flow.
\index{Fokker-Planck@Fokker--Planck!equation}% strictindex
\index{logarithmic Sobolev inequality}% strictindex
\index{Wasserstein!PL inequality}% strictindex
...
\index{gradient!flow}% strictindex
	...
	\]
	Because $\rho_\infty>0$ almost everywhere and $\nabla\delta f(\alpha_\infty)$ is continuous, this gradient vanishes throughout $\Omega$. Connectedness then implies that $\delta f(\alpha_\infty)$ is constant. Consequently, for every $\beta\in\Pp(\Omega)$,
	\[
...
\section{Generalized Dynamic Wasserstein Flows}
...
This section explains how changing the metric geometry changes the associated descent PDE.

\paragraph{Generalized Wasserstein flows.}
\index{implicit Euler scheme}% strictindex
\index{JKO scheme}% strictindex
\index{curve of maximal slope}% strictindex
The JKO construction is not tied to the quadratic Wasserstein distance. Once a space of probability measures is equipped with a metric or extended metric \(d\), it gives the following generalized descent construction.
\begin{defn}[Generalized action gradient flow]\label{def-generalized-action-gradient-flow}
\index{generalized!action gradient flow}% strictindex
...
for every smooth field $v$, because $v$ need not be the minimal finite-action representative of the induced tangent perturbation. Whenever a minimizing-movement scheme for $\Wass_\gamma$ converges to a smooth curve whose metric derivative is represented by this minimal action, it produces the same evolution described below.
...
\begin{proof}
	...
	\).
	Inserting this velocity into the continuity equation~\eqref{eq-generalized-flow-continuity-motivation} gives~\eqref{eq-normalized-spectral-flow}. Proposition~\ref{prop-normalized-spectral-polar} gives the explicit inverse-trace formula.
...
The effect of this spectral normalization is visible already in the homogeneous two-layer ReLU model discussed in Section~\ref{sec-wasserstein-flows-mlp}. Figure~\ref{fig:gradflow-mlp-w2-vs-muon} reproduces, in the style of the book figures, the numerical comparison from~\cite{peyre2026muon}: the same teacher network, same initialization and same empirical first variation are evolved either by the $\Wass_2$ particle flow or by the operator-gauge Muon direction.

\begin{figure}[H]
\centering
\begin{tabular}{@{}ccc@{}}
\scriptsize $\Wass_2$ trajectories & \scriptsize Muon trajectories & \scriptsize risk decay \\[-.15em]
...
\section{Nonlocal Wasserstein Flows}
...
\index{Markov semigroup}% strictindex
	Let $\mathcal X=\RR^d$, let $\mathfrak m=\mathcal L^d$, and let $K(x,\d y)=\nu(\d y-x)$ be translation invariant. Assume the metric hypotheses of Proposition~\ref{prop-nonlocal-distance-properties} and the heat-kernel and moment conditions of Assumptions~5.5--5.6 in~\cite{Erbar2012JumpEntropy}. Then the Markov semigroup generated by the closure of
	\[
		L\psi(x)
		=
...
\begin{proof}[Proof idea]
	...
	\]
	which is the weak form of \(\partial_t\rho=L\rho\). This calculation identifies the formal descent equation. Under the stated heat-kernel and moment assumptions, Theorem~5.11 of~\cite{Erbar2012JumpEntropy} proves the corresponding evolution variational inequality, and hence the rigorous metric gradient-flow statement.
\end{proof}
...
\index{Levy process@L\'evy process}% strictindex
...
\index{Fokker-Planck@Fokker--Planck!fractional equation}% strictindex
\end{rem}
...
\[
\dot\rho_i
=-\sum_jK_{i,j}\theta(\rho_i,\rho_j)(\log\rho_i-\log\rho_j)
=
\sum_jK_{i,j}(\rho_j-\rho_i),
\]
...
\section{Dynamic Unbalanced OT and WFR Flows}
...
\]
with squared norm $\int(\norm{v_t}^2+\kappa^2 g_t^2)\rho_t\d x$, namely the integral of \(A_\kappa(\rho_t,v_t,g_t)\). The vector field \(v_t\) accounts for displacement, while \(g_t\) is a Fisher--Rao growth rate. This is the infinitesimal version of the balance-equation action in Section~\ref{sec-unbalanced-ot}.
...
\index{relative!entropy}% strictindex
\index{Fokker-Planck@Fokker--Planck!equation}% strictindex
	Let $\beta=\density{\beta}\d x$ and use the finite-measure relative entropy
	\[
		f(\alpha)=\KL(\alpha|\beta)
		=
		\int
			\left(\rho\log\frac{\rho}{\density{\beta}}-\rho+\density{\beta}\right)\d x,
		\qquad
		\alpha=\rho\d x.
	\]
	This is the Csisz{\'a}r divergence on positive measures; it is nonnegative and is minimized, without imposing a mass constraint, at $\alpha=\beta$. Its first variation is $\delta f(\alpha)=\log(\rho/\density{\beta})$. The balanced $\Wass_2$ flow, when the total mass is fixed, is the Fokker--Planck equation
\index{Fokker-Planck@Fokker--Planck!equation}% strictindex
	\[
		\partial_t\rho_t
		=
			\diverg\left(\rho_t\nabla\log\frac{\rho_t}{\density{\beta}}\right)
			=
	...
	\begin{equation}\label{eq-wfr-kl-flow}
		...
		=
			\Delta\rho_t-\diverg(\rho_t\nabla\log\density{\beta})
			-
			\frac{1}{\kappa^2}\rho_t\log\frac{\rho_t}{\density{\beta}}.
	\end{equation}
	The first two terms transport and diffuse mass, while the last term kills mass where $\rho_t>\density{\beta}$ and creates it where $\rho_t<\density{\beta}$.
\end{example}
...
\section{Second-Order Momentum Flows}
\label{sec-second-order-momentum-flows}
\index{flow!momentum}% strictindex
\index{flow!second-order}% strictindex
...
Let $F:\RR^{N}\to\RR$ be a smooth finite-dimensional objective. Polyak's heavy-ball method~\cite{Polyak1964HeavyBall} augments gradient descent with a velocity variable. With step size $h>0$ and damping $\lambda_{\rm fric}>0$, one convenient velocity form is
...
\begin{equation}\label{eq-nesterov-momentum}
	...
	X^{(\ell+1)}=Y^{(\ell)}-h\nabla F(Y^{(\ell)}),
\end{equation}
with $\theta^{(\ell)}=(\ell-1)/(\ell+r-1)$ for the classical convex schedule $r=3$. Its scaling limit is the time-dependent damping equation
...
Equivalently,
\[
	\ddot x_i(t)+\lambda_{\rm fric}\dot x_i(t)
	=
	v_{\al_t}(x_i(t))
	=
	-\Wgrad f(\al_t)(x_i(t)).
...
\index{phase-space equation}% strictindex
	\begin{equation}\label{eq-second-order-entropy-phase-space}
		...
		\nabla_x\cdot(s\eta_t)
		+
		\nabla_s\cdot\bigl((-\lambda_{\rm fric} s-\nabla\log\rho_t(x))\eta_t\bigr)
		=0,
		\qquad
