...
\end{equation}
The Wasserstein analogue uses the metric derivative $|\dot\alpha_t|_{\Wass_2}$ of an absolutely continuous curve. In the smooth formal setting, a Wasserstein gradient flow with velocity $v_t=-\Wgrad f(\alpha_t)$ satisfies the energy-dissipation identity
\[
	f(\alpha_s)-f(\alpha_t)
	=
...
\]
for $0\leq s<t$. In the abstract metric theory, the same formula is expressed as an energy-dissipation identity for curves of maximal slope, as developed in the metric-gradient-flow framework of Ambrosio--Gigli--Savar\'e and in the optimal-transport presentation of Santambrogio~\cite{ambrosio2006gradient,SantambrogioBook}; if one uses only the weaker inequality convention, the estimate below remains true up to the inessential constant \(\sqrt2\). In the smooth setting considered here, Cauchy's inequality yields
...
\end{equation}
This estimate is weaker than a convergence rate: it does not identify a minimizer and it gives no decay law. It does, however, rule out escape to infinity in finite metric time whenever the energy is bounded below on finite time intervals. Indeed the trajectory segment remains in the \(\Wass_2\)-ball centered at \(\alpha_s\) with radius given by~\eqref{eq-wasserstein-gradient-flow-length}. Non-coercivity of the objective means that sublevel sets need not be compact, but an energy-dissipating curve still cannot travel an infinite \(\Wass_2\)-distance in finite time without an infinite energy drop. The PL and convexity estimates below add the missing quantitative information that forces convergence toward minimizers.
...
The generalized distances of Section~\ref{sec-generalized-dynamic-wasserstein-distances} are useful because they change what \emph{steepest descent} means. The energy functional can be the same, but the tangent norm, mobility, reaction term, graph structure, spectral normalization or jump kernel changes the resulting PDE. This section mirrors that dynamic-distance taxonomy: each paragraph takes one distance defined in Chapter~\ref{sec-dynamic-optimal-transport} and records the corresponding formal gradient-flow equation or algorithmic interpretation.
...
\paragraph{Dynamic unbalanced OT and WFR flows.}
...
\]
with squared norm $\int(\norm{v_t}^2+\kappa^2 g_t^2)\rho_t\d x$. This is the infinitesimal version of the balance-equation action in Section~\ref{sec-unbalanced-ot}.
...
\index{JKO scheme}% strictindex
\index{minimizing movement}% strictindex
The normalized dynamics can be obtained formally from a JKO scheme associated with the static spectral distance \(\Wass_\gamma\). At small time steps only the tangent expansion matters, so the same infinitesimal flow is described by the dynamic path-length geometry of \eqref{eq-dynamic-spectral-wasserstein}:
...
\paragraph{Fractional diffusion as a transport gradient flow.}
...
\paragraph{Connection with stochastic gradient dynamics.}
