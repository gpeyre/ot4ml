...
The metric ingredient needed below is the local action \(q_\alpha\) introduced in Section~\ref{sec-generalized-dynamic-wasserstein-distances}. It is the second-order tangent cost in \eqref{eq-generalized-distance-tangent-form}. For action densities \(A(\rho,m)\), the instantaneous action \(\ell_{A,\alpha}\) is defined in \eqref{eq-local-action-density-induced}; if \(A\) is \(r\)-homogeneous in the momentum and the induced distance is \(\mathcal A_A^{1/r}\), then \(q_\alpha(v)=\ell_{A,\alpha}(v)^{2/r}\). Thus \(\Wass_p\) gives the Finsler action \(q_{p,\alpha}(v)=(\int\norm{v}^p\d\alpha)^{2/p}\), while quadratic geometries give \(q_\alpha=\ell_{A,\alpha}\). This section keeps the same notation but uses it for steepest descent rather than for defining distances.
...
\index{L{\'e}vy process}% strictindex
