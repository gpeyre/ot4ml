...
\textbf{Input:} Gaussian measures $\Gaussian(\mean_s,\cov_s)$ with $\cov_s\succeq0$ and at least one positive-definite active covariance, positive weights $\lambda_s$ summing to one, tolerance $\mathrm{tol}$, iteration budget $K\geq1$.
...
\end{algblock}
\algreturnskip
\textbf{Return} $\Gaussian(\mean,\cov^{(K)})$.
\end{alg}
