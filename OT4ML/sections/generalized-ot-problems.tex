...
\begin{proof}
	...
	\begin{equation}\label{eq-radon-fourier-slice}
...
\textbf{Initialize:} Set \(u_s=\ones_{n_s}\) for all $s$, residual \(\eta=+\infty\), and $k=0$.
...
\(\overline{\P}_{ij}=u_{ij}a_i b_j\), the corresponding problem is
...
\textbf{Initialize:} Set $\P=\K$, correction matrices $\R_1=\R_2=\R_3=\ones_{n\times m}$, residual $r=+\infty$, and $k=0$.

\textbf{While} $r>\mathrm{tol}$ and $k<L$ \textbf{do}:
\begin{algblock}
\textbf{Set} $k\leftarrow k+1$ and $\P_{\rm old}=\P$.
