...
\includegraphics[width=.32\linewidth]{figures/inverse-ot-gap-loss/gap-loss-n100.pdf}
\end{tabular}
\caption{\emph{Empirical inverse-OT losses approach a curved population geometry.} Panel (a) shows $n=10$ i.i.d.\ pairs from a fixed plan $\pi_0=(\Id,T)_\sharp\alpha$ generated by a nonlinear Brenier map $T$ for $A_0=-\Id$. Panels (b,c) plot $t\mapsto\mathcal F_0(c_{A_t}\mid\widehat\pi_n)$ for $n=10$ and $n=100$ on the same vertical scale. Each finite-sample curve is convex and polyhedral; increasing $n$ resolves progressively more affine pieces around the generating parameter $t=0$.}
