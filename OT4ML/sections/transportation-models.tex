...
\begin{prop}[Flow matching vector field]\label{prop-flow-matching-vector-field}
	Assume that $t\mapsto I_t(u)$ is absolutely continuous for $\pi$-almost every $u$, that its derivative $(t,u)\mapsto\partial_t I_t(u)$ is jointly measurable, and that
	\[
		\int_0^1\!\int_{\RR^{d'}}\norm{\partial_t I_t(u)}^2\d\pi(u)\d t<+\infty.
	\]
	For almost every $t$, consider the flow-matching problem over measurable fields $v_t:\RR^d\to\RR^d$
\index{flow matching}% strictindex
	\begin{equation}
...
\index{conditional!expectation}% strictindex
	...
	\end{equation}
	Then $v_t\in L^2(\alpha_t;\RR^d)$ for almost every $t$, and the pair $(\alpha_t,v_t)$ satisfies the continuity equation~\eqref{eq:eulerian-advection} in the distributional sense on $(0,1)\times\RR^d$.
\index{continuity equation}% strictindex
\end{prop}
...
\index{push-forward}% strictindex
	...
	\]
	so the Eulerian velocity is square-integrable in space--time. Formally, this can be read as
\index{conditional!expectation}% strictindex
\index{flow matching}% strictindex
	\[
	...
	\begin{equation}\label{eq:flow-matching-weak-target}
		...
		=0.
	\end{equation}
	By the pathwise chain rule, $t\mapsto\varphi(I_t(u))$ is absolutely continuous for $\pi$-almost every $u$. The space--time integrability assumption and Fubini's theorem justify integration of its derivative, so~\eqref{eq:flow-matching-pushforward-test} gives, for almost every $t$,
\index{push-forward}% strictindex
\index{flow matching}% strictindex
...
\index{conditional!expectation}% strictindex
\index{Radon-Nikodym@Radon--Nikodym derivative}% strictindex
...
\index{latent!variable}% strictindex
\index{flow!probability ODE}% strictindex
In the terminology of Albergo--Boffi--Vanden-Eijnden~\cite{Albergo2025StochasticInterpolants}, randomness can enter through a static auxiliary variable without turning the path into an SDE.
\begin{defn}[Stochastic interpolant]\label{def-stochastic-interpolant}
Let \((X_0,X_1)\sim\pi\in\Couplings(\alpha_0,\alpha_1)\), and let \(Z\) be an auxiliary random variable independent of \((X_0,X_1)\). A stochastic interpolant is
\[
    X_t = I_t(X_0,X_1,Z),
\]
...
\index{flow!probability ODE}% strictindex
One can also represent an interpolating marginal curve by Brownian-in-time dynamics. This is a different construction from the static-noise bridge of Definition~\ref{def-stochastic-interpolant}. Let $Z_t$ solve the It\^o equation
\[
...
\]
one obtains the forward probability-flow velocity $v_\tau(z)=-z-\sigma^2\nabla\log p_\tau(z)$. Sampling integrates this ODE backward in $\tau$. Equivalently, after introducing reverse time $s$, the velocity changes sign and becomes $z+\sigma^2\nabla\log p_\tau(z)$. The resulting trajectories are more curved than for the linear bridge because the centers and variances evolve according to the OU/Fokker--Planck scaling rather than by affine interpolation. Numerically, the integration is stopped at a small positive noising time before the Dirac endpoint, where the score becomes singular.
\index{Fokker-Planck@Fokker--Planck!equation}% strictindex

The finite-time coefficients $a_t=\cos(\pi t/2)$ and $b_t=\sin(\pi t/2)$ are not a new spatial interpolant: they are exactly the OU coefficients after the time change $\tau=-\log\cos(\pi t/2)$. A genuinely different scalar bridge is
...
Figure~\ref{fig:generative-diffusion-versus-ot-2d} contrasts the reverse OU probability-flow trajectories with straight quadratic-OT displacement rays toward the same target atoms.
...
Figure~\ref{fig:generative-diffusion-schedule-comparison} compares the linear, variance-preserving OU and overshooting bridges, isolating how the interpolation schedule bends the reverse trajectories.
...
\begin{tabular}{@{}>{\centering\arraybackslash}m{2.5em}@{\hspace{.15em}}c@{\hspace{.25em}}ccccc@{}}
{\small $\Wass_2$} &
...
\[
	...
	\d\alpha(x).
\]
The bracket is the Langevin--Stein operator applied to $v$ and averaged under $\alpha$; because it only evaluates $v$, $\diverg v$, and the target score at sample locations, it remains meaningful when $\alpha$ is empirical. The reproducing property shows that the standard, unnormalized SVGD field is the unique solution of the quadratically penalized local problem
\[
	\min_{v\in\RKHS_k^d}
	\left\{
		\left.\frac{\d}{\d s}\KL\bigl((\Id+s v)_\sharp\alpha\,|\,\beta\bigr)\right|_{s=0}
		+\frac12\norm{v}_{\RKHS_k^d}^2
	\right\}.
\]
It is explicitly
\begin{equation}\label{eq-svgd-velocity}
	...
	\d\alpha(y).
\end{equation}
If $v_{\alpha}^{\mathrm{SVGD}}\neq0$, the optimizer of the corresponding linear descent problem over the RKHS unit ball is instead $v_{\alpha}^{\mathrm{SVGD}}/\norm{v_{\alpha}^{\mathrm{SVGD}}}_{\RKHS_k^d}$. Thus the normalized and standard fields have the same direction but a different state-dependent time parametrization.
...
\index{consensus!Dobrushin contraction}% strictindex
	...
	\[
		X^{(\ell+1)}
		=
		\bigl((1-\tau)\Id_n+\tau\mathsf P_{X^{(\ell)}}\bigr)X^{(\ell)},
		\qquad 0<\tau\leq1,
	\]
	define
	\[
		\delta^{(\ell)}=\delta(\mathsf P_{X^{(\ell)}}),
		\qquad
		q^{(\ell)}=1-\tau(1-\delta^{(\ell)}),
		\qquad
	...
	\[
		\max_i\norm{x_i^{(\ell)}-x_\infty^{\rm d}}
		\leq
		\frac{D_0}{1-\bar\delta}q_\tau^\ell.
	\]
...
\index{two-layer neural network}% strictindex
	...
	\[
		G_{\alpha_t}(z)=\Sigma_{v,u}(\alpha_t)z.
	\]
...
\index{neuron law}% strictindex
	...
	\[
		E_\alpha\eqdef \Sigma_{v,u}(\alpha)S-R.
	\]
