...
\paragraph{Consensus and clustering.}
...
\begin{equation}\label{eq-general-mean-shift-field}
	...
	\frac{\int (y-x)K(x,y)\d\alpha(y)}
	     {\int K(x,y)\d\alpha(y)}.
\end{equation}
For an empirical law $\alpha_t=\sum_{i=1}^n a_i\delta_{x_i(t)}$, with positive fixed weights summing to one, the transport equation becomes the row-normalized particle system
\begin{equation}\label{eq-mean-shift-particle-system}
	\dot x_i(t)
	=
	\frac{\sum_j a_jK(x_i(t),x_j(t))(x_j(t)-x_i(t))}
	     {\sum_j a_jK(x_i(t),x_j(t))}.
\end{equation}
The following theorem treats this finite system and its compactly supported measure-valued extension simultaneously.

\begin{thm}[Exponential consensus for positive mean shift]\label{thm-mean-shift-consensus}
\index{convex hull}% strictindex
\index{diameter!contraction}% strictindex
	Let $\alpha_0\in\Pp(\RR^d)$ have compact support $S_0$, and set
	\[
		C_0=\operatorname{conv}(S_0),
		\qquad
		D_0=\operatorname{diam}(S_0).
	\]
	Assume that $K:C_0\times C_0\to(0,+\infty)$ is Lipschitz and satisfies
	\[
		0<k_-\leq K(x,y)\leq k_+<+\infty.
	\]
	Let $\alpha_t$ be the characteristic solution of
	\begin{equation}\label{eq-positive-mean-shift-pde}
		\partial_t\alpha_t+\diverg\bigl(\alpha_tM_K[\alpha_t]\bigr)=0,
		\qquad
		\alpha_{t=0}=\alpha_0.
	\end{equation}
	Then this solution is globally defined, its convex hulls are nested,
	\[
		...
		\qquad (t\geq s\geq0),
	\]
	and, with $\kappa_K=k_-/k_+$,
	\begin{equation}\label{eq-mean-shift-diameter-contraction}
		D(t)\eqdef\operatorname{diam}(\supp\alpha_t)
		\leq
		e^{-\kappa_K t}D_0.
	\end{equation}
	Consequently there exists $x_\infty\in C_0$ such that
	\begin{equation}\label{eq-mean-shift-dirac-convergence}
		\Wass_\infty(\alpha_t,\delta_{x_\infty})
		\leq
		\frac{D_0}{\kappa_K}e^{-\kappa_K t}.
	\end{equation}
	For the Gaussian kernel $K_\epsilon(x,y)=e^{-\norm{x-y}^2/(2\epsilon)}$, the identity $\operatorname{diam}(C_0)=D_0$ gives
	\[
		k_+=1,
		\qquad
		k_-=e^{-D_0^2/(2\epsilon)},
		\qquad
		\kappa_K=e^{-D_0^2/(2\epsilon)}.
	\]
\end{thm}

\begin{proof}
	Since $K$ is positive and Lipschitz on the compact set $C_0\times C_0$, the denominator in $M_K$ is bounded below by $k_-$. On probability measures supported in $C_0$, the map $(x,\alpha)\mapsto M_K[\alpha](x)$ is therefore uniformly Lipschitz in $x$ and Lipschitz in $\alpha$ for $\Wass_1$. Standard characteristic theory gives a unique solution as long as its support remains in $C_0$.

	For $x\in\supp\alpha_t$, introduce the probability measure
	\[
		p_{t,x}(\d y)
		=
		\frac{K(x,y)}{\int K(x,z)\d\alpha_t(z)}\d\alpha_t(y).
	\]
	Then $M_K[\alpha_t](x)=\int y\,p_{t,x}(\d y)-x$ points from $x$ toward a point of $\operatorname{conv}(\supp\alpha_t)$. Hence no characteristic can cross an outward supporting hyperplane, which proves that the convex hulls are nested and closes the global-existence argument.

	The same observation yields a quantitative contraction. Fix $\theta\in\Sphere^{d-1}$ and write
	\[
		h_\theta^+(t)=\max_{x\in\supp\alpha_t}\dotp{\theta}{x},
		\qquad
		h_\theta^-(t)=\min_{x\in\supp\alpha_t}\dotp{\theta}{x},
		\qquad
		\bar h_\theta(t)=\int\dotp{\theta}{y}\d\alpha_t(y).
	\]
	Because $\int K(x,z)\d\alpha_t(z)\leq k_+$, one has the measure inequality
	\[
		p_{t,x}\geq\kappa_K\alpha_t.
	\]
	Thus $p_{t,x}=\kappa_K\alpha_t+(1-\kappa_K)q_{t,x}$ for some probability measure $q_{t,x}$ supported on $\supp\alpha_t$. At a point attaining $h_\theta^+$, the $q_{t,x}$-average cannot exceed $h_\theta^+$; at a point attaining $h_\theta^-$, it cannot be smaller than $h_\theta^-$. The standard envelope argument for extrema of transported compact sets therefore gives, for almost every $t$,
	\[
		\frac{\d}{\d t}h_\theta^+(t)
		\leq
		\kappa_K\bigl(\bar h_\theta(t)-h_\theta^+(t)\bigr),
		\qquad
		\frac{\d}{\d t}h_\theta^-(t)
		\geq
		\kappa_K\bigl(\bar h_\theta(t)-h_\theta^-(t)\bigr).
	\]
	Subtracting and applying Gronwall's lemma yields
	\[
		h_\theta^+(t)-h_\theta^-(t)
		\leq
		e^{-\kappa_Kt}
		\bigl(h_\theta^+(0)-h_\theta^-(0)\bigr).
	\]
	Taking the supremum over $\theta\in\Sphere^{d-1}$ proves~\eqref{eq-mean-shift-diameter-contraction}.

	Finally, every characteristic satisfies $\abs{\dot X_t}\leq D(t)$. It is therefore Cauchy, and any two characteristics have the same limit because their distance is bounded by $D(t)\to0$. Calling this common limit $x_\infty$ and integrating the velocity bound from $t$ to $+\infty$ gives
	\[
		\abs{X_t-x_\infty}
		\leq
		\int_t^{+\infty}D(s)\d s
		\leq
		\frac{D_0}{\kappa_K}e^{-\kappa_Kt}.
	\]
	Pushing $\alpha_0$ forward by the characteristic flow proves~\eqref{eq-mean-shift-dirac-convergence}.
\end{proof}

For the Gaussian kernel, the proof yields more than the uniform rate stated in the theorem. Repeating the directional-width estimate with the diameter of the current hull gives, for $t\geq s$,
\begin{equation}\label{eq-gaussian-mean-shift-adaptive-contraction}
	D(t)
	\leq
	D(s)\exp\!\left(
		-\int_s^t e^{-D(r)^2/(2\epsilon)}\d r
	\right).
\end{equation}
Since $D(r)\leq D_0$, this implies~\eqref{eq-mean-shift-diameter-contraction}; as the cloud contracts, the guaranteed instantaneous rate improves and tends to one.

The theorem concerns the self-consistent flow, in which $\alpha_t$ changes inside the mean-shift field. Because row normalization makes the pairwise coefficients asymmetric, $x_\infty$ need not equal the barycenter of $\alpha_0$; that barycenter is conserved for the constant kernel, but not in general. The self-consistent flow should also not be confused with classical mode seeking, where the data measure is frozen and different initial points may converge to different modes. Nor does one-point consensus hold for the hard confidence kernel $K_R(x,y)=\mathbf{1}_{\{\norm{x-y}\leq R\}}$: then $k_-=0$ whenever the cloud spans more than one confidence neighborhood. The discrete Hegselmann--Krause dynamics converges instead to opinion clusters~\cite{BlondelHendrickxTsitsiklis2009Krause}; under the hypotheses of the Eulerian theory, the limiting measure is a finite convex combination of Dirac masses separated by at least $R$~\cite{CanutoFagnaniTilli2012EulerianKrause}. For the Gaussian kernel, global positivity ultimately forces a single Dirac, although the initial lower bound $e^{-D_0^2/(2\epsilon)}$ can be very small and the multimodal transient can be long.
