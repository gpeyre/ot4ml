...
To handle an arbitrary number of tokens, we define $\alpha = \frac{1}{n} \sum_i \delta_{x_i}$ as the empirical measure of tokens. The resulting context-dependent map is the basic mean-field attention operator.
\index{empirical!measure}% strictindex
\index{transformer}% strictindex
\begin{defn}[Mean-field attention map]\label{def-mean-field-attention-map}
For \(\theta=(K,Q,V)\), define
\begin{equation*}
    \Gamma_\theta[\alpha](x) :=
    \frac{\int e^{\langle Q x, K y \rangle} V y \, \d \alpha(y)}
    {\int e^{\langle Q x, K y \rangle} \, \d \alpha(y)}.
\end{equation*}
One residual attention step of size \(\tau\) is the measure map
\[
    \mathcal A_{\theta,\tau}(\alpha)
    \eqdef
    (\Id+\tau\Gamma_\theta[\alpha])_\sharp\alpha.
\]
\end{defn}
For the empirical token measure, this sends each token to \(x_i+\tau\Gamma_\theta[\alpha](x_i)\).
