...
\end{defn}
Entropy regularization gives the following discrete transport problem.
...
\end{defn}
This is a strictly convex approximation of the discrete Kantorovich problem~\eqref{eq-kanto-discr}.

The temperature $\epsilon$ controls the compromise between transport cost and entropy. For small $\epsilon$, the cost dominates and most of the visible mass follows a concentrated geometric matching. Increasing $\epsilon$ rewards more diffuse couplings, so the significant mass leaving each source point is spread over a wider range of targets; in the large-temperature limit, the geometry of the cost is progressively washed out and the plan approaches the independent coupling $\a\otimes\b$, as proved later in Proposition~\ref{prop-convergence-eps}. Figure~\ref{fig:sinkhorn-plan-epsilon} makes this transition visible. Although only the largest entries are drawn, every entry of each displayed plan is positive, as proved next.
...
\includegraphics[width=.21\linewidth]{figures/sinkhorn-entropy-lp-geometry/path.pdf} \\[.35em]
...
\textbf{Input:} Positive weights $\a,\b$, cost matrix $\C$, regularization $\epsilon>0$, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.

\textbf{Output:} Entropic coupling $\P$.

\textbf{Initialize:} Set $\K_{i,j}=e^{-\C_{i,j}/\epsilon}$, $\vD^{(0)}=\ones_m$, \(r^{(0)}=+\infty\), and \(\ell=0\).
...
More generally, for balanced OT with fixed positive marginals, changing the tensor-product reference only adds a constant to the objective, as shown in the following proposition.
...
One can consider arbitrary measures by replacing the discrete entropy with relative entropy with respect to the product measure $\d\al\otimes\d\be(x,y) \eqdef \d\al(x)\d\be(y)$.
\index{product!measure}% strictindex
...
This relative entropy yields the continuous counterpart of Definition~\ref{def-discrete-entropic-ot}.
...
The multiplicative scaling structure of~\eqref{eq-scaling-form} extends from matrices to positive functions. Define the Gibbs kernel and its two integral operators by
\index{Gibbs!kernel}% strictindex
\index{Sinkhorn!continuous iteration}% strictindex
...
Because the reference measure $\al\otimes\be$ already contains the prescribed marginals, their target densities with respect to $\al$ and $\be$ are both one. The marginal constraints are therefore equivalent to
\eq{
...
\index{Sinkhorn!closed-form coupling}% strictindex
\index{Gibbs!kernel!push-forward}% strictindex
Let $\be_0$ be a sigma-finite reference measure on $\Yy$ and suppose that
\[
	Z_\epsilon(x)\eqdef\int_\Yy k_\epsilon(x,y)\d\be_0(y)
	\in(0,+\infty)
	\qquad\text{for $\al$-a.e. }x.
\]
...
\[
	\frac{\d\be_\epsilon}{\d\be_0}(y)
	=
	\int_\Xx k_\epsilon(x,y)\d\al(x).
\]
For example, let $\Xx=\Yy=\RR^d$, let $\be_0$ be Lebesgue measure, and take $c(x,y)=\norm{x-y}^2$. Then $Z_\epsilon=(\pi\epsilon)^{d/2}$ and
\[
	p_\epsilon(x,y)
	=
	(\pi\epsilon)^{-d/2}e^{-\norm{x-y}^2/\epsilon},
...
\)
for independent $X\sim\al$ and $G\sim\Gaussian(0,\Id)$. Time-indexed Gaussian blurrings of this type are the forward noising mechanism behind diffusion models, with an additional deterministic rescaling for variance-preserving Ornstein--Uhlenbeck schedules; see the \hyperref[par-diffusion-model-connection]{diffusion-model discussion} in Section~\ref{sec-generative-flow-matching}.
\index{Gaussian!blurring}% strictindex
\index{diffusion model!Gaussian noising}% strictindex

For discrete measures, setting $\uD_i=\a_i u(x_i)$ and $\vD_j=\b_j v(y_j)$ gives
\(
	\P_{i,j}=\a_i\b_j u(x_i)\K_{i,j}v(y_j)=\uD_i\K_{i,j}\vD_j
\),
so~\eqref{eq-continuous-sinkhorn-iteration} reduces exactly to the matrix iteration~\eqref{eq-sinkhorn}. Its logarithmic interpretation as continuous dual block ascent is derived in the \hyperref[par-continuous-dual-sinkhorn]{dual Sinkhorn paragraph}. Continuous convergence is revisited through Fortet monotonicity in Section~\ref{sec-sinkhorn-monotone}; the finite-dimensional linear rate is studied through Hilbert's metric in Section~\ref{sec-sinkhorn-hilbert}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Dual of Sinkhorn}\label{sec-dual-sinkhorn}
...
\begin{algH}[Log-domain Sinkhorn by soft transforms]\label{alg:log-domain-sinkhorn}
\textbf{Input:} Positive weights $\a,\b$, cost matrix $\C$, regularization $\epsilon>0$, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.
...
\begin{algblock}
\textbf{Set} $M_i=\max_j\{\gD_j^{(\ell-1)}-\C_{i,j}\}$.

\textbf{Set} $\fD_i^{(\ell)}=-M_i-\epsilon\log\sum_j\b_j
\exp((\gD_j^{(\ell-1)}-\C_{i,j}-M_i)/\epsilon)$.
\end{algblock}

\textbf{For} $j=1,\ldots,m$ \textbf{do}:
\begin{algblock}
\textbf{Set} $N_j=\max_i\{\fD_i^{(\ell)}-\C_{i,j}\}$.

\textbf{Set} $\gD_j^{(\ell)}=-N_j-\epsilon\log\sum_i\a_i
\exp((\fD_i^{(\ell)}-\C_{i,j}-N_j)/\epsilon)$.
\end{algblock}

\textbf{Set} $\P_{i,j}^{(\ell)}=\a_i\b_j
\exp((\fD_i^{(\ell)}+\gD_j^{(\ell)}-\C_{i,j})/\epsilon)$.
...
The continuous dual formula is not a discretization heuristic: it follows from the same entropy conjugacy as its matrix counterpart, with the two marginal constraints represented by continuous potentials.

\begin{prop}[Continuous entropic duality]\label{prop-continuous-entropic-duality}
\index{duality!entropic OT}% strictindex
...
\begin{proof}
	...
	\[
		\int_{\Yy}r(x,y)\d\be(y)=1\quad\al\text{-a.e.},
		\qquad
		\int_{\Xx}r(x,y)\d\al(x)=1\quad\be\text{-a.e.}
	\]
...
\index{Sinkhorn!continuous dual iteration}% strictindex
\index{dual!block maximization}% strictindex

The soft transforms are not only regularized analogues of hard $c$-transforms: they are the exact block-maximization steps of the continuous dual objective~\eqref{eq-dual-sinkhorn-objective}. Indeed, for fixed $g$ and a perturbation $h\in\Cc(\Xx)$,
\eq{
...
The decorations record the domains: the $\bar c$-transform~\eqref{eq-soft-c-cont-g} sends a potential on $\Yy$ to one on $\Xx$, while the $c$-transform~\eqref{eq-soft-c-cont-f} sends a potential on $\Xx$ to one on $\Yy$.

This dual iteration is exactly the logarithmic form of the continuous scaling iteration~\eqref{eq-continuous-sinkhorn-iteration}. More precisely, setting
\eq{
	u^{(\ell)}=e^{f^{(\ell)}/\epsilon},
	\qquad
...
Schr\"odinger's reciprocal problem is naturally posed on paths rather than on endpoint pairs. A noisy reference dynamics defines a probability law on trajectories; after optimizing out the conditional law of a path given its endpoints, only the endpoint coupling remains, and the Sinkhorn problem appears.
...
%
The path-space formulation optimizes directly over laws of trajectories.
\index{path-space formulation}% strictindex
...
In finite dimension, the second part of Definition~\ref{def-marginal-dependent-entropic-transport} gives the corresponding matrix problem, with reference weights satisfying $a_i,b_j>0$.
...
\index{generalized Sinkhorn algorithm}% strictindex
\textbf{Input:} Reference weights $\a,\b$, cost $\C$, convex marginal penalties $\mathsf F,\mathsf G$, regularization $\epsilon>0$, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.

\textbf{Output:} Coupling $\P$ and optimized marginals $r=\P\ones_m$, $s=\transp{\P}\ones_n$.

\textbf{Set} $\K_{i,j}=a_i b_j e^{-\C_{i,j}/\epsilon}$.

\textbf{Initialize:} $\uD=\ones_n$, $\vD=\ones_m$, $\eta=+\infty$, and $\ell=0$.
...
The scalar factor is absorbed by the Sinkhorn scalings and therefore does not change the optimal coupling. On a Riemannian manifold or surface $M$, let $L=-\Delta_M$ be the positive Laplace--Beltrami operator and replace the dense Gibbs kernel by the intrinsic heat operator
\[
	\mathsf H_\epsilon\eqdef e^{-(\epsilon/4)L},
\]
whose integral kernel is $h_{\epsilon/4}(x,y)$. For histograms on the same discretized domain, the scaling steps then read
...
where $\mathsf H_\epsilon$ denotes the discretized integral operator, including quadrature weights, and $\mathsf H_\epsilon^\top$ its discrete adjoint. In a symmetric mass-normalized discretization, the two operators coincide. Thus heat diffusion fits Sinkhorn at its only expensive operation: multiplication by the Gibbs kernel. Equivalently, it uses the effective cost
\[
...
\]
so convolutional Sinkhorn recovers the squared geodesic ground cost at small temperature without first computing all pairwise geodesic distances~\cite{varadhan-1967,2015-solomon-siggraph}. The same asymptotic relation is the analytical basis of geodesics-in-heat distance estimation~\cite{Crane2013}.
...
\]
On a triangulated surface or image grid, replace $L$ by a sparse positive semidefinite Laplacian $L_h$ and set
\[
	A_{\epsilon,q}\eqdef \Id+\frac{\epsilon}{4q}L_h.
\]
For fixed $(\epsilon,q)$, one computes a sparse Cholesky factorization $A_{\epsilon,q}=RR^\top$ once. Each application of $\mathsf H_\epsilon$ is then approximated by $q$ successive solves with $A_{\epsilon,q}$, and each solve uses only the two triangular substitutions associated with $R$. Since~\eqref{eq-convolutional-sinkhorn} repeatedly applies the same kernel, this factorization is reused throughout all Sinkhorn iterations~\cite{2015-solomon-siggraph}. This avoids both a dense kernel matrix and an all-pairs geodesic-distance computation. A temperature-scaling strategy requires one factorization for each value of $\epsilon$; on a finite-element mesh, the analogous shifted system is written with the mass and stiffness matrices. The small-temperature limit remains delicate because the diffusion length, of order $\sqrt\epsilon$, must be resolved by the mesh: if $\epsilon$ vanishes faster than the squared grid spacing, metrication artifacts and inconsistent discretizations dominate the intended geodesic limit.
\index{heat kernel}% strictindex
\index{mesh size}% strictindex
\index{discrete!Laplacian}% strictindex
\index{resolvent}% strictindex
\index{Cholesky factorization}% strictindex
\index{convolutional Sinkhorn}% strictindex
...
\end{tabular}
\caption{\emph{Geodesics-in-heat approximation of the distance to a dense red non-convex source curve on a two-dimensional grid.} The first panel shows the exact Euclidean distance to the curve. The other panels diffuse a narrow band by the one-step approximation $(\Id+\epsilon L_h/4)^{-1}$ with Neumann boundary conditions, then recover a distance by the normalized-gradient Poisson solve. Each panel is robustly normalized, and contour values are selected from quantiles of the displayed field. Increasing the Sinkhorn temperature $\epsilon$ suppresses unresolved grid-scale artifacts but progressively rounds the non-convex level-set geometry.}
\index{geodesics in heat}% strictindex
...
\paragraph{Density-ratio regularization.}
...
Equivalently, when $\phi^*$ is differentiable, $u=g^{\bar c,\epsilon,\phi}(x)$ is characterized by
\[
	\int_{\Yy}
	(\phi^*)'\!\left(\frac{u+g(y)-c(x,y)}{\epsilon}\right)
	\d\be(y)
...
Figure~\ref{fig:sinkhorn-phi-soft-c-transforms} shows how changing $\phi$ modifies the smoothing of the dual envelope.
...
Figure~\ref{fig:sinkhorn-entropic-versus-quadratic-regularization} shows the corresponding change in the concentration and support of the primal coupling.
...
This distinction leads to a second regularized transport problem with the same marginal constraints.
...
\index{dual!potential}% strictindex
	...
	\[
		...
		\Phi^*(\delta\Phi(\xi)),
	\]
	which yields~\eqref{eq-bregman-regularized-ot-dual}. Equality in Fenchel's inequality gives the optimality condition for $\pi^\star$. The density-ratio dual and density law are exactly those of Proposition~\ref{prop-phi-regularized-ot-dual}. Placing the two formulas side by side shows the structural difference: Bregman regularization translates the reference measure in the dual coordinate $\delta\Phi$, whereas $\phi$-regularization applies a scalar nonlinearity to the density with respect to the moving product reference $\al\otimes\be$.
\index{phi-divergence regularized OT}% strictindex
\index{Bregman!regularization}% strictindex
\index{reference!measure}% strictindex
\index{density!ratio}% strictindex
...
Thus, at large $\epsilon$, the raw entropic cost approaches the bilinear cross-energy $(\al,\be)\mapsto\iint c\,\d\al\d\be$, not a distance. The following special case makes the resulting attraction explicit.

\begin{example}[Large-temperature collapse for quadratic costs]
\index{large-temperature collapse}% strictindex
...
\begin{prop}[Non-negativity of Sinkhorn divergences]\label{prop-sinkhorn-positive}
	Assume that the entropic dual admits optimal potentials and that the symmetric kernel $k_\epsilon(x,y)=e^{-c(x,y)/\epsilon}$ is positive semidefinite. Then $\bar\MK_\c^\epsilon(\al,\be)\geq0$.
\end{prop}
...
\index{strict!positivity}% strictindex
		If, in addition, $k_\epsilon$ is universal on a compact space, then $\bar\MK_\c^\epsilon(\al,\be)=0$ implies $\al=\be$, and the Sinkhorn divergence metrizes weak convergence~\cite{feydy2018interpolating}. Positive semidefiniteness alone proves only non-negativity and may leave a nontrivial null space.
...
Let $\al\in\Pp(\Xx)$ and $\be\in\Pp(\Yy)$. Whenever the following integral operators are well defined, set, for $\epsilon\in\CC\setminus\{0\}$,
\[
...
\begin{equation}\label{eq-complex-measure-fixed-point}
	u(x)\int_\Yy K_\epsilon(x,y)v(y)\,\d\be(y)=1
	\quad \text{for }\al\text{-a.e. }x,
	\qquad
	v(y)\int_\Xx K_\epsilon(x,y)u(x)\,\d\al(x)=1
...
\begin{equation}\label{eq-complex-measure-iteration}
	u^{(\ell+1)}(x)=
	\left(\int_\Yy K_\epsilon(x,y)v^{(\ell)}(y)\,\d\be(y)\right)^{-1},
	\qquad
	v^{(\ell+1)}(y)=
	\left(\int_\Xx K_\epsilon(x,y)u^{(\ell+1)}(x)\,\d\al(x)\right)^{-1},
\end{equation}
...
\index{soft!transform}% strictindex
\[
	f(x)=-\epsilon\log\!\int_\Yy \exp\!\left(\frac{g(y)-c(x,y)}{\epsilon}\right)\d\be(y),
	\qquad
	g(y)=-\epsilon\log\!\int_\Xx \exp\!\left(\frac{f(x)-c(x,y)}{\epsilon}\right)\d\al(x).
\]
...
If $\al=\sum_i \a_i\delta_{x_i}$ and $\be=\sum_j \b_j\delta_{y_j}$, set $\C_{i,j}=c(x_i,y_j)$ and define the complex Gibbs matrix
\index{complex!Gibbs kernel}% strictindex
\[
	\K_\epsilon(\C)_{i,j}\eqdef \exp(-\C_{i,j}/\epsilon).
\]
The direct discretization of~\eqref{eq-complex-measure-coupling} is
\[
	\P_{\epsilon,ij}=\a_i\b_j u_i\K_\epsilon(\C)_{i,j}v_j,
\]
with fixed-point equations
\[
	u_i\sum_j \b_j\K_\epsilon(\C)_{i,j}v_j=1,
	\qquad
	v_j\sum_i \a_i\K_\epsilon(\C)_{i,j}u_i=1 .
\]
...
\begin{equation}\label{eq-complex-sinkhorn-iteration}
	\uD^{(\ell+1)}
	=
	\a \oslash \bigl(\K_\epsilon(\C)\vD^{(\ell)}\bigr),
	\qquad
	\vD^{(\ell+1)}
	=
	\b \oslash \bigl(\K_\epsilon(\C)^\top\uD^{(\ell+1)}\bigr),
\end{equation}
...
The following finite-dimensional result was communicated by Guillaume Carlier. It uses absorbed log-scalings: if $(\widehat\fD,\widehat\gD)$ are the KL-normalized dual potentials of~\eqref{eq-dual-formulation}, then
\index{absorbed log-scaling}% strictindex
\[
	\fD=\widehat\fD+\epsilon\log\a,
	\qquad
	\gD=\widehat\gD+\epsilon\log\b,
\]
so that the marginal factors $a_i b_j$ are incorporated into the exponential coupling formula.
...
\index{holomorphic implicit function theorem}% strictindex
	Fix positive histograms $\a^0\in\simplex_n$, $\b^0\in\simplex_m$, a finite real cost matrix $\C^0$, and a real temperature $\epsilon_0>0$. Let $(\fD^0,\gD^0)$ be the absorbed log-scalings at this point, normalized by $\sum_i \a_i^0 f_i^0=0$. Then there are complex neighborhoods of $(\epsilon_0,\a^0,\b^0,\C^0)$, inside the affine constraint $\sum_i \a_i=\sum_j \b_j$, and a unique holomorphic map
	\[
	...
	\[
		\P_{\epsilon,ij}
		=
		\exp\!\left(\frac{f_{\epsilon,i}+g_{\epsilon,j}-\C_{i,j}}{\epsilon}\right),
		\qquad
		\P_\epsilon\ones_m=\a,
		\qquad
		\P_\epsilon^\top\ones_n=\b .
	\]
	Consequently the gauge-fixed scalings $\uD_\epsilon=e^{\fD_\epsilon/\epsilon}$, $\vD_\epsilon=e^{\gD_\epsilon/\epsilon}$ and the coupling $\P_\epsilon$ are holomorphic in $(\epsilon,\a,\b,\C)$ near the base point.
\end{thm}
...
\begin{proof}
	...
	\[
		R_i=\sum_j
		\exp\!\left(\frac{f_i+g_j-\C_{i,j}}{\epsilon}\right)-\a_i,
		\qquad
		S_j=\sum_i
		\exp\!\left(\frac{f_i+g_j-\C_{i,j}}{\epsilon}\right)-\b_j,
		\qquad
		G=\sum_i \a_i^0 f_i ,
	\]
	...
	\[
		\delta \P_{i,j}
		=
		\frac{1}{\epsilon_0}\P^0_{i,j}(\delta f_i+\delta g_j).
	\]
	...
	\[
		0
		=
		\sum_i \overline{\delta f_i}\sum_j\delta \P_{i,j}
		+
		\sum_j \overline{\delta g_j}\sum_i\delta \P_{i,j}
		=
		\frac{1}{\epsilon_0}
		\sum_{i,j}\P^0_{i,j}\abs{\delta f_i+\delta g_j}^2 .
	\]
	Since $\P^0_{i,j}>0$, one has $\delta f_i+\delta g_j=0$ for all $i,j$. Hence $\delta\fD$ is constant and $\delta\gD$ is the opposite constant. The gauge derivative $\sum_i \a_i^0\delta f_i=0$ forces this constant to vanish. The derivative is thus invertible, and the holomorphic implicit-function theorem gives the local holomorphic potentials. The formulas for $\uD_\epsilon$, $\vD_\epsilon$ and $\P_\epsilon$ are holomorphic compositions of these potentials.
\end{proof}
...
\index{quadratic!scaling}% strictindex
	...
	\[
		v^{(\ell)}(y)=\exp(\eta^{(\ell)}+q^{(\ell)} y^2),
		\qquad
		u^{(\ell+1)}(x)=\exp(\gamma^{(\ell+1)}+p^{(\ell+1)}x^2),
	\]
	then, with
	\[
	...
	\[
		p^{(\ell+1)}=T_t(q^{(\ell)}),
		\qquad
		\gamma^{(\ell+1)}=-\eta^{(\ell)}+\frac12\log\bigl(2tA_t(q^{(\ell)})\bigr),
	\]
	and
	\[
		q^{(\ell+1)}=T_s(p^{(\ell+1)}),
		\qquad
		\eta^{(\ell+1)}=-\gamma^{(\ell+1)}+\frac12\log\bigl(2sA_s(p^{(\ell+1)})\bigr),
	\]
	with the logarithm branch chosen from the positive real solution. Thus the infinite-dimensional fixed point has been reduced to a two-dimensional rational iteration for $(p^{(\ell)},q^{(\ell)})$ plus scalar gauge constants.
