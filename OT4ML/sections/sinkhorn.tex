...
\begin{proof}
	...
	\[
		...
		\int c\,\d\al\d\be .
	\]
	Hence $\KL(\pi_\epsilon|\al\otimes\be)\leq \epsilon^{-1}\int c\,\d\al\d\be$. Pinsker's inequality gives total-variation convergence to $\al\otimes\be$, and the convergence of the values follows by continuity of $c$.
\end{proof}
...
\index{Schrodinger problem@Schr\"odinger problem}% strictindex
\index{Fisher information}% strictindex
	Let $\Omega\subset\RR^d$ be a smooth compact domain and let $\al=\rho_0\,\d x$, $\be=\rho_1\,\d x$ have smooth positive densities. Assume that the quadratic optimal map $T$ from $\al$ to $\be$ is a smooth diffeomorphism and that the McCann interpolation $\al_t=\rho_t\,\d x=((1-t)\Id+tT)_\sharp\al$ satisfies
	\[
		\Ii_0(\al,\be)\eqdef
		\int_0^1\int_\Omega \norm{\nabla\log\rho_t(x)}^2\rho_t(x)\,\d x\,\d t<+\infty .
	\]
	For $c(x,y)=\norm{x-y}^2$, define $\mathrm H(\al)\eqdef\int_\Omega\rho_0\log\rho_0\,\d x$ and similarly for $\be$. Then, as $\epsilon\downarrow0$,
	\[
		...
		+\frac{\epsilon^2}{16}\Ii_0(\al,\be)
		+o(\epsilon^2).
	\]
	If $f_\epsilon,g_\epsilon$ are optimal Sinkhorn potentials and $f_0,g_0$ are Kantorovich potentials for the quadratic cost, normalized by $\int g_\epsilon\,\d\be=\int g_0\,\d\be=0$, then under the same smooth non-degeneracy assumptions,
	\[
		f_\epsilon+\frac{d\epsilon}{2}\log(\pi\epsilon)\longrightarrow f_0,
		\qquad
		g_\epsilon\longrightarrow g_0,
	\]
	locally uniformly. The order-$\epsilon$ correction to the potentials is obtained by the Laplace prefactor in the soft $c$-transform equations and depends on the endpoint densities and on the Hessian of the quadratic contact phase.
\end{prop}
...
\begin{proof}
	...
	\[
		...
		+\frac{T^2}{8}\Ii_0(\al,\be)+o(T^2),
	\]
	under the stated regularity assumptions~\cite{ConfortiTamanini2021EntropicDerivative}. Since
	\[
		\mathscr C_T(\al,\be)
		=
		\inf_{\pi\in\Couplings(\al,\be)}
		\KL(\pi|p_T(x,y)\,\d x\d y),
		\qquad
		p_T(x,y)=(2\pi T)^{-d/2}e^{-\norm{x-y}^2/(2T)},
	\]
	multiplying by $2T$ and setting $\epsilon=2T$ gives
	\[
		\MK_{\norm{\cdot}^2}^{\epsilon}(\al,\be)
		=
		...
		-dT\log(2\pi T),
	\]
	which is exactly the displayed expansion because $2T=\epsilon$. The potential statement follows by applying Laplace's method to the soft $c$-transform equations. In the chosen gauge the universal Gaussian prefactor is placed in $f_\epsilon$; changing the additive gauge redistributes the same scalar shift between the two potentials.
\end{proof}
...
\index{complex!potential}% strictindex
\index{soft!transform}% strictindex
\[
