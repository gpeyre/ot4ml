...
\textbf{While} \(r_k>\mathrm{tol}\) and $k<L$ \textbf{do}:
\begin{algblock}
...
\begin{algblock}
\textbf{Set} $M_i=\max_j\{\gD_j^{(k-1)}-\C_{ij}\}$.

\textbf{Set} $\fD_i^{(k)}=-M_i-\epsilon\log\sum_j\b_j
\exp((\gD_j^{(k-1)}-\C_{ij}-M_i)/\epsilon)$.
\end{algblock}

\textbf{For} $j=1,\ldots,m$ \textbf{do}:
\begin{algblock}
\textbf{Set} $N_j=\max_i\{\fD_i^{(k)}-\C_{ij}\}$.

\textbf{Set} $\gD_j^{(k)}=-N_j-\epsilon\log\sum_i\a_i
\exp((\fD_i^{(k)}-\C_{ij}-N_j)/\epsilon)$.
\end{algblock}

\textbf{Set} $\P_{ij}^{(k)}=\a_i\b_j
...
\textbf{Initialize:} $\uD=\ones_n$, $\vD=\ones_m$, $\eta=+\infty$, and $k=0$.

\textbf{While} $\eta>\mathrm{tol}$ and $k<L$ \textbf{do}:
\begin{algblock}
\textbf{Set} $k\leftarrow k+1$.
...
\eql{\label{eq-convolutional-sinkhorn}
	\uD^{(k+1)}
	=
	\a\oslash\bigl(\mathsf H_\epsilon\vD^{(k)}\bigr),
	\qquad
	\vD^{(k+1)}
	=
	\b\oslash\bigl(\mathsf H_\epsilon^\top\uD^{(k+1)}\bigr),
}
...
\begin{equation}\label{eq-complex-measure-iteration}
	u^{(k+1)}(x)=
	\left(\int_\Yy K_\epsilon(x,y)v^{(k)}(y)\,\d\be(y)\right)^{-1},
	\qquad
	v^{(k+1)}(y)=
	\left(\int_\Xx K_\epsilon(x,y)u^{(k+1)}(x)\,\d\al(x)\right)^{-1},
\end{equation}
