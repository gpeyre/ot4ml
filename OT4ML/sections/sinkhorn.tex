...
\index{Shannon-Boltzmann@Shannon--Boltzmann entropy}% strictindex
...
\index{entropic!OT!discrete}% strictindex
...
\index{Sinkhorn!log-domain}% strictindex
...
\).
The same identity also exhibits the promised projection geometry. Indeed, for every
$\P\in\CouplingsD(\a,\b)$,
...
Consequently, entropic OT is the generalized-KL projection of the tilted Gibbs reference
$(\a\otimes\b)\odot e^{-\C/\epsilon}$ onto the transport polytope. Section~\ref{sec-convergence-init} develops this interpretation and shows that the two Sinkhorn half-steps are precisely the KL projections onto the row- and column-marginal constraint sets.
...
\index{entropic!OT!continuous}% strictindex
For $\epsilon>0$, define
...
\index{Fisher information}% strictindex
	...
	\[
		f_\epsilon\longrightarrow f_0\quad\text{in }L^1(\al),
		\qquad
		g_\epsilon\longrightarrow g_0\quad\text{in }L^1(\be)
	\]
	as $\epsilon\downarrow0$~\cite{NutzWiesel2022EntropicPotentials}. If the potentials are chosen as their continuous soft $c$-transform representatives, this convergence is uniform on $\Supp(\al)$ and $\Supp(\be)$. Moreover, the scalar part of the first potential is fixed by
	\[
		\int f_\epsilon\,\d\al
		=
		\Wass_2^2(\al,\be)
...
\begin{proof}
	...
	It remains to justify the uniform statement, which does not follow from the citation alone. On the two compact supports, the quadratic cost has a common modulus of continuity in either variable. The soft $c$-transform formula therefore gives, uniformly in $\epsilon$,
	\[
		\abs{f_\epsilon(x)-f_\epsilon(x')}
		\leq\sup_{y\in\Supp(\be)}\abs{c(x,y)-c(x',y)},
	\]
...
Under stronger uniform non-degeneracy and boundary assumptions, Laplace's method applied to the two soft $c$-transform equations also gives spatial order-$\epsilon$ corrections involving the endpoint densities and the Hessian of the contact phase. This is a formal refinement, not part of Proposition~\ref{prop-small-epsilon-expansion}.
...
\index{Sinkhorn!generalized}% strictindex
...
\index{Kullback-Leibler@Kullback--Leibler!proximal operator}% strictindex
\index{Sinkhorn!scaling}% strictindex
\index{unbalanced!OT}% strictindex
...
\index{entropic!OT!marginal-dependent}% strictindex
...
\index{Sinkhorn!generalized}% strictindex
\textbf{Input:} Positive reference weights $\a,\b$, finite cost $\C$, convex marginal penalties $\mathsf F,\mathsf G$ satisfying the strict-feasibility assumptions of Proposition~\ref{prop-marginal-dependent-dual-scaling}, regularization $\epsilon>0$, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.
...
\index{Hopf-Cole@Hopf--Cole transform}% strictindex
...
\index{Laplace-Beltrami@Laplace--Beltrami operator}% strictindex
...
\]
On a triangulated surface or image grid, replace $L$ by a sparse positive semidefinite Laplacian $L_h$ and set $A_{\epsilon,q}\eqdef \Id+\frac{\epsilon}{4q}L_h$.
...
\index{Sinkhorn!convolutional}% strictindex
...
\index{Hopf-Lax@Hopf--Lax formula}% strictindex
\index{Hopf-Cole@Hopf--Cole transform}% strictindex
\index{log-sum-exp}% strictindex

The Hopf--Lax formula is the Hamilton--Jacobi incarnation of the hard $c$-transform. We use the normalized quadratic cost $c(x,y)=\frac12\norm{x-y}^2$, so that the Hopf--Lax operator applied to an initial datum $h$ is precisely
\[
...
\index{Legendre-Fenchel@Legendre--Fenchel transform}% strictindex
...
\index{phi-divergence regularized OT}% strictindex
	Set $L\eqdef\phi'_\infty$ and assume exact Fenchel--Rockafellar duality for the marginalization constraint. This holds, in particular, when $\Xx,\Yy$ are compact metric spaces, $\al,\be$ have full support, $c$ is Lipschitz, and $\phi$ is of Legendre type~\cite{TerjekGonzalezSanchez2022}. Then
...
\index{convex!function}% strictindex
	\[
		...
		\d\al(x)\d\be(y),
	\]
	provided $f\oplus g-c\leq\epsilon L$, and it is $-\infty$ otherwise. This gives~\eqref{eq-phi-regularized-ot-dual}. Equality in the Fenchel inequality yields both the subgradient inclusion
\index{phi-divergence regularized OT}% strictindex
	\[
...
To type the function-space dual precisely, let $(\mathsf E,\mathsf E^*)$ be a separated locally convex dual pair in which $\mathsf E$ contains the signed measures on $\Xx\times\Yy$ under consideration and $\mathsf E^*$ contains the admissible test functions. Let $\mathsf U_\Xx$ and $\mathsf U_\Yy$ be potential spaces such that $f\oplus g\in\mathsf E^*$, and let $\mathsf A\pi=(\pi_1,\pi_2)$ denote the continuous marginalization map for these pairings.
...
\]
and the second satisfies the symmetric equation. Thus an interior update normalizes one absolutely continuous conditional density. When the left derivative exists, a boundary minimizer instead satisfies the same integral inequality with right-hand side at most $1$; the missing mass is singular and supported where the recession cap is active, in agreement with Proposition~\ref{prop-phi-regularized-ot-dual}. For finite discrete marginals with positive weights, Burg updates are interior because $\phi^*(s)=-\log(1-s)$ diverges at $s=1$, so the update remains a one-dimensional monotone root solve. In general, however, a root-only implementation requires an interior-solution hypothesis; the constrained $\argmin$ above also covers boundary updates~\cite{TerjekGonzalezSanchez2022}.
...
for the $\phi$ and Bregman models, respectively. For KL, which admits both descriptions, the two lines coincide with the usual soft $c$-transform iteration~\eqref{eq-continuous-dual-sinkhorn-iteration}, hence recover Sinkhorn's algorithm.
...
\end{tabular}
\caption{\emph{Density-ratio regularizers produce qualitatively different coupling supports.} The red and blue side curves are fixed one-dimensional Gaussian-mixture marginals. Dense violet level sets display the square root of the coupling density on a common scale, using $\epsilon=.06$. KL regularization gives the usual diffuse positive plan, the Burg barrier keeps a positive but differently tailed support, and the quadratic soft $c$-transform produces the sparse rightmost plan through its positive-part law. All three displayed matrices satisfy both marginal constraints to $\ell^1$ error below $3\cdot10^{-13}$; the retained Burg computation has primal--dual gap $1.9\cdot10^{-4}$.}
\index{Gaussian!mixture}% strictindex
\index{density!ratio}% strictindex
...
\index{large-temperature collapse}% strictindex
	...
	$\be_\epsilon \rightharpoonup \delta_{y^\star(\al)}$, where $y^\star(\al)=\uargmin{y} V_\al(y)$.
...
\begin{rem}[Reference-measure invariance]\label{rem-sinkhorn-reference-invariance}
	The product reference carried by each pair of marginals in Definition~\ref{def-continuous-entropic-ot} is convenient but not intrinsic to the debiased quantity. Let $\eta$ and $\zeta$ be auxiliary probability measures on the common state space such that $\al\ll\eta$, $\be\ll\zeta$ and $\KL(\al|\eta)+\KL(\be|\zeta)<+\infty$, and denote by
	\[
...
\end{tabular}
\caption{\emph{Finite-step point descent visualizes the debiasing effect.} The red level sets show a fixed two-Gaussian target density $\be$, while the blue atoms are finite-step Adam endpoints initialized identically in all panels. The optimized functional is the displayed raw or debiased cost plus the mild confinement $0.006\int\norm{x}^2\d\al_n(x)$. For small $\epsilon$, both objectives keep the two overlapping modes; for large $\epsilon$, the raw cost moves the atoms toward the barycenter while the self-cost subtraction in~\eqref{eq-sinkhorn-divergence} keeps a bimodal cloud. These endpoints are illustrative rather than certified global minimizers; the notebook retains objective decreases, terminal first-variation norms, marginal residuals below $6\cdot10^{-16}$, and verifies that the window-clipping branch is never activated.}
\index{Sinkhorn!divergence}% strictindex
...
\index{Sinkhorn!self}% strictindex
	...
	\[
		\d\al=(k_\epsilon\tilde\mu)\,\d\tilde\mu=\d\be,
		\qquad
		(k_\epsilon\tilde\mu)(x)\eqdef\int k_\epsilon(x,y)\d\tilde\mu(y),
	\]
	so $\al=\be$. The converse follows directly from the definition. Finally, on a compact space, continuity of the entropic value under weak convergence and compactness of $\Pp(\Xx)$ turn this definiteness into the stated equivalence of convergences; see~\cite{feydy2018interpolating}.
\end{proof}
...
\index{complex!epsilon@$\epsilon$}% strictindex
...
Let $\al\in\Pp(\Xx)$ and $\be\in\Pp(\Yy)$. For $\epsilon\in\CC\setminus\{0\}$, reuse the Gibbs kernel and integral operators of~\eqref{eq-continuous-sinkhorn-operators} whenever the defining integrals exist. The density factorization~\eqref{eq-continuous-sinkhorn-scaling}, the marginal equations that follow it, and the scaling updates~\eqref{eq-continuous-sinkhorn-iteration} then remain valid verbatim for complex-valued $u$ and $v$, provided that the divisions are well defined. For real $\epsilon>0$ these are the usual Sinkhorn equations. For complex $\epsilon$ they are only local analytic identities: they assert neither positivity of $\pi_\epsilon$ nor convergence of the alternating iteration.
...
Figure~\ref{fig:sinkhorn-complex-epsilon-continuation} visualizes the continuation at the level of the coupling, without choosing logarithm branches. To keep the strength of the regularization fixed, it follows the constant-modulus arc $\epsilon_\theta=\epsilon_0e^{i\theta}$ and displays $\abs{\P_{\epsilon_\theta}}$ at four increasing phases. The complex matrix $\P_{\epsilon_\theta}$ always has marginals $(\a,\b)$, but taking its entrywise modulus destroys these linear identities. The red and blue side profiles therefore show the prescribed marginals, while the violet profiles show the row and column sums of $\abs{\P_{\epsilon_\theta}}$ and reveal the cancellations hidden by the modulus. The branch is followed by gauge-fixed Newton continuation in small angular steps, and every point on the computed path is checked against the marginal equations.

\begin{figure}[ht]
\centering
\setlength{\tabcolsep}{2pt}
\begin{tabular}{@{}cccc@{}}
\includegraphics[width=.242\linewidth]{figures/sinkhorn-complex-epsilon-continuation/theta-0p00.pdf} &
\includegraphics[width=.242\linewidth]{figures/sinkhorn-complex-epsilon-continuation/theta-0p40.pdf} &
\includegraphics[width=.242\linewidth]{figures/sinkhorn-complex-epsilon-continuation/theta-0p80.pdf} &
\includegraphics[width=.242\linewidth]{figures/sinkhorn-complex-epsilon-continuation/theta-1p20.pdf} \\[-.1em]
\small $\theta=0$ & \small $\theta=0.40$ & \small $\theta=0.80$ & \small $\theta=1.20$
\end{tabular}
\caption{\emph{The magnitude of a complex Sinkhorn coupling exposes the oscillations created by a complex phase.} Two Gaussian-mixture histograms and $\epsilon_0=0.12$ are fixed, while $\epsilon_\theta=\epsilon_0e^{i\theta}$ is continued through $\theta\in\{0,0.40,0.80,1.20\}$, so every panel has the same regularization amplitude $\abs{\epsilon_\theta}=0.12$. All panels use the same intensity scale. The attached red and blue profiles are the prescribed marginals $(\a,\b)$ of the complex coupling; the violet profiles are the marginals of its entrywise modulus and separate from them as cancellation increases. The maximum marginal residual over the full continuation path is below $10^{-11}$.}
