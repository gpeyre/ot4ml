...
\textbf{Input:} Endpoint laws $\al,\be$, a static optimizer $\pi_\epsilon^\star$, reference bridge kernel $(x,y)\mapsto\Rr^{\epsilon,x,y}$, sample count $R\geq1$.

\textbf{Output:} Schr\"odinger path law $M_\epsilon^\star$ and independent paths $(\omega^{(r)})_{r=1}^R$ sampled from it.

\textbf{Set} exact lifted law:
\(M_\epsilon^\star=\int \Rr^{\epsilon,x,y}\,\d\pi_\epsilon^\star(x,y).\)

\textbf{For} $r=1,\ldots,R$ \textbf{do}:
\begin{algblock}
\textbf{Draw} $(X_0^{(r)},X_1^{(r)})\sim\pi_\epsilon^\star$.

\textbf{Draw} \(\omega^{(r)}\sim\Rr^{\epsilon,X_0^{(r)},X_1^{(r)}}.\)
\end{algblock}
\algreturnskip
\textbf{Return} $M_\epsilon^\star$ and $(\omega^{(r)})_{r=1}^R$.
\end{alg}
