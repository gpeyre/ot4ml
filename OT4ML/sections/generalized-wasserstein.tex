...
\d\pi(x,y).
\end{align*}
This factorization motivates two scalar costs: the local reverse cost and its homogeneous marginal perspective.
...
\[
	...
	\psi_2(0)\be^\perp(\Yy).
\]
The homogeneous perspective in Definition~\ref{def-unbalanced-local-costs} defines
...
\index{sliced Wasserstein!distance}% strictindex
	...
	\]
	To make the metric meaning of ``pull-back'' explicit, and to distinguish it from the pull-back of test functions introduced together with push-forwards in Remark~\ref{rem-pullback-pushforward}, equip measurable Radon fields $h:\Sphere^{d-1}\to\Pp_p(\RR)$ having finite integrated $p$th moment with the metric
\index{metric!pull-back}% strictindex
	\[
		...
		\right)^{1/p}.
	\]
	This is also exactly a conditional Wasserstein metric. Indeed, associate with each Radon field $h$ the joint law
	\[
...
\\[-.1em]
\small first quantile-PCA mode & \small second quantile-PCA mode
\end{tabular}
\caption{\emph{One-dimensional linear OT PCA for synthetic two-Gaussian mixtures.} The exact empirical Wasserstein barycenter has quantile \(Q_{\bar\alpha}=N^{-1}\sum_iQ_{\alpha_i}\). Each panel shows nine densities with quantiles \(Q_{\bar\alpha}+a e_k\), including the barycenter at the violet midpoint \(a=0\), with \(a\) increasing from red to blue. The first mode is a common translation and the second changes the separation of the two components. Since every displayed quantile is increasing, these are valid probability laws in exact one-dimensional Wasserstein coordinates.}
\index{linear!OT}% strictindex
\index{quantile!function}% strictindex
\index{principal component analysis}% strictindex
...
\index{positive!semidefinite matrix}% strictindex
	...
	\]
\end{defn}
The monotonicity condition means that increasing the displacement covariance in Loewner order cannot decrease the transport penalty. A fundamental family is obtained from Schatten norms.
\index{displacement covariance}% strictindex
...
Restricting the robust directions to orthogonal projections gives a non-convex, dimension-controlled counterpart of spectral Wasserstein transport.
...
The Ky Fan gauge supplies the convex spectral relaxation of this rank-constrained construction.
