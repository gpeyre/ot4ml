...
\]
The normalization $\bar\psi_s(1)=0$ alone does not imply a balanced large-$\tau$ limit: an entropy may vanish away from $1$, or even identically. A precise sufficient regime is the following. Assume that $\Xx$ and $\Yy$ are compact metric spaces, $c$ is lower semicontinuous, $\alpha$ and $\beta$ have the same finite mass and admit a finite-cost coupling, and
$\bar\psi_s^{-1}(\{0\})=\{1\}$. If the induced divergences are lower semicontinuous and have compact sublevels for weak convergence of finite measures, then the functionals in the preceding display are equicoercive and $\Gamma$-converge in that topology, as $\tau\to+\infty$, to the balanced Kantorovich functional. Indeed, bounded energy forces both marginal divergences to vanish, so every limit plan has marginals $(\alpha,\beta)$; a balanced plan is its own recovery sequence. Consequently, the values converge to $\MK_c(\alpha,\beta)$, and every cluster point of minimizers is a balanced optimal plan. Small $\tau$ instead makes creation and destruction cheap; after division by $\tau$, the diagonal zero-transport part reveals the pure divergence geometry.

Marginal relaxation changes which measures are compared, but not the transport problem once those modified marginals are fixed. To state this precisely, extend the Kantorovich value~\eqref{eq-mk-generic} by positive homogeneity: for finite measures $(\widetilde\alpha,\widetilde\beta)$ with common mass $M$,
\index{marginal!relaxed}% strictindex
\index{Kantorovich!finite equal-mass measures}% strictindex
...
\index{balanced!transport between relaxed marginals}% strictindex
...
\index{mass!shape separation}% strictindex

Proposition~\ref{prop-uot-balanced-relaxed-marginals} separates transport from marginal relaxation for every $\phi$-divergence. KL penalties allow one further simplification: the common mass of the relaxed marginals separates from their normalized shapes. A scalar optimization then determines how much mass is transported, while a probability-level transport problem determines how it is arranged.
...
\begin{proof}
	...
	\[
		M\mathcal E_{c,\tau_0,\tau_1}(\widehat\alpha,\widehat\beta)
		+\tau_0\big(M\log(M/A)-M+A\big)
		+\tau_1\big(M\log(M/B)-M+B\big).
	\]
...
Figure~\ref{fig:unbalanced-divergence-choice} now keeps the geometry, entropic plan regularization and relaxation strength fixed, and changes only the marginal divergence. This isolates the effect of the penalty: KL gives smooth rescaling, Burg discourages complete deletion of prescribed modes, while a smooth approximation of total variation produces sharper active-mass selection.

\begin{figure}[H]
\centering
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}ccc@{}}
\small KL & \small Burg & \small smoothed TV \\[-.15em]
...
\end{tabular}
\caption{\emph{Effect of the marginal divergence in unbalanced entropic OT.} The geometric cost, entropic plan regularization $\epsilon$, and relaxation strength $\tau$ are fixed; only the marginal penalty changes. KL allows smooth mass variation, Burg keeps transported marginals from vanishing on prescribed modes, and the displayed differentiable TV surrogate gives a sharper active-mass selection. The strictly positive plan is recovered from the Fenchel dual without an imposed numerical floor; every exported panel is checked for solver success and marginal first-order residual. The side plots use the same convention as Figure~\ref{fig:unbalanced-mass-relaxation}.}
...
\begin{proof}
	...
	entropy--transport duality theorem of Liero, Mielke and Savar\'e~\cite{LieroMielkeSavareLong}
	justifies exchanging the infimum and the supremum. This gives
	\[
...
\d\pi(x,y).
\end{align*}
This factorization motivates two scalar costs: the local reverse cost and its closed homogeneous marginal perspective.
...
\end{defn}
The closure in this definition is essential: it supplies the correct axis costs that model pure creation and destruction; these costs are finite whenever the relevant entropy values are finite. By construction, $H_c$ is lower semicontinuous and positively $1$-homogeneous. For the unit KL entropy $\psi(t)=t\log t-t+1$, direct minimization in the positive quadrant followed by this closure gives, with $e^{-\infty}=0$,
\[
	H_c(r,s)=r+s-2\sqrt{rs}\,e^{-c/2},
	\qquad c\in[0,+\infty],\quad r,s\geq0.
\]
In particular, $H_c(r,0)=r$, $H_c(0,s)=s$ and $H_c(0,0)=0$.
If $\al=F\pi_1+\al^\perp$ and $\be=G\pi_2+\be^\perp$ are the Lebesgue decompositions of the reference marginals with respect to the transported marginals, then the reverse formulation reads
\index{recession convention}% strictindex
...
\[
	...
	\psi_2(0)\be^\perp(\Yy).
\]
The closed perspective absorbs these singular terms into its axis values. For the cone construction, and also for the proof of equivalence, define the homogeneous value through its semi-coupling form~\cite{LieroMielkeSavareLong}. Here one uses a base measure $\lambda$ on $\Xx\times\Yy$ and two nonnegative weights $u,v$ attached to each transported pair:
...
\begin{proof}
	This is the homogeneous-reformulation theorem of Liero, Mielke and Savar\'e~\cite[Theorems~5.5 and~5.8]{LieroMielkeSavareLong}. Its interior mechanism is worth recording. The reverse representation and $H_c\leq\widetilde H_c\leq L_c$ show that every unbalanced competitor induces a semi-coupling of no larger cost, including singular mass through the closed axis values; hence $\HW_c\leq\UW_c$.
...
\index{Wasserstein-Fisher-Rao@Wasserstein--Fisher--Rao!distance}% strictindex
For $p=2$ and
\[
\De((x,r),(y,s))^2=r^2+s^2-2rs\cos(d(x,y)\wedge\pi/2),
\]
...
\begin{proof}
	The equality $\UW=\HW$ is Proposition~\ref{prop-homogeneous-unbalanced}. It remains to compare the semi-coupling representation~\eqref{eq-homogeneous} with the cone value.
...
\index{Kullback-Leibler@Kullback--Leibler!relaxation}% strictindex
...
\index{soft!c-transform}% strictindex
	...
	\[
		d_\infty((f^{(\ell)},g^{(\ell)}),(f^\star,g^\star))
		\leq
		\omega^\ell d_\infty((f^{(0)},g^{(0)}),(f^\star,g^\star)).
	\]
	Equivalently, for $u^{(\ell)}=e^{f^{(\ell)}/\epsilon}$ and
	$v^{(\ell)}=e^{g^{(\ell)}/\epsilon}$,
	\[
		d_T((u^{(\ell)},v^{(\ell)}),(u^\star,v^\star))
		\leq
		\omega^\ell d_T((u^{(0)},v^{(0)}),(u^\star,v^\star)).
	\]
\end{prop}
\begin{proof}
...
\(K_{i,j}=e^{-\C_{i,j}/\epsilon}\a_i\b_j, \quad \omega=\frac{\tau}{\tau+\epsilon}, \quad u^{(0)}=\ones_n, \quad v^{(0)}=\ones_m, \quad \eta^{(0)}=+\infty, \quad \ell=0\).

\textbf{While} \(\eta^{(\ell)}>\mathrm{tol}\) and $\ell<L$ \textbf{do}:
\begin{algblock}

\textbf{Set} \(\ell\leftarrow \ell+1\).

\(u^{(\ell)} = \left(\frac{\a}{K v^{(\ell-1)}}\right)^\omega, \qquad v^{(\ell)} = \left(\frac{\b}{\transp{K}u^{(\ell)}}\right)^\omega\).

\textbf{Set} \(\eta^{(\ell)}=\epsilon\max\{\norm{\log u^{(\ell)}-\log u^{(\ell-1)}}_\infty,\norm{\log v^{(\ell)}-\log v^{(\ell-1)}}_\infty\}\).
\end{algblock}
\algreturnskip
\textbf{Return} \(\P^{(\ell)}=\diag(u^{(\ell)})K\diag(v^{(\ell)})\).
\end{alg}

\paragraph{Gaussian Hellinger--Kantorovich transport.}
\index{Hellinger-Kantorovich@Hellinger--Kantorovich!Gaussian}% strictindex
\index{Gaussian!unbalanced}% strictindex
For arbitrary finite nonnegative measures, quadratic transport with equal KL marginal penalties defines the following discrepancy.
...
\end{equation}
\end{defn}
	Following Liero, Mielke and Savar\'e~\cite{LieroMielkeSavareLong}, this is called the \emph{Gaussian Hellinger--Kantorovich} (GHK) distance. The adjective ``Gaussian'' does not constrain the input measures: $\alpha$ and $\beta$ in Definition~\ref{def-gaussian-ghk} need not be Gaussian. It refers to the Gaussian kernel $k_\tau(x,y)=\exp(-\norm{x-y}^2/(2\tau))$ appearing in the associated homogeneous cone cost. In the normalization of~\eqref{eq-dynamic-unbalanced-ot}, the intrinsic length metric induced by $\operatorname{GHK}_\tau$ is $\WFR_{\sqrt{\tau}/2}$. Indeed, the static cone cost~\eqref{eq-wfr-scaled-cone-metric} then has prefactor $\tau$ and angular argument $d(x,y)/\sqrt{\tau}$. Thus $\tau=1$ gives $\WFR_{1/2}$, namely the unscaled cosine-cone convention of Definition~\ref{def-wfr-distance}; see Proposition~\ref{prop-static-dynamic-unbalanced}.

We now specialize~\eqref{eq-gaussian-ghk-endpoint} to scaled Gaussian inputs,
\[
\alpha=r_0\Gaussian(m_0,\Sigma_\alpha),
\qquad
\beta=r_1\Gaussian(m_1,\Sigma_\beta),
\qquad
r_0,r_1>0,\quad \Sigma_\alpha,\Sigma_\beta\succ0,
\]
where $r_i$ is the total mass and $m_i\in\RR^d$ is the mean, while $\Sigma_\alpha$ and $\Sigma_\beta$ are the respective covariances. These inputs provide one of the few continuous unbalanced problems with an explicit endpoint cost. The graph-supported zero-coupling-entropy formulas below follow~\cite{YangZhang2026}; the nondegenerate positive-coupling-entropy counterpart recalled afterward is from~\cite{janati2020gaussian}.

Write $\bar\alpha=\Gaussian(m_0,\Sigma_\alpha)$ and $\bar\beta=\Gaussian(m_1,\Sigma_\beta)$. The covariance relaxation associated with these normalized inputs is governed by a matrix Bregman divergence. Let $\Phi_{\mathrm{LD}}(P)=-\log\det P$ on $\mathbb S_{++}^d$; since $\nabla\Phi_{\mathrm{LD}}(\Sigma)=-\Sigma^{-1}$, its Bregman divergence in the sense of Definition~\ref{def-bregman-divergence} is the LogDet divergence below.
\index{LogDet divergence}% strictindex
...
\end{equation}
\end{defn}
The notation anticipates the hard-penalty limit below; it does not assert that $\Bb_\tau$ is a metric for finite $\tau$.

Although~\eqref{eq-gaussian-ghk-covariance-envelope} is variational, both its minimizer and its value are explicit.
...
\begin{proof}
For $P,Q\succ0$, let $L$ be the Gaussian optimal map from $P$ to $Q$, so $Q=LPL$. The Frobenius gradients of the squared Bures term are $\Id-L$ and $\Id-L^{-1}$~\cite{bhatia2018bures}; hence stationarity of~\eqref{eq-gaussian-ghk-covariance-envelope} reads
...
2\eta^{-1}P_\tau^{-1}
=(\Id-L_\tau)^2
+\eta^{-1}\Sigma_\alpha^{-1}
+\eta^{-1}L_\tau\Sigma_\beta^{-1}L_\tau
\succ0.
\]
...
\begin{rem}[Hard Bures limit]\label{rem-gaussian-uot-bures-limit}
\index{Bures!soft Procrustes limit}% strictindex
For fixed $\Sigma_\alpha,\Sigma_\beta\succ0$, the function $\tau\mapsto\Bb_\tau^2(\Sigma_\alpha,\Sigma_\beta)$ is nondecreasing and
\begin{equation}\label{eq-gaussian-uot-hard-bures-limit}
\lim_{\tau\to+\infty}\Bb_\tau^2(\Sigma_\alpha,\Sigma_\beta)
=
\Bb_\infty^2(\Sigma_\alpha,\Sigma_\beta)
=
\Bb^2(\Sigma_\alpha,\Sigma_\beta).
\end{equation}
Indeed, monotonicity follows directly from~\eqref{eq-gaussian-ghk-covariance-envelope}, while the competitor $(P,Q)=(\Sigma_\alpha,\Sigma_\beta)$ gives $\Bb_\tau^2\leq\Bb^2$. For minimizers $(P_\tau,Q_\tau)$, this bound implies $D_{\mathrm{LD}}(P_\tau|\Sigma_\alpha)+D_{\mathrm{LD}}(Q_\tau|\Sigma_\beta)\to0$. Coercivity and strict positivity of the LogDet divergence away from its base point yield $P_\tau\to\Sigma_\alpha$ and $Q_\tau\to\Sigma_\beta$. Since $\Bb_\tau^2\geq\Bb^2(P_\tau,Q_\tau)$, continuity of the Bures cost gives the matching lower limit. Together with Proposition~\ref{prop-gaussian-ghk-endpoint}, this recovers the Gaussian $\Wass_2^2$ formula~\eqref{eq-dist-gauss}; for $r_0=r_1=r$, the endpoint value~\eqref{eq-gaussian-ghk-endpoint} converges to $r\Wass_2^2(\bar\alpha,\bar\beta)$.
\end{rem}
...
\index{Hellinger!geodesic}% strictindex
	Although the GHK optimizer between scaled Gaussians is a scaled Gaussian coupling~\cite{YangZhang2026}, the ambient WFR geodesic defined in Section~\ref{sec-unbalanced-ot} need not remain Gaussian. This contrasts with the $\Wass_2$ geometry of Proposition~\ref{prop-gaussian-w2-bures}, whose displacement interpolation preserves Gaussianity.

	The failure is already explicit at the pure Hellinger endpoint of WFR\@. If $\rho_0$ and $\rho_1$ are the densities of $\Gaussian(m_0,\Sigma_\alpha)$ and $\Gaussian(m_1,\Sigma_\beta)$, respectively, then the Hellinger geodesic is
	\begin{equation}\label{eq-hellinger-gaussian-three-mixture}
		...
		=
		(1-t)^2\rho_0
		+2t(1-t)\mathfrak h_{0,1}\rho_{0,1}
		+t^2\rho_1.
	\end{equation}
	Here $\mathfrak h_{0,1}=\int\sqrt{\rho_0\rho_1}$, and $\rho_{0,1}$ is the Gaussian density with covariance $2(\Sigma_\alpha^{-1}+\Sigma_\beta^{-1})^{-1}$ and mean $(\Sigma_\alpha^{-1}+\Sigma_\beta^{-1})^{-1}(\Sigma_\alpha^{-1}m_0+\Sigma_\beta^{-1}m_1)$. Thus, except in degenerate cases, every interior point is an unnormalized mixture of three distinct Gaussian densities rather than a single Gaussian.
\end{rem}

\begin{rem}[Entropic Gaussian extension]\label{rem-entropic-gaussian-uot}
	The Gaussian closed forms above extend to KL-regularized unbalanced OT: add a KL penalty on the coupling in~\eqref{eq-unbalanced-entropic-primal}, while retaining quadratic ground cost and KL marginal penalties. The optimizer is again a scaled Gaussian coupling, and its transported mass, covariance and objective value are available in closed form; see Janati, Muzellec, Peyr\'e and Cuturi~\cite{janati2020gaussian}.
\end{rem}

\paragraph{Partial optimal transport.}
\index{partial transport}% strictindex
Partial OT allows only part of the available mass to be matched. One may prescribe the active mass, price the unmatched mass, or regard the latter problem as unbalanced OT with total-variation marginal penalties. The following definition separates these three formulations.
\index{active mass}% strictindex
...
The equivalence shows that $\lambda$ is the Lagrange multiplier associated with transported mass: increasing it makes unmatched mass more expensive and therefore favors larger active masses.
...
\end{equation}
Consequently, $(\al,\be)\mapsto\operatorname{POT}_m(\al,\be)^{1/p}$ is a distance on the space of finite-$p$th-moment measures with total mass $m$.
\end{prop}
...
The fixed-total-mass restriction is essential: on a larger class of measures, two distinct inputs may share a common submeasure of mass $m$ and therefore have zero partial cost. The constrained theory, including active regions and free boundaries, was developed by Caffarelli--McCann and Figalli~\cite{CaffarelliMcCannPartial,FigalliPartial}. Modern computational and learning applications include partial Wasserstein and partial Gromov--Wasserstein variants, for instance in Chapel--Alaya--Gasso~\cite{ChapelAlayaGassoPartial}.
...
\end{defn}
Since each projected problem can be solved by the sorting rule of Proposition~\ref{prop-matching-1d-monotone} or the quantile formula of Proposition~\ref{prop-wass-quantile-1d}, $\SW_p$ is much cheaper to approximate numerically than high-dimensional OT\@. It metrizes the same weak-plus-moment topology as $\Wass_p$, but its geometry is not bi-Lipschitz equivalent to $\Wass_p$ in high dimension~\cite{nadjahi2019asymptotic}.
...
\index{sliced Wasserstein!distance}% strictindex
	...
	\]
	To make the metric meaning of ``pull-back'' explicit, and to distinguish it from the pull-back of test functions introduced together with push-forwards in Remark~\ref{rem-pullback-pushforward}, equip measurable Radon fields $h:\Sphere^{d-1}\to\Pp_p(\RR)$ having finite integrated $p$th moment with the pseudometric
\index{metric!pull-back}% strictindex
	\[
		...
		\right)^{1/p}.
	\]
	It is a metric after quotienting fields by $\sigma$-almost-everywhere equality. On these equivalence classes it is also exactly a conditional Wasserstein metric. Indeed, associate with each Radon field $h$ the joint law
	\[
		\widehat h(\d\theta,\d s)
		\eqdef
		h(\theta)(\d s)\sigma(\d\theta)
	\]
	on $\Sphere^{d-1}\times\RR$. Its first marginal is $\sigma$, so $\widehat h\in\Pp_{p,\sigma}(\Sphere^{d-1}\times\RR)$, and Definition~\ref{def-conditional-wasserstein-distance}, with condition space $S=\Sphere^{d-1}$ and fiber $\Omega=\RR$, gives
	\[
		d_{\mathrm{Rad},p}(h_0,h_1)
		=
...
proposition below gives the general upper bound. The two remarks that follow
show that it is locally sharp on fixed-weight atomic strata but can be strict
for smooth measures.
...
\begin{rem}[$\SW_2$ and $\Wass_2$ local equality for discrete measures]\label{rem-sliced-local-atomic-equality}
\index{sliced Wasserstein!atomic local equality}% strictindex
	Let $a_i>0$ satisfy $\sum_i a_i=1$, let $x_1,\ldots,x_n$ be pairwise distinct, and let $v_i\in\RR^d$. For
	\[
	...
	\]
	Indeed, for all sufficiently small $t$, the diagonal coupling between $x_i$ and $x_i+t v_i$ is optimal for $\Wass_2$. Writing $c_{i,j}(t)=\norm{x_i-x_j-tv_j}^2$, the discrete dual potentials $\fD_i=0$ and $\gD_j=c_{j,j}(t)$ are feasible for small $t$, because $c_{j,j}(t)<c_{i,j}(t)$ whenever $i\neq j$, and they certify the diagonal plan. Thus
	\[
		\Wass_2(\eta,\eta_t)^2
		=t^2\sum_{i=1}^n a_i\norm{v_i}^2.
	\]
	For every $\theta$ outside the finite union of great spheres $\{\dotp{\theta}{x_i-x_j}=0\}$, the projected atoms are distinct and their order is unchanged for sufficiently small $t$. The one-dimensional monotone coupling then pairs each projected atom with its perturbation, so
	\[
		\lim_{t\to0}\frac{1}{t^2}
		\Wass_2\big((P_\theta)_\sharp\eta,(P_\theta)_\sharp\eta_t\big)^2
		=
		\sum_{i=1}^n a_i\abs{\dotp{\theta}{v_i}}^2.
	\]
	The diagonal projected coupling provides the same expression as an integrable upper bound. Dominated convergence and the spherical second-moment identity give the sliced limit.
\end{rem}

\begin{rem}[$\SW_2$ and $\Wass_2$ strict local inequality]\label{rem-sliced-local-gaussian-strict}
\index{sliced Wasserstein!strict local inequality}% strictindex
	The upper bound in Proposition~\ref{prop-sliced-first-order-tangent} can be strict at order $t^2$. Let $d\geq2$, let $\alpha=\Gaussian(0,\Id)$, and take $\varphi(x)=\frac12x^\top A x$ for a symmetric matrix $A$ that is not a scalar multiple of $\Id$. Then
	\[
	...
	\]
	Indeed, $T_t=\Id+tA$, and the projected source and target in direction $\theta$ are centered one-dimensional Gaussians with standard deviations $1$ and $\norm{(\Id+tA)\theta}$. Consequently,
	\[
		\lim_{t\to0}\frac{1}{t^2}
		\Wass_2\big((P_\theta)_\sharp\alpha,(P_\theta)_\sharp\alpha_t\big)^2
		=
	...
	\[
		...
		\frac{\tr(A)^2+2\tr(A^2)}{d(d+2)}.
	\]
	On the other hand, the Brenier formula gives $\Wass_2(\alpha,\alpha_t)^2=t^2\tr(A^2)$. Strictness follows from $\tr(A)^2<d\tr(A^2)$ when $A$ is symmetric and non-scalar.
\end{rem}
...
$\SW_2$-absolutely continuous curve. Park and Slep\v{c}ev prove that the infimum is attained, so
...
Wasserstein geometry. Remark~\ref{rem-sliced-local-atomic-equality} shows that
the sliced metric derivative equals $d^{-1/2}$ times the Wasserstein metric
derivative along finite atomic curves with fixed weights. By contrast, the
Gaussian deformation in Remark~\ref{rem-sliced-local-gaussian-strict} is
strictly slower for $\SW_2$. The sliced speed along
...
The associated Gaussian sliced-Wasserstein flow is revisited in
Proposition~\ref{prop-centered-gaussian-covariance-catalogue}.
...
\begin{rem}[Proportional and rank-one covariances]\label{rem-sliced-gaussian-special-cases}
	If $\Sigma_\alpha=s^2\Sigma$ and $\Sigma_\beta=t^2\Sigma$ for
	$s,t\geq0$, then
	\[
		\SW_2(\alpha,\beta)^2
		=
		\frac{\norm{m_\alpha-m_\beta}^2+(s-t)^2\tr(\Sigma)}{d}.
	\]
	For the rank-one covariances
	$\Sigma_\alpha=a^2u\transp{u}$ and
	$\Sigma_\beta=b^2v\transp{v}$, where $u$ and $v$ are unit vectors, set
	...
	\end{aligned}
	\]
	The inequality is the $p=2$ case of
	Proposition~\ref{prop-sliced-wasserstein-metric}, and the final identity
	follows from the Gaussian $\Wass_2$ formula.
...
$Q\in\mathrm O(k)$, because $Q^\top$ is an isometry of $\RR^k$. Thus the
construction depends only on the subspace spanned by the columns of $U$; the
Stiefel manifold is a convenient parametrization of the corresponding
Grassmann manifold.
...
\index{Min-SW!discrepancy}% strictindex
\index{Min-SW!lifted plan}% strictindex
...
\begin{prop}[Min-SW lower bound, topology and metric status]\label{prop-min-sw-comparison}
	...
	\begin{equation}\label{eq-min-sw-comparison}
		...
		\MinSW_2(\alpha,\beta).
	\end{equation}
	Consequently, convergence for $\MinSW_2$ implies convergence for $\Wass_2$. The converse fails in every dimension $d\geq2$: there are measures $\alpha_n$ such that
...
\begin{proof}
	...
	It remains to prove the topological claim. Write $\gamma_k=\Gaussian(0,\Id_k)$ and let
...
Despite the topological obstruction above, Min-SW is exact on the finite-dimensional family of nondegenerate Gaussian laws.
...
\begin{proof}
	...
	\]
	The projected variables are therefore related by an increasing affine map, so $(P_\theta,P_\theta)_\sharp(\Id,T)_\sharp\alpha$ is their monotone one-dimensional coupling $\varpi_\theta$. Hence the optimal Gaussian coupling $(\Id,T)_\sharp\alpha$ belongs to $\mathcal C_\theta(\alpha,\beta)$ in Definition~\ref{def-min-sw}. It is feasible for~\eqref{eq-min-sw-definition}, which gives $\MinSW_2\leq\Wass_2$. The reverse inequality is~\eqref{eq-min-sw-comparison}; formula~\eqref{eq-dist-gauss} concludes the proof.
\end{proof}

For comparison, fixing $\theta$ before comparing the measures restores a metric on equal-weight $n$-point clouds whose $\theta$-projections are injective: it is $n^{-1/2}$ times the Euclidean distance between the two tuples ordered along $\theta$. The constrained fixed-direction extension is likewise a metric on the class of measures with atomless $\theta$-projections~\cite{TanguyChapelDelon2025SlicedTransportPlans}. It is the subsequent pair-dependent minimization over $\theta$ that destroys the triangle inequality. The topological obstruction in Proposition~\ref{prop-min-sw-comparison} has a different origin: a one-dimensional projection does not resolve the $(d-1)$ transverse coordinates of a continuous law. Even though empirical measures converge in $\Wass_2$, each projected atom can represent at most finitely many transverse locations, leaving a non-vanishing quantization error. Accordingly, the weak-convergence result of Mahey et al.~\cite{MaheyChapelGassoBonetCourty2023MinSW} concerns the fixed-cardinality empirical space $\Pp_2^n(\RR^d)$ and does not extend to the full Wasserstein space.
...
This is an extrinsic counterpart of the Gromov--Wasserstein viewpoint developed in Section~\ref{sec-gromov-wasserstein}. Procrustes alignment searches only over ambient rigid motions, whereas GW compares the metric-measure structures themselves. Proposition~\ref{prop-gw-wasserstein-procrustes-comparison} places the Wasserstein--Procrustes distance between GW and ordinary Wasserstein in~\eqref{eq-gw-wp-wasserstein-comparison}, and Remark~\ref{rem-gw-wp-topologies} explains the resulting quotient topologies. Thus a good Procrustes registration forces small GW distortion; the M\'emoli profile bound in Proposition~\ref{prop-memoli-gw-profile-lower-bound} gives a complementary inexpensive intrinsic certificate.
\index{Wasserstein!Procrustes}% strictindex
\index{Gromov-Wasserstein@Gromov--Wasserstein}% strictindex
\index{profile lower bound}% strictindex

The empirical problem naturally suggests an alternating minimization. Given a current rigid motion $(R^{(\ell)},t^{(\ell)})$, first compute the OT coupling between the registered source cloud and the target cloud,
\index{rigid motion}% strictindex
...
\index{Procrustes!rigid update}% strictindex
	...
	\[
		...
		M_\P=\sum_{i,j} \P_{i,j}(y_j-\bar y)(x_i-\bar x)^\top.
	\]
...
\begin{proof}
	...
	\[
		\sum_{i,j}\P_{i,j}\dotp{R(x_i-\bar x)}{y_j-\bar y}
		=
		\tr(R^\top M_\P).
	\]
...
\textbf{Set} $\bar x=\sum_i\a_i x_i$, $\bar y=\sum_j\b_j y_j$, $\ell=0$ and $\eta^{(0)}=+\infty$.

\textbf{While} $\eta^{(\ell)}>\mathrm{tol}$ and $\ell<L$ \textbf{do}:
\begin{algblock}

\textbf{Set} \(\C_{i,j}^{(\ell)}=\norm{R^{(\ell)}x_i+t^{(\ell)}-y_j}^2\).

\textbf{Solve} \(\P^{(\ell)}\in\argmin_{\P\in\CouplingsD(\a,\b)}\sum_{i,j}\P_{i,j}\C_{i,j}^{(\ell)}\).

\textbf{Compute} \(M_{\P^{(\ell)}}=\sum_{i,j}\P^{(\ell)}_{i,j}(y_j-\bar y)(x_i-\bar x)^\top\).

\textbf{Factorize} \(M_{\P^{(\ell)}}=U\Sigma V^\top\).

\textbf{Set} \(D=\Id\) for $\mathrm O(d)$; set \(D=\diag(1,\ldots,1,\det(UV^\top))\) for $\mathrm{SO}(d)$.

\textbf{Set} \(R^{(\ell+1)}=UDV^\top\) and \(t^{(\ell+1)}=\bar y-R^{(\ell+1)}\bar x\).

\textbf{Set} \(\eta^{(\ell+1)}=\norm{R^{(\ell+1)}-R^{(\ell)}}_{\mathrm F}+\norm{t^{(\ell+1)}-t^{(\ell)}}\).

\textbf{Set} \(\ell\leftarrow \ell+1\).
\end{algblock}
\algreturnskip
\textbf{Solve} \(\P^{(\ell)}\in\argmin_{\P\in\CouplingsD(\a,\b)}\sum_{i,j}\P_{i,j}\norm{R^{(\ell)}x_i+t^{(\ell)}-y_j}^2\).

\textbf{Return} \(\P^{(\ell)},R^{(\ell)},t^{(\ell)}\).
\end{alg}

Figure~\ref{fig:wasserstein-procrustes-rigid-motion} follows a pose-damped variant through a deliberately large translation, showing how the transport correspondences and the rigid registration stabilize together. At each iteration, the exact Procrustes candidate is computed and then interpolated with the current pose along $\mathrm{SO}(2)\times\RR^2$. This continuation heuristic differs from the full block update in Algorithm~\ref{alg:wasserstein-procrustes}, but every displayed state remains a rigid transform of the source.
...
\end{tabular}
\caption{\emph{Pose-damped Wasserstein--Procrustes alignment of two bunny silhouettes.} The target is black, while the rigidly transformed source is colored from red to blue at iterations \(1,2,3,5,10\). Each step solves an equal-weight assignment and computes the closed-form Procrustes candidate of Proposition~\ref{prop-wasserstein-procrustes-rigid-update}; rotation is then damped along $\mathrm{SO}(2)$ and translation linearly before the next assignment. Faint segments show selected current correspondences. This rigid continuation heuristic visualizes the registration path but is not the exact full block update of Algorithm~\ref{alg:wasserstein-procrustes}.}
...
Linear OT starts from the multivariate analogue of quantile coordinates. The one-dimensional quantile function represents a probability measure by the monotone map sending a fixed reference law to it; in dimension $d>1$, Brenier's theorem gives the corresponding construction after choosing an absolutely continuous reference probability $\gamma$, typically the uniform law on a convex body or a standard Gaussian.
\index{probability measure}% strictindex
...
\[
	...
	\qquad
	\bar\alpha_{\LOT}=\bar T_\sharp\gamma.
\]
...
\small 1D maps $T_\alpha,T_\beta,T_\eta$ & \small measures $\alpha,\beta,\eta$
\end{tabular}
...
\index{Brenier!map}% strictindex
	\[
		\Wass_2(\alpha,\beta)\leq \LOT_\gamma(\alpha,\beta)
		\quad\text{and}\quad
		\LOT_\gamma(\alpha,\beta)
		\leq
...
\end{prop}
\begin{proof}
	The first inequality is immediate: $(T_\alpha,T_\beta)_\sharp\gamma$ is a feasible coupling between $\alpha$ and $\beta$. The second inequality is the quantitative stability theorem of M\'erigot, Delalande and Chazal~\cite{merigot2020stability}. Its proof first controls the difference of the two Kantorovich potentials in terms of $\Wass_1(\alpha,\beta)$, then combines convexity and interpolation estimates to control the gradients in $L^2(\gamma)$; the resulting dimension-independent H\"older exponent is $2/15$.
...
\]
The map \(\Id+\bar\xi\) defines the linear OT mean \((\Id+\bar\xi)_\sharp\gamma\). The empirical covariance operator on \(L^2(\gamma;\RR^d)\) is the finite-rank operator
\[
	\mathcal C_{\LOT} h
	\eqdef
	\frac1N\sum_{i=1}^N
	...
	(\xi_i-\bar\xi),
\]
and its leading orthonormal eigenvectors \(e_k\) define the principal linear OT modes. Equivalently, one diagonalizes the \(N\times N\) Gram matrix \(G_{i,j}=\frac1N\dotp{\xi_i-\bar\xi}{\xi_j-\bar\xi}_{L^2(\gamma)}\). If \(Gv_k=\lambda_k v_k\) with \(\lambda_k>0\) and \(\norm{v_k}=1\), then
\[
...
\]
For small excursions around the mean displacement, this gives a practical tangent-space PCA for probability measures: it captures dominant modes of deformation while avoiding repeated pairwise OT computations. For large coefficients, \(T_a\) may fail to be the Brenier map from \(\gamma\) to \(\alpha_a\), and may even leave the regular chart where \(T_a\) is a gradient of a convex function. Thus \(a\mapsto \alpha_a\) should be read as a chart-dependent linearized visualization rather than as an intrinsic Wasserstein geodesic. This LOT-PCA viewpoint was introduced for image variability analysis in~\cite{wang2013linear} and developed in transport-based signal analysis~\cite{thorpe2017transportation,kolouri2017optimal}; it complements intrinsic principal-geodesic or geodesic-PCA approaches, which optimize directly in the curved Wasserstein space~\cite{SeguyCuturi,bigot2017geodesic}.
\index{covariance!operator}% strictindex
\index{principal component analysis}% strictindex
\index{Brenier!map}% strictindex
\index{Wasserstein!geodesic}% strictindex

Figure~\ref{fig:linear-ot-1d-pca} first shows the exact one-dimensional case. A balanced training ensemble of well-separated two-Gaussian mixtures varies their common translation, relative separation and, more weakly, their scales and weights. Its empirical Wasserstein barycenter is computed exactly by averaging the finely sampled quantile functions, after which ordinary PCA is performed in \(L^2(0,1)\). The three leading modes are converted back into densities by pushing the uniform law on \((0,1)\) through enlarged excursions along the PCA lines.
\index{quantile!function}% strictindex
...
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}ccc@{}}
\includegraphics[width=.31\linewidth]{figures/linear-ot-1d-pca/mode-1.pdf} &
\includegraphics[width=.31\linewidth]{figures/linear-ot-1d-pca/mode-2.pdf} &
\includegraphics[width=.31\linewidth]{figures/linear-ot-1d-pca/mode-3.pdf}
\\[-.1em]
\small first quantile-PCA mode & \small second quantile-PCA mode & \small third quantile-PCA mode
\end{tabular}
\caption{\emph{One-dimensional linear OT PCA for synthetic two-Gaussian mixtures.} The exact empirical Wasserstein barycenter has quantile \(Q_{\bar\alpha}=N^{-1}\sum_iQ_{\alpha_i}\). Each panel shows nine pushforwards \(\alpha_{k,a}=(Q_{\bar\alpha}+a e_k)_\sharp\mathrm{Unif}(0,1)\), including the barycenter at the violet midpoint \(a=0\), with \(a\) increasing from red to blue over an enlarged range. The first mode is a common translation, the second changes the component separation, and the third captures the leading residual shape variation. At extreme amplitudes the displayed map need not remain monotone, but its pushforward is still a valid probability law.}
\index{linear!OT}% strictindex
\index{quantile!function}% strictindex
\index{principal component analysis}% strictindex
...
Figure~\ref{fig:linear-ot-mnist-pca} then illustrates a regularized numerical approximation of the same construction on MNIST digit-zero images. Before normalization, a background value $0.020$ is added to every pixel; this prevents zero rows in the entropic computations but changes the input histograms. A Sinkhorn barycenter with regularization $0.004$ is used as the reference \(\gamma\), entropic couplings from \(\gamma\) to the training images are converted into barycentric-projection maps, and PCA is applied to the resulting approximate displacement fields.
...
\end{tabular}
\end{tabular}
\caption{\emph{Principal components in linear OT coordinates for MNIST digit-zero histograms.} The left panel shows the Sinkhorn barycenter reference \(\gamma\). Each mode panel is a triptych obtained by pushing \(\gamma\) through \(x\mapsto x+\bar\xi(x)+a e_k(x)\) with a negative, zero, and positive coefficient \(a\). The transport computations use densities obtained by adding $0.020$ to every pixel before normalization, with entropic regularization $0.004$; white and black respectively denote zero and high displayed mass. The modes capture rotations, aspect-ratio changes, and stroke-thickness deformations in the tangent chart around the barycenter.}
\index{linear!OT}% strictindex
\index{principal component analysis}% strictindex
...
Spectral OT changes the scalar quadratic cost by measuring the whole displacement second-moment matrix through a matrix gauge. The same object admits a robust projected formulation: instead of fixing one projection, one maximizes over the polar set of the gauge. Projection robust OT is the non-convex max--min version of this idea, while subspace robust OT reverses the order into a min--max problem~\cite{paty2019subspace}; spectral gauges express the latter through a convex matrix gauge and connect to recent spectral-gradient viewpoints such as Muon dynamics~\cite{peyre2026muon}.
\index{spectral!gauge}% strictindex
...
\index{displacement second moment}% strictindex
\index{quadratic!cost}% strictindex
...
\index{positive!semidefinite matrix}% strictindex
	...
	\]
\end{defn}
The monotonicity condition means that increasing the displacement second-moment matrix in Loewner order cannot decrease the transport penalty. A fundamental family is obtained from Schatten norms.
\index{displacement second moment}% strictindex
...
Restricting the robust directions to orthogonal projections gives the non-convex max--min counterpart of spectral Wasserstein transport.
...
Ky Fan's variational formula identifies the min--max value with a spectral Wasserstein distance and makes its relation to the max--min value explicit.

\begin{prop}[Projection/subspace robust comparison]\label{prop-ky-fan-srw-comparison}
\index{Ky Fan gauge}% strictindex
\index{polar set}% strictindex
...
\begin{proof}
	...
	\[
		...
		=
		\max_{0\preceq A\preceq\Id,\ \tr(A)\leq k}\tr(AM).
	\]
...
Figure~\ref{fig:spectral-wasserstein-gauge} compares the trace and top-eigenvalue geometries at both levels: each row is the exact discrete displacement interpolation induced by the corresponding computed plan, followed only by KDE smoothing for visualization. The coupling panels display the largest positive edges of those plans.
...
\end{tabular}
\caption{\emph{Trace and spectral gauges for displacement second moments.} Both plans use the same equal-weight discretizations of a shifted two-disk source and a shifted heart target. The trace gauge gives ordinary quadratic OT, while the $\lambda_{\max}$ gauge minimizes the largest mean squared projected displacement and is approximated with finitely many directions. The coupling panels show the largest positive plan entries. Each density row is the exact weighted pushforward $((1-t)x+ty)_\sharp\pi$ of the corresponding full discrete plan, KDE-smoothed on one fixed spatial frame; white is zero density and high density saturates in the red-to-blue time color.}
...
\index{optimal coupling!fiberwise}% strictindex
