...
\index{Hellinger!geodesic}% strictindex
	...
	\[
		\mathfrak h_{0,1}=\int\sqrt{\rho_0\rho_1},
		\qquad
		\Sigma_{0,1}=2(\Sigma_0^{-1}+\Sigma_1^{-1})^{-1},
		\qquad
		m_{0,1}=(\Sigma_0^{-1}+\Sigma_1^{-1})^{-1}
		(\Sigma_0^{-1}m_0+\Sigma_1^{-1}m_1).
	\]
	Completing the square gives $\sqrt{\rho_0\rho_1}=\mathfrak h_{0,1}\rho_{0,1}$, where $\rho_{0,1}$ is the density of $\Gaussian(m_{0,1},\Sigma_{0,1})$. The Hellinger geodesic is therefore
...
\begin{proof}
	...
	Indeed, writing $c_{i,j}(t)=\norm{x_i-x_j-tv_j}^2$, the discrete dual
	potentials $f_i=0$ and $g_j=c_{j,j}(t)$ are feasible for small $t$, because
	$c_{j,j}(t)<c_{i,j}(t)$ whenever $i\neq j$, and they certify the diagonal
	plan. Thus
	\[
		\Wass_2(\eta,\eta_t)^2
		=t^2\sum_{i=1}^n a_i\norm{v_i}^2.
...
\begin{proof}
	...
	\[
		\SW_2(\alpha,\beta)^2
		=
		\frac{\norm{m_\alpha-m_\beta}^2+\tr(\Sigma_\alpha)+\tr(\Sigma_\beta)}{d}
		-2\int_{\Sphere^{d-1}}
		\sqrt{(\transp{\theta}\Sigma_\alpha\theta)
		(\transp{\theta}\Sigma_\beta\theta)}\,\d\sigma(\theta).
	\]
...
\begin{rem}[Proportional and rank-one covariances]\label{rem-sliced-gaussian-special-cases}
	If $\Sigma_\alpha=s^2\Sigma$ and $\Sigma_\beta=t^2\Sigma$, then
	\[
		\SW_2(\alpha,\beta)^2
		=
		\frac1d\Wass_2(\alpha,\beta)^2
	...
	\]
	This includes two isotropic covariances. If both covariances are positive
	definite, these proportional pairs are exactly the equality cases; one
	isotropic covariance alone does not suffice.

	For the rank-one covariances
	$\Sigma_\alpha=a^2u\transp{u}$ and
	$\Sigma_\beta=b^2v\transp{v}$, set
	$\chi=\abs{\dotp{u}{v}}$. Formula~\eqref{eq-sliced-gaussian} reduces to
	\[
		\SW_2(\alpha,\beta)^2
		=
		\frac1d\left[
		...
		\right].
	\]
	Thus aligned directions attain the upper bound, whereas centered,
	orthogonal and equal-scale covariances satisfy
	$\SW_2=\sqrt{1-2/\pi}\,\Wass_2/\sqrt d$. The associated Gaussian
	sliced-Wasserstein flow is revisited in
...
\[
	...
	(z_i-\bar z),
\]
and its leading orthonormal eigenvectors \(e_k\) define the principal linear OT modes. Equivalently, one diagonalizes the \(N\times N\) Gram matrix \(G_{i,j}=\frac1N\dotp{z_i-\bar z}{z_j-\bar z}_{L^2(\alpha_{\rm ref})}\). If \(Gv_k=\lambda_k v_k\) with \(\lambda_k>0\) and \(\norm{v_k}=1\), then
\[
