...
\paragraph{Mass--shape separation.}
\index{mass!shape separation}% strictindex

An unbalanced plan chooses both how much mass to transport and how to arrange it. These variables can always be separated algebraically, independently of the ground cost and of the marginal divergences. For a general divergence, however, the normalized shape problem still depends on the transported mass; KL relaxation removes this residual dependence.

Let $A=\alpha(\Xx)>0$, $B=\beta(\Yy)>0$, and write the normalized input shapes as $\widehat\alpha=\alpha/A$ and $\widehat\beta=\beta/B$. For an entropy $\psi$, a scalar $s\in\operatorname{ri}(\dom\psi)$, and $q_s\in\partial\psi(s)$, introduce the recentered entropy
\begin{equation}\label{eq-recentered-mass-shape-entropy}
	\psi^{[s]}(r)
	\eqdef
	\psi(sr)-\psi(s)-s q_s(r-1).
\end{equation}
Convexity gives $\psi^{[s]}(1)=0$ and $\psi^{[s]}\geq0$, so $\psi^{[s]}$ again generates a divergence between probability measures.
Although the subgradient $q_s$ need not be unique, changing it only adds an affine function of $r-1$; the resulting divergence between probability measures is therefore unchanged.

\begin{prop}[Mass--shape decomposition for marginal divergences]\label{prop-uot-general-mass-shape}
	For $M>0$ and probability measures $\widetilde\alpha,\widetilde\beta$, one has
	\begin{equation}\label{eq-divergence-mass-shape-identity}
		\Dd_{\psi_1}(M\widetilde\alpha|A\widehat\alpha)
		=
		A\psi_1(M/A)
		+A\Dd_{\psi_1^{[M/A]}}(\widetilde\alpha|\widehat\alpha),
	\end{equation}
	and analogously for $(\psi_2,B,\widetilde\beta,\widehat\beta)$. Define the normalized shape envelope at transported mass $M$ by
	\begin{equation}\label{eq-uot-mass-dependent-shape-envelope}
		\mathcal E_M^{c,\psi_1,\psi_2}(\alpha,\beta)
		\eqdef
		\inf_{\substack{\widetilde\alpha\in\Pp(\Xx)\\
			\widetilde\beta\in\Pp(\Yy)}}
		\left\{
			\MK_c(\widetilde\alpha,\widetilde\beta)
			+\frac{A}{M}\Dd_{\psi_1^{[M/A]}}(\widetilde\alpha|\widehat\alpha)
			+\frac{B}{M}\Dd_{\psi_2^{[M/B]}}(\widetilde\beta|\widehat\beta)
		\right\}.
	\end{equation}
	Then the unbalanced problem reduces to the scalar mass minimization
	\begin{equation}\label{eq-uot-general-mass-shape}
		\UW_c(\alpha,\beta)
		=
		\min\left\{
			A\psi_1(0)+B\psi_2(0),
			\inf_{M>0}
			\left[
				M\mathcal E_M^{c,\psi_1,\psi_2}(\alpha,\beta)
				+A\psi_1(M/A)+B\psi_2(M/B)
			\right]
		\right\}.
	\end{equation}
	Boundary values are understood by lower-semicontinuous extension when $M/A$ or $M/B$ lies on the boundary of an entropy domain.
\end{prop}

\begin{proof}
	Write every nonzero plan as $\pi=M\bar\pi$, where $M=\pi(\Xx\times\Yy)$ and $\bar\pi$ is a probability coupling with marginals $(\widetilde\alpha,\widetilde\beta)$. At fixed marginals, minimizing its transport term gives $M\MK_c(\widetilde\alpha,\widetilde\beta)$.

	For~\eqref{eq-divergence-mass-shape-identity}, write the Lebesgue decomposition $\widetilde\alpha=r\widehat\alpha+\widetilde\alpha^\perp$. The recession slope of~\eqref{eq-recentered-mass-shape-entropy} is $s((\psi_1)'_{\infty}-q_s)$. Since $\int(r-1)\d\widehat\alpha=-\widetilde\alpha^\perp(\Xx)$, the affine recentering cancels between the absolutely continuous and recession terms, and Definition~\ref{def_divergence} gives the identity with $s=M/A$. Applying the same argument to the second marginal, factoring out $M$, and minimizing first over the normalized shapes gives~\eqref{eq-uot-mass-dependent-shape-envelope}. The remaining minimization in $M$ gives the second term in~\eqref{eq-uot-general-mass-shape}; the first is the value of the zero plan.
\end{proof}

For general entropies, the shape envelope~\eqref{eq-uot-mass-dependent-shape-envelope} depends on $M$, so mass and shape remain coupled. KL is distinguished by an exact cancellation of this dependence.
At the opposite hard-constrained extreme, $\psi_1=\psi_2=\iota_{\{1\}}$ forces $M=A=B$ and $(\widetilde\alpha,\widetilde\beta)=(\widehat\alpha,\widehat\beta)$. Thus the value is infinite when $A\neq B$, whereas for $A=B$ formula~\eqref{eq-uot-general-mass-shape} reduces to the balanced Kantorovich cost.
\index{power divergence}% strictindex
For example, the power-divergence generator $\phi_\gamma$ introduced in Section~\ref{sec-divergences-dual-norms} satisfies $(\phi_\gamma)^{[s]}=s^\gamma\phi_\gamma$ (with the usual continuous extensions at $\gamma=0,1$). For a marginal of mass $A$ penalized by $\tau\Dd_{\phi_\gamma}$, its normalized-shape term in~\eqref{eq-uot-mass-dependent-shape-envelope} therefore carries the coefficient $\tau M^{\gamma-1}A^{1-\gamma}$; the target term has the analogous coefficient with $A$ replaced by $B$. Thus the whole power family admits an explicit mass--shape reduction, while only its KL member $\gamma=1$ makes the shape problem independent of $M$.
...
\begin{prop}[Exact mass--shape separation for KL relaxation]\label{prop-uot-kl-mass-shape}
	...
	\begin{equation}\label{eq-uot-kl-mass-shape}
		...
		\right).
	\end{equation}
	If the shape envelope has a minimizer, an optimal plan transports the mass
	\begin{equation}\label{eq-uot-kl-optimal-mass}
		...
		\right)
		\end{equation}
	times an optimal probability coupling in~\eqref{eq-uot-kl-shape-envelope}.
\end{prop}

\begin{proof}
	For $s>0$, direct substitution in~\eqref{eq-recentered-mass-shape-entropy} gives $(\tau_i\phi_{\KL})^{[s]}=\tau_i s\phi_{\KL}$. Hence~\eqref{eq-uot-general-mass-shape} reduces, after the shape minimization, to
	\[
		M\mathcal E_{c,\tau_0,\tau_1}(\widehat\alpha,\widehat\beta)
		+\tau_0\big(M\log(M/A)-M+A\big)
		+\tau_1\big(M\log(M/B)-M+B\big).
	\]
	Its unique positive minimizer is~\eqref{eq-uot-kl-optimal-mass}; substitution gives~\eqref{eq-uot-kl-mass-shape}. If the shape value is infinite, the same formula returns the zero-plan value.
\end{proof}
...
\(K_{ij}=e^{-\C_{ij}/\epsilon}\a_i\b_j, \quad \omega=\frac{\tau}{\tau+\epsilon}, \quad u^{(0)}=\ones_n, \quad v^{(0)}=\ones_m, \quad \eta^{(0)}=+\infty, \quad \ell=0.\)
...
%%%
\paragraph{Gaussian KL-unbalanced transport.}
\index{Gaussian Hellinger-Kantorovich}% strictindex
\index{Gaussian!unbalanced}% strictindex
Gaussian inputs provide one of the few continuous unbalanced problems with an explicit endpoint cost. The terminology nevertheless requires care. With a quadratic ground cost, KL marginal penalties and no entropy on the coupling, the static problem below is commonly called the \emph{squared Gaussian Hellinger--Kantorovich endpoint cost}. The Wasserstein--Fisher--Rao distance is instead the intrinsic length metric represented by the cosine cone cost in Section~\ref{sec-unbalanced-ot}; see Proposition~\ref{prop-static-dynamic-unbalanced} and~\cite{LieroMielkeSavareLong}. The graph-supported zero-coupling-entropy formulas below follow~\cite{YangZhang2026}; the nondegenerate positive-coupling-entropy counterpart recalled afterward is from~\cite{janati2020gaussian}.
...
\right\}.
\end{equation}
Write $\bar\alpha=\Gaussian(m_0,\Sigma_0)$ and $\bar\beta=\Gaussian(m_1,\Sigma_1)$, and denote by
\begin{equation}\label{eq-gaussian-ghk-shape-envelope}
\mathcal E_\tau
\eqdef
\mathcal E_{c_2,\tau,\tau}(\bar\alpha,\bar\beta),
\qquad
c_2(x,y)=\norm{x-y}^2,
\end{equation}
the normalized KL shape envelope from~\eqref{eq-uot-kl-shape-envelope}.

The first step is independent of the closed-form matrix calculation: among all normalized marginals, Gaussian ones are optimal. For $P,\Sigma\in\mathbb S_{++}^d$, introduce the LogDet divergence
...
and set
\begin{align}
G_\tau&=\Sigma_0+\Sigma_1+\frac{\tau}{2}\Id,
\label{eq-gaussian-ghk-mean-matrix}\\
\mathcal C_\tau(\Sigma_0,\Sigma_1)
&\eqdef
\min_{P,Q\in\mathbb S_{++}^d}
\left\{
\Bb^2(P,Q)
+\frac{\tau}{2}D_{\mathrm{ld}}(P|\Sigma_0)
+\frac{\tau}{2}D_{\mathrm{ld}}(Q|\Sigma_1)
\right\}.
\label{eq-gaussian-ghk-covariance-envelope}
\end{align}

\begin{prop}[Gaussian reduction of the KL shape envelope]\label{prop-gaussian-ghk-reduction}
Let $\Delta m=m_0-m_1$. Then
\begin{equation}\label{eq-gaussian-ghk-reduced-value}
\mathcal E_\tau
=
\frac{\tau}{2}\norm{\Delta m}_{G_\tau^{-1}}^2
+\mathcal C_\tau(\Sigma_0,\Sigma_1).
\end{equation}
The unique minimizing normalized marginals are Gaussian. Their means are
\begin{equation}\label{eq-gaussian-ghk-adjusted-means}
u_\tau=m_0-\Sigma_0G_\tau^{-1}\Delta m,
\qquad
v_\tau=m_1+\Sigma_1G_\tau^{-1}\Delta m,
\end{equation}
and their covariances minimize~\eqref{eq-gaussian-ghk-covariance-envelope}.
\end{prop}

\begin{proof}
Consider finite-objective competitors $\widetilde\alpha,\widetilde\beta$ in~\eqref{eq-gaussian-ghk-shape-envelope}, with respective means and covariances $(u,P)$ and $(v,Q)$. Finite relative entropy with respect to a nondegenerate Gaussian gives finite second moments and $P,Q\succ0$. Gelbrich's inequality, Theorem~\ref{thm-gelbrich-projection}, lowers the transport cost after replacing the competitors by $\Gaussian(u,P)$ and $\Gaussian(v,Q)$. Moment matching also gives the information-projection identity
\[
\KL(\widetilde\alpha|\bar\alpha)
=
\KL\bigl(\widetilde\alpha|\Gaussian(u,P)\bigr)
+\KL\bigl(\Gaussian(u,P)|\bar\alpha\bigr),
\]
and similarly for the second marginal. Hence Gaussian replacement cannot increase the objective.

The Gaussian formulas for $\Wass_2^2$ and KL separate the mean and covariance variables. The first-order equations for the mean part are
\[
u-v+\frac{\tau}{2}\Sigma_0^{-1}(u-m_0)=0,
\qquad
v-u+\frac{\tau}{2}\Sigma_1^{-1}(v-m_1)=0.
\]
Solving them gives~\eqref{eq-gaussian-ghk-adjusted-means}; substitution gives the first term in~\eqref{eq-gaussian-ghk-reduced-value}, while the covariance part is~\eqref{eq-gaussian-ghk-covariance-envelope}. The resulting finite-dimensional objective is coercive in the means and covariances and diverges at the boundary of $\mathbb S_{++}^d$, so it attains its minimum. Finally, Proposition~\ref{prop-kantorovich-value-curvature} gives joint convexity of the transport term, and each KL term is strictly convex in its first argument. Thus the minimizing pair is unique.
\end{proof}

The covariance envelope is a strictly convex optimization problem, despite its nonlinear matrix square roots. Its first-order equations reduce to one positive-definite Riccati equation.

\begin{prop}[Adjusted covariance pair]\label{prop-gaussian-ghk-adjusted-covariances}
Set $\eta=2/\tau$, $R_i=\Sigma_i^{-1}+\eta\Id$, and
\begin{equation}\label{eq-gaussian-ghk-adjusted-map}
L_\tau
=
R_1^{-1/2}
\left(R_1^{1/2}R_0R_1^{1/2}\right)^{1/2}
R_1^{-1/2}.
\end{equation}
Then $L_\tau R_1L_\tau=R_0$, and the unique minimizer of~\eqref{eq-gaussian-ghk-covariance-envelope} is
\begin{equation}\label{eq-gaussian-ghk-adjusted-covariances}
P_\tau=\left[\Sigma_0^{-1}+\eta(\Id-L_\tau)\right]^{-1},
\qquad
...
P^{-1}=\Sigma_0^{-1}+\eta(\Id-L),
\qquad
Q^{-1}=\Sigma_1^{-1}+\eta(\Id-L^{-1}).
\end{equation}
Substituting $Q^{-1}=L^{-1}P^{-1}L^{-1}$ gives $LR_1L=R_0$, whose unique positive-definite solution is~\eqref{eq-gaussian-ghk-adjusted-map}. Back-substitution gives~\eqref{eq-gaussian-ghk-adjusted-covariances}. Positivity follows directly from
\[
2\eta^{-1}P_\tau^{-1}
=(\Id-L_\tau)^2
+\eta^{-1}\Sigma_0^{-1}
+\eta^{-1}L_\tau\Sigma_1^{-1}L_\tau
\succ0.
\]
...
+\frac{\tau}{2}
\left[
\tr(\Sigma_0^{-1}P)+\tr(\Sigma_1^{-1}Q)-2d
\right]
=0.
\]
...
The preceding convex covariance problem has a factor interpretation that directly extends the ordinary Bures--Procrustes formula. For a full-row-rank matrix $Z\in\RR^{d\times d'}$, define
\begin{equation}\label{eq-soft-procrustes-penalty}
\Psi(Z)
\eqdef
\norm{Z}_{\mathrm F}^2-\log\det(ZZ^\top)-d.
\end{equation}

\begin{prop}[Soft Procrustes formula]\label{prop-gaussian-uot-soft-procrustes}
Let $S_i=\Sigma_i^{1/2}$ and $d'\geq d$. Then
\begin{equation}\label{eq-gaussian-uot-soft-procrustes}
\mathcal C_\tau(\Sigma_0,\Sigma_1)
=
\min_{\substack{Z_0,Z_1\in\RR^{d\times d'}\\
\rank(Z_0)=\rank(Z_1)=d}}
\left\{
\norm{S_0Z_0-S_1Z_1}_{\mathrm F}^2
+\frac{\tau}{2}\Psi(Z_0)
+\frac{\tau}{2}\Psi(Z_1)
\right\}.
\end{equation}
Moreover, $\Psi(Z)\geq0$, with equality if and only if $ZZ^\top=\Id$. Thus the LogDet terms soften the row-Stiefel constraints in the ordinary Bures--Procrustes problem.
\end{prop}

\begin{proof}
Insert the free-factor Bures formula of Proposition~\ref{prop-bures-procrustes} into~\eqref{eq-gaussian-ghk-covariance-envelope}. Every full-row-rank factor of $P_i\succ0$ can be written $F_i=S_iZ_i$, and cyclicity of the trace together with similarity invariance of the determinant gives
\[
D_{\mathrm{ld}}(F_iF_i^\top|\Sigma_i)
=
\tr(Z_iZ_i^\top)-\log\det(Z_iZ_i^\top)-d
=
\Psi(Z_i).
\]
This proves~\eqref{eq-gaussian-uot-soft-procrustes}. If $s_1,\ldots,s_d$ are the singular values of $Z$, then $\Psi(Z)=\sum_j(s_j^2-2\log s_j-1)$; each summand is nonnegative and vanishes exactly at $s_j=1$.
\end{proof}

The covariance problem~\eqref{eq-gaussian-ghk-covariance-envelope} is convex, but its factorized form~\eqref{eq-gaussian-uot-soft-procrustes} is generally not convex in $(Z_0,Z_1)$. It is an exact redundant parameterization, invariant under the simultaneous right action $(Z_0,Z_1)\mapsto(Z_0R,Z_1R)$ for $R\in\mathrm O(d')$.

\begin{rem}[Hard Bures limit]\label{rem-gaussian-uot-bures-limit}
\index{Bures!soft Procrustes limit}% strictindex
\index{Procrustes!soft}% strictindex
Formula~\eqref{eq-gaussian-uot-soft-procrustes} is the penalized counterpart of Proposition~\ref{prop-bures-procrustes}. Indeed, row-Stiefel competitors give $\mathcal C_\tau\leq\Bb^2(\Sigma_0,\Sigma_1)$. Therefore any minimizing factors satisfy $\Psi(Z_i)\to0$ as $\tau\to+\infty$. Coercivity of $\Psi$ and lower semicontinuity then reduce the limiting problem to the hard row-Stiefel constraints, yielding
\[
\mathcal C_\tau(\Sigma_0,\Sigma_1)
\longrightarrow
\Bb^2(\Sigma_0,\Sigma_1).
\]
Together with Proposition~\ref{prop-gaussian-ghk-reduction}, this recovers the Gaussian $\Wass_2^2$ formula~\eqref{eq-dist-gauss}. In particular, when $r_0=r_1=r$, the endpoint value~\eqref{eq-gaussian-ghk-endpoint} converges to $r\Wass_2^2(\bar\alpha,\bar\beta)$.
\end{rem}

We can now state the endpoint formula in terms of the adjusted Gaussian marginals. Define
\begin{equation}\label{eq-gaussian-ghk-affinity}
\mathfrak a_\tau
\eqdef
\exp\!\left(-\frac14\norm{m_0-m_1}_{G_\tau^{-1}}^2\right)
\left(
\frac{\det P_\tau\det Q_\tau}
{\det\Sigma_0\det\Sigma_1}
\right)^{1/4}.
\end{equation}
...
\right).
\end{equation}
The optimal transported mass is $M_\tau=\sqrt{r_0r_1}\,\mathfrak a_\tau$. More precisely, the unique optimal plan is
\begin{equation}\label{eq-gaussian-ghk-optimal-plan}
\begin{aligned}
\pi_\tau
&=
M_\tau(\Id,\T_\tau)_\sharp\Gaussian(u_\tau,P_\tau),\\
\T_\tau(x)&=v_\tau+L_\tau(x-u_\tau).
\end{aligned}
\end{equation}
\end{prop}

\begin{proof}
Propositions~\ref{prop-gaussian-ghk-reduction} and~\ref{prop-gaussian-ghk-adjusted-covariances} give
\[
\exp\!\left(-\frac{\mathcal E_\tau}{2\tau}\right)
=
\mathfrak a_\tau.
\]
The common-penalty case of the general KL mass--shape formula~\eqref{eq-uot-kl-mass-shape} therefore gives~\eqref{eq-gaussian-ghk-closed-form} and the mass $M_\tau$. The optimal normalized coupling is the Gaussian $\Wass_2$ graph coupling between the adjusted marginals from Propositions~\ref{prop-gaussian-ghk-reduction} and~\ref{prop-gaussian-ghk-adjusted-covariances}, which is precisely~\eqref{eq-gaussian-ghk-optimal-plan}. The scalar mass minimization and the normalized marginals are unique; since the adjusted source is nondegenerate, Brenier uniqueness then gives uniqueness of the full plan.
\end{proof}
...
\index{Hellinger!geodesic}% strictindex
	Gaussian optimal-plan closure is established at zero coupling entropy in~\cite{YangZhang2026} and at positive coupling entropy in~\cite{janati2020gaussian}. In the first case, the optimizer is a scaled Gaussian Wasserstein coupling between two adjusted Gaussian marginals; in the second, it is a non-degenerate scaled Gaussian on the product space. Consequently, linearly interpolating a pair drawn from either plan produces Gaussian one-time marginals. This interpolation should not be identified with an ambient WFR geodesic, which is governed by the cosine cone geometry of Proposition~\ref{prop-static-dynamic-unbalanced} and need not remain Gaussian.
...
\textbf{Solve} \(\P^{(\ell)}\in\argmin_{\P\in\CouplingsD(\a,\b)}\sum_{i,j}\P_{ij}\C_{ij}^{(\ell)}\).

\textbf{Compute} \(M_{\P^{(\ell)}}=\sum_{i,j}\P^{(\ell)}_{ij}(y_j-\bar y)(x_i-\bar x)^\top\).
...
\end{algblock}
\algreturnskip
\textbf{Solve} \(\P^{(\ell)}\in\argmin_{\P\in\CouplingsD(\a,\b)}\sum_{i,j}\P_{ij}\norm{R^{(\ell)}x_i+t^{(\ell)}-y_j}^2\).
...
\[
	...
	(z_i-\bar z),
\]
and its leading orthonormal eigenvectors \(e_k\) define the principal linear OT modes. Equivalently, one diagonalizes the \(N\times N\) Gram matrix \(G_{ij}=\frac1N\dotp{z_i-\bar z}{z_j-\bar z}_{L^2(\alpha_{\rm ref})}\). If \(Gv_k=\lambda_k v_k\) with \(\lambda_k>0\) and \(\norm{v_k}=1\), then
\[
	e_k=\frac{1}{\sqrt{N\lambda_k}}\sum_{i=1}^N (v_k)_i(z_i-\bar z)
\]
