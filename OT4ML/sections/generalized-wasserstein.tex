...
For the cone construction it is useful to make explicit the equivalent
semi-coupling form of the homogeneous value~\cite{LieroMielkeSavareLong}.
Here one uses a base measure $\lambda$ on $\Xx\times\Yy$ and two nonnegative
weights $u,v$ attached to each transported pair:
...
\begin{aligned}
	...
	\; ; \\
	&(\mathrm{p}_1)_\#(u\lambda)=\al,\qquad
...
\index{marginal!constraint}% strictindex
	...
	Assume now that $\De$ is a distance on $\mathfrak C[\Xx]$. Nonnegativity
	and symmetry are immediate. The triangle inequality is the standard
	gluing property of the homogeneous cone construction: one applies the
	gluing lemma to almost optimal cone couplings and then Minkowski's
	inequality for the metric $\De$; the linear weighted projection constraints
	are preserved under this gluing~\cite{LieroMielkeSavareLong}. Finally, if
	$\CW(\al,\be)=0$, any minimizing sequence is supported in the limit on the
	diagonal of the cone, because $\De$ is a distance. On this diagonal one has
	$x=y$ and $r=s$, so the two weighted base projections coincide. Therefore
	$\al=\be$.
...
\[
	f \leftarrow \rho\, g^{\bar c,\epsilon},
	\qquad
	g \leftarrow \rho\, f^{c,\epsilon},
	\qquad
	\rho\eqdef\frac{\tau}{\tau+\epsilon},
\]
...
\index{soft c-transform}% strictindex
	Assume that $c$ is bounded, or more generally that the soft transforms below
	are finite on the bounded potentials under consideration. Let
	$\rho=\tau/(\tau+\epsilon)<1$, and define
	\[
		d_\infty((f,g),(f',g'))
		\eqdef
		\max\{\norm{f-f'}_\infty,\norm{g-g'}_\infty\}.
	...
	\[
		...
		\qquad
		\tilde f=\rho\,g^{\bar c,\epsilon},
		\qquad
		\tilde g=\rho\,\tilde f^{c,\epsilon},
	\]
	is a contraction:
	\[
		d_\infty\big(\mathcal T_{\rm GS}(f,g),\mathcal T_{\rm GS}(f',g')\big)
		\leq
		\rho\, d_\infty((f,g),(f',g')).
	\]
	Consequently the unbalanced Sinkhorn potentials have a unique bounded fixed point
	$(f^\star,g^\star)$ and the iterates satisfy
	\[
		d_\infty((f^{(k)},g^{(k)}),(f^\star,g^\star))
		\leq
		\rho^k d_\infty((f^{(0)},g^{(0)}),(f^\star,g^\star)).
	\]
	...
	\[
		...
		\leq
		\rho^k d_T((u^{(0)},v^{(0)}),(u^\star,v^\star)).
	\]
\end{prop}
\begin{proof}
...
\index{sliced Wasserstein distance}% strictindex
	...
	\]
	With this notation, \eqref{eq-sliced-wasserstein} says that $\SW_p$ is the pull-back, through $\mathfrak R$, of the $L^p(\Sphere^{d-1};\Wass_p)$ distance on Radon data. The full all-direction transform is injective on probability measures by the Cram\'er--Wold theorem, which is why sliced Wasserstein separates measures. The warning is numerical and variational: finitely sampled Radon data are not injective, and independently constructed one-dimensional Radon data need not satisfy the range conditions of an actual image. This is precisely the issue behind sliced and Radon barycenter reconstructions discussed in Section~\ref{sec-barycenters} and illustrated in Figure~\ref{fig:sliced-radon-barycenter}.
...
comparison is clearest along smooth Brenier perturbations: if a measure is
displaced by a small gradient field, then the usual Wasserstein distance sees
the full $L^2(\alpha)$ norm of this field, whereas the sliced distance sees its
...
\index{geodesic!completion}% strictindex
...
\index{sliced Wasserstein!length}% strictindex
	Let $\tau_h(x)=x+h$ for $h\in\RR^d$. For any $\alpha\in\Pp_2(\RR^d)$,
	\[
	...
	\]
	Consequently, even the intrinsic length metric generated by $\SW_2$ is not the usual $\Wass_2$ metric when $d>1$.
\end{prop}
...
\begin{proof}
	...
	\[
		\Wass_2\big((P_\theta)_\sharp\alpha,(P_\theta)_\sharp(\tau_h)_\sharp\alpha\big)^2
		=
		\abs{\dotp{\theta}{h}}^2.
	\]
	After integration on the sphere,
	\[
		\SW_2\big(\alpha,(\tau_h)_\sharp\alpha\big)^2
		=
		\int_{\Sphere^{d-1}}\abs{\dotp{\theta}{h}}^2\d\sigma(\theta)
		=
		\frac{\norm{h}^2}{d}.
	\]
	For the translation curve $\alpha_s=(\tau_{sh})_\sharp\alpha$, the same computation gives
	\[
		\SW_2(\alpha_s,\alpha_t)=\abs{s-t}\frac{\norm{h}}{\sqrt d}.
	\]
...
The proposition gives a useful sanity check on the expression ``geodesic completion of sliced Wasserstein''. The Hilbert embedding below supplies straight chords in projected-quantile coordinates, but such chords need not satisfy the Radon consistency constraints required to come from an actual measure on $\RR^d$. The intrinsic completion is therefore a sliced, projected geometry; it is not obtained by replacing $\SW_2$ with $\Wass_2$. Bonnotte's discussion of sliced geodesics should be read with this distinction in mind: the displacement geodesics used there are the usual $\Wass_2$ geodesics along which the squared sliced distance admits convexity-type estimates, not a statement that the intrinsic length metric of $\SW_2$ is $\Wass_2$.
...
\index{Wasserstein-Procrustes}% strictindex
\index{orthogonal Procrustes}% strictindex
