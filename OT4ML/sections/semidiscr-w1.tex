...
The full objective and the result of eliminating its source potential will be used repeatedly in this chapter. We write \(\Dd_0(f,g)\) for the hard two-potential dual objective and \(\Ee_0(g)\) for its one-potential semi-dual; the subscript \(0\) records the absence of entropic regularization. Section~\ref{sec-sinkhorn-local-acceleration} uses the parallel notation \(\Dd_\epsilon(f,g)\) and \(\Ee_\epsilon(g)\) at positive temperature.
...
\end{defn}
Thus the dual problem~\eqref{eq-dual-generic} is $\MK_\c(\al,\be)=\max_{f,g}\Dd_0(f,g)$. For fixed $g$, feasibility is equivalent to $f\leq g^{\bar c}$. Since $\al$ is nonnegative, the largest admissible choice $f=g^{\bar c}$ maximizes the objective. Consequently,
\[
	\MK_\c(\al,\be)
	=
	\sup_{g\in\Cc(\Yy)}\Ee_0(g),
	\qquad
	\Ee_0(g)=\sup_{f\in\Cc(\Xx)}\Dd_0(f,g).
\]
\index{semi-dual}% strictindex
Partial maximization of a jointly concave objective preserves concavity, so $\Ee_0$ is concave. The semi-dual is invariant under $g\mapsto g+s$: indeed, $(g+s)^{\bar c}=g^{\bar c}-s$ and both measures have unit mass. Thus the objective has an additive gauge: if $g$ is optimal, so is $g+s$. Optimizers need not otherwise be unique; suitable nondegeneracy or connectivity assumptions can yield uniqueness modulo this gauge. The main computational advantage is that the semi-dual is unconstrained.

\paragraph{Discrete semi-dual.}
\index{semi-dual!discrete}% strictindex
...
Let $\al=\sum_{i=1}^n\a_i\de_{x_i}$,
$\be=\sum_{j=1}^m\b_j\de_{y_j}$, and $\C_{i,j}=c(x_i,y_j)$,
where $\a\in\simplex_n$ and $\b\in\simplex_m$. We use the same notation for functions and vectors: in the discrete setting,
\[
	\Dd_0(\fD,\gD)
	\eqdef
	\begin{cases}
...
\]
The function $\Ee_0$ is concave and piecewise affine. It is invariant under $\gD\mapsto\gD+s\ones_m$, because the two marginals have equal mass. If a tie-breaking rule selects $\sigma_\gD(i)\in\argmin_j(\C_{i,j}-\gD_j)$, the induced target masses determine a supergradient explicitly.
...
Thus the semi-dual supergradient is exactly the mismatch between prescribed target mass and mass currently attracted by each target coordinate. At ties, convex combinations of active selections, equivalently splitting each source mass among its tied minimizers, give the full superdifferential. The auction method developed next specializes this same target-potential formulation to uniform square assignments and couples targeted dual updates with ownership changes.
...
\index{auction algorithm!coordinate ascent}% strictindex
\index{coordinate ascent}% strictindex
\index{Laguerre cell!discrete}% strictindex
...
\index{complementary slackness!epsilon-approximate@$\epsilon$-approximate}% strictindex
...
Compared with a single $\eta$-auction started from zero, whose dense bound grows as $O(n^2(1+R_\C/\eta))$, scaling replaces the linear accuracy factor by a logarithmic number of $O(n^3)$ phases. This is a high-accuracy guarantee rather than an unconditional speedup: at coarse tolerances the cold-start bound can be smaller, while the scaling bound improves it once $R_\C/\eta$ is large compared with $n\bigl(1+\log_+(R_\C/\eta)\bigr)$. For integer costs with $R_\C>0$, choosing, for example, $\eta=1/(2n)$ gives an exact assignment with complexity $O\bigl(n^3(1+\log_+(nR_\C))\bigr)$; the case $R_\C=0$ terminates immediately.
...
\index{dual!weight}% strictindex
%
Figure~\ref{fig:semidiscrete-laguerre-cells} follows this adjustment from unweighted Voronoi cells to a power diagram whose cell masses approximately match the prescribed discrete target weights.
...
\small approximately balanced cells \\[-.15em]
...
\index{semi-discrete!one-dimensional formula}% strictindex
\index{quantile!semi-discrete OT}% strictindex

In one dimension, the Laguerre geometry of Definition~\ref{def-laguerre-power-cells} becomes explicit. Consider the quadratic cost $c(x,y)=\abs{x-y}^2$, assume that $\al$ has a density and that $y_1<\cdots<y_m$, and set $B_0=0$ and $B_j=\sum_{k=1}^j\b_k$. For $j<k$, the difference between the weighted costs $c(x,y_j)-\gD_j$ and $c(x,y_k)-\gD_k$ is affine and increasing in $x$. The active site index is therefore nondecreasing, so the nonempty Laguerre cells are intervals ordered like the sites. If $\gD^\star$ maximizes the semi-dual problem~\eqref{eq-semi-dual-discr}, its induced map pushes $\al$ to $\be$. Consequently, the optimal cells agree, up to $\al$-null sets, with the quantile intervals:
...
Here the harmless endpoint conventions are $\cumul{\al}^{-1}(0)=-\infty$ and $\cumul{\al}^{-1}(1)=+\infty$. Thus, after tie-breaking on the null boundaries, the optimal semi-discrete map sends the $j$-th quantile interval to $y_j$; this is the Laguerre-cell realization of the monotone rearrangement in Theorem~\ref{prop-1d-quantile-map}. For equal target weights, $B_j=j/m$, and the same interval partition yields a closed-form quadratic cost through the integrated quantile.

\begin{prop}[Closed-form one-dimensional semi-discrete transport]\label{prop-semidiscrete-1d-quantile}
\index{semi-discrete!closed form}% strictindex
...
The formula isolates the dependence on the source in its second moment and in the increments of $M_\al$ across the uniform quantile bins. In higher dimensions, no global order determines the cells, and the Laguerre-cell machinery becomes necessary.

\paragraph{Mass balance.}
\index{mass!balance}% strictindex

Returning to arbitrary dimension and target weights, one has \(\gD^{\bar\c}(x)=c(x,y_j)-\gD_j\) on each tie-broken cell \(\Laguerre_j(\gD)\). Splitting the integral in the semi-dual objective~\eqref{eq-semi-dual-discr} over this partition gives
...
\textbf{Input:} Source measure $\al$, target atoms $(y_j,\b_j)$, cost $c$, steps $(\tau^{(\ell)})_{\ell=0}^{L-1}$, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.

\textbf{Output:} Semi-discrete dual weights $\gD$ and selected Laguerre cells.
\index{Laguerre cell}% strictindex
...
\textbf{Select active sites and cells:}
\(j^{(\ell)}(x)=\min\argmin_{1\leq r\leq m}\{c(x,y_r)-\gD_r^{(\ell)}\}\),
\(\Laguerre_j^{(\ell),\mathrm{sel}}=\enscond{x}{j^{(\ell)}(x)=j}\).

\textbf{Compute masses:}
\(m_j^{(\ell)}=\int_{\Laguerre_j^{(\ell),\mathrm{sel}}}\d\al \).

\textbf{If} $\max_j\abs{m_j^{(\ell)}-\b_j}\leq\mathrm{tol}$ \textbf{then}:
\begin{algblock}
\textbf{Return} $\gD^{(\ell)}$ and $\bigl(\Laguerre_j^{(\ell),\mathrm{sel}}\bigr)_{j=1}^m$.
\end{algblock}

\textbf{Update}
\(\gD^{(\ell+1)} = \gD^{(\ell)}+\tau^{(\ell)}\bigl(\b-m^{(\ell)}\bigr)\).
\end{algblock}
\algreturnskip
\textbf{Compute} $j^{(L)}(x)=\min\argmin_r\{c(x,y_r)-\gD_r^{(L)}\}$ and $\Laguerre_j^{(L),\mathrm{sel}}=\{x:j^{(L)}(x)=j\}$.

\textbf{Return} $\gD^{(L)}$ and these selected cells.
\end{alg}

The deterministic selector makes the returned cells a disjoint partition, so their masses sum to one, and $\b-m^{(\ell)}$ is a selected supergradient of the concave semi-dual objective. For an atomic source, exact prescribed masses may nevertheless require splitting atoms tied between several cells and need not be realizable by a deterministic map.
...
\]
which is an unbiased estimator of $\nabla\Ee_0(\gD)$ when cell boundaries have $\al$-measure zero. One can therefore maximize~\eqref{eq-semi-disc-energy} without first discretizing $\al$: the measure is used as a black box from which independent samples are drawn, a natural setup in high-dimensional statistics and machine learning.
...
\[
	...
	\gD_j^{(\ell)}
	+
	\tau^{(\ell)}\bigl(\b_j-\ones_{\{j=j^{(\ell)}\}}\bigr).
\]
...
for instance $\tau^{(\ell)}=\tau^{(0)}(1+\ell/\ell_0)^{-q}$ with $1/2<q\leq1$. A finite-horizon $O(L^{-1/2})$ guarantee instead uses averaging and a horizon-dependent step, as made precise next.
\index{rate!sublinear}% strictindex

The following estimate is the classical averaged stochastic-supergradient bound for concave optimization, instantiated for the semi-dual objective; see Nemirovski, Juditsky, Lan and Shapiro~\cite{nemirovski2009robust} and its OT specialization by Genevay et al.~\cite{genevay2016stochastic}.
\begin{prop}[Averaged stochastic semi-dual rate]\label{prop-semidiscrete-sgd-rate}
	...
	\[
		\Ee_0(\gD^\star)-\EE\bigl[\Ee_0(\bar\gD_L)\bigr]
		\leq
		\frac{RG}{\sqrt L}.
	\]
\end{prop}
...
Here one may take $G\leq2$ because each sample supergradient is $\b-e_{j^{(\ell)}}$. This stochastic viewpoint is one of the main algorithmic advantages of the semi-discrete formulation~\cite{Merigot11,genevay2016stochastic}.
\index{semi-discrete!OT}% strictindex

\begin{alg}[Stochastic semi-discrete ascent]\label{alg:semidiscrete-stochastic-ascent}
\textbf{Input:} Source sampler $x\sim\al$, target atoms $(y_j,\b_j)$, steps $(\tau^{(\ell)})_{\ell=0}^{L-1}$, iteration count $L$.
...
\textbf{Draw} $x^{(\ell)}\sim\al$.

\textbf{Set} \(j^{(\ell)}=\min\argmin_j\bigl(c(x^{(\ell)},y_j)-\gD_j^{(\ell)}\bigr)\).
...
This deterministic best-placement rate should not be identified with the rate of an empirical measure. Empirical OT also contains sampling fluctuations, whose dependence on $d$, $p$, moments and regularity is analyzed in Section~\ref{sec-sample-complexity}; it matches the spacing rate only in appropriate high-dimensional regimes. For the regular nonconstant one-dimensional laws covered below, deterministic quadratic quantization has $\Wass_2$ error of order $m^{-1}$, whereas empirical sampling has root-mean-square $\Wass_2$ error of order $m^{-1/2}$. Zador's theorem refines the deterministic statement by identifying the sharp asymptotic constant and the limiting density of optimal codepoints~\cite{graf2000foundationsquantization}.
...
\]
For the squared Euclidean cost, this becomes the fixed-point equation $y_j=\frac{\int_{\VV_j(Y)} x\d\al(x)}{\int_{\VV_j(Y)} \d\al}$.
...
\[
	y_j^{(\ell+1)}=y_j^{(\ell)}+\tau\big(b_j(Y^{(\ell)})-y_j^{(\ell)}\big),
	\qquad 0<\tau\leq 1,
\]
...
\)
can be described by a site density \(\sigma\). Thus \(m\sigma(x)\d x\) represents the number of codepoints near \(x\): regions where \(\sigma\) is large are quantized more finely. If \(\al=\rho\d x\), \(r=p/d\), the empirical site measures converge to \(\sigma\d x\), and the cells are asymptotically locally optimal, the high-resolution ansatz reads~\cite{graf2000foundationsquantization}
\[
	\begin{aligned}
	...
	\end{aligned}
\]
where \(C_{p,d}>0\) is the universal quantization constant. Thus the unscaled distortion is of order \(m^{-p/d}\) along such configurations. Here \(r=2/d\) for the quadratic cost considered in this chapter, \(\sigma\) describes the codepoints, and \(\rho\) is the source density being quantized.
...
\[
	...
	\right),
\]
on a periodic domain or with no-flux boundary conditions. For a uniform source density \(\rho=1\), this becomes \(\partial_t\sigma=-(r+1)\Delta(\sigma^{-r})\), an ultrafast-diffusion equation. Its negative power produces a strong redistribution where codepoints are scarce, and its stationary site density satisfies \(\sigma_\star\propto\rho^{d/(d+p)}\).

The rigorous one-dimensional discrete-to-continuum result starts instead from the ordinary Euclidean site gradient flow \(\dot y_j=-\nabla_{y_j}\Ff(Y)\), together with an \(m\)-dependent time and metric rescaling~\cite{caglioti2015gradient}. It does not directly give a limit for the mass-preconditioned Lloyd flow~\eqref{eq-lloyd-preconditioned-flow}: the two velocities differ by the cell-dependent factors \(2a_j(Y)\), so in general they cannot be related by one global change of time. Related works study the resulting one-dimensional diffusion, perturbations of the hexagonal lattice in dimension two, and the weighted PDE in arbitrary dimension~\cite{iacobelli2019asymptotic,caglioti2018quantization2d,iacobelli2019weighted}.
...
\textbf{Input:} Source measure $\al$, initial codepoints $Y^{(0)}=(y_j^{(0)})_{j=1}^m$, squared Euclidean cost, tolerance $\mathrm{tol}$, iteration budget $L\geq1$.

\textbf{Output:} Codepoints $Y=(y_j)_{j=1}^m$.

\textbf{Initialize:} Set \(d^{(0)}=+\infty\) and \(\ell=0\).

\textbf{While} \(d^{(\ell)}>\mathrm{tol}\) and $\ell<L$ \textbf{do}:
\begin{algblock}

\textbf{Set} \(\ell\leftarrow \ell+1\).

\textbf{Compute the tie-broken Voronoi partition:}
\(\VV_j(Y^{(\ell-1)}) = \enscond{x}{j=\min\argmin_{1\leq r\leq m}\norm{x-y_r^{(\ell-1)}}^2}\).

\textbf{For} each nonempty cell $\VV_j$ \textbf{do}:
\begin{algblock}
\(y_j^{(\ell)} = \frac{\int_{\VV_j(Y^{(\ell-1)})}x\,\d\al(x)} {\int_{\VV_j(Y^{(\ell-1)})}\d\al(x)}\).
\end{algblock}
\textbf{For} each empty cell $\VV_j$ \textbf{do}:
\begin{algblock}

\textbf{Set} $y_j^{(\ell)}=y_j^{(\ell-1)}$.

\end{algblock}

\textbf{Set} \(d^{(\ell)}=\max_j\norm{y_j^{(\ell)}-y_j^{(\ell-1)}}\).
\end{algblock}
\algreturnskip
\textbf{Return} $Y^{(\ell)}$.
\end{alg}
...
\begin{prop}[One-dimensional equal-weight quantization]\label{prop-1d-equal-weight-quantization}
	Let $\al\in\Mm_+^1(\RR)$ have finite second moment and quantile function $Q=\cumul{\al}^{-1}$. For
		\[
			...
			\Wass_2^2(\al,\frac1m\sum_{i=1}^m\de_{y_i}),
	\]
...
\begin{proof}
	...
	\]
	Write $X_i=Q(U_i)$ with $U_i$ i.i.d.\ uniform on $[0,1]$, and let $\widehat U_m^{-1}$ be the empirical quantile function of the $U_i$. Then $\widehat Q_m=Q\circ \widehat U_m^{-1}$. The classical uniform quantile-process theorem~\cite{vanDerVaartWellner1996,BobkovLedoux2019EmpiricalKantorovich} and the functional delta method show that $\sqrt m\,(\widehat Q_m-Q)$ converges in law in $L^2(0,1)$ to $Q'$ times a standard Brownian bridge, up to an immaterial sign. Standard fourth-moment bounds for the uniform quantile process, together with the boundedness of $Q'$, give uniform integrability of the squared $L^2$ norms and hence convergence of expectations. A standard Brownian bridge has pointwise second moment $u(1-u)$, so Fubini's theorem gives the displayed limit.
\end{proof}

Combining Propositions~\ref{prop-1d-equal-weight-quantization} and~\ref{prop-1d-random-quantile-process}, one obtains a sharp contrast between optimal placement and random placement on the line. If $Q$ is nonconstant, deterministic equal-weight quantization has squared error of order $m^{-2}$, hence $\Wass_2$ error of order $m^{-1}$, while i.i.d.\ empirical sampling has expected squared error of order $m^{-1}$, hence root-mean-square $\Wass_2$ error of order $m^{-1/2}$. This one-dimensional comparison is consistent with the broader empirical OT sample-complexity theory~\cite{dereich2013constructive,fournier2015rate,weed2017sharp}.

Figure~\ref{fig:semidiscrete-quantile-quantization-rates} illustrates both parts of this comparison using one bin-average atom per equal quantile interval. On the squared-error scale, the deterministic curve decays one power of $m$ faster than the empirical curve; equivalently, their $\Wass_2$ exponents differ by one half.
...
\\[-.2em]
\small equal-quantile-bin atoms &
\small deterministic versus i.i.d.\ decay
\end{tabular}
\caption{\emph{One-dimensional equal-weight quantization in quantile coordinates.} Left: for a smooth positive density on $[0,1]$, the colored atoms are bin averages of the inverse CDF over equal quantile intervals, while the gray atoms show one i.i.d.\ empirical draw with the same number of particles. Right: deterministic squared errors and Monte Carlo estimates of expected empirical squared errors. The bin averages and midpoint quantiles follow the $m^{-2}$ law, whereas i.i.d.\ empirical measures follow the slower $m^{-1}$ law.}
...
The correct flux space is generally not $L^1(\RR^d;\RR^d)$. For instance, transporting one Dirac mass to another produces a flux concentrated on a segment. The appropriate norm measures both diffuse and singular vector-valued fluxes.

\begin{defn}[Total variation norm of a vector-valued measure]\label{def-vector-measure-tv}
\index{total variation!vector-valued measure}% strictindex
\index{vector-valued measure}% strictindex
	...
	\[
		\norm{\flow}_{\TV}
		\eqdef
		\sup_{\zeta\in C_c(\RR^d;\RR^d)}
		\enscond{\int_{\RR^d}\dotp{\zeta}{\d\flow}}
		{\sup_{x\in\RR^d}\norm{\zeta(x)}_2\leq1}.
	\]
\end{defn}

\begin{rem}[Absolutely continuous vector measures]\label{rem-vector-measure-tv-density}
	If $\flow=w\,\d x$ with $w\in L^1(\RR^d;\RR^d)$, then
	\[
		\norm{\flow}_{\TV}
		=
		\int_{\RR^d}\norm{w(x)}_2\d x.
	\]
	Thus the total variation norm extends the usual $L^1$ norm of a vector field while also assigning a finite cost to singular fluxes.
\end{rem}
