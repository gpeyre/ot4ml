...
\caption{\emph{Graph Beckmann formulation of $\Wass_1$ on two Delaunay graphs $G=(V,E)$.} The left panel uses a quasi-regular vertex set, while the right panel uses a denser nonuniform vertex set sampled from a two-component Gaussian mixture. Red and blue disks encode the positive and negative parts of $r=\alpha-\beta$, localized on six vertices each. The darker graph edges show the triangulation, while the violet arrows display the optimal signed edge flow $m$: orientation gives the sign, width is proportional to $\sqrt{|m_e|}$, and the flow satisfies $\operatorname{div}_G m=r$.}
