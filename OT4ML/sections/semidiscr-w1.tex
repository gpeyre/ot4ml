...
Under the compactness and continuity assumptions of Chapter~\ref{sec-dual}, write the dual problem~\eqref{eq-dual-generic} as
\index{dual!problem}% strictindex
\eq{
	\usup{f,g \in \Cc(\Xx) \times \Cc(\Yy)} \Ee(f,g)
}
where $\Ee(f,g)$ is the dual objective, extended by $-\infty$ outside the feasible set. For fixed $f$, feasibility is equivalent to $g\leq f^c$. Since $\be$ is nonnegative, the largest admissible choice $g=f^c$ maximizes the objective. Consequently,
\index{semi-dual}% strictindex
\eql{\label{eq-semi-dual}
	\usup{f \in \Cc(\Xx)} \tilde\Ee(f) \eqdef \Ee(f,f^c) = \usup{g} \Ee(f,g) =
	\int_\Xx f \d \al + \int_\Yy f^c \d \be.
}
Partial maximization of a jointly concave objective preserves concavity, so $\tilde \Ee$ is concave. The semi-dual is invariant under $f\mapsto f+s$: indeed, $(f+s)^c=f^c-s$ and both measures have unit mass. Potentials are therefore determined only up to an additive constant, exactly as in the full dual. The main computational advantage is that the semi-dual is unconstrained.
