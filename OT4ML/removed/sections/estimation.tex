...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Wasserstein Estimation}
\index{Wasserstein!estimation}% strictindex


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Wasserstein Loss}
\index{Wasserstein!loss}% strictindex
...
\index{parametric model}% strictindex
%
...
Of course, one can consider more complicated problems: for instance, the barycenter problem described in Section~\ref{sec-bary} consists in a sum of such terms. However, most of these more advanced problems can usually be solved by adapting tools defined for the basic case: either using the chain rule to compute derivatives explicitly, or using automatic differentiation. %  as advocated in~\S\ref{rem-auto-diff}.

\begin{example}[Application to structured prediction losses]\label{ex-structured-prediction-wasserstein-loss}
\index{Wasserstein!loss}% strictindex
\index{structured prediction}% strictindex
	When a label is itself a distribution, such as a segmentation mask, spatial heatmap, histogram or semantic probability vector, a prediction \(h_\theta(x_i)\) can be trained with
	\[
		\umin{\theta}\frac1n\sum_{i=1}^n \MK_c(h_\theta(x_i),\be_i).
	\]
	Here \(\be_i\) denotes the target output distribution attached to the input \(x_i\), and \(h_\theta(x_i)\) is constrained or normalized to have the same total mass. The cost \(c\) encodes spatial or semantic similarity between output bins. Thus a small displacement of probability mass is penalized less than a far-away error, unlike cross-entropy or an entrywise loss. Wasserstein losses were introduced for supervised learning in~\cite{FrognerNIPS}; scalable structured-label variants exploit cost structure and regularization~\cite{ToyokuniYokoiKashimaYamada2021WassersteinStructuredLabels}.
\end{example}
...
\index{Wasserstein!distance}% strictindex
\index{dictionary learning}% strictindex
\index{convex!combination}% strictindex


A practical problem of paramount importance in statistics and machine learning is density fitting.
\index{density fitting}% strictindex
%
...
In the case where $\al_\th$ has a density $\density{\th} \eqdef \density{\al_\th}$ with respect to the Lebesgue measure (or any other fixed reference measure), the maximum likelihood estimator (MLE) is obtained by solving
\index{Lebesgue measure}% strictindex
\index{reference!measure}% strictindex
\index{maximum likelihood estimator}% strictindex
\eq{
...
This MLE approach is known to lead to optimal estimation procedures in many cases (see for instance~\cite{owen2001empirical}). However, it fails to work when estimating singular distributions, typically when $\al_\th$ does not have a density (so that $\Ll_{\text{MLE}}(\al_\th,\be) = +\infty$) or when $(x_i)_i$ are samples from some singular $\bar\be$ (so that $\al_\th$ should share the same support as $\be$ for $\KL(\al|\bar\be)$ to be finite, but this support is usually unknown). Another issue is that in several cases of practical interest, the density $\density{\th}$ is inaccessible (or too hard to compute).
...
In order to cope with such difficult scenarios, one has to use weak metrics in place of the MLE functional $\Ll_{\text{MLE}}$, which can be written in dual form as
...
The class of estimators obtained using $\Ll=\MK_\c$, often called ``Minimum Kantorovich Estimators'' (MKE), was initially introduced in~\cite{bassetti2006minimum}, see also~\cite{CanasRosasco}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Wasserstein Derivatives}

Wasserstein losses are differentiable with respect to model parameters only through the dependence $\theta \mapsto \alpha_\theta$. There are two complementary viewpoints.
%
In the Eulerian viewpoint, one differentiates the loss with respect to the measure itself. When the dual potential $f_\theta$ between $\alpha_\theta$ and $\beta$ is unique, it acts as a first variation:
\index{first variation}% strictindex
\index{dual!potential}% strictindex
\eq{
	\Ll(\alpha_\theta+\delta\alpha,\beta)-\Ll(\alpha_\theta,\beta)
	= \int f_\theta \d \delta\alpha + o(\norm{\delta\alpha}).
}
%
In the Lagrangian viewpoint, when $\alpha_\theta=\fPF_{\theta,\sharp}\zeta$ and the map $\fPF_\theta$ is differentiable, the chain rule gives the parameter derivative
\eq{
	\partial_\theta \Ll(\alpha_\theta,\beta)
	= \int_{\Zz} \nabla f_\theta(\fPF_\theta(z)) \partial_\theta \fPF_\theta(z) \d\zeta(z),
}
whenever the potential is sufficiently smooth. This formula is the basis for gradient-based fitting of generative models with Wasserstein losses.
\index{generative model}% strictindex
%
Entropic regularization is often used in this context because it replaces non-smooth Kantorovich potentials by smooth Sinkhorn potentials. This makes the numerical gradients more stable, at the price of introducing a bias controlled by the regularization parameter $\epsilon$.
...
%
For both Wasserstein distances $\Wass_p$ (see~\ref{eq-defn-wass-dist}) and MMD norms (see Section~\ref{sec-dual-norms}), a straightforward estimator of the unknown distance between distributions is to compute it directly between the empirical measures, hoping ideally that one can control the rate of convergence of the latter to the former,
...
For such a dual distance that metrizes the weak convergence (see Definition~\ref{dfn-weak-conv}), since $\hat\al_n$ converges weakly to $\al$, one has $D(\al_n,\be_n) \rightarrow D(\al,\be)$ as $n \rightarrow +\infty$.
...
For $\X=\RR^\dim$ and measures supported on a bounded domain, it is shown by~\cite{dudley1969speed} that for $\dim>2$, and $1 \leq p < +\infty$,
\eq{
...
\index{nonasymptotic rate}% strictindex
%
...
\index{concentration inequality}% strictindex
...
%
For instance, denoting $\xi=\al-\be$ and $\xi_n=\al_n-\be_n$, one has
\eq{
	\norm{\xi_n}_{\Krkhs}^2 - \norm{\xi}_{\Krkhs}^2
	=
	\int_{\X \times \X} k(x,y) \d(\xi_n \otimes \xi_n - \xi \otimes \xi)(x,y).
}
Thus estimating the squared MMD amounts to approximating an integral by empirical averages of kernel evaluations. Standard Monte-Carlo bounds give a variance of order $1/n$ for bounded kernels, hence an expected error of order $1/\sqrt n$ after Jensen's inequality. The hidden constant depends on bounds on the kernel, and these bounds may themselves depend on the ambient dimension or on kernel parameters such as the bandwidth.
...
Entropic regularization improves the statistical behavior of OT because the dual potentials become smoother. For a fixed $\epsilon>0$, the relevant class of Sinkhorn potentials behaves like a smooth function class rather than a Lipschitz one, and its empirical process complexity can lead to dimension-free rates under suitable assumptions~\cite{genevay2018sample}. The tradeoff is that the constants deteriorate as $\epsilon \rightarrow 0$, reflecting the return to the non-smooth Wasserstein regime.
\index{dual!potential}% strictindex
\index{entropic regularization}% strictindex
